...
def _wall_quad(x0: float, z0: float, x1: float, z1: float, bot_y: float, top_y: float) -> dict:
    return {
        "corners": [
            [x0, bot_y, z0],
            [x1, bot_y, z1],
...
def _rect_room(
    *,
    ...
    """Rectangular room with 4 walls along each edge, all at floor_y..floor_y+wall_height."""
...
def test_rejects_pair_with_wall_height_delta_over_threshold():
    ...
    a = _rect_room(story=0, floor_y=0.00, wall_height=2.50)
    b = _rect_room(story=0, floor_y=0.03, wall_height=2.60, x0=5.0, x1=9.0)  # 10 cm taller
...
def test_lineage_records_on_modified_elements():
    ...
    for room in (a, b):
        entries = [e for e in room.get("lineage", []) if e["step"] == STEP_ALIGN_ROOM_HEIGHTS]
        assert len(entries) == 1
        assert "floor_y=" in entries[0]["detail"]
        for wall in room["walls_computed"]:
            wentries = [e for e in wall.get("lineage", []) if e["step"] == STEP_ALIGN_ROOM_HEIGHTS]
