...
def test_top_boundary_graph_demotes_attic_floor_inferred_under_strong_slope_without_exact_cell() -> None:
    ...
    assert flat_atom["role"] == "attic_floor_candidate"
    assert flat_atom["reason"] == "attic_inferred_demoted_strong_slope_without_exact_cell"
    assert room_summary["has_candidate_attic_relation"] is True
...
def test_top_boundary_graph_demotes_attic_floor_inferred_when_same_part_neighbour_has_oblique() -> None:
    ...
    building_part_graph = {
        "nodes": [
            {"id": "part:0", "type": "BuildingPart", "roof_family_guess": "gable_or_multi_slope"}
        ],
    ...
    flat_atom = next(node for node in top_boundary["nodes"] if node.get("id") == "atom:flat:r0")
    room_summary = top_boundary["room_summaries"]["room:0"]
...
def test_top_boundary_graph_demotes_attic_inferred_when_flat_cap_under_slope_with_same_part_oblique() -> None:
