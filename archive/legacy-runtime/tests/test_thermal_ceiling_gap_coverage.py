...
the roof (room fp ∪ within-story gap polygons) and get split by walls whose
...
# x ∈ [2, 4], leaving a non-empty eave cap over x ∈ [0, 2]. A probe wall at
...
def test_almost_reaching_wall_splits_eave_cap():
    ...
    probes = _probe_points(cap_entries)
    assert not probes["on_wall"], "almost-reaching wall (within REACH) must split the cap"
    assert probes["left_in"] and probes["right_in"]
