...
def _rect_floor(x0: float, z0: float, x1: float, z1: float, *, y: float = 0.0) -> list[list[float]]:
    return [
        [x0, y, z0],
        [x1, y, z0],
        [x1, y, z1],
...
def test_has_floor_above_uses_graph_partial_relation_when_geometric_probe_misses() -> None:
    bldg = {
        "rooms": [
            {"story": 0, "floor_polygon": _rect_floor(0.0, 0.0, 2.0, 2.0)},
            {"story": 1, "floor_polygon": _rect_floor(100.0, 100.0, 102.0, 102.0, y=3.0)},
        ]
    }
    graph = TopologyGraph(
        version="test",
...
def test_has_floor_above_ignores_weak_graph_relation_and_falls_back_to_geometry() -> None:
    bldg = {
        "rooms": [
            {"story": 0, "floor_polygon": _rect_floor(0.0, 0.0, 2.0, 2.0)},
            {"story": 1, "floor_polygon": _rect_floor(100.0, 100.0, 102.0, 102.0, y=3.0)},
