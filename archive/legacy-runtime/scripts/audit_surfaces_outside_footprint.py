...
def _poly_from_xz(corners):
    if not corners or len(corners) < 3:
        return None
    pts = [(c[0], c[2]) for c in corners if isinstance(c, (list, tuple)) and len(c) >= 3]
    if len(pts) < 3:
        return None
    try:
        p = Polygon(pts)
...
def _footprint_poly(bldg, py_entry):
    """Union of every room's floor polygon across all stories (building ground footprint)."""
    polys = []
...
def _iter_surfaces(bldg, py_entry):
    ...
    for i, c in enumerate(ceiling.get("thermal") or []):
        yield (f"thermal:{c.get('kind','?')}", f"thermal:{c.get('kind','?')}:{i}", c.get("poly"))
    # dormers
...
def main() -> int:
    ...
    buildings = buildings_data.get("buildings") if isinstance(buildings_data, dict) else buildings_data
    ...
    for idx, bldg in enumerate(buildings, 1):
        ...
        per_building.append({"uuid": uuid, "footprint_area_m2": fp_area, "offenders": offenders})
        if offenders:
            worst = max(o["outside_area_m2"] for o in offenders)
            print(f"[{idx}/{total}] {uuid}  offenders={len(offenders):3d}  worst_outside={worst:.2f} m²")
        elif idx % 25 == 0:
    ...
    for b in ranked[:15]:
        ...
        print(f"  {b['uuid']}  n={len(b['offenders']):3d}  worst_outside={worst:.2f} m²  footprint={b['footprint_area_m2']:.1f} m²")

    out = Path("/tmp/surfaces_outside_footprint.json")
    out.write_text(json.dumps({
        "thresholds": {"area_m2": AREA_THRESHOLD_M2, "ratio": RATIO_THRESHOLD, "footprint_buffer_m": FOOTPRINT_BUFFER_M},
