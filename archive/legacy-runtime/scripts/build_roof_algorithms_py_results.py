...
def main() -> int:
    parser = argparse.ArgumentParser(description="Rebuild reconcile/roof_algorithms_py_results.json from the current graph-aware roof pipeline.")
    parser.add_argument("uuids", nargs="*", help="Optional building UUIDs to rebuild. Defaults to all pipeline-outputs/*/merged.json buildings.")
    parser.add_argument("--output", default=str(OUTPUT_PATH), help="Output JSON path.")
    parser.add_argument("--checkpoint-every", type=int, default=10, help="Write partial results every N successful buildings.")
    ...
    for index, uuid in enumerate(uuids, start=1):
        ...
        try:
            ...
            print(f"[{index}/{len(uuids)}] error {uuid} {exc}", file=sys.stderr, flush=True)
