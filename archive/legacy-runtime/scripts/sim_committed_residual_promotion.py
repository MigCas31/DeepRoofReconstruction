...
DEFAULT_SIDECAR = Path(".context/raw_ceiling_plane_scorer_v2_full/plane_extent_splits.json")
...
def simulate(sidecar_path: Path) -> dict[str, Any]:
    ...
    for uuid, pieces in buildings.items():
        ...
        final_polys: list[tuple[str, Polygon, tuple[float, float, float, float] | None]] = []
        for p in pieces:
        ...
        for p in pieces:
            ...
            cand_plane = (
                tuple(float(c) for c in cand_coeffs) if cand_coeffs else None
            )
...
def _print_report(s: dict[str, Any]) -> None:
    ...
    print(f"  REPAIRED (now final):              {s['repaired']}   total {s['repaired_area_m2']:.1f} m²")
    print(f"  UNCHANGED · part-id guard:         {s['unchanged_part_guard']}")
    print(f"  UNCHANGED · coplanar overlap guard:{s['unchanged_coplanar_guard']}   total {s['coplanar_guard_area_m2']:.1f} m²")
    print(f"  REGRESSED (residual):              {s['regressed']}   total {s['regressed_area_m2']:.1f} m²")
    print()
    ...
    for ex in s["repaired_examples"][:10]:
        print(f"  {ex['uuid'][:8]}… piece={ex['piece_id'].split('::')[-1]}  area={ex['area']:.2f} m²  max_overlap={ex['overlap_area']:.2f} m²")
    print()
    print("Top coplanar-guard examples (predicate refinement skipped promotion):")
    for ex in s["coplanar_guard_examples"][:10]:
        print(f"  {ex['uuid'][:8]}… piece={ex['piece_id'].split('::')[-1]}  area={ex['area']:.2f} m²  overlap={ex['overlap_area']:.2f} m²  peer={ex['overlap_peer']}")
    print()
    ...
    for ex in s["unchanged_examples"][:8]:
        print(
            f"  {ex['uuid'][:8]}… piece={ex['piece_id'].split('::')[-1]}  area={ex['area']:.2f} m²"
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--out", type=Path, default=Path(".context/sim_committed_residual_promotion.json"))
