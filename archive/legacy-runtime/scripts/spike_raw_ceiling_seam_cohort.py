    ...
    for i in range(len(targets)):
        for j in range(i + 1, len(targets)):
            ...
            writer({
                ...
                "seam_disp_m": round(seam_disp, 2) if seam_disp == seam_disp else None,
                "overlap_m2": round(float(a["poly_xz"].intersection(b["poly_xz"]).area), 2),
            })
            pairs_emitted += 1
...
def main():
    ...
    ap.add_argument("--out", type=Path, default=REPO / ".context" / "intersection_seam_cohort.jsonl")
    args = ap.parse_args()
    ...
    print(f"Buildings with ≥2 oblique roofs: {len(roof_obliques_by_uuid)}", file=sys.stderr)
    ...
    with open(BUILDINGS_PATH) as fh, open(args.out, "w") as out_fh:
        ...
        for building in ijson.items(fh, "item"):
            ...
            if args.limit and processed >= args.limit:
                break
            if processed % 25 == 0:
                print(f"  ... {processed} buildings, {total_pairs} pairs", file=sys.stderr)
    print(f"Done. {processed} buildings, {total_pairs} pairs → {args.out}", file=sys.stderr)
