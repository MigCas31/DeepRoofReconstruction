...
def render(uuid: str, highlight_atom: str | None = None) -> Path:
    ...
    for partition in (b.get("ceiling_partitions") or {}).get("flat") or []:
        ...
        flat_total_count += 1
        signature = bisects_gable(cap_poly, cap_y, oblique_records) if area >= 0.1 else None
        if signature:
            flat_cohort_count += 1
        ...
        patch = MplPolygon(coords, closed=True, facecolor=color, edgecolor=edge, linewidth=1.5, alpha=alpha)
        ax_f.add_patch(patch)
        cx, cy = cap_poly.centroid.x, cap_poly.centroid.y
        ax_f.text(cx, cy, f"{area:.1f}m²\ny={cap_y:.2f}", ha="center", va="center", fontsize=6, color="#111")
    ...
    for partition in (b.get("ceiling_partitions") or {}).get("oblique") or []:
        ...
        patch = MplPolygon(coords, closed=True, facecolor="#10b981", edgecolor="#065f46", linewidth=1.5, alpha=0.5)
        ax_o.add_patch(patch)
        cx, cy = p.centroid.x, p.centroid.y
        ax_o.text(cx, cy, f"{area:.1f}m²", ha="center", va="center", fontsize=6, color="#111")

    # oblique ROOF surfaces shown on both for context
    for ob in oblique_records:
        coords = list(ob["poly"].exterior.coords)
        for ax in (ax_f, ax_o):
            patch = MplPolygon(coords, closed=True, facecolor="none", edgecolor="#475569", linewidth=0.8, linestyle="--", alpha=0.8)
            ax.add_patch(patch)

    for ax, title in [
        (ax_f, f"Flat ceiling partitions ({flat_cohort_count}/{flat_total_count} match bisects-gable)"),
    ...
    legend_handles = [
        mpatches.Patch(color="#ef4444", label="flat cap: bisects-gable predicate match"),
        mpatches.Patch(color="#60a5fa", label="flat cap: other"),
        mpatches.Patch(color="#10b981", label="oblique ceiling partition"),
        mpatches.Patch(facecolor="none", edgecolor="#475569", label="oblique roof surface (dashed)"),
    ]
    if highlight_atom:
        legend_handles.append(mpatches.Patch(color="#fbbf24", label=f"user-reported: {highlight_atom}"))
    fig.legend(handles=legend_handles, loc="lower center", ncol=2, fontsize=8, bbox_to_anchor=(0.5, -0.02))
