...
def _lift_polygon_to_surface(xz_poly: Polygon, surface_corners) -> list[list[float]]:
    ...
    if fit is None:
        ...
        if abs(ny) < 1e-6:
            ...
            y_fn = lambda x, z: cy - (nx * (x - cx) + nz * (z - cz)) / ny
    poly = xz_poly
    if poly.geom_type == "MultiPolygon":
        poly = max(poly.geoms, key=lambda g: g.area)
    if poly.geom_type != "Polygon" or poly.is_empty:
        return []
    return [[float(x), float(y_fn(x, z)), float(z)] for x, z in poly.exterior.coords[:-1]]
    ...
    if overlapping:
        ...
        if raw_union is not None and not raw_union.is_empty:
            ...
            try:
                ...
                over_xz = None
            if over_xz is not None and not over_xz.is_empty and over_xz.area >= MIN_OVERLAP_AREA_M2:
            ...
            overextend_buildings.setdefault(uuid, []).append(
                {
                    ...
                    "overextend_element_id": (
                        f"{uuid}::roof-overextend::{sub}:{idx}"
                    ),
                    "surface_kind": kind,
                    "surface_index": idx,
                    "story": story,
