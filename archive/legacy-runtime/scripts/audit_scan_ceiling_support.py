...
def _y_at_from_plane_eq(centroid: np.ndarray, normal: np.ndarray) -> Callable[[float, float], float]:
...
def _score_surface(
    ...
    return {
        ...
        "surface_y_min_at_match": float(min(sampled_surface_ys)) if sampled_surface_ys else None,
        "surface_y_max_at_match": float(max(sampled_surface_ys)) if sampled_surface_ys else None,
    }


def _classify(scores: dict[str, Any]) -> str:
    if scores["xz_coverage"] < MIN_XZ_COVERAGE_FOR_JUDGEMENT or scores["median_dy"] is None:
        return "none"
...
def _audit_committed_oblique(uuid: str, ob: dict, idx: int, raw_by_story: dict[int, list[RawCeiling]]) -> dict | None:
...
def _audit_committed_flat(uuid: str, fl: dict, idx: int, raw_by_story: dict[int, list[RawCeiling]]) -> dict | None:
...
def _audit_candidate_plane(uuid: str, plane: dict, idx: int, raw_by_story: dict[int, list[RawCeiling]]) -> dict | None:
        ...
        summary.append(
            {
                ...
                "candidate_minus_committed_weak": cand.get("weak", 0) - ob.get("weak", 0),
            }
        )
    return summary
...
def _print_top_table(summary: list[dict], n: int = 20) -> None:
    ...
    print(f"{'uuid':<40} {'ob_w':>5} {'ob_s':>5} {'ob_n':>5} {'cand_w':>7} {'cand_s':>7} {'cand_n':>7}")
    for row in ranked[:n]:
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--buildings", type=Path, default=BUILDINGS_PATH,
        help="Path to buildings_3d.json (default: %(default)s)",
    )
    parser.add_argument(
        "--roof-results", type=Path, default=ROOF_RESULTS_PATH,
    ...
    if args.building is not None:
        ...
        if not buildings:
            print(f"No building with uuid {args.building} in {args.buildings}", file=sys.stderr)
            return 2
    ...
    print(f"Audited {len(summary)} buildings ({skipped_no_roof} skipped: no roof results).")
