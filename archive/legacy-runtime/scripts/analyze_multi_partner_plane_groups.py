...
def _room_floor_union(building: dict[str, Any]) -> Polygon | None:
    ...
    if not geoms:
        return None
    return max((geom for geom in geoms if isinstance(geom, Polygon)), key=lambda geom: float(geom.area), default=None)
...
def _building_shape_summary(building: dict[str, Any], roof_result: dict[str, Any] | None) -> dict[str, Any]:
    stories = sorted({int(room.get("story", 0)) for room in building.get("rooms") or []})
    top_story = stories[-1] if stories else None
    top_story_room_count = (
        sum(1 for room in building.get("rooms") or [] if int(room.get("story", 0)) == top_story)
    ...
    return {
        ...
        "footprint_concavity_ratio": round(float(concavity_ratio), 6) if concavity_ratio is not None else None,
    ...
    *,
    ...
    for other_id, other in plane_groups_by_id.items():
        ...
        if other.get("azimuth_deg") is None or other.get("inclination_deg") is None:
            continue
        azimuth_delta = _wrapped_angle_delta_deg(subject_azimuth, other.get("azimuth_deg"))
        if azimuth_delta is None or azimuth_delta > azimuth_tol_deg:
            continue
        if abs(float(subject_inclination) - float(other.get("inclination_deg"))) > inclination_tol_deg:
            continue
        count += 1
    return count
...
def analyze_multi_partner_plane_groups(
    ...
    buildings_by_uuid = {str(building.get("uuid")): building for building in buildings if building.get("uuid")}
    ...
    for entry in entries:
        ...
        multi_partner_nodes = [node_id for node_id, neighbors in adjacency.items() if len(neighbors) > 1]
        ...
        for node_id in sorted(multi_partner_nodes):
            ...
            row = {
                ...
                "neighbor_family_count": _neighbor_family_count(node_id, neighbors, plane_groups_by_id),
                "same_side_sibling_count": _same_side_sibling_count(node_id, plane_groups_by_id),
                "best_partner_plane_group_id": plane_groups_by_id.get(node_id, {}).get("best_partner_plane_group_id"),
                "azimuth_deg": plane_groups_by_id.get(node_id, {}).get("azimuth_deg"),
                "inclination_deg": plane_groups_by_id.get(node_id, {}).get("inclination_deg"),
                "pattern": pattern,
            }
        ...
        building_rows.append(
            {
                ...
                **shape,
                ...
                "max_multi_partner_degree": max(len(adjacency[node_id]) for node_id in multi_partner_nodes),
...
def main() -> None:
    ...
    parser.add_argument("--uuid", help="Only emit analysis rows for a single building UUID")
    ...
    analysis = analyze_multi_partner_plane_groups(buildings, roof_results, ridge_eave_scores)
    if args.uuid:
        analysis["buildings"] = [
            row for row in analysis["buildings"] if str(row.get("uuid")) == str(args.uuid)
        ]
