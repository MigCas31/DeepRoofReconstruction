...
from reconcile.complexity_tiers import classify_building  # noqa: E402
...
def facade_continuity_metrics(
    eave_a: list, eave_b: list, ridge: dict
) -> dict:
...
def ridge_continuity_metrics(
    top_a: list, top_b: list, ridge: dict
) -> dict:
...
def face_metrics(face: dict, plane: dict, ridge: dict) -> dict:
    ...
    return {
        ...
        "top_y_avg": (
            round(sum(top_corner_ys) / len(top_corner_ys), 3)
            if top_corner_ys
            else None
        ),
    }
...
def audit_building(uuid: str, building: dict, roof: dict, v3_entry: dict | None) -> dict:
    ...
    if pair is None:
        return {
            ...
            "v3_status": (v3_entry.get("parts") or [{}])[0].get("gable_extension", {}).get("status")
            if v3_entry
            else None,
            "buckets": ["no_pair"],
        }
    ...
    continuity = {"overlap_ratio": None, "gap_3d_m": None,
                  "ridge_extent_a_m": None, "ridge_extent_b_m": None}
    if ridge is not None and "A" in top_corners_per_face and "B" in top_corners_per_face:
    ...
    if ridge is not None and eave_corners_per_face.get("A") and eave_corners_per_face.get("B"):
    ...
    return {
        ...
        "wall_top_y_max": round(wall_top_y_max, 3) if wall_top_y_max is not None else None,
...
def main() -> int:
    ...
    for uid in tier6_uuids:
        ...
        print(
            ...
            f"facade asym={fac.get('facade_asymmetry_m')} cov={fac.get('coverage_ratio')} "
    ...
    for r in sorted(records, key=lambda x: (sorted(x["buckets"]), x["uuid"])):
        ...
        if len(r["faces"]) >= 1:
            ...
            a_y = f"{f.get('y_min', 0.0):+.2f}..{f.get('y_max', 0.0):+.2f}"
        if len(r["faces"]) >= 2:
            f = r["faces"][1]
            b_area = f"{f['area_m2']:.1f}"
            b_y = f"{f.get('y_min', 0.0):+.2f}..{f.get('y_max', 0.0):+.2f}"
        outside = (
        ...
        gap = (
            f"{cont['gap_3d_m']:.2f}"
            if cont.get("gap_3d_m") is not None
            else "—"
        )
