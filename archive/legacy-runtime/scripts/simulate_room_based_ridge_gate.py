        ...
        cluster_results.append(
            {
                ...
                "verdict_change": ("recovered" if room_pass and not seg_pass else
                                   ("kept" if room_pass and seg_pass else
                                    ("still_dropped" if not room_pass and not seg_pass else "would_drop"))),
            }
        )
...
def main() -> int:
    ...
    for r in reports.values():
        ...
        would_drop = sum(1 for c in r["clusters"] if c["verdict_change"] == "would_drop")
    ...
    print(f"buildings with at least one recovered cluster:               {n_recovered_buildings}")
    print(f"clusters that would NOW DROP (passed seg, fail room):        {n_would_drop}  (regression risk)")
    ...
    if target in reports:
        ...
        for c in reports[target]["clusters"]:
            print(
                f"  cluster[{c['cluster_index']}] az={c['az']:>6.2f} incl={c['incl']:>5.2f} "
