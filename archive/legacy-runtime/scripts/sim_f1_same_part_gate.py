...
def simulate_building(building: dict) -> dict:
    ...
    atom_regions = coverage_graph.get("atom_regions") or {}
    ...
    for cell in cells:
        ...
        atom_stays_upper_void = (
            flat_role in UPPER_VOID_FLAT_ROLES or room_oblique.get(room_key, False)
        )
...
def main() -> None:
    ...
    hdr = f"{'uuid':<38} {'cohort':>6} {'F1':>5} {'F1+F1b':>7} {'lost(F1)':>9} {'lost(F1+F1b)':>13} {'ambig':>6}"
    print(hdr)
    for uuid, out in top:
        print(
