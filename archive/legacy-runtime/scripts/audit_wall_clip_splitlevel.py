...
def _scan_room(room):
    ...
    new_bottoms = [_min_y(w["corners"]) for w in clipped if len(w.get("corners") or []) >= 3]
    orig_bottom_median = statistics.median(orig_bottoms)
    new_bottom_median = statistics.median(new_bottoms) if new_bottoms else orig_bottom_median
...
def run(input_path: Path, top: int) -> int:
    ...
    if flagged:
        ...
        for r in flagged:
            ratio = r["n_walls_clipped"] / r["n_walls_total"] if r["n_walls_total"] else 0
            if ratio >= 1.0:
                ratio_bins["100"] += 1
            elif ratio >= 0.75:
                ratio_bins["75-99"] += 1
