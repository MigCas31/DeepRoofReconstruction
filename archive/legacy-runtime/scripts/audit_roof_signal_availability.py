...
Signals measured per building:
  ...
  - oblique_azimuth_bins  : distinct 30 deg bins among oblique surfaces (azimuth mod 180)
...
def polygon_area_xz(corners: list) -> float:
    ...
    n = len(corners)
    for i in range(n):
        x1, _, z1 = corners[i][0], corners[i][1] if len(corners[i]) > 1 else 0.0, corners[i][-1]
...
def main() -> int:
    ...
    underbuilt = [
        s
        for s in summaries
        if s["concavity"] >= 0.10 and (s["oblique_surfaces"] <= 1 or s["oblique_az_bins"] <= 1)
    ]
    underbuilt_strong = [
        s
        for s in summaries
        if s["concavity"] >= 0.20 and s["oblique_az_bins"] <= 1
    ]
    ...
    print(f"very concave (>=0.20): {len(very_concave)} ({100*len(very_concave)/n:.1f}%)")
    print()
    ...
    print(f"floor_delta_present:        {sig_floor_delta}/{n}  ({100*sig_floor_delta/n:.1f}%)")
    print(f"has_slanted_ceiling:        {sig_slanted_ceiling}/{n}  ({100*sig_slanted_ceiling/n:.1f}%)")
    print(f"ceiling_planes > 0:         {sig_ceiling_planes}/{n}  ({100*sig_ceiling_planes/n:.1f}%)")
    print(f"multi-story (>=2):          {sig_multi_story}/{n}  ({100*sig_multi_story/n:.1f}%)")
    print(f"cross_floor_gaps present:   {sig_cross_floor_gaps}/{n}  ({100*sig_cross_floor_gaps/n:.1f}%)")
    print(f"oblique segments present:   {sig_segments}/{n}  ({100*sig_segments/n:.1f}%)")
    print(f"multi-azimuth clusters:     {sig_multi_az_clusters}/{n}  ({100*sig_multi_az_clusters/n:.1f}%)")
    ...
    for s in examples:
        print(
            ...
            f"valid_clusters={s['valid_clusters']}  cluster_az_bins={s['cluster_az_bins']}  "
            f"dropped={s['dropped_clusters']}  slanted_ceiling={s['has_slanted_ceiling']}  "
