...
def _fit_plane_svd(corners: list[list[float]]) -> tuple[np.ndarray, np.ndarray] | tuple[None, None]:
...
def _normal_to_azimuth_inclination(normal: np.ndarray) -> tuple[float, float]:
    ...
    azimuth = float((math.degrees(math.atan2(float(roof_up[0]), float(roof_up[2]))) + 360.0) % 360.0)
    inclination = float(math.degrees(math.acos(max(-1.0, min(1.0, abs(float(roof_up[1])))))))
...
def _candidate_normal(plane: dict[str, Any]) -> np.ndarray:
    n = plane["n"]
    return _normalize_roof_up(np.array([float(n["x"]), float(n["y"]), float(n["z"])], dtype=float))


def _wall_top_segments(room: dict[str, Any]) -> list[tuple[tuple[float, float, float], tuple[float, float, float]]]:
...
def _dist_point_to_segment_3d(
    ...
    ap = np.array([point[0] - start[0], point[1] - start[1], point[2] - start[2]], dtype=float)
    ab = np.array([end[0] - start[0], end[1] - start[1], end[2] - start[2]], dtype=float)
...
def _min_dist_to_wall_tops(
    ...
    if not wall_segments:
        return float("inf")
    return min(_dist_point_to_segment_3d(point, start, end) for start, end in wall_segments)
...
def _raw_plane_is_local_target_match(
    ...
    if raw_overlap_fraction < LOW_TRUST_PROMOTION_MIN_RAW_OVERLAP_FRACTION:
        return False
    normal_dot = abs(float(np.dot(_normalize_roof_up(record.normal), _normalize_roof_up(target.normal))))
    if normal_dot < LOW_TRUST_PROMOTION_MIN_NORMAL_DOT:
        return False
    y_on_target = _plane_y_at(target, float(record.centroid[0]), float(record.centroid[2]))
        ...
        if not usable and _base_raw_plane_usable(record):
            for target in targets_by_story.get(record.story, []):
                if _raw_plane_is_local_target_match(record, target):
                    usable = True
                    break
        promoted.append(record if usable == record.usable_for_support else replace(record, usable_for_support=usable))
    return promoted


def _footprint_lines(roof_result: dict[str, Any]) -> list[tuple[float, float, float, float]]:
...
def _distance_point_to_segment_2d(px: float, pz: float, ax: float, az: float, bx: float, bz: float) -> float:
    vx, vz = bx - ax, bz - az
    wx, wz = px - ax, pz - az
    denom = vx * vx + vz * vz
    if denom < 1e-12:
...
def _endpoints_match(a1: list[float], b1: list[float], a2: list[float], b2: list[float]) -> bool:
    def close(p: list[float], q: list[float]) -> bool:
        return (
            math.hypot(float(p[0]) - float(q[0]), float(p[2]) - float(q[2])) <= RIDGE_MATCH_XZ_TOL_M
...
def collect_raw_edges(
    ...
    records_by_story: dict[int, list[tuple[RawPlaneRecord, list[float], list[float], float, float | None]]] = defaultdict(list)
    for record in usable_records:
        ...
        for idx in range(count):
            ...
            if dy > HORIZONTAL_DY_M or length < MIN_EDGE_LEN_M:
                continue
            records_by_story[record.story].append((record, start, end, length, _edge_azimuth_xz_deg(start, end)))
    ...
    for story, story_edges in records_by_story.items():
        for idx, (record, start, end, length, edge_azimuth) in enumerate(story_edges):
            ...
            label = "isolated"
            best_angle = 0.0
            for other_idx, (other_record, other_start, other_end, _other_len, _other_azimuth) in enumerate(story_edges):
                ...
                if angle >= RIDGE_MIN_ANGLE_DEG and angle > best_angle:
                    label = "ridge_or_hip"
                    best_angle = angle
            if label != "ridge_or_hip" and _edge_near_footprint(start, end, footprint_lines):
                label = "eave"
            midpoint_xz = ((float(start[0]) + float(end[0])) * 0.5, (float(start[2]) + float(end[2])) * 0.5)
            edges.append(
                RawEdgeRecord(
                    story=story,
...
def collect_conflict_pairs(raw_records: list[RawPlaneRecord]) -> list[ConflictPairRecord]:
    usable_records = [record for record in raw_records if record.usable_for_support]
...
def _edge_chain_connects(left: RawEdgeRecord, right: RawEdgeRecord) -> bool:
    ...
    if left.story != right.story:
        return False
    if left.start_xz is None or left.end_xz is None or right.start_xz is None or right.end_xz is None:
        return False
    if left.y_mid is None or right.y_mid is None:
        return False
    if _wrapped_angle_delta_mod180_deg(left.edge_azimuth_xz_deg, right.edge_azimuth_xz_deg) > EAVE_CHAIN_ANGLE_TOL_DEG:
        return False
    ...
    left_to_right = _distance_point_to_infinite_line_2d(
        left_mid[0], left_mid[1], right.start_xz[0], right.start_xz[1], right.end_xz[0], right.end_xz[1]
    )
    right_to_left = _distance_point_to_infinite_line_2d(
        right_mid[0], right_mid[1], left.start_xz[0], left.start_xz[1], left.end_xz[0], left.end_xz[1]
    )
    ...
    line_gap = LineString([left.start_xz, left.end_xz]).distance(LineString([right.start_xz, right.end_xz]))
    return endpoint_gap <= EAVE_CHAIN_GAP_M or line_gap <= EAVE_CHAIN_COLINEAR_OFFSET_M


def build_eave_chains(uuid: str, raw_edges: list[RawEdgeRecord]) -> list[EaveChainRecord]:
        ...
        for start_idx in range(len(story_edges)):
            ...
            if longest.start_xz is not None and longest.end_xz is not None:
                longest_vec = np.array(
                    [longest.end_xz[0] - longest.start_xz[0], longest.end_xz[1] - longest.start_xz[1]],
                    dtype=float,
                )
            ...
            chains.append(
                EaveChainRecord(
                    ...
                    azimuth_deg=float(math.degrees(math.atan2(float(axis[0]), float(axis[1]))) % 180.0),
                    y_mean=y_mean,
                    start_xz=(float(start[0]), float(start[1])),
                    end_xz=(float(end[0]), float(end[1])),
                    line_xz=LineString([(float(start[0]), float(start[1])), (float(end[0]), float(end[1]))]),
                    member_plane_ids=tuple(sorted({edge.plane_element_id for edge in component})),
                )
            )
            chain_index += 1
    return chains
...
def build_story_extent_envelopes(building: dict[str, Any]) -> dict[int, Polygon]:
    ...
    for story, polys in by_story.items():
        ...
        if not candidates:
            continue
        envelope = max(candidates, key=lambda poly: float(poly.area)) if len(candidates) == 1 else unary_union(candidates)
...
def _plane_coeff_point_and_normal(plane: list[float], poly: Polygon) -> tuple[tuple[float, float, float], np.ndarray] | tuple[None, None]:
    if len(plane) != 4:
        return None, None
...
def _infer_story_from_envelopes(poly: Polygon, story_extent_envelopes: dict[int, Polygon]) -> int:
    best_story = 0
    best_overlap = -1.0
    for story, envelope in story_extent_envelopes.items():
        try:
            ...
            overlap = 0.0
        if overlap > best_overlap + 1e-9 or (abs(overlap - best_overlap) <= 1e-9 and int(story) > best_story):
...
def collect_ridge_eave_target_diagnostics(
    ...
    for plane_group in (ridge_eave_entry.get("plane_groups") or []):
        ...
        if not plane_group_suffix:
            continue
        target_element_id = f"{uuid}::ridge-eave-candidate::plane-group::{plane_group_suffix}"
        member_candidates = [
        ...
        total_candidate_area = float(sum(float(candidate.get("area_m2") or 0.0) for candidate in member_candidates))
        ...
        provenance_flag = (
            ...
            if (
                ...
                and (not has_partner or creator_negative_top_fraction >= CREATOR_NEG_TOP_SUSPECT_MIN)
            )
            else "normal"
        )
...
def collect_ridge_eave_target_anchor_masks(
    ...
    for plane_group in (ridge_eave_entry.get("plane_groups") or []):
        ...
        for room_id in (diagnostics.get("creator_source_room_ids") or []):
            if not isinstance(room_id, str) or not room_id.startswith("room:") or building is None:
                continue
            try:
            ...
            if room_index < 0 or room_index >= len(rooms):
                continue
            room_poly = _xz_polygon((rooms[room_index] or {}).get("floor_polygon") or [])
        ...
        if source_room_polys and hasattr(seed_union, "geoms"):
            ...
            if source_union is not None and not getattr(source_union, "is_empty", True):
                ...
                if scored_components:
                    best_overlap = max(overlap_area for overlap_area, _component in scored_components)
                    kept_components = [
                        component
                        for overlap_area, component in scored_components
...
def _augment_raw_records_with_source_rooms(
    ...
    if not source_room_keys:
        return raw_records
    raw_by_id: dict[str, RawPlaneRecord] = {record.element_id: record for record in raw_records}
    all_room_records = collect_raw_plane_records(building, roof_result, exposed_only=False)
...
def collect_target_plane_records(roof_result: dict[str, Any], uuid: str) -> list[TargetPlaneRecord]:
    targets: list[TargetPlaneRecord] = []
    for idx, plane in enumerate((roof_result.get("ceiling") or {}).get("planes") or []):
        ...
        targets.append(
            TargetPlaneRecord(
                ...
                area_xz_m2=float(poly.area),
                plane_point=(float(plane["ref"]["x"]), float(plane["ref"]["y"]), float(plane["ref"]["z"])),
            )
        )
    for idx, surface in enumerate((roof_result.get("roof_surfaces") or {}).get("oblique") or []):
        ...
        if ridge_norm < 1e-9:
            ...
            ridge_norm = math.hypot(ridge_x, ridge_z)
        ridge_dir_xz = (ridge_x / ridge_norm, ridge_z / ridge_norm) if ridge_norm >= 1e-9 else (1.0, 0.0)
        azimuth_deg, inclination_deg = _normal_to_azimuth_inclination(normal)
        targets.append(
            TargetPlaneRecord(
                uuid=uuid,
...
def _ridge_edge_support_length(target: TargetPlaneRecord, raw_edges: list[RawEdgeRecord]) -> float:
    ridge_azimuth = _ridge_dir_to_azimuth_mod180(target.ridge_dir_xz)
    total = 0.0
    for edge in raw_edges:
        if edge.story != target.story or edge.label != "ridge_or_hip":
            continue
        if _wrapped_angle_delta_mod180_deg(edge.edge_azimuth_xz_deg, ridge_azimuth) > EDGE_ALIGNMENT_TOL_DEG:
            continue
...
def _eave_edge_support_length(target: TargetPlaneRecord, raw_edges: list[RawEdgeRecord]) -> float:
    ridge_azimuth = _ridge_dir_to_azimuth_mod180(target.ridge_dir_xz)
    total = 0.0
    for edge in raw_edges:
...
def _conflicting_pair_count(target: TargetPlaneRecord, conflicts: list[ConflictPairRecord]) -> int:
    count = 0
...
def _score_flags(orientation_support_score: float, retention_support_score: float, conflicting_raw_pair_count: int) -> tuple[str, str, bool]:
...
def _ridge_axis_unit(target: TargetPlaneRecord) -> np.ndarray:
    axis = np.asarray([float(target.ridge_dir_xz[0]), float(target.ridge_dir_xz[1])], dtype=float)
...
def score_plane_eave_chain_supports(
    ...
    for target in targets:
        ...
        for chain in chains_by_story.get(target.story, []):
            angle_delta = _wrapped_angle_delta_mod180_deg(chain.azimuth_deg, ridge_azimuth)
            ...
            boundary_score = 1.0 - min(boundary_distance / PLANE_EAVE_CHAIN_BOUNDARY_TOL_M, 1.0)
            overlap_length = float(target.poly_xz.buffer(PLANE_EAVE_CHAIN_BUFFER_M).intersection(chain.line_xz).length)
            ...
            if y_samples:
                ...
                height_score = 1.0 - min(height_residual / PLANE_EAVE_CHAIN_HEIGHT_TOL_M, 1.0)
            else:
                height_score = 0.0
...
def _chain_endpoint_gap(chain_a: EaveChainRecord, chain_b: EaveChainRecord) -> float:
    ...
    for ax, az in endpoints_a:
        for bx, bz in endpoints_b:
            min_gap = min(min_gap, math.hypot(float(ax) - float(bx), float(az) - float(bz)))
    return min_gap


def _chains_share_facade_component(chain_a: EaveChainRecord, chain_b: EaveChainRecord) -> bool:
    if chain_a.story != chain_b.story:
        return False
    if _wrapped_angle_delta_mod180_deg(chain_a.azimuth_deg, chain_b.azimuth_deg) > EAVE_CHAIN_ANGLE_TOL_DEG:
        return False
...
def expand_plane_eave_chain_supports_by_facade_continuity(
    ...
    supports_by_target: dict[str, dict[str, PlaneEaveChainSupportRecord]] = defaultdict(dict)
    ...
    for target_supports in supports_by_target.values():
        seed_ids = {chain_id for chain_id, support in target_supports.items() if support.supported}
...
def _projection_interval(axis: np.ndarray, start_xz: tuple[float, float], end_xz: tuple[float, float]) -> tuple[float, float]:
...
def _polygon_projection_bounds(poly: Polygon, axis: np.ndarray) -> tuple[float, float] | None:
    if poly.is_empty:
        return None
    values: list[float] = []
    ...
    for part in polygons:
        ...
        for x, z in list(part.exterior.coords):
            values.append(float(np.dot(axis, np.asarray([float(x), float(z)], dtype=float))))
...
def _axis_strip_polygon(target_poly: Polygon, axis: np.ndarray, start_t: float, end_t: float) -> Polygon:
    minx, minz, maxx, maxz = target_poly.bounds
    width = math.hypot(maxx - minx, maxz - minz) + 2.0 * (PLANE_EAVE_CHAIN_BOUNDARY_TOL_M + EAVE_CHAIN_GAP_M) + 1.0
...
def _target_uphill_sweep_direction(target: TargetPlaneRecord, axis: np.ndarray) -> np.ndarray | None:
    ...
    for direction in candidate_directions:
        ...
        if line_cover is not None and not getattr(line_cover, "is_empty", True):
            pieces = [
                poly
                for poly in pieces
                if not poly.intersection(line_cover).is_empty
            ]
        if not pieces:
            continue
        try:
        ...
        if plane_direction is not None and float(np.dot(direction, plane_direction)) > 0.0:
...
def trim_ridge_eave_supported_pieces_to_chain_run_bands(
    ...
    for story_rows in piece_rows_by_story.values():
        ...
        while pending:
            ...
            while changed:
                ...
                for row in list(pending):
                    if any(
                        _targets_are_local_ownership_competitors(row["target"], member["target"])
                        for member in group
                    ):
                        group.append(row)
                        pending.remove(row)
        ...
        for family in family_groups:
            ...
            family_ortho = np.asarray([-float(family_axis[1]), float(family_axis[0])], dtype=float)
            ...
            for row in family:
                ...
                for chain_id in row["signature"]:
                    ...
                    interval_values.extend(
                        [
                            float(np.dot(family_axis, np.asarray(chain.start_xz, dtype=float))),
                            float(np.dot(family_axis, np.asarray(chain.end_xz, dtype=float))),
                        ]
                    )
                if not interval_values:
                    continue
            ...
            for left_row, right_row in zip(normalized_rows, normalized_rows[1:]):
                if _chain_signature_relation(left_row["signature"], right_row["signature"]) != "disjoint":
                    continue
                boundary_t = 0.5 * (
                    float(left_row["interval_max_t"]) + float(right_row["interval_min_t"])
                )
    ...
    if not trimmed_by_piece_id:
        return split_pieces
    return [
        trimmed_by_piece_id.get(str(piece.piece_id), piece)
        for piece in split_pieces
    ]


def _ring_to_plane_corners(target: TargetPlaneRecord, coords: list[tuple[float, float]]) -> list[list[float]]:
    corners: list[list[float]] = []
    for x, z in coords:
...
def _target_plane_coeffs(target: TargetPlaneRecord) -> tuple[float, float, float, float] | None:
    if target.plane_point is None:
        return None
    normal = _normalize_roof_up(np.asarray(target.normal, dtype=float))
    if normal.shape[0] < 3:
...
def _half_plane_polygon_for_bounds(
    ...
    *,
    ...
    for idx, point in enumerate(bbox):
        ...
        crosses = (value > eps and next_value < -eps) or (value < -eps and next_value > eps)
        if not crosses:
            continue
...
def _merge_supported_gap_polygons(
    ...
    try:
        supported_union = unary_union([candidate["poly"] for candidate in supported_candidates])
    except Exception:
        return supported_candidates, []
    ...
    for residual in residual_polys:
        ...
        if gap_polys:
            ...
            for gap_overlap in _iter_polygons(residual.intersection(gap_union) if gap_union is not None else None):
    ...
    if gap_union is not None:
        try:
            direct_gap_geom = base_poly.intersection(gap_union).difference(supported_union)
        except Exception:
        ...
        for gap_slice in _iter_polygons(direct_gap_geom):
            if float(gap_slice.area) < MIN_GAP_CONTINUATION_OVERLAP_M2:
                continue
            near_supported = float(supported_union.distance(gap_slice)) <= PLANE_EAVE_CHAIN_BUFFER_M
            in_support_hull = (
                support_hull is not None
                and support_hull.buffer(1e-6).contains(gap_slice.representative_point())
            )
    ...
    try:
        merged_union = unary_union([candidate["poly"] for candidate in supported_candidates] + bridged_residuals)
    except Exception:
        return supported_candidates, []
    ...
    for poly in _iter_polygons(merged_union):
        ...
        merged_candidates.append(
            {
                "poly": poly,
                "support_score": max(float(candidate["support_score"]) for candidate in touching),
                "chain_ids": tuple(
                    sorted(
                        {
                            chain_id
...
def _ridge_eave_chain_owner_equivalent(
    ...
    *,
    ...
    left_height = float(left.height_residual_m) if left.height_residual_m is not None else float("inf")
    right_height = float(right.height_residual_m) if right.height_residual_m is not None else float("inf")
    return (
        abs(left_height - right_height) <= eps
        and abs(float(left.overlap_fraction) - float(right.overlap_fraction)) <= eps
        and abs(float(left.boundary_distance_m) - float(right.boundary_distance_m)) <= eps
...
def select_locally_owned_ridge_eave_chain_supports(
    ...
    supports_by_story_chain: dict[tuple[int, str], list[PlaneEaveChainSupportRecord]] = defaultdict(list)
    for support in plane_chain_supports:
        if (
            not support.supported
            or support.target_element_id not in ridge_targets
        ):
            continue
    ...
    for support in plane_chain_supports:
        ...
        if (
            support.target_element_id not in ridge_targets
...
def build_plane_extent_split_pieces(
    ...
    sibling_polys_by_key: dict[tuple[str, int, str], list[tuple[str, Polygon]]] = defaultdict(list)
    ...
    for target in targets:
        ...
        allowed_part_ids = (target_allowed_part_ids or {}).get(target.element_id) or set()
        envs: list[Any] = []
        ...
        if (
            ...
            try:
                support_base_poly = unary_union([support_base_poly, anchor_support_geom])
            except Exception:
                pass
            if extent_poly is not None:
                try:
                    ...
                    pass
        if getattr(support_base_poly, "is_empty", True) or float(getattr(support_base_poly, "area", 0.0) or 0.0) < MIN_SPLIT_PIECE_AREA_M2:
            continue
        ...
        support_map = {
            ...
            if support.chain_id in chain_by_id and chain_by_id[support.chain_id].story == target.story
        }
        if not support_map:
            continue
        ...
        while remaining_chain_ids:
            ...
            while queue:
                ...
                for neighbor_id in chain_neighbors.get(chain_id, set()):
                    if neighbor_id in remaining_chain_ids and neighbor_id not in component_chain_ids:
                        queue.append(neighbor_id)
        ...
        if (
            ...
            for component_supports in connected_components:
                ...
                for support in component_supports:
                    ...
                    try:
                        overlap_area += float(
                            chain.line_xz.buffer(PLANE_EAVE_CHAIN_BUFFER_M, cap_style=2).intersection(anchor_support_geom).area
                        )
                    except Exception:
                        continue
            ...
            if max_anchor_overlap > 0.0:
                connected_components = [
                    component_supports
                    for component_supports, overlap_area in zip(connected_components, component_anchor_overlaps)
        ...
        for component_supports in connected_components:
            if target.target_kind == "ridge_eave_plane_group":
                ...
                for support in sorted(component_supports, key=lambda row: (-row.support_score, row.chain_id)):
                    chain = chain_by_id.get(support.chain_id)
                    if chain is None:
                        continue
                    sweep_poly = _chain_inward_sweep_polygon(target, support_base_poly, chain, axis)
                    if sweep_poly is None or sweep_poly.is_empty:
                        continue
                    sweep_polys.append(sweep_poly)
                    component_chain_ids.add(chain.chain_id)
                    component_support_score = max(component_support_score, float(support.support_score))
                if not sweep_polys:
                    continue
                try:
            ...
            for support in sorted(component_supports, key=lambda row: (-row.support_score, row.chain_id)):
                ...
                if chain is None:
                    continue
                start_t, end_t = _projection_interval(axis, chain.start_xz, chain.end_xz)
                start_u, end_u = _projection_interval(ortho, chain.start_xz, chain.end_xz)
            ...
            if not interval_rows:
                continue
            interval_rows.sort(key=lambda row: (float(row["start_t"]), float(row["end_t"])))
            ...
            for row in interval_rows:
                ...
                if float(row["start_t"]) <= float(prev["end_t"]) + EAVE_CHAIN_GAP_M:
                    ...
                    prev["support_score"] = max(float(prev["support_score"]), float(row["support_score"]))
                else:
                    component_intervals.append(row)
            ...
            component_rows.append(
                {
                    "start_t": min(float(row["start_t"]) for row in component_intervals),
                    ...
                    "support_score": max(float(row["support_score"]) for row in component_intervals),
                }
            )
        ...
        if component_rows:
            component_rows.sort(key=lambda row: ((float(row["min_u"]) + float(row["max_u"])) * 0.5, float(row["start_t"])))
            ...
            for component_index, component in enumerate(component_rows):
                ...
                component_bands.append({**component, "band_min_u": lower_u, "band_max_u": upper_u})

            for component_index, component in enumerate(component_bands):
        ...
        for component_index, candidate in enumerate(supported_candidates):
            ...
            if extension_provenance_out is not None and target_extension_info is not None:
...
def trim_ridge_eave_supported_pieces_to_room_ownership(
    ...
    *,
    ...
    for piece in split_pieces:
        room_regions_by_story.setdefault(piece.story, _story_room_regions(building, piece.story))
    ...
    for piece in split_pieces:
        ...
        if allowed_part_ids:
            ...
            if part_polys:
                try:
                    allowed_part_union_by_piece_id[piece.piece_id] = unary_union(part_polys)
                except Exception:
                    pass
    ...
    for piece in split_pieces:
        if piece.piece_role != "supported" or piece.target_kind != "ridge_eave_plane_group":
        ...
        if owned_regions:
            ...
            if not owned_union.is_empty:
                try:
                    ...
                    trimmed.append(piece)
                    continue
        if allowed_part_union is not None and not getattr(allowed_part_union, "is_empty", True):
            try:
                allowed_mask = allowed_part_union
            except Exception:
                trimmed.append(piece)
                continue
            trim_mask = allowed_mask if trim_mask is None else trim_mask.intersection(allowed_mask)
        ...
        if not clipped_polys:
            trimmed.append(piece)
            continue
        clipped_polys = sorted(clipped_polys, key=lambda poly: float(poly.area), reverse=True)[:1]
        for poly_index, poly in enumerate(clipped_polys):
            record = _piece_records_from_polygon(
                target,
                poly,
                piece_id=piece.piece_id if poly_index == 0 else f"{piece.piece_id}:{poly_index}",
                piece_index=piece.piece_index,
                piece_role=piece.piece_role,
                support_score=piece.support_score,
                chain_ids=piece.chain_ids,
    ...
    *,
    ...
    for piece in split_pieces:
        ...
        partner_plane_group_id = str(
            meta_by_target.get(piece.target_element_id, {}).get("best_partner_plane_group_id") or ""
        )
        ...
        if coeffs is None:
            ...
            continue
        try:
            bounds = unary_union([piece_poly, target.poly_xz, partner_target.poly_xz]).bounds
        except Exception:
...
def _room_part_membership(
    ...
    return {
        ...
        for room_id, part_ids in (building_part_graph.get("room_membership") or {}).items()
    }
...
def _ridge_eave_plane_group_meta_by_target(
    ...
    for plane_group in (ridge_eave_entry.get("plane_groups") or []):
        ...
        meta[target_element_id] = {
            "best_partner_plane_group_id": plane_group.get("best_partner_plane_group_id"),
...
def diagnose_ridge_eave_supported_piece_ownership(
    ...
    *,
    ...
    supported_ridge_eave_pieces_by_signature: dict[tuple[int, tuple[str, ...]], list[TargetSplitPieceRecord]],
    ...
    subject_quality = float(
        piece.support_score if piece.support_score is not None else subject_score_row.get("retention_support_score") or 0.0
    )
    piece_signature = _chain_signature(piece.chain_ids)
    signature_class = supported_ridge_eave_pieces_by_signature.get((piece.story, piece_signature), [])
    local_signature_competitors = [
        ...
        if (
            ...
            and _targets_are_local_ownership_competitors(target, targets_by_id[other_piece.target_element_id])
        )
    ]
    ...
    for room_region in room_regions:
        ...
        for other_target in story_targets:
            ...
            if other_score_row is None:
                continue
            try:
                overlap_area = float(region_poly.intersection(other_target.poly_xz).area)
            except Exception:
                overlap_area = 0.0
            if overlap_area < MIN_SPLIT_PIECE_AREA_M2:
                continue
        ...
        for other_piece in local_signature_competitors:
            ...
            other_quality = float(
                ...
                else (target_scores_by_id.get(other_target.element_id, {}).get("retention_support_score") or 0.0)
            )
            competitor_score = (overlap_area / max(region_area, 1e-9)) * other_quality
            if competitor_score > best_competitor_score + 1e-6:
                best_competitor_score = competitor_score
    ...
    occupied_piece = unary_union(occupied_intersections) if occupied_intersections else piece_poly
    ...
    competition_class_target_ids = sorted({other_piece.target_element_id for other_piece in signature_class})
    return {
        "uuid": piece.uuid,
        "story": piece.story,
        ...
        "local_competitor_loss_fraction": round(
            ...
            6,
        ) if crossed_room_area_m2 > 1e-9 else 0.0,
        "local_top_competitor_ids": [
            competitor_id
            for competitor_id, _count in competitor_counter.most_common(3)
        ],
        ...
        "chain_signature_id": _chain_signature_id(piece.uuid, piece.story, piece.chain_ids),
        ...
        "mirror_support_score": round(float(ridge_meta["best_score"]), 6) if ridge_meta.get("best_score") is not None else None,
        "creator_eave_proximity": round(float(ridge_meta["creator_eave_proximity"]), 6) if ridge_meta.get("creator_eave_proximity") is not None else None,
        "best_supported_chain_id": chain_support.chain_id if chain_support is not None else None,
        "best_supported_chain_score": round(float(chain_support.support_score), 6) if chain_support is not None else None,
        "best_supported_chain_height_residual_m": round(float(chain_support.height_residual_m), 6) if chain_support is not None and chain_support.height_residual_m is not None else None,
        "creator_source_room_ids": list(target_diag.get("creator_source_room_ids") or []),
...
def diagnose_ridge_eave_piece_ownership(
    ...
    supported_ridge_eave_pieces_by_signature: dict[tuple[int, tuple[str, ...]], list[TargetSplitPieceRecord]] = defaultdict(list)
    for piece in split_pieces:
        if piece.target_kind != "ridge_eave_plane_group" or piece.piece_role != "supported":
            continue
        signature = _chain_signature(piece.chain_ids)
    ...
    ridge_eave_meta_by_target = _ridge_eave_plane_group_meta_by_target(str(building["uuid"]), ridge_eave_entry)
...
def annotate_ridge_eave_rows_with_creator_source_overlap(
    ...
    for uuid, building in buildings_by_uuid.items():
        ...
        building_part_graph = (roof_results_by_uuid.get(uuid) or {}).get("building_part_graph") or {}
    ...
    creator_union_cache: dict[tuple[str, tuple[str, ...]], tuple[Any | None, Any | None, tuple[str, ...]]] = {}
    ...
    for row in split_piece_rows:
        ...
        creator_source_room_ids = tuple(
            sorted(str(room_id) for room_id in (row.get("creator_source_room_ids") or []) if room_id)
        )
        piece_poly = _row_piece_polygon(row)
        if not uuid or not creator_source_room_ids or piece_poly is None or piece_poly.is_empty:
        ...
        if cached is None:
            ...
            creator_part_polys = [
                part_unions.get(part_id)
                for part_id in creator_part_ids
            ]
            creator_part_polys = [
                poly
        ...
        if creator_room_union is not None and not getattr(creator_room_union, "is_empty", True):
            try:
                room_overlap_area = float(piece_poly.intersection(creator_room_union).area)
            except Exception:
                room_overlap_area = 0.0
        part_overlap_area = 0.0
        if creator_part_union is not None and not getattr(creator_part_union, "is_empty", True):
            try:
                part_overlap_area = float(piece_poly.intersection(creator_part_union).area)
            except Exception:
                part_overlap_area = 0.0
        ...
        out["creator_source_room_overlap_fraction"] = round(room_overlap_area / piece_area, 6)
        out["creator_source_part_overlap_area_m2"] = round(part_overlap_area, 6)
        out["creator_source_part_overlap_fraction"] = round(part_overlap_area / piece_area, 6)
        annotated_rows.append(out)
    ...
    *,
    ...
    for row in split_piece_rows:
        ...
        for other in supported_ridge_rows:
            if other is row or str(other.get("target_element_id") or "") == row_target_id:
                continue
            ...
            has_reciprocal_partner = (
                (other_plane_group_id is not None and row_partner_id == other_plane_group_id)
                or (row_plane_group_id is not None and other_partner_id == row_plane_group_id)
            )
        ...
        extra_partner_polys = [
            poly
            for idx, poly in enumerate(partner_polys)
            if idx != seam_partner_index
        ]
        try:
        ...
        clipped_area = float(getattr(clipped, "area", 0.0) or 0.0)
        original_area = max(float(piece_poly.area), 1e-9)
        if clipped.is_empty or clipped_area < MIN_SPLIT_PIECE_AREA_M2 or clipped_area >= 0.995 * original_area:
            trimmed_rows.append(dict(row))
            continue
        clipped_polys = _iter_polygons(clipped)
        if not clipped_polys:
        ...
        for poly_index, poly in enumerate(sorted(clipped_polys, key=lambda item: float(item.area), reverse=True)):
...
def _rounded_loop_3d(corners: list[list[float]], *, ndigits: int = 6) -> list[list[float]]:
    rounded: list[list[float]] = []
    for corner in corners:
        if len(corner) < 3:
            continue
        point = [round(float(corner[0]), ndigits), round(float(corner[1]), ndigits), round(float(corner[2]), ndigits)]
...
def _committed_supported_piece_hypothesis_metadata(
    ...
    for uuid, roof_result in roof_results_by_uuid.items():
        ...
        for hypothesis_id, part_ids in (building_part_graph.get("hypothesis_membership") or {}).items():
        ...
        roof_surfaces_by_uuid[uuid] = list((roof_result.get("roof_surfaces") or {}).get("oblique") or [])
    ...
    for row in split_piece_rows:
        ...
        if target_index is None or target_index < 0 or target_index >= len(roof_surfaces):
            continue
        ...
        part_ids = [
            str(part_id)
            for part_id in (
                ((roof_results_by_uuid.get(uuid) or {}).get("building_part_graph") or {})
        ...
        misaligned = False
        if poly is not None and not poly.is_empty and part_union is not None and not part_union.is_empty:
            try:
...
def _committed_target_hypothesis_metadata(
    ...
    for target in committed_targets:
        ...
        if target_index < 0 or target_index >= len(roof_surfaces):
            continue
        hypothesis_id = str((roof_surfaces[target_index] or {}).get("roof_hypothesis_id") or "")
        if not hypothesis_id:
            continue
        part_ids = [
            str(part_id)
            for part_id in (building_part_graph.get("hypothesis_membership") or {}).get(hypothesis_id, [])
            if part_id
        ]
        ...
        if part_union is not None and not part_union.is_empty:
            try:
                ...
                overlap_area = 0.0
            overlap_fraction = round(overlap_area / max(float(target.poly_xz.area), 1e-9), 6)
            misaligned = overlap_area <= 1e-9
...
def build_target_face_runs(
    ...
    committed_targets = [target for target in targets if target.target_kind == "committed_oblique"]
    target_metadata_by_id = _committed_target_hypothesis_metadata(committed_targets, building, roof_result)
        ...
        run_entries.append(
            {
                ...
                "hypothesis_ids": {
                    str(metadata.get("roof_hypothesis_id") or "")
                }
    ...
    for ridge_target in ridge_eave_targets:
        ...
        for committed_target in committed_targets:
            ...
            if bool(metadata.get("hypothesis_part_misaligned")):
                continue
            if not _targets_are_local_ownership_competitors(ridge_target, committed_target):
                continue
            try:
            ...
            ridge_y = _plane_y_at(ridge_target, float(overlap_point.x), float(overlap_point.y))
            committed_y = _plane_y_at(committed_target, float(overlap_point.x), float(overlap_point.y))
            ...
            if best_key is None or score_key > best_key:
                best_key = score_key
                best_index = run_index_by_committed_target_id.get(committed_target.element_id)
    ...
    for entry in run_entries:
        ...
        try:
            core_union = unary_union(entry["core_polys"]) if entry["core_polys"] else None
        except Exception:
            core_union = None
        face_runs.append(
            FaceRunSeedRecord(
                uuid=str(entry["uuid"]),
                face_run_id=_face_run_id(str(entry["uuid"]), committed_target_ids, ridge_target_ids),
                story=int(entry["story"]),
                azimuth_deg=float(entry["azimuth_deg"]) if entry["azimuth_deg"] is not None else None,
                inclination_deg=float(entry["inclination_deg"]) if entry["inclination_deg"] is not None else None,
                member_target_ids=tuple(sorted(set(committed_target_ids).union(ridge_target_ids))),
                ...
                hypothesis_ids=tuple(sorted(value for value in entry["hypothesis_ids"] if value)),
                hypothesis_part_ids=tuple(sorted(value for value in entry["hypothesis_part_ids"] if value)),
                core_target_union=core_union if core_union is not None and not getattr(core_union, "is_empty", True) else None,
            )
        )
...
def build_face_runs(
    ...
    for uuid, building_rows in grouped.items():
        committed_rows: list[tuple[dict[str, Any], Polygon, tuple[tuple[float, float, float], np.ndarray] | tuple[None, None]]] = []
        ...
        for row, poly, _plane_model in committed_rows:
            ...
            hypothesis_ids = tuple(
                value
                for value in [str(row.get("roof_hypothesis_id") or "")]
                if value
            )
            hypothesis_part_ids = tuple(
                sorted({str(part_id) for part_id in (row.get("hypothesis_part_ids") or []) if part_id})
            )
            run_entries.append(
                {
                    "uuid": uuid,
                    ...
                    "committed_rows": [row],
                    "ridge_rows": [],
                    "core_committed_rows": [] if bool(row.get("hypothesis_part_misaligned")) else [row],
                    "core_polys": [] if bool(row.get("hypothesis_part_misaligned")) else [poly],
        ...
        for row in building_rows:
            ...
            best_index = None
            best_key: tuple[float, float] | None = None
            for index, (committed_row, committed_poly, committed_plane) in enumerate(committed_rows):
                ...
                if overlap.is_empty or float(overlap.area) < MIN_SPLIT_PIECE_AREA_M2:
                    continue
                overlap_point = overlap.representative_point()
                row_y = _row_plane_y_at(row_plane, float(overlap_point.x), float(overlap_point.y))
                committed_y = _row_plane_y_at(committed_plane, float(overlap_point.x), float(overlap_point.y))
        ...
        for entry in run_entries:
            committed_piece_ids = tuple(
                sorted({str(row.get("piece_id") or "") for row in entry["committed_rows"] if row.get("piece_id")})
            )
            ridge_piece_ids = tuple(
                sorted({str(row.get("piece_id") or "") for row in entry["ridge_rows"] if row.get("piece_id")})
            )
            ...
            face_runs.append(
                FaceRunRecord(
                    uuid=uuid,
                    face_run_id=_face_run_id(uuid, committed_piece_ids, ridge_piece_ids),
                    story=int(entry["story"]),
                    azimuth_deg=float(entry["azimuth_deg"]) if entry["azimuth_deg"] is not None else None,
                    inclination_deg=float(entry["inclination_deg"]) if entry["inclination_deg"] is not None else None,
                    member_piece_ids=tuple(sorted(set(committed_piece_ids).union(ridge_piece_ids))),
                    ...
                    core_union=core_union if core_union is not None and not getattr(core_union, "is_empty", True) else None,
                )
            )
    return face_runs
...
def resolve_split_piece_rows_with_face_runs(
    ...
    for row in split_piece_rows:
        ...
        def _annotate_face_run(record: dict[str, Any], *, role: str) -> dict[str, Any]:
            annotated = dict(record)
            annotated["face_run_id"] = face_run.face_run_id
            annotated["face_run_role"] = role
            annotated["face_run_committed_target_ids"] = list(face_run.core_committed_target_ids)
            annotated["face_run_hypothesis_ids"] = list(face_run.hypothesis_ids)
            annotated["face_run_hypothesis_part_ids"] = list(face_run.hypothesis_part_ids)
            return annotated
        ...
        if (
            ...
            merge_fraction = max(0.0, min(1.0, 1.0 - residual_area / max(float(piece_poly.area), 1e-9)))
            for poly_index, poly in enumerate(sorted(residual_polys, key=lambda item: float(item.area), reverse=True)):
                record = _row_piece_records_from_polygon(
                    row,
                    poly,
                    piece_id=piece_id if poly_index == 0 else f"{piece_id}:{poly_index}",
                ...
                annotated["same_plane_committed_core_fraction"] = round(merge_fraction, 6)
                annotated["same_plane_committed_core_target_ids"] = list(face_run.core_committed_target_ids[:3])
                resolved_rows.append(annotated)
            continue

        role = "committed_core" if piece_id in face_run.core_committed_piece_ids else (
            "committed_misaligned" if piece_id in face_run.committed_piece_ids else "ridge_run"
        )
...
def resolve_split_piece_rows_with_target_face_runs(
    ...
    for row in split_piece_rows:
        ...
        def _annotate(record: dict[str, Any], *, role: str) -> dict[str, Any]:
            annotated = dict(record)
            annotated["face_run_id"] = face_run.face_run_id
            annotated["face_run_role"] = role
            annotated["face_run_committed_target_ids"] = list(face_run.core_committed_target_ids)
...
def _chain_signature_id(uuid: str, story: int, chain_ids: Any) -> str | None:
    ...
    if not signature:
        return None
    digest = hashlib.blake2b(repr((uuid, story, signature)).encode("utf-8"), digest_size=6).hexdigest()
...
def _targets_are_local_ownership_competitors(
    ...
    azimuth_delta = _wrapped_angle_delta_deg(subject_target.azimuth_deg, other_target.azimuth_deg)
    if azimuth_delta is None or azimuth_delta > RIDGE_EAVE_OWNERSHIP_MAX_AZIMUTH_DELTA_DEG:
        return False
    inclination_delta = abs(float(subject_target.inclination_deg) - float(other_target.inclination_deg))
...
def _rows_share_ridge_eave_competition_class(
    ...
    if (
        ...
        return True
    relation = _chain_signature_relation(subject_row.get("chain_ids"), other_row.get("chain_ids"))
    if relation is None:
    ...
    for (uuid, _story), story_rows in grouped.items():
        ...
        for row in story_rows:
            ...
            stronger_rows = [
                ...
                and not (
                    ...
                    and (
                        (
                            bool(row_partner_id)
                            and bool(_ridge_eave_target_to_plane_group_id(str(other.get("target_element_id") or "")))
                            and row_partner_id == _ridge_eave_target_to_plane_group_id(str(other.get("target_element_id") or ""))
                        )
                        or (
                            bool(str(other.get("mirror_partner_plane_group_id") or ""))
                            and bool(row_plane_group_id)
                            and str(other.get("mirror_partner_plane_group_id") or "") == row_plane_group_id
                        )
                    )
                )
            ]
            ...
            for other in stronger_rows:
                ...
                covering_ids.append((str(other["target_element_id"]), float(overlap.area)))

            if cover_parts:
                try:
                    higher_union = unary_union(cover_parts)
                    cover_fraction = float(poly.intersection(higher_union).area) / max(float(poly.area), 1e-9)
                except Exception:
                    cover_fraction = 0.0
            else:
                cover_fraction = 0.0
            ...
            if row_kind == "ridge_eave_plane_group":
                ...
                row_plane_group_id = _ridge_eave_target_to_plane_group_id(str(row.get("target_element_id") or ""))
                row_partner_id = str(row.get("mirror_partner_plane_group_id") or "")
                for other in building_rows:
                    ...
                    if other_kind in {"committed_oblique", "candidate_oblique"}:
                        ...
                        roof_surface_cover_ids.append((str(other["target_element_id"]), float(overlap.area)))
                        if other_kind == "committed_oblique":
                            committed_cover_parts.append(other_poly)
                            committed_cover_ids.append((str(other["target_element_id"]), float(overlap.area)))
                            local_roof_cover_parts.append(other_poly)
                            local_roof_cover_ids.append((str(other["target_element_id"]), float(overlap.area)))
                        continue
                    ...
                    if _piece_ownership_rank(other) > row_rank:
                        ...
                        relation = _chain_signature_relation(row.get("chain_ids"), other.get("chain_ids"))
                        if (
                            azimuth_delta is not None
                            and azimuth_delta <= RIDGE_EAVE_OWNERSHIP_MAX_AZIMUTH_DELTA_DEG
                            and relation == "subset"
                            and not str(other.get("mirror_partner_plane_group_id") or "")
                        ):
                            same_side_superset_cover_parts.append(other_poly)
                            same_side_superset_cover_ids.append(
                                (str(other["target_element_id"]), float(overlap.area))
                            )
                    ...
                    if _rows_are_local_ownership_competitors(row, other):
                        continue
                    other_plane_group_id = _ridge_eave_target_to_plane_group_id(str(other.get("target_element_id") or ""))
                    other_partner_id = str(other.get("mirror_partner_plane_group_id") or "")
                    if not (
                        (other_plane_group_id is not None and row_partner_id == other_plane_group_id)
                        or (row_plane_group_id is not None and other_partner_id == row_plane_group_id)
            ...
            if committed_cover_parts:
                try:
                    ...
                    committed_cover_fraction = float(poly.intersection(committed_union).area) / max(float(poly.area), 1e-9)
                except Exception:
            ...
            if roof_surface_cover_parts:
                try:
                    ...
                    roof_surface_cover_fraction = float(poly.intersection(roof_surface_union).area) / max(
            ...
            if local_roof_cover_parts:
                try:
                    ...
                    local_roof_cover_fraction = float(poly.intersection(local_roof_union).area) / max(float(poly.area), 1e-9)
                except Exception:
            ...
            if same_side_superset_cover_parts:
                try:
                    same_side_superset_union = unary_union(same_side_superset_cover_parts)
                    same_side_superset_cover_fraction = float(poly.intersection(same_side_superset_union).area) / max(
                        float(poly.area), 1e-9
                    )
                except Exception:
            ...
            signature_id = _chain_signature_id(str(row.get("uuid") or ""), int(row.get("story") or 0), row.get("chain_ids"))
            ...
            annotated["roof_surface_cover_fraction"] = round(roof_surface_cover_fraction, 6)
            annotated["roof_surface_covering_target_ids"] = [
            ...
            annotated["same_side_superset_cover_fraction"] = round(same_side_superset_cover_fraction, 6)
            annotated["same_side_superset_covering_target_ids"] = [
                target_id for target_id, _area in same_side_superset_cover_ids[:3]
            ]
            annotated["ownership_redundant"] = (
                cover_fraction >= 0.98
                or committed_cover_fraction >= RIDGE_EAVE_COMMITTED_COVER_REDUNDANT_MIN_FRACTION
                or roof_surface_cover_fraction >= RIDGE_EAVE_COMMITTED_COVER_REDUNDANT_MIN_FRACTION
        ...
        for row in story_rows:
            ...
            for other in building_rows:
                ...
                support_covering_target_ids.append(
                    (str(other.get("target_element_id") or ""), overlap_area)
                )
    ...
    * ridge_eave_plane_group rows are classified per supported piece first, then
    ...
    for row in split_piece_rows:
        ...
        if target_kind == "ridge_eave_plane_group":
            ...
            strict_disjoint_extension_bypass = (
                source_crossed_disjoint
                and str(row.get("provenance_relevance_flag") or "") == "suspect_interior_slice"
            ...
            if competitor_loss is not None:
                try:
                    owns_local_rooms = float(competitor_loss) <= RIDGE_EAVE_FINAL_MAX_LOCAL_COMPETITOR_LOSS
                except (TypeError, ValueError):
                    owns_local_rooms = False
            mirror_score = row.get("mirror_support_score")
            anchored_suspect = (
                str(row.get("provenance_relevance_flag") or "") == "suspect_interior_slice"
                and mirror_score is not None
            )
            if anchored_suspect:
                try:
                    anchored_suspect = float(mirror_score) >= RIDGE_EAVE_SUSPECT_ANCHOR_MIN_MIRROR_SCORE
                except (TypeError, ValueError):
            ...
            area_fraction_of_target_max = (
                _piece_numeric_value(row, "area_xz_m2", 0.0) / max(max_supported_area, 1e-9)
                if max_supported_area > 1e-9
                else 0.0
            )
            is_mirror_sliver = (
                max_supported_area > 1e-9
                and area_fraction_of_target_max <= RIDGE_EAVE_MIRROR_SLIVER_MAX_AREA_FRACTION
                and _piece_numeric_value(row, "through_ratio", 0.0) >= RIDGE_EAVE_MIRROR_SLIVER_MIN_THROUGH_RATIO
                and mirror_score is not None
            )
    ...
    for rows in ridge_rows_by_target.values():
        positive_overlap_rows = [
            row
            for row in rows
            if _piece_numeric_value(row, "creator_source_part_overlap_area_m2", 0.0) >= MIN_SPLIT_PIECE_AREA_M2
        ]
        ...
        has_positive_final = any(
            ...
            and _piece_numeric_value(row, "creator_source_part_overlap_area_m2", 0.0) >= MIN_SPLIT_PIECE_AREA_M2
            for row in rows
        )
    ...
    committed_rows_by_target: dict[tuple[str, str], list[dict[str, Any]]] = defaultdict(list)
    for row in classified_rows:
        if str(row.get("target_kind") or "") != "committed_oblique":
            continue
        if str(row.get("piece_role") or "") != "supported":
        ...
        if not target_element_id:
            continue
        committed_rows_by_target[(str(row.get("uuid") or ""), target_element_id)].append(row)
...
def prune_ridge_eave_rows_with_unreliable_mirror_pairs(
    ...
    for row in split_piece_rows:
        ...
        partner_rows = [
            candidate
            for candidate in rows_by_plane_group_id.get(mirror_partner_plane_group_id, [])
        ...
        is_redundant_candidate = (
            not is_final_layer and bool(row.get("ownership_redundant"))
        )
...
def score_target(
    ...
    if matches and target.area_xz_m2 > 1e-9:
        try:
            ...
            raw_xz_coverage = float(covered.area) / target.area_xz_m2 if not covered.is_empty else 0.0
        except Exception:
            raw_xz_coverage = 0.0
        raw_room_trust_mean = float(np.mean([record.room_trust_score for record in matches]))
        normal_dots = [abs(float(np.dot(_normalize_roof_up(target.normal), _normalize_roof_up(record.normal)))) for record in matches]
        raw_normal_dot_p50 = float(np.median(normal_dots)) if normal_dots else None
        azimuth_deltas = [
            delta
            for record in matches
            for delta in [_wrapped_angle_delta_deg(target.azimuth_deg, record.azimuth_deg)]
            if delta is not None
        ]
        inclination_deltas = [abs(float(target.inclination_deg) - float(record.inclination_deg)) for record in matches]
        raw_azimuth_delta_p50_deg = float(np.median(azimuth_deltas)) if azimuth_deltas else None
        raw_inclination_delta_p50_deg = float(np.median(inclination_deltas)) if inclination_deltas else None
    ...
    supported_chain_supports = [support for support in chain_supports if support.supported]
    best_chain = max(chain_supports, key=lambda support: support.support_score, default=None)
    ...
    return {
        ...
        "raw_normal_dot_p50": round(raw_normal_dot_p50, 6) if raw_normal_dot_p50 is not None else None,
        "raw_azimuth_delta_p50_deg": round(raw_azimuth_delta_p50_deg, 6) if raw_azimuth_delta_p50_deg is not None else None,
        "raw_inclination_delta_p50_deg": round(raw_inclination_delta_p50_deg, 6) if raw_inclination_delta_p50_deg is not None else None,
        ...
        "best_eave_chain_support_score": round(best_chain.support_score, 6) if best_chain is not None else None,
        "conflicting_raw_pair_count": conflicting_raw_pair_count,
...
def _summarize_story_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for (uuid, story), story_rows in sorted(grouped.items()):
        candidate_rows = [row for row in story_rows if row["target_kind"] == "candidate_oblique"]
        committed_rows = [row for row in story_rows if row["target_kind"] == "committed_oblique"]
        top_retention = sorted(
            story_rows,
            key=lambda row: (float(row["retention_support_score"]), str(row["element_id"])),
            reverse=True,
        )[:3]
        top_orientation = sorted(
            story_rows,
            key=lambda row: (float(row["orientation_support_score"]), str(row["element_id"])),
            reverse=True,
        )[:3]
        summary_rows.append(
            {
                ...
                "split_target_count": sum(1 for row in story_rows if bool(row["split_flag"])),
...
def _summary_payload(rows: list[dict[str, Any]], per_story_rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    candidate_vs_committed_by_building: dict[str, list[dict[str, Any]]] = defaultdict(list)
    ...
    for (uuid, story), story_rows in rows_by_story.items():
        ...
        if not candidate_rows or not committed_rows:
            continue
        candidate_mean = float(np.mean([float(row["retention_support_score"]) for row in candidate_rows]))
        committed_mean = float(np.mean([float(row["retention_support_score"]) for row in committed_rows]))
    ...
    return {
        "thresholds": {
            ...
            "low_trust_promotion_min_raw_overlap_fraction": LOW_TRUST_PROMOTION_MIN_RAW_OVERLAP_FRACTION,
            "low_trust_promotion_min_normal_dot": LOW_TRUST_PROMOTION_MIN_NORMAL_DOT,
            "low_trust_promotion_max_height_residual_m": LOW_TRUST_PROMOTION_MAX_HEIGHT_RESIDUAL_M,
            "min_gap_continuation_overlap_m2": MIN_GAP_CONTINUATION_OVERLAP_M2,
            "min_gap_continuation_overlap_fraction": MIN_GAP_CONTINUATION_OVERLAP_FRACTION,
        ...
        "counts": {
            "n_targets": len(rows),
            "n_candidate_targets": sum(1 for row in rows if row["target_kind"] == "candidate_oblique"),
            "n_committed_targets": sum(1 for row in rows if row["target_kind"] == "committed_oblique"),
        ...
        "top_buildings_where_committed_scores_worse_than_candidate": [
            {
                ...
                "mean_gap": round(float(np.mean([float(entry["gap"]) for entry in stories])), 6),
                "stories": sorted(stories, key=lambda entry: (-float(entry["gap"]), int(entry["story"]))),
            }
...
def score_buildings(
    ...
    for building in buildings:
        ...
        raw_records = collect_raw_plane_records(building, roof_result, exposed_only=True)
        ...
        split_target_score_rows = [
            score_target(target, raw_records, raw_edges, conflicts, plane_chain_supports)
            for target in split_targets
        ]
        ...
        plane_chain_rows.extend(
            {
                ...
                "height_residual_m": round(support.height_residual_m, 6) if support.height_residual_m is not None else None,
        ...
        split_piece_rows.extend(
            {
                ...
                "target_azimuth_deg": round(target.azimuth_deg, 6),
                "target_inclination_deg": round(target.inclination_deg, 6),
                "support_score": round(piece.support_score, 6) if piece.support_score is not None else None,
            ...
            for piece in split_pieces
            for target in [next(t for t in split_targets if t.element_id == piece.target_element_id)]
        )
        if split_pieces:
            split_piece_rows[-len(split_pieces):] = annotate_rows_with_target_face_runs(
                split_piece_rows[-len(split_pieces):],
                target_face_annotations,
            )
        rows.extend(
            split_target_score_by_id[str(target.element_id)]
            for target in targets
        )
    ...
    summary["counts"]["n_suspect_interior_slice_split_pieces"] = sum(
        1 for row in split_piece_rows if row.get("provenance_relevance_flag") == "suspect_interior_slice"
    )
    ...
    summary["counts"]["n_ridge_eave_supported_piece_ownership_rows"] = len(ownership_rows)
    summary["counts"]["n_ridge_eave_piece_competitor_losses"] = sum(
        1 for row in ownership_rows if float(row.get("local_competitor_loss_area_m2") or 0.0) > 0.0
    )
    buildings_by_uuid = {
    ...
    split_piece_rows = merge_split_piece_rows_with_ownership(split_piece_rows, ownership_rows)
    split_piece_rows = annotate_committed_supported_pieces_with_hypothesis_part_overlap(
        split_piece_rows,
        buildings_by_uuid,
        roof_results,
    )
    split_piece_rows = resolve_split_piece_rows_with_target_face_runs(split_piece_rows, face_run_seeds)
    ...
    split_piece_rows, mirror_prune_diag = prune_ridge_eave_rows_with_unreliable_mirror_pairs(
        split_piece_rows
    )
    ...
    return rows, per_story_rows, chain_rows, plane_chain_rows, split_piece_rows, ownership_rows, summary
...
def _shadow_diff_payload(
    ...
    return {
        ...
        "schema": {
            "v1_only_target_fields": sorted(set(v1_target_fields) - set(v2_target_fields)),
            "v2_only_target_fields": sorted(set(v2_target_fields) - set(v1_target_fields)),
...
def main() -> None:
    ...
    parser.add_argument("--ridge-eave-scores", type=Path, default=RIDGE_EAVE_SCORES_PATH)
    ...
    if args.uuid:
        buildings = [building for building in buildings if str(building.get("uuid")) == args.uuid]
        if not buildings:
