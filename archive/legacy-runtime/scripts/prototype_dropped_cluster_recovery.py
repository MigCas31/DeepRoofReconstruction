...
def plane_normal(azimuth_deg: float, incl_deg: float) -> np.ndarray:
    ...
    return np.array([math.sin(az) * math.sin(inc), math.cos(inc), math.cos(az) * math.sin(inc)])
...
def project_on_ridge(points: np.ndarray, az_deg: float, ref: np.ndarray) -> tuple[float, float]:
...
def project_on_slope(points: np.ndarray, az_deg: float, ref: np.ndarray) -> tuple[float, float]:
    az = math.radians(az_deg)
...
def evaluate_uuid(uuid: str, rec: dict) -> dict:
    ...
    for o in obliques:
        cl = o.get("cluster") or {}
        committed_keys.add((round(float(cl.get("avgAzimuth") or 0), 2), round(float(cl.get("avgIncl") or 0), 2)))
    ...
    for ci, cl in enumerate(clusters):
        ...
        for fp in fragment_planes:
            ...
            if d_angle <= PLANE_NORMAL_TOL_DEG and d_offset <= PLANE_OFFSET_TOL_M:
                matched_fragments.append({**fp, "delta_angle": d_angle, "delta_offset": d_offset})

        matched_pts = (
            np.concatenate([m["pts"] for m in matched_fragments]) if matched_fragments else np.zeros((0, 3))
        )
        if matched_pts.size:
            frag_min_r, frag_max_r = project_on_ridge(matched_pts, slope["az"], slope["ref"])
            frag_ridge_extent = frag_max_r - frag_min_r
            frag_min_s, frag_max_s = project_on_slope(matched_pts, slope["az"], slope["ref"])
    ...
    for c in cluster_reports:
        c["dominance_pct"] = (
            round(100.0 * c["matched_area_m2"] / total_matched_area, 1) if total_matched_area > 0 else 0.0
        )
    ...
    return {
        ...
        "fragments_total_area_m2": round(sum(f.get("area_m2") or 0 for f in fragments), 2),
...
def main() -> int:
    ...
    for uuid, r in sorted(reports.items()):
        committed = [c for c in r["clusters"] if c["currently_committed"]]
        dropped = [c for c in r["clusters"] if not c["currently_committed"]]
        recoverable = [c for c in dropped if c["would_pass_gate"]]
        print(f"\n{uuid}  fragments={r['fragment_count']} (matched_area={r['fragments_matched_area_m2']:.1f}/{r['fragments_total_area_m2']:.1f} m²)")
        for c in r["clusters"]:
            tag = "COMMITTED" if c["currently_committed"] else ("RECOVERABLE" if c["would_pass_gate"] else "STILL DROPPED")
            print(
                f"  [{c['cluster_index']}] az={c['az']:>6.2f} incl={c['incl']:>5.2f}  "
                f"segs={c['segs_count']:>1}  seg_ridge={c['segments_ridge_extent_m']:>5.2f}m  "
                f"frag_match={c['matched_fragment_count']:>2} (area={c['matched_area_m2']:>5.1f} m², "
