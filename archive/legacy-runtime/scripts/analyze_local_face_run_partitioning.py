...
def _summarize_target_signatures(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        signature = tuple(sorted(str(chain_id) for chain_id in (row.get("chain_ids") or [])))
        if not signature:
            continue
...
def analyze_local_face_run_partitioning(
    *,
    ...
    multi_partner = analyze_multi_partner_plane_groups(buildings, roof_results, ridge_eave_scores)
    cohort_uuids = {str(row["uuid"]) for row in multi_partner["buildings"]}
    buildings_subset = [building for building in buildings if str(building.get("uuid")) in cohort_uuids]
    ridge_by_uuid = {
    ...
    for building_row in multi_partner["buildings"]:
        ...
        subpart_count = len(
            (
                (roof_results.get(uuid) or {}).get("roof_coverage_graph") or {}
            ).get("subparts")
            or []
        )
        for node in building_row["multi_partner_plane_groups"]:
            involved_plane_group_ids = [
                ...
                *[str(plane_group_id) for plane_group_id in node["neighbor_plane_group_ids"]],
            ]
            target_summaries: list[dict[str, Any]] = []
            for plane_group_id in involved_plane_group_ids:
                target_token = _plane_group_token(plane_group_id)
                signature_rows = _summarize_target_signatures(split_index[uuid].get(target_token, []))
            ...
            for target in target_summaries:
                ...
                for idx, sig_a in enumerate(signatures):
                    for sig_b in signatures[idx + 1 :]:
                        within_target_signature_relation_counts[_signature_relation(sig_a, sig_b)] += 1
                ...
                for target_b in target_summaries[idx + 1 :]:
                    ...
                    for sig_a in signatures_a:
                        for sig_b in signatures_b:
                            across_target_signature_relation_counts[_signature_relation(sig_a, sig_b)] += 1

            case, details = _classify_node_partition_case(target_summaries)
            case_counts[case] += 1
            case_counts_by_arch_context[str(building_row["architectural_context"])][case] += 1
            node_rows.append(
                {
                    "uuid": uuid,
                    ...
                    "neighbor_plane_group_ids": [str(plane_group_id) for plane_group_id in node["neighbor_plane_group_ids"]],
                    "partition_case": case,
        ...
        examples_by_case[case].append(
            {
                ...
                "multi_signature_plane_group_ids": row["multi_signature_plane_group_ids"],
    ...
    return {
        "summary": {
            ...
            "within_target_signature_relation_counts": dict(within_target_signature_relation_counts),
            "across_target_signature_relation_counts": dict(across_target_signature_relation_counts),
