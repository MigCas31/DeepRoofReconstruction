    ...
    out.update(
        {
            ...
            "crosses_main_extension_boundary": bool(context.crosses_main_extension_boundary),
        }
    )
    return out
...
def score_buildings_v2(
    ...
    for building in buildings:
        ...
        graph = build_plane_relation_graph(split_targets, contexts_by_target, config.relation)
        ...
        for support in chain_bundle.plane_chain_supports:
            if not support.supported:
                continue
            supports_by_key[(str(support.target_element_id), str(support.chain_id))] = support

        piece_rows = [_attach_context_fields(row, contexts_by_target.get(str(row.get("target_element_id") or ""))) for row in piece_rows]
        piece_rows = [_attach_piece_anchor_fields(row, supports_by_key) for row in piece_rows]
        piece_rows = annotate_split_rows_with_ownership(piece_rows, graph, building, roof_result)
        ...
        target_rows = [_attach_context_fields(row, contexts_by_target.get(str(row.get("element_id") or ""))) for row in target_rows]
...
def score_building_v2(
    ...
    return BuildingResult(
        building_uuid=uuid,
        per_target_rows=[row for row in corpus.per_target_rows if str(row.get("uuid") or "") == uuid],
        per_story_rows=[row for row in corpus.per_story_rows if str(row.get("uuid") or "") == uuid],
        split_piece_rows=[row for row in corpus.split_piece_rows if str(row.get("uuid") or "") == uuid],
