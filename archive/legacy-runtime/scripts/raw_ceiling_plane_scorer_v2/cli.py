...
def main() -> None:
    parser = argparse.ArgumentParser(description="Relation-first raw ceiling plane scorer (V2).")
    ...
    parser.add_argument("--ridge-eave-scores", type=Path, default=legacy.RIDGE_EAVE_SCORES_PATH)
    ...
    config = ScorerV2Config(
        global_selection=GlobalSelectionConfig(enabled=bool(args.enable_global_selection_ilp))
    )
