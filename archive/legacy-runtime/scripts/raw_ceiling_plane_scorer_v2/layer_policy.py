...
def _covering_relations_for_target(target_id: str, graph: RelationGraph) -> list:
    return [
        ...
        if relation.relation_kind == "covering" and relation.covered_target_id == target_id
    ]
...
def _is_anchor_valid(row: dict, context, cfg: LayerPolicyConfig) -> bool:
    ...
    if piece_anchor_count is not None and int(piece_anchor_count) > 0 and not multi_chain_mirror_pair:
...
def _ridge_anchor_valid_for_poly(
    ...
    if float(getattr(poly, "area", 0.0) or 0.0) < cfg.ridge_post_clip_overlap_check_min_area_m2:
        return True

    try:
        chain_mask = line_union.buffer(cfg.ridge_post_clip_source_edge_buffer_m, cap_style=2)
    except Exception:
        return False
...
def _rescue_unanchored_ridge_coverage(rows: list[dict], cfg: LayerPolicyConfig) -> list[dict]:
    rescue_indexes = [
        index
        for index, row in enumerate(rows)
        if str(row.get("target_kind") or "") == "ridge_eave_plane_group"
        and str(row.get("piece_role") or "") == "supported"
        and str(row.get("final_layer_reason") or "") == "ridge_eave_post_clip_unanchored"
    ...
    visible_polys = [
        poly for poly in visible_polys if poly is not None and not getattr(poly, "is_empty", True)
    ]
    ...
    for row_index in rescue_indexes:
        ...
        for overlap_area, peer_index in peer_indexes:
            ...
            if abs(peer_y - visible_reference_y) <= cfg.ridge_post_clip_rescue_max_visible_y_delta_m:
                in_band_peer_indexes.append((overlap_area, peer_index))
        if in_band_peer_indexes:
            peer_indexes = in_band_peer_indexes

        def _peer_rank(item: tuple[float, int]) -> tuple[float, float, float, float, str]:
        ...
        if not bool(best_row.get("overlay_suppressed")):
            ...
            if best_poly is not None and not best_poly.is_empty:
                try:
                    visible_union = best_poly if visible_union is None else unary_union([visible_union, best_poly])
                except Exception:
                    pass
...
def _apply_xy_overlap_resolution(
    ...
    *,
    ...
    residual_polys = _iter_large_polygons(residual_geom, cfg.xy_conflict_min_residual_area_m2)
    ...
    first_corner = next((corner for corner in (row.get("corners") or []) if len(corner) >= 3), None)
...
def _promote_ridge_eave_mirror_rescued_residuals(rows: list[dict]) -> list[dict]:
    ...
    for row in rows:
        ...
        if str(row.get("piece_role") or "") != "supported":
            continue
        if str(row.get("final_layer_reason") or "") not in _RIDGE_EAVE_ACCEPTED_OWNER_REASONS:
            continue
...
def _enforce_final_xy_disjointness(rows: list[dict], cfg: LayerPolicyConfig) -> list[dict]:
    final_indexes = [
        index
        for index, row in enumerate(rows)
        if bool(row.get("final_layer")) and not bool(row.get("overlay_suppressed"))
    ...
    final_indexes.sort(
        key=lambda index: (
            ...
            str(rows[index].get("piece_id") or rows[index].get("target_element_id") or ""),
        ),
        reverse=True,
    )
...
def classify_split_piece_rows(
    ...
    for target_id, rows in committed_supported_by_target.items():
        ...
        committed_owner_piece_by_target[target_id] = str(owner_row.get("piece_id") or "")
    ...
    for row in split_piece_rows:
        ...
        if target_kind == "committed_oblique":
            ...
            if is_owner:
                ...
                source_part_overlap = float(row.get("source_part_overlap_fraction") or 0.0)
        ...
        if target_kind == "ridge_eave_plane_group":
            ...
            # exterior roof face.
            if (
                float(row.get("creator_rain_area_fraction") or 0.0) == 0.0
                and not bool(row.get("creator_has_partner"))
            ):
                annotated["final_layer"] = False
            ...
            crossed_part_ids = set(context.crossed_part_ids) if context is not None else set()
            ...
            has_covering_relation = bool(_covering_relations_for_target(target_id, graph))
            has_local_competitor = bool(row.get("local_competitor_target_ids"))
            has_mirror_partner = bool(row.get("mirror_partner_target_ids"))

            if has_covering_relation and not disjoint_source_crossed and not through_building_owner:
                annotated["final_layer"] = False
                annotated["final_layer_reason"] = "ridge_eave_covered_by_relation"
                out.append(annotated)
                continue

            if has_local_competitor and not has_mirror_partner and not disjoint_source_crossed:
                annotated["final_layer"] = False
...
def _apply_same_face_source_suppression(
    ...
    for relation in graph.relations:
        ...
        if "candidate_oblique" in kinds_a and "committed_oblique" in kinds_b:
            candidate_same_face_to_committed[relation.a_target_id].add(relation.b_target_id)
        if "candidate_oblique" in kinds_b and "committed_oblique" in kinds_a:
            candidate_same_face_to_committed[relation.b_target_id].add(relation.a_target_id)

    for candidate_target_id, committed_target_ids in candidate_same_face_to_committed.items():
        ...
        for candidate_row in list(candidate_rows):
            ...
            for committed_target_id in committed_target_ids:
                ...
                if _element_kind(committed_target_id) != candidate_kind:
                    continue
                for committed_row in committed_winner_rows_by_target.get(committed_target_id, []):
                    ...
                    if overlap_fraction > best_overlap:
                        ...
                        best_committed_piece_id = str(committed_row.get("piece_id") or "")
            if best_overlap >= cfg.same_face_shadow_min_overlap_fraction:
                ...
                candidate_row["same_face_shadow_overlap_fraction"] = round(best_overlap, 6)
    ...
    for target_id, committed_rows in rows_by_target.items():
        ...
        if len(committed_supported) < 2:
            continue
        owner_rows = [row for row in committed_supported if bool(row.get("final_layer"))]
        demoted_rows = [
            row
        ...
        for demoted_row in list(demoted_rows):
            ...
            if best_overlap >= cfg.same_face_shadow_min_overlap_fraction:
                ...
                demoted_row["same_face_shadow_overlap_fraction"] = round(best_overlap, 6)
                extra_rows = _apply_xy_overlap_resolution(
                    demoted_row,
                    owner_cover_polys,
                    cfg,
...
def _chain_id_set(row: dict) -> set[str]:
    return {
        str(chain_id)
        for chain_id in (row.get("chain_ids") or [])
        if str(chain_id)
    }
...
def _apply_same_face_chain_subset_suppression(
    ...
    for target_id, target_rows in rows_by_target.items():
        ...
        if not candidate_rows:
            continue
        candidate_chain_ids = set().union(*(_chain_id_set(row) for row in candidate_rows))
    ...
    for relation in graph.relations:
        ...
        if (
            ...
            covering_owner_target_ids_by_candidate[covered_target_id].add(covering_target_id)
    ...
    for candidate_target_id, payload in suppression_by_candidate_target.items():
        ...
        for candidate_row in candidate_rows_by_target.get(candidate_target_id, []):
            ...
            candidate_row["same_face_shadow_piece_id"] = owner_best_piece_id_by_target.get(
                owner_target_id
            )
