...
def build_run_output(
    ...
    summary["counts"]["n_plane_extent_split_pieces_candidate"] = len(split_piece_rows) - final_count
    ...
    relation_sidecar = {
        ...
        "buildings": {
            uuid: {
                "contexts": [asdict(context) for context in graph.contexts_by_target.values()],
...
def write_outputs(out_dir: Path, output: RunOutput) -> None:
    ...
    (out_dir / "summary.json").write_text(json.dumps(output.summary, indent=2), encoding="utf-8")
