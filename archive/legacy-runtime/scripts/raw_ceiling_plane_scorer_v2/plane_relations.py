...
def _boundary_height_residual_m(
    ...
    try:
        ...
        return None
    if getattr(shared, "is_empty", True) or float(getattr(shared, "length", 0.0) or 0.0) <= 1e-9:
        return None
    ...
    for a, b in _pairwise_targets(targets):
        ...
        if overlap_m2 >= cfg.min_relation_overlap_m2:
            if (
                ...
                same_story
                and azimuth_delta is not None
                and cfg.mirror_min_azimuth_delta_deg <= azimuth_delta <= cfg.mirror_max_azimuth_delta_deg
            ):
        ...
        relations.append(
            PlaneRelation(
                ...
                azimuth_delta_deg=float(azimuth_delta) if azimuth_delta is not None else None,
                inclination_delta_deg=float(inclination_delta),
                height_residual_m=float(height_residual) if height_residual is not None else None,
