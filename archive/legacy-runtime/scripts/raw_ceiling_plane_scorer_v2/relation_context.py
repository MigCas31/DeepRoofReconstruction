...
def _main_part_ids(room_membership: dict[str, set[str]]) -> set[str]:
    ...
    candidates = sorted(part_id for part_id, count in counts.items() if count == max_count)
...
def _parts_for_poly(poly: Any, room_polys: dict[str, Any], room_membership: dict[str, set[str]]) -> set[str]:
    parts: set[str] = set()
    if poly is None or poly.is_empty:
        return parts
...
def _target_base_part_ids(
    ...
    if target.target_kind == "committed_oblique":
        ...
        building_part_ids.update(_parts_for_poly(target.poly_xz, room_polys_story, room_membership))
        source_part_ids.update(building_part_ids)

    if not building_part_ids:
        building_part_ids.update(_parts_for_poly(target.poly_xz, room_polys_all, room_membership))
...
def build_plane_contexts(
    ...
    supports_by_target: dict[str, list[legacy.PlaneEaveChainSupportRecord]] = defaultdict(list)
    ...
    for target in targets:
        ...
        source_edge_ids = tuple(sorted({str(support.chain_id) for support in supports if support.chain_id}))
        source_edge_overlap_m = float(sum(float(support.chain_length_m) for support in supports))
        source_edge_alignment_deg = _median([float(support.angle_delta_deg) for support in supports])
        ...
        for row in piece_rows_by_target.get(target.element_id, []):
            ...
            if piece_poly is None or piece_poly.is_empty:
                continue
            piece_story = int(row.get("story") if row.get("story") is not None else target.story)
        ...
        if not crossed_part_ids:
            crossed_part_ids.update(_parts_for_poly(target.poly_xz, room_polys_all, room_membership))
        ...
        in_main_body = bool((building_part_ids or crossed_part_ids_for_flags) & main_part_ids)
        in_extension = bool({*building_part_ids, *crossed_part_ids_for_flags} - main_part_ids)
        crosses_boundary = (
            (in_main_body and in_extension)
            or (bool(crossed_part_ids_for_flags & main_part_ids)
