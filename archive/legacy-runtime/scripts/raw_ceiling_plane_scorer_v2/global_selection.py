...
def _safe_union(polys: list[Any]) -> Any | None:
    valid = [poly for poly in polys if poly is not None and not getattr(poly, "is_empty", True)]
    if not valid:
        return None
    try:
...
def _fill_small_holes(geom: Any, max_hole_area_m2: float = _ENV_SMALL_HOLE_MAX_AREA_M2) -> Any:
...
def _exposed_room_keys(roof_result: dict[str, Any] | None) -> set[tuple[int, int]] | None:
...
def _rain_story_envelopes(
    ...
    for room_index, room in enumerate(building.get("rooms") or []):
        ...
        part_ids = room_membership.get(room_id) or room_membership.get(f"room:{room_index}") or set()
...
def _piece_envelopes(
    ...
    base_envelope = _safe_union(part_envelopes) if part_envelopes else story_env.get(int(story))
...
def _best_distinct_part_assignment(
    ...
    for member_index in members:
        ...
        candidate_parts = [part_id for part_id in piece.part_ids if part_id in story_part_polys]
    ...
    available_parts = sorted(
        {part_id for member_index in eligible_members for part_id in candidate_parts_by_member[member_index]}
    )
    ...
    for part_selection in permutations(available_parts, member_count):
        mapping = {eligible_members[idx]: part_selection[idx] for idx in range(member_count)}
        valid = True
        score_total = 0.0
...
def _partition_group_geometry_by_members(
    ...
    try:
        ...
        leftover = selected_group_geom.difference(covered) if covered is not None else selected_group_geom
    except Exception:
        leftover = None
    if leftover is not None and not getattr(leftover, "is_empty", True):
        ...
        if best_member is not None and best_score > 1e-9:
            ...
            merged = _safe_union([current, leftover]) if current is not None else leftover
...
def _row_output_plane_coeffs(row: dict[str, Any]) -> tuple[float, float, float, float] | None:
...
def _plane_y_at(plane_coeffs: tuple[float, float, float, float] | None, x: float, z: float) -> float:
...
def _ring_to_3d_loop(
    ...
    out: list[list[float]] = []
    for x, z in coords:
        y = _plane_y_at(plane_coeffs, float(x), float(z)) if plane_coeffs is not None else fallback_y
...
def _is_dormer_exception(
    ...
    is_dormer = (
        ...
        and cfg.dormer_min_outside_fraction <= outside_soft_fraction <= cfg.dormer_max_outside_fraction
    )
...
def _apply_story_batch(
    ...
    for group_id, member_indexes in group_member_indexes.items():
        group_poly = _safe_union([piece_entries[index].poly for index in member_indexes])
        ...
        group_entries.append(
            _GroupEntry(
                group_id=group_id,
                poly=group_poly,
                support_score=max(piece_entries[index].support_score for index in member_indexes),
                prior_final=max(piece_entries[index].prior_final for index in member_indexes),
    ...
    if soft_budget_base is not None:
        budget = float(soft_budget_base.area) * cfg.envelope_soft_outside_budget_fraction
        soft_rows: list[int] = []
        soft_cols: list[int] = []
...
def apply_global_xy_selection(
    ...
    *,
    ...
    # Pieces finalized by layer_policy before ILP (e.g. ridge_eave_mirror_rescued_residual)
    # are excluded from eligible_entries; without pre-clipping, ILP can select pieces that
    ...
    for row in rows_out:
        ...
        if target_kind_obs not in cfg.include_target_kinds:
            continue
        if str(row.get("piece_role") or "") == "supported" and _hard_invalid_pre_ilp_reason(row, cfg) is None:
            continue
    ...
    for row_index, row in enumerate(rows_out):
        ...
        if bool(row.get("overlay_suppressed")):
            ...
            if not (
                ...
                and str(row.get("final_layer_reason") or "") in _RIDGE_ILP_ELIGIBLE_REASONS
            ):
                continue
