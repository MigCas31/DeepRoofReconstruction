            ...
            rows.append(
                {
                    ...
                    "raw_area_vs_floor": round(raw_area / floor_area, 3) if floor_area > 0 else 0.0,
...
def _analyze_bins(rows: list[dict]) -> dict[str, Any]:
    ...
    num_specs = {
        ...
        "raw_area_vs_floor": ([0, 0.5, 0.9, 1.1, 1.5, 999], ["<0.5", "0.5-0.9", "0.9-1.1", "1.1-1.5", "1.5+"]),
        ...
        "incl_range_deg": ([0, 5, 20, 45, 90, 180], ["<5", "5-20", "20-45", "45-90", "90+"]),
        "incl_std_deg": ([0, 2, 10, 25, 180], ["<2", "2-10", "10-25", "25+"]),
        "azimuth_range_deg": ([0, 30, 90, 180, 361], ["<30", "30-90", "90-180", "180+"]),
        "n_floor_corners": ([0, 5, 7, 10, 99], ["<=4", "5-6", "7-9", "10+"]),
    }

    report: dict[str, Any] = {
        "corpus": {"n_planes": corpus_n, "n_disagree": corpus_d, "rate": round(corpus_rate, 4)},
        "bool_features": {},
    ...
    for feat in bool_features:
        ...
        report["bool_features"][feat] = {
            "true": {"rooms": len(pos), "planes": n_p, "disagree": d_p, "rate": round(rate_p, 4)},
            "false": {"rooms": len(neg), "planes": n_n, "disagree": d_n, "rate": round(rate_n, 4)},
...
def _print_table(report: dict[str, Any]) -> None:
    ...
    for feat, data in report["bool_features"].items():
        ...
        print(
            f"  {feat:<18}  true:  rooms={t['rooms']:>4}  planes={t['planes']:>4}  rate={t['rate']:.3f}"
            f"   false: rooms={f['rooms']:>4}  planes={f['planes']:>4}  rate={f['rate']:.3f}"
            f"   RR={rr}"
        )
    print()
