...
def _wall_top_segments(room: dict) -> list[tuple[tuple[float, float, float], tuple[float, float, float]]]:
    segs = []
    for wall in room.get("walls_computed") or []:
        ...
        top_y = max(ys)
        tops = [tuple(float(v) for v in c) for c in corners if abs(float(c[1]) - top_y) <= WALL_TOP_TOL_M]
        if len(tops) < 2:
            continue
...
def main() -> None:
        ...
        for ri, room in enumerate(building.get("rooms") or []):
            ...
            for pi, plane in enumerate(planes):
                ...
                if len(corners) < 3:
                    continue
                dists = [
                    _min_dist_to_wall_tops((float(c[0]), float(c[1]), float(c[2])), wall_segs)
                    for c in corners
                ]
    ...
    summary = {
        ...
        "trust_score_p50": round(float(np.median(trust_scores)), 3) if trust_scores else 0.0,
        "trust_score_p25": round(float(np.percentile(trust_scores, 25)), 3) if trust_scores else 0.0,
        ...
        "worst_rooms": [
            {k: r[k] for k in ("uuid", "story", "room_index", "trust_score", "p95_dist_m", "n_planes")}
    ...
    print(f"p95 distance to wall-top — median={summary['p95_dist_m_p50']} m, p95={summary['p95_dist_m_p95']} m")
