...
def evaluate_uuid(uuid: str, rec: dict) -> dict:
    ...
    singletons = [s for s in segments if (tuple(s["a"]), tuple(s["b"])) not in clustered]
    ...
    for s in singletons:
        ...
        for ci, c in enumerate(clusters):
            ...
            mirror_rooms = {(int(seg.get("story", -1)), int(seg.get("room_idx", -1))) for seg in c.get("segs") or []}
            ...
            footprint = np.concatenate(footprint_pts) if footprint_pts else np.zeros((0, 3))
            ...
            candidates.append(
                {
                    ...
                    "would_pass_gate": ridge_extent >= RIDGE_EXTENT_GATE_M and singleton_in_mirror_wing,
                }
            )
...
def main() -> int:
    ...
    n_with_passing = sum(
        1 for r in reports.values() if any(c["would_pass_gate"] for c in r["candidates"])
    )
    n_total_candidates = sum(len(r["candidates"]) for r in reports.values())
    n_passing = sum(
        sum(1 for c in r["candidates"] if c["would_pass_gate"]) for r in reports.values()
    )
    print(f"buildings with at least one mirror-singleton candidate: {n_with_candidates}")
    ...
    for uuid, r in sorted(reports.items()):
        ...
        for c in passing:
            print(
                f"  singleton az={c['singleton_az']:>6.2f} incl={c['singleton_incl']:>5.2f} "
                f"len={c['singleton_len']:>4.2f}m story={c['singleton_story']} room={c['singleton_room']}"
            )
            print(
                f"    mirrors cluster[{c['mirror_cluster_index']}] az={c['mirror_cluster_az']:>6.2f} "
                f"incl={c['mirror_cluster_incl']:>5.2f} size={c['mirror_cluster_size']}"
            )
            print(
                f"    wing rooms: {c['wing_room_count']} rooms (singleton in wing: {c['singleton_in_mirror_wing']})"
            )
            print(
                f"    wing ridge extent: {c['wing_ridge_extent_m']:.2f} m  (gate: {RIDGE_EXTENT_GATE_M:.1f} m)"
            )
    ...
    for uuid, r in sorted(reports.items()):
        ...
        for c in r["candidates"]:
            ...
            print(
                f"  {uuid}  {tag}  s_az={c['singleton_az']:>6.2f} s_incl={c['singleton_incl']:>5.2f} "
