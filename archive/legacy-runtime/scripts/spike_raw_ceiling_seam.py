...
def _fit_plane(corners):
    ...
    pts = np.array([[float(c[0]), float(c[1]), float(c[2])] for c in corners], dtype=float)
    if len(pts) < 3:
        return None, None
...
def _collect_obliques(roof_result, uuid):
    """Return list of dicts: {idx, element_id, corners, poly_xz, centroid, normal, plane}."""
    out = []
    for idx, surf in enumerate((roof_result.get("roof_surfaces") or {}).get("oblique") or []):
...
def _collect_raw_ceilings(building):
    """Flatten raw_ceiling_planes across rooms; return [{room_index, story, plane_index, corners, centroid, normal, area_3d, area_xz}]."""
    out = []
...
def _evidence_seam(footprint_a, footprint_b):
    ...
    try:
        ...
        shared = None
    if shared is not None and not getattr(shared, "is_empty", True) and float(getattr(shared, "length", 0.0) or 0.0) > 0:
...
def _polygon_summary(poly):
    ...
    if hasattr(poly, "geoms"):
        ...
        return f"MultiPolygon(area={poly.area:.2f}m² n={n} bounds={tuple(round(v,2) for v in poly.bounds)})"
    return f"Polygon(area={poly.area:.2f}m² bounds={tuple(round(v,2) for v in poly.bounds)})"


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--uuid", default=DEFAULT_UUID)
    ap.add_argument("--pair", nargs=2, type=int, default=list(DEFAULT_PAIR),
    ...
    for t in targets:
        ...
        n = t["normal"]
        print(f"  [{t['idx']}] az={t['azimuth_deg']:.0f}° inc={t['inclination_deg']:.0f}° "
              f"normal=({n[0]:.2f},{n[1]:.2f},{n[2]:.2f}) area={t['poly_xz'].area:.2f}m² bbox={bx}")

    ceilings = _collect_raw_ceilings(building)
    print(f"\nRaw scan ceilings found: {len(ceilings)} (across {len(building.get('rooms') or [])} rooms)")
    ...
    for i, j in pairs:
        ...
        print(f"  Δazimuth = {abs(ti['azimuth_deg']-tj['azimuth_deg']):.0f}°  "
        ...
        print(f"  raw ceilings assigned to oblique:{i} = {len(ass_i)} (Σarea_xz = {sum(c['area_xz_m2'] for c in ass_i):.2f}m²)")
        print(f"  raw ceilings assigned to oblique:{j} = {len(ass_j)} (Σarea_xz = {sum(c['area_xz_m2'] for c in ass_j):.2f}m²)")
        ...
        if seam_geom is None:
            print(f"  geometric seam:  degenerate (coeffs={tuple(round(c,3) for c in seam_coeffs)})")
        else:
            (x1, z1), (x2, z2) = list(seam_geom.coords)
            print(f"  geometric seam:  ({x1:.2f},{z1:.2f}) → ({x2:.2f},{z2:.2f})  "
        ...
        if ev_seam is None:
            ...
            print(f"  evidence seam:   {ev_kind}  type={ev_seam.geom_type}  length={length:.2f}m  bounds={tuple(round(v,2) for v in ev_seam.bounds)}")
        ...
        print(f"  evidence gate (≥{MIN_EVIDENCE_AREA_M2}m² each): {'PASS' if gate else 'FAIL'}  (ai={ai:.2f}m², aj={aj:.2f}m²)")
