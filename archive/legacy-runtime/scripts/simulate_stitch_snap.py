...
def _notch_predicate(wall_origin, wall_dir, wall_len, stitch_along, perp_offset, room_poly):
    ...
    for key in ("doors", "windows", "openings"):
        for item in room.get(key, []):
            ...
            if 0.1 < abs(perp) < perp_offset + 0.3 and (
                (perp_offset > 0 and 0 < perp < perp_offset) or
                (perp_offset < 0 and perp_offset < perp < 0)
            ):
...
def simulate():
    ...
    for b in buildings:
        ...
        for si, sw in enumerate(sws):
            ...
            sr = sw.get("room_indices") or ([sw.get("room_index")] if sw.get("room_index") is not None else [])
            sr = set(r for r in sr if r is not None)

            for (ri, w_story, wi, (wa, wb, wd, wL), wdict) in wall_index:
                if w_story != s_story:
                    continue
                if ri in sr:
                    continue
    ...
    totals["total_events"] = totals["repaired"] + totals["unchanged"] + totals["regressed"]
