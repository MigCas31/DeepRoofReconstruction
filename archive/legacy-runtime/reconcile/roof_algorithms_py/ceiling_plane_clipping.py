...
def _shared_partition_for_gable_pair(
    ...
    def _proj_i(t: float, plane_other: dict) -> float:
        ...
        dot = (
            plane_other["ridgeX"] * rx + plane_other["ridgeZ"] * rz
        )
    ...
    envelope = clip_poly_by_ridge(
        envelope, rx, rz, refx, refz, combined_min, True
    )
    if len(envelope) < 3:
        return None
    envelope = clip_poly_by_ridge(
        envelope, rx, rz, refx, refz, combined_max, False
    )
    if len(envelope) < 3:
        return None
    ...
    ai, bi = plane_i["n"]["x"] / plane_i["n"]["y"], plane_i["n"]["z"] / plane_i["n"]["y"]
    ci = plane_i["ref"]["y"] + ai * plane_i["ref"]["x"] + bi * plane_i["ref"]["z"]
    aj, bj = plane_j["n"]["x"] / plane_j["n"]["y"], plane_j["n"]["z"] / plane_j["n"]["y"]
