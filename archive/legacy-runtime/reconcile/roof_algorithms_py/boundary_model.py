...
def _boundary_record(
    *,
    ...
    return {
        "id": boundary_id or f"boundary:{_stable_hash([role, room_id or '', face_id], 20)}",
        "face_id": face_id,
        "role": role,
        "room_id": room_id,
...
def build_boundary_face_model(bldg: dict, roof_result: dict, graph=None) -> dict[str, Any]:
            ...
            if len(corners) >= 3:
                face = (
                    _make_face(
                        ...
                        story=story,
                        corners=corners,
                        source={"room_index": room_idx, "room_id": room_id, "wall_id": wall.get("id")},
                ...
                if len(strip) >= 3:
                    ...
                    boundaries.append(
                        _boundary_record(
                            ...
                            graph_surface_id=wall_binding.get("graph_surface_id"),
                            boundary_id=wall_binding.get("boundary_id"),
                            source={"room_index": room_idx, "wall_id": wall.get("id"), "extension": True},
                        )
                    )

    oblique_out: list[dict[str, Any]] = []
    for idx, surf in enumerate((roof_result.get("roof_surfaces") or {}).get("oblique", [])):
        ...
        for room_id in roof_room_ids or [None]:
            boundaries.append(
                _boundary_record(
                    ...
                    role="roof",
                    room_id=room_id,
                    boundary_id=f"boundary:roof:{_stable_hash([face['id'], room_id or '', 'oblique'], 20)}",
                    source={"roof_index": idx, "surface_kind": "roof_oblique"},
                )
            )

    flat_out: list[dict[str, Any]] = []
    for idx, surf in enumerate((roof_result.get("roof_surfaces") or {}).get("flat", [])):
        corners = surf.get("corners") or []
        if len(corners) < 3:
            continue
        face = _make_face(
        ...
        if graph is not None:
            ...
            graph_room = match_graph_room(graph, story, fp) if isinstance(story, int) else None
            room_id = graph_room.id if graph_room is not None else None
        boundaries.append(
            _boundary_record(
                face_id=face["id"],
                role="roof",
                room_id=room_id,
                boundary_id=f"boundary:roof:{_stable_hash([face['id'], room_id or '', 'flat'], 20)}",
...
def derive_roof_surfaces_from_boundary_model(boundary_model: dict[str, Any]) -> dict[str, list[dict[str, Any]]]:
    boundaries_by_face = {}
