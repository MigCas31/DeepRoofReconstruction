...
def _poly_xz(points3d: list) -> Polygon | None:
    ...
    if not poly.is_valid:
        try:
            ...
            return None
    if poly is not None and not poly.is_empty and isinstance(poly, Polygon) and poly.area > 1e-6:
        return poly
    try:
        hull = MultiPoint(coords).convex_hull
    except Exception:
        hull = None
    if hull is None or hull.is_empty or not isinstance(hull, Polygon) or hull.area <= 1e-6:
        return None
    return hull
...
def _graph_room_polygons(graph) -> dict[str, Polygon]:
    ...
    cell_complex = ((getattr(graph, "geometry_index", None) or {}).get("cell_complex") or {})
    ...
    for room_id, polys in polygons.items():
        ...
        try:
            ...
            unioned = None
        if unioned is not None and isinstance(unioned, Polygon) and not unioned.is_empty:
...
def match_graph_room(graph, story: int, floor_polygon: list) -> object | None:
    ...
    if floor_poly is not None and room_polys:
        ...
        for room in graph.nodes_by_type("Room"):
            ...
            if overlap > best_overlap + 1e-6 or (abs(overlap - best_overlap) <= 1e-6 and iou > best_iou):
                best = room
