    ...
    *,
    ...
    def _graph_cap_for_plane(plane, poly2d):
        ...
        for room_idx in plane.get("room_indices", []):
            if not isinstance(room_idx, int) or room_idx < 0 or room_idx >= len(bldg.get("rooms", [])):
                continue
            room = bldg["rooms"][room_idx]
            graph_room = match_graph_room(graph, int(room.get("story", 0)), room.get("floor_polygon") or [])
            ...
            for upper_room_id, relation_state in graph_upper_room_ids(graph, room_id):
                ...
                if room_cell is not None:
                    ...
                    if isinstance(footprint, list) and len(footprint) >= 3:
                        upper_poly = Polygon([(float(p[0]), float(p[1])) for p in footprint])
                        overlap_ok = upper_poly.intersects(plane_poly) and upper_poly.intersection(plane_poly).area > 0.01
                elif upper_room.bbox_xz:
                    overlap_ok = _bbox_overlap(_poly_bbox(poly2d), tuple(float(v) for v in upper_room.bbox_xz))
                if not overlap_ok:
                    continue
