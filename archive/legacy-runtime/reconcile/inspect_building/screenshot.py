...
([dx, dy, dz]) => {
  ...
  const THREE_BOX = new (camera.constructor.prototype.constructor.toString().includes('Perspective')
    ? window.THREE?.Box3 || (function(){ throw new Error('THREE.Box3 not on window'); })()
    : null);
}
"""
...
([dx, dy, dz]) => {
  ...
  scene.traverse((obj) => {
    if (!(obj.isMesh && obj.userData?.tierPreview && !obj.userData.pickOnly && !obj.userData.framingIgnore)) return;
...
def capture(uuid: str, eid: str | None, out_dir: Path, base_url: str, timeout_ms: int = 30000) -> list[Path]:
    ...
    with sync_playwright() as p:
        ...
        for name, deltas in PRESETS.items():
            ...
            if not ok:
                print(f"[screenshot] preset {name}: no meshes to frame", file=sys.stderr)
                continue
        ...
        if eid:
            ...
            try:
                page.wait_for_function(
                    "(uid) => window.__tierState && window.__tierState.selectedLocator === uid",
                    arg=eid,
                    timeout=5000,
                )
