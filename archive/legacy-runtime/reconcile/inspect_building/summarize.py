...
def _render_realism_section(realism: dict) -> list[str]:
    ...
    if extents:
        lines.append(
            f"- Extents (x, y, z): {extents.get('x', 0):.2f} × "
            f"{extents.get('y', 0):.2f} × {extents.get('z', 0):.2f} m"
        )
    ...
    if realism.get("volume_m3") is not None:
        lines.append(f"- Volume: {realism['volume_m3']:.0f} m³")
    if realism.get("convex_hull_volume_m3") is not None:
        lines.append(f"- Convex hull volume: {realism['convex_hull_volume_m3']:.0f} m³")
...
def _render_audit_section(audit: dict) -> list[str]:
    ...
    if coverage:
        ...
        lines.append("")
        lines.append("| Room | Story | Floor (m²) | Covered (m²) | Coverage |")
        lines.append("| --- | ---: | ---: | ---: | ---: |")
    ...
    if outside:
        ...
        lines.append("")
        lines.append("| Kind | Locator | Outside (m²) | Outside ratio |")
        lines.append("| --- | --- | ---: | ---: |")
...
def render_markdown(report: dict, out_dir: Path) -> str:
    ...
    if tier:
        ...
        lines.append("")
        for key in (
            "tier", "tier_label", "roof_type", "n_stories", "n_rooms",
            "n_oblique", "n_flat", "has_gable", "has_half_height",
            "knee_walls", "gaps",
        ):
