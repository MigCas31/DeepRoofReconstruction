...
def _check_ceiling_coverage(payload):
    ...
    issues: list[dict] = []
    for room in rooms:
        floor = (room.get("floor") or {}).get("corners") or []
        if not floor:
            continue
        ...
        # Ceilings above this room: those whose mean Y is at or above the wall top minus a slack.
        slack_m = 0.5
        candidate_polys = []
        for piece in ceilings:
            ...
            if yr is None:
                continue
            if wall_y_max is not None and yr[0] < wall_y_max - slack_m:
                # Ceiling sits well below the room top — not above this room.
                continue
            poly = _safe_polygon(_corners_xz(corners))
            if poly:
            ...
            for index, opening in enumerate(room.get(opening_kind) or []):
                ...
                if len(corners) < 3:
                    drops.append({
                        ...
                        "locator_id": f"{room.get('locator_id')}:{opening_kind[:-1]}:{index}",
                        "reason": f"corners.length={len(corners)} (<3)",
                    })
                    continue
                ...
                if w < OPENING_MIN_DIM or h < OPENING_MIN_DIM:
                    drops.append({
                        "kind": f"room.{opening_kind}",
                        "locator_id": f"{room.get('locator_id')}:{opening_kind[:-1]}:{index}",
                        "reason": f"width={w:.3f} m, height={h:.3f} m (min={OPENING_MIN_DIM})",
                    })
    return drops
...
def _check_story_coverage(payload):
    ...
    stories: dict[int, dict] = defaultdict(
        lambda: {"room_count": 0, "y_min": float("inf"), "y_max": float("-inf"), "ceiling_count": 0}
    )
    for room in rooms:
        story = room.get("story")
        if story is None:
...
def _gap_census(payload):
    ...
    return [{"kind": k, "scope": s, "count": c} for (k, s), c in sorted(counter.items())]
