...
def _ground_room_floor_sum(tier_payload: dict | None) -> float | None:
    ...
    for room in rooms:
        if room.get("story") != ground_story:
            continue
        corners = (room.get("floor") or {}).get("corners") or []
        if len(corners) < 3:
            continue
        pts = [(c.get("x", 0.0), c.get("z", 0.0)) for c in corners]
        # Shoelace on the XZ projection.
        area = 0.0
...
def compute_realism(obj_path: Path, tier_payload: dict | None) -> dict:
    ...
    if len(components) > 1:
        _flag(
            out, "multiple_components", "fail",
            value=len(components),
            threshold=1,
            reason=(
                f"Mesh has {len(components)} disconnected pieces; expected 1."
            ),
        )
    else:
    ...
    if out["volume_m3"] is not None:
        ...
        if v < VOLUME_MIN_M3:
            _flag(
                out, "implausible_volume", "fail", v, f"[{VOLUME_MIN_M3}, {VOLUME_MAX_M3}]",
                f"Volume {v:.0f} m³ is below {VOLUME_MIN_M3} m³ — too small to be a building.",
            )
        elif v > VOLUME_MAX_M3:
            _flag(
                out, "implausible_volume", "warn", v, f"[{VOLUME_MIN_M3}, {VOLUME_MAX_M3}]",
                f"Volume {v:.0f} m³ exceeds {VOLUME_MAX_M3} m³ — large for a single home.",
            )
        else:
            _flag(out, "implausible_volume", "pass", v, f"[{VOLUME_MIN_M3}, {VOLUME_MAX_M3}]", "Plausible volume.")
    ...
    if out.get("convex_hull_volume_m3") and out.get("volume_m3"):
        ...
        if ratio < CONVEXITY_MIN:
            _flag(
                out, "extreme_convexity", "fail", ratio, f"[{CONVEXITY_MIN}, {CONVEXITY_MAX}]",
                f"Convexity {ratio:.2f} below {CONVEXITY_MIN} — bizarre carved shape.",
            )
        elif ratio > CONVEXITY_MAX:
            _flag(
                out, "extreme_convexity", "warn", ratio, f"[{CONVEXITY_MIN}, {CONVEXITY_MAX}]",
                f"Convexity {ratio:.2f} above {CONVEXITY_MAX} — indistinguishable from a box.",
            )
        else:
            _flag(out, "extreme_convexity", "pass", ratio, f"[{CONVEXITY_MIN}, {CONVEXITY_MAX}]", "Plausible convexity.")
    ...
    if aspect > ASPECT_MAX_HEIGHT_RATIO:
        _flag(
            out, "implausible_aspect", "fail", aspect, ASPECT_MAX_HEIGHT_RATIO,
            f"Height {height_m:.1f} m vs footprint {horiz_max:.1f} m → flagpole shape.",
        )
    else:
        _flag(out, "implausible_aspect", "pass", aspect, ASPECT_MAX_HEIGHT_RATIO, "Plausible aspect.")
    ...
    if dist["wall_pct"] < WALL_PCT_MIN:
        _flag(
            out, "no_walls", "fail", dist["wall_pct"], WALL_PCT_MIN,
            f"Only {dist['wall_pct']*100:.0f}% of face area is wall-like — reconstruction has no walls.",
        )
    else:
    ...
    if classified_stories is not None:
        ...
        if delta > STORY_COUNT_TOL:
            ...
            _flag(out, "story_count_mismatch", "pass",
                  {"implied": implied_stories, "classified": classified_stories},
                  f"|delta| <= {STORY_COUNT_TOL}", "Story count agrees.")
    ...
    if lod2_footprint and tier_ground:
        ...
        if delta > FOOTPRINT_DELTA_MAX:
            _flag(
                out, "footprint_mismatch", "fail", delta, FOOTPRINT_DELTA_MAX,
                (
                    f"LoD2 ground area {lod2_footprint:.1f} m² vs sum of ground-story "
                    f"room floors {tier_ground:.1f} m² differ by {delta*100:.0f}%."
                ),
            )
        else:
            _flag(out, "footprint_mismatch", "pass", delta, FOOTPRINT_DELTA_MAX,
