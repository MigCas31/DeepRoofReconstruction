...
def _winner_wall_covers_overlap_segment(
    ...
    *,
    ...
    try:
        ...
        return False, {"reason": "invalid_overlap_segment", "overlap_length_m": round(overlap_len, 6)}
...
def _compute_gap_walls(
    ...
    for room in rooms_out:
        ...
        for w in room["walls_computed"]:
            ...
            if ext and len(ext) >= 1:
                ...
                ec = wc
            _add_wall_edge(story, ec, room_index=room.get("room_index"), wall_id=w.get("id"))
    ...
    def _build_snapped_vertices(edge_verts, story, floor_y):
        ...
        for i, c in enumerate(edge_verts):
            ...
            ytop = _interp_top_profile(picked["wall"]["top_profile"], picked["t_profile"])
            ytop = _clamp_to_sloped_ceiling(picked["proj"], story, floor_y, ytop)
            snapped.append({"xz": picked["proj"], "ybot": ybot, "ytop": ytop, "wall": picked["wall"]})
    ...
    for gap in gaps:
        ...
        if gap_type == "cross_story":
            ...
            if len(edge_verts) < 3:
                continue
            snapped_below = _build_snapped_vertices(edge_verts, below, story_y_map[below])
            draped = [
        ...
        for piece_idx, piece_snapped in enumerate(pieces_for_caps):
            ...
            xz_2d = [
                (float(v["xz"][0]), float(v["xz"][1])) for v in piece_snapped
            ]
            ti = 0
...
def _compute_gap_closures(indicators, rooms_out):
    ...
    for ind in indicators:
        ...
        p_top_profile, p_bot_profile = _wall_top_bottom_profiles(pc_raw, axis2d, origin_xz)
        w_top_profile, w_bot_profile = _wall_top_bottom_profiles(wc_raw, axis2d, origin_xz)
