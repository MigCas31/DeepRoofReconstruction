...
* Tier payload kinds (``tier-room``, ``tier-wall``, ``tier-ceiling``,
  ``tier-knee-wall``, ...) resolve against
...
def _tier_corners(element: dict) -> list:
    return element.get("corners") or element.get("floor", {}).get("corners") or []
...
def find_tier_payload_element(tier_payload: dict, parsed: ParsedElementId) -> dict:
    ...
    for room_index, room in enumerate(tier_payload.get("rooms") or []):
        if room.get("locator_id") == token:
            ...
            corners = floor.get("corners") or []
            return {
                "json_path": f"rooms[{room_index}].floor",
                "room_index": room_index,
                "story": room.get("story"),
                "source": "tier_payload",
                "corners_count": len(corners),
        ...
        for wall_index, wall in enumerate(room.get("walls") or []):
            ...
            if f"{wall.get('locator_id')}:extension" == token:
                ...
                return {
                    "json_path": f"rooms[{room_index}].walls[{wall_index}].extension_strip",
                    "room_index": room_index,
                    "story": room.get("story"),
                    "source": "tier_payload",
                    "corners_count": len(corners),
        ...
        for collection_key in ("doors", "windows"):
            for element_index, element in enumerate(room.get(collection_key) or []):
                candidate = f"{room.get('locator_id')}:{collection_key[:-1]}:{element_index}"
                if candidate == token:
                    corners = element.get("corners") or []
                    return {
                        "json_path": f"rooms[{room_index}].{collection_key}[{element_index}]",
                        "room_index": room_index,
                        "story": room.get("story"),
                        "source": "tier_payload",
                        "corners_count": len(corners),
    ...
    for collection_key, json_key in top_level:
        for index, element in enumerate(tier_payload.get(collection_key) or []):
            ...
            return {
                ...
                "room_index": element.get("room_index"),
                "story": element.get("story"),
                "source": element.get("source") or element.get("kind") or "tier_payload",
                "corners_count": len(corners),
                "element": element,
            }
...
def find_ceiling_raw_element(building: dict, parsed: ParsedElementId) -> dict:
    ...
    try:
        ...
        raise LookupError(
            f"ceiling-raw id must be <story>:<room_index>:<plane_index>, got '{parsed.element_id}'"
        ) from exc
...
def find_thermal_ceiling_element(
    ...
    if len(parts) == 3:
        ...
        thermal = (
            ((roof_results_for_uuid or {}).get("ceiling") or {}).get("thermal") or []
        )
...
def find_candidate_face_element(candidate_faces: dict | list, parsed: ParsedElementId) -> dict:
    ...
    if not faces:
        ...
        faces = [
            face for face in (per_uuid.get("candidates") or []) if isinstance(face, dict)
        ]
    full_id = f"{parsed.building_uuid}::candidate::{parsed.element_id}"
    for idx, face in enumerate(faces):
        candidate_id = str(face.get("id") or "")
        if parsed.element_id in {candidate_id, _id_tail(candidate_id)} or candidate_id == full_id:
            corners = face.get("footprint_xz") or []
            return {
                "building_uuid": parsed.building_uuid,
                "kind": parsed.kind,
                "id": parsed.element_id,
                "json_path": f"candidate_faces[{parsed.building_uuid}].{collection_key}[{idx}]",
...
def find_reconstruction_face_element(
    ...
    selected_faces = [
        face for face in (per_uuid.get("selected_faces") or []) if isinstance(face, dict)
    ]
    if not selected_faces and candidate_faces is not None:
        candidate_entry = _sidecar_entry_for_uuid(candidate_faces, parsed.building_uuid) or {}
        candidate_rows = candidate_entry.get("faces") or candidate_entry.get("candidates") or []
        candidate_by_id = {
    ...
    for idx, face in enumerate(selected_faces):
        ...
        if parsed.element_id in {face_id, _id_tail(face_id)}:
            ...
            return {
                ...
                "kind": parsed.kind,
                "id": parsed.element_id,
                "json_path": f"reconstruction[{parsed.building_uuid}].selected_faces[{idx}]",
                "room_index": face.get("room_index"),
                "story": face.get("story"),
    ...
    for idx, group in enumerate(per_uuid.get("plane_groups") or []):
        ...
        if base_plane_group_id in {
            ...
            return {
                ...
                "kind": parsed.kind,
                "id": parsed.element_id,
                "json_path": f"ridge_eave_scores[{parsed.building_uuid}].plane_groups[{idx}]",
                "room_index": None,
    ...
    for idx, candidate in enumerate(per_uuid.get("candidates") or []):
        ...
        if element_id in {candidate_id, _id_tail(candidate_id)}:
            return {
                ...
                "kind": parsed.kind,
                "id": parsed.element_id,
                "json_path": f"ridge_eave_scores[{parsed.building_uuid}].candidates[{idx}]",
...
def find_roof_overextend_element(
    ...
    rows = (
        ((computed_overextend or {}).get("buildings") or {}).get(parsed.building_uuid)
        or []
    )
    for idx, piece in enumerate(rows):
        ...
        return {
            ...
            "kind": parsed.kind,
            "id": parsed.element_id,
            "json_path": f"computed_overextend.buildings[{parsed.building_uuid}][{idx}]",
            "room_index": piece.get("room_index"),
            "story": piece.get("story"),
...
def find_raw_disagreement_element(raw_disagreement: dict, parsed: ParsedElementId) -> dict:
    rows = (
        ((raw_disagreement or {}).get("buildings") or {}).get(parsed.building_uuid)
        or []
    )
    for idx, piece in enumerate(rows):
...
def find_clean_ceiling_element(ceiling_replacement: dict, parsed: ParsedElementId) -> dict:
    rows = (
        ((ceiling_replacement or {}).get("buildings") or {}).get(parsed.building_uuid)
        or []
    )
    ...
    for idx, piece in enumerate(rows):
        ...
        return {
            ...
            "kind": parsed.kind,
            "id": parsed.element_id,
            "json_path": f"ceiling_replacement.buildings[{parsed.building_uuid}][{idx}]",
            "room_index": piece.get("room_index"),
            "story": piece.get("story"),
...
def find_ontology_element(
    ...
    continuation_regions = (
        (roof_results_for_uuid.get("roof_continuation_diagnostics") or {}).get(
            "continuation_regions"
        )
        or []
    )
    ...
    if atom is None and atom_id in {"building-part:full-building", "building-part:unassigned"}:
    ...
    if atom is None and atom_id.startswith("unresolved-coverage:"):
        ...
        if isinstance(room_summary, dict):
            ...
            provenance_paths.append(
                f"top_boundary_graph.room_summaries[{room_id}]"
            )
...
def find_element(
    ...
    *,
    ...
    if parsed.kind in RAW_EAVE_SPLIT_KINDS:
        ...
        sidecar = (
            raw_ceiling_plane_splits_v2
            if version == "v2"
            else raw_ceiling_plane_splits
        )
        if sidecar is None:
            if version == "v2":
                raise LookupError(
...
def _cli() -> int:
    ...
    parser.add_argument(
        ...
        help="Directory containing per-building tier_payload.json files (default: pipeline-outputs)",
    )
    parser.add_argument(
        "--roof-results",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(".context/raw_ceiling_plane_scorer_v2_full/plane_extent_splits.json"),
        help=(
            "Path to .context/raw_ceiling_plane_scorer_v2_full/plane_extent_splits.json "
            "(needed for raw-eave-split-v2 kinds)"
        ),
    )
    parser.add_argument(
        "--candidate-faces",
    ...
    parser.add_argument(
        ...
        help="Path to reconstruction selections sidecar (needed for reconstruction-face kinds)",
    )
    parser.add_argument(
        "--ridge-eave-scores",
        type=Path,
        default=Path("reports/ridge_eave_scores_20260420/scores.json"),
        help="Path to ridge/eave scores sidecar (needed for ridge-eave-candidate kinds)",
    )
    parser.add_argument(
        "--computed-overextend",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="Path to clean ceiling replacement sidecar (needed for clean-ceiling kinds)",
    )
    parser.add_argument(
        "--element-id",
        required=True,
    ...
    if args.raw_ceiling_plane_splits.exists():
        raw_ceiling_plane_splits = json.loads(
            args.raw_ceiling_plane_splits.read_text()
        )
