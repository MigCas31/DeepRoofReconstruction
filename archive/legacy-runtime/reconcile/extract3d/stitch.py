...
def dedup_duplicate_vertical_stitches(stitch_walls):
    ...
    def parallel(n1, n2, tol=0.02):  # ~8 degrees — well under 90° between adjacent walls
    ...
    for story, rooms in story_rooms.items():
        ...
        for room_idx, room in rooms:
            for wall_idx, wall in enumerate(room["walls_computed"]):
                ...
                endpoints.append(
                    (corners[0][0], corners[0][2], y_low, top_y, key, 0)
                )
                endpoints.append(
                    (corners[1][0], corners[1][2], y_low, top_y, key, 1)
                )
        ...
        for idx in range(len(endpoints)):
            ...
            def _build_l_candidate(
                p_anchor, p_other, dir_anchor_out, dir_other_out
            ):
                ax, az = p_anchor
                ox, oz = p_other
...
def _drop_stitches_crossing_rooms(stitch_walls, rooms_out):
    ...
    for entry in stitch_walls:
        ...
        if inside / total >= INSIDE_FRACTION_MAX:
            record(entry, STEP_STITCH_WALLS, "dropped_crosses_room", f"inside_fraction={inside/total:.2f}")
            continue
        kept.append(entry)
    return kept
...
def _drop_degenerate_stitches(stitch_walls):
    ...
    ``_MIN_LEG_LENGTH_M``: a 6-mm-wide × 2-m-tall quad has 4 unique 3D
    ...
    for entry in stitch_walls:
        ...
        for c in corners:
            ...
            if not same:
                unique.append(c)
        if len(unique) < 3:
            record(entry, STEP_STITCH_WALLS, "dropped_degenerate", f"unique_vertices={len(unique)}")
            continue
