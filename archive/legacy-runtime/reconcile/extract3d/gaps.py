    ...
    while len(work) > 3:
        ...
        if not ear_found:
            for k in range(1, len(work) - 1):
                triangles.append(
                    (src_idx[work[0]], src_idx[work[k]], src_idx[work[k + 1]])
                )
            return triangles
...
def compute_cross_floor_gaps(rooms_out):
    ...
    for story, polys in sorted(story_rooms.items()):
        ...
        phase1_cover = (
            make_valid(unary_union(phase1_parts)) if phase1_parts else None
        )
...
def compute_gap_walls(
    ...
    for room in rooms_out:
        ...
        for wall in room["walls_computed"]:
            ...
            if ext and len(ext) >= 1:
                ...
                ec = wc
            add_wall_edge(story, ec, room_index=room.get("room_index"), wall_id=wall.get("id"))
    ...
    for s, polys in rooms_by_story.items():
        ...
        try:
            story_room_boundary[s] = u.boundary if (u is not None and not u.is_empty) else None
        except Exception:
    ...
    for gap in gaps:
        ...
        if gap_type == "cross_story":
            ...
            if len(edge_verts) < 3:
                continue
            snapped_below = build_snapped_vertices(edge_verts, below, story_y_map[below])
            draped = [
        ...
        for piece_idx, piece_snapped in enumerate(pieces_for_caps):
            ...
            xz_2d_p = [
                (float(s["xz"][0]), float(s["xz"][1])) for s in piece_snapped
            ]
            ti = 0
