...
def _room_from_dict(index: int, room: dict) -> V3Room:
    return V3Room(
        ...
        doors=[
            {"id": d.get("id") or f"door:{i}", "corners": [_to_vec3(c) for c in d.get("corners", [])]}
            for i, d in enumerate(room.get("doors") or [])
        ],
        windows=[
            {"id": w.get("id") or f"window:{i}", "corners": [_to_vec3(c) for c in w.get("corners", [])]}
    ...
    for b in buildings:
        if b.get("uuid") == uuid:
            return V3Input(
                ...
                rooms=[_room_from_dict(i, r) for i, r in enumerate(b.get("rooms") or [])],
            )
