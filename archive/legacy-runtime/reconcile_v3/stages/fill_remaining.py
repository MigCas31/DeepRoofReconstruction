    ...
    for part in parts:
        ...
        for piece_idx, piece in enumerate(_iter_polygons(leftovers)):
            ...
            if slanted_hit is not None:
                hp_pt, hyp, room = slanted_hit
                room_poly = _safe_polygon_xz(room.floor_polygon)
                try:
                    clipped = (
                        piece.intersection(room_poly) if room_poly is not None else piece
                    )
                except Exception:
                    clipped = piece
                ...
                for sub_idx, sub in enumerate(clipped_pieces):
                    ...
                    seeded = [(float(x), float(y_ref), float(z)) for x, _, z in sub_corners]
                    corners_3d = project_xz_onto_plane(seeded, plane)
                    suffix = f"fill-{inner}" if sub_idx == 0 else f"fill-{inner}-{sub_idx}"
                    inferred_slants.append(
                        V3SlantedRoof(
                            ...
                            trace=HypothesisTrace(
                                ...
                                decision_reason=(
                                    "Uncovered piece contains a SlopeHypothesis centroid "
                                    "— inherit slope; polygon clipped to room footprint."
                                ),
                            ),
                        )
                    )
                    ...
                    ceilings.append(
                        V3FlatCeiling(
                            ...
                            trace=HypothesisTrace(
                                ...
                                decision_reason=(
                                    "Piece lies inside a top-story room with flat wall tops — "
            ...
            unresolved.append(
                V3UnresolvedRegion(
                    id=make_element_id(
                        building_uuid, "v3-unresolved", f"fill-{inner}"
                    ),
                    room_id=None,
                    ...
                    context={
                        ...
                        "containing_room_id": containing_room[0].identifier if containing_room else None,
                    },
                    trace=HypothesisTrace(
                        stage="fill_remaining",
