...
def _polygon_to_xz_corners(poly: Polygon, floor_y: float = 0.0) -> list[tuple[float, float, float]]:
...
def identify_building_parts(
    ...
    buffered = unary_union([p.buffer(SCAN_NOISE_M, join_style=2) for _, p in room_polys])
    components = decompose_polys(make_valid(buffered.buffer(-SCAN_NOISE_M, join_style=2)))
    ...
    for room, poly in room_polys:
        ...
        unresolved.append(
            V3UnresolvedRegion(
                id=make_element_id(building_uuid, "v3-unresolved", f"orphan-room-{room.index}"),
