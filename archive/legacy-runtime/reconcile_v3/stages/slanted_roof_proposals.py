"""slanted_roof_proposals — permissive (slanted-segment × rain-exposed slab piece) generator.
...
Slant threshold is deliberately very permissive (1° < incl < 89°) so edge
...
def _collect_permissive_slanted_segments(rooms: list[V3Room]) -> list[dict]:
    ...
    Much more permissive than the default oblique segment collector (1° vs 5°
...
def _polygon_min_width(poly: Polygon) -> float:
    """Effective width for sliver detection: 2 × area / perimeter.

    For a true rectangle W × L this returns W exactly; for irregular ribbon
...
def _rain_exposure(
    ...
    for other in rooms:
        ...
        if isinstance(other_poly, Polygon) and not other_poly.is_empty and other_poly.is_valid:
...
def _exposed_pieces(
    ...
    if isinstance(exposure, MultiPolygon):
        candidates = [g for g in exposure.geoms if isinstance(g, Polygon) and not g.is_empty]
...
def _plane_y_at(point_xz: tuple[float, float], plane: tuple[float, float, float, float]) -> float:
    a, b, c, d = plane
    if abs(b) < 1e-6:
        return 0.0
...
def _slant_delta_over_polygon(poly: Polygon, plane: tuple[float, float, float, float]) -> float:
    a, b, c, d = plane
    if abs(b) < 1e-6:
        return 0.0
...
def _compute_features(
    *,
    ...
    """Build the flat feature vector for a (segment × rain-exposed-piece) pair.
    ...
    return {
        ...
        "rain_exposure_ratio": round(piece_area / slab_area, 4) if slab_area > 0 else 0.0,
        # relation
        ...
        "slant_delta_over_piece_m": round(_slant_delta_over_polygon(piece_poly, plane), 4),
    }
...
def _collect_target_pieces(
    ...
    for room in rooms:
        ...
        if not isinstance(full_poly, Polygon) or full_poly.is_empty or not full_poly.is_valid:
            continue
    ...
    for gap in gaps:
        ...
        if not pieces:
            continue
        gap_tail = gap.id.rsplit("::", 1)[-1] if isinstance(gap.id, str) else str(gap.id)
        targets.append(
            {
                "kind": "gap",
...
def propose_slanted_roofs(
    ...
    """Permissive (slanted-segment × rain-exposed piece) proposer.
    ...
    for segment in segments:
        ...
        for target in targets:
            ...
            for piece_index, piece_poly in enumerate(target["pieces"]):
                ...
                inner = (
                    f"segment-{segment['seg_idx']}:{target['id_token']}:piece-{piece_index}"
                )
                proposals.append(
                    V3RoofProposal(
                        ...
                        trace=HypothesisTrace(
                            ...
                            decision_reason=(
                                ...
                                "closed gap footprints — a sealed gap is rain-exposed, "
                                "so without it the merged stage has no cluster to "
                                "intersect above the gap and downstream roof coverage "
                                "vanishes. "
                                f"Pieces below {EXPOSED_PIECE_MIN_AREA_M2} m² area or "
                                f"{EXPOSED_PIECE_MIN_WIDTH_M} m effective width "
                                "(2·area/perimeter) are dropped. Permissive slant threshold "
                                f"({PERMISSIVE_MIN_INCL_DEG}° < incl < "
                                f"{PERMISSIVE_MAX_INCL_DEG}°) to surface edge cases."
                            ),
                        ),
                    )
                )
