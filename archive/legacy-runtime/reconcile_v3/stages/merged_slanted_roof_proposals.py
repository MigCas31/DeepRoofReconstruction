...
#: (``azimuth<30°``, ``incl<15°``, ``COPLANAR_TOL=0.5m``): a dot of ~0.94
#: approximates the 30°×15° angular cone, and 0.5m matches ``COPLANAR_TOL``.
NORMAL_DOT_MIN = 0.94
D_ABS_MAX = 0.50
#: Azimuth cosine below which two clusters are considered "opposing" (>30°
#: apart in horizontal direction). Matches the viewer threshold.
OPPOSING_COS_AZIMUTH_MAX = 0.866
#: Minimum segment area in m² — below this we treat the piece as a split
...
#: area ≥ 0.05 m² but width ≪ 1 cm). Mirrors ``EXPOSED_PIECE_MIN_WIDTH_M``
...
def _half_plane_polygon(
    ...
    if abs(A) < 1e-12 and abs(B) < 1e-12:
        ...
        trivial = C >= 0
        return box(*bounds) if (trivial and not keep_negative) or (not trivial and keep_negative) else None
...
def _polygon_min_width(poly: Polygon) -> float:
    """Ribbon-thickness metric: 2 * area / perimeter. A 1 m × 1 m square gets
...
def build_merged_roof_segments(
    ...
    for cluster in clusters:
        ...
        for opp in opposing:
            ...
            for region in regions:
                ...
                if region_poly is None or region_poly.is_empty:
                    continue
                rain_half = _half_plane_polygon(coeffs, region_poly.bounds, keep_negative=True)
                covered_half = _half_plane_polygon(coeffs, region_poly.bounds, keep_negative=False)
        ...
        opposing_planes = tuple(tuple(float(x) for x in opp["plane"]) for opp in opposing)
        ...
        # per (cluster × opposing-sides × piece) connected component.
        segment_counter = 0
        for region in regions:
            region_poly = region["poly"]
            ...
            for piece in rain_pieces:
                ...
                for connected in _expand_multipolygon(sub):
                    ...
                    inner = f"{cluster['canonical_id'].rsplit('::', 1)[-1]}:seg-{segment_counter}"
                    seg_id = make_element_id(building_uuid, "v3-merged-roof-segment", inner)
                        ...
                        V3MergedRoofSegment(
                            ...
                            trace=HypothesisTrace(
                                ...
                                decision_reason=(
                                    ...
                                    f"segments, azimuth cos < {OPPOSING_COS_AZIMUTH_MAX}), "
