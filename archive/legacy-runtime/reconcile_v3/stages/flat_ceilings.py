...
def _outlier_fraction(walls: list[dict], top_ys: list[float]) -> float:
    ...
    total_len = sum(l for _, l in wall_data if l > 0)
    if total_len == 0:
        return 1.0
    outlier_len = sum(l for y, l in wall_data if y is not None and y < min_top + SCAN_NOISE_M)
...
def flat_ceilings_unambiguous(
    ...
    for room in rooms:
        ...
        top_ys = [
            y
            for y in (_wall_top_y(w) for w in room.walls)
            if y is not None
        ]
        ...
        if len(top_ys) < 3:
            unresolved.append(
                V3UnresolvedRegion(
                    id=make_element_id(building_uuid, "v3-unresolved", f"ceiling-{room_eid_inner}"),
                    ...
                    trace=HypothesisTrace(
                        ...
                        inputs={"room_id": room.identifier, "wall_top_count": len(top_ys)},
                        decision_reason="Fewer than 3 wall tops to derive a flat ceiling.",
                    ),
                )
            )
            continue
                ...
                V3UnresolvedRegion(
                    ...
                    trace=HypothesisTrace(
                        ...
                        decision_reason=(
                            ...
                            "asymmetric slope. Deferred to slanted_roofs/fill_remaining."
                        ),
                    ),
                )
            )
            ...
            if spread <= FLAT_CEILING_SPREAD_M and out_frac < OUTLIER_FRACTION_MAX:
                ...
                ceilings.append(
                    V3FlatCeiling(
                        ...
                        room_id=room.identifier,
                        footprint_xz=[(float(c[0]), y, float(c[2])) for c in room.floor_polygon],
                        y=y,
                        over="room",
                        trace=HypothesisTrace(
                            stage="flat_ceilings",
                            ...
                            decision_reason=(
                                "Spread above scan-noise but below flat-ceiling tolerance, "
