...
def test_candidates_split_and_tagged_by_authoritative_zone() -> None:
    ...
    faces = build_candidate_faces(
        _BLDG,
        [_segment(f"{_BLDG}::v3-merged-roof-segment::lone", plane, seg_fp, area_m2=24.0)],
...
def test_zone_filter_rejects_weak_overlap_with_incompatible_azimuth() -> None:
    ...
    faces = build_candidate_faces(
        _BLDG,
        [_segment(f"{_BLDG}::v3-merged-roof-segment::lone", plane, [(0.0, 0.0), (4.0, 0.0), (4.0, 1.0), (0.0, 1.0)], area_m2=4.0)],
...
def test_gap_derived_segment_carries_gap_axis_metadata() -> None:
    ...
    assert math.isclose(faces[0].source_gap_major_axis_azimuth_deg or 0.0, 90.0, abs_tol=0.1)
