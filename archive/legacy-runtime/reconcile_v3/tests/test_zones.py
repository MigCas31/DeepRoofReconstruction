...
def _ring_xyz(x0: float, z0: float, x1: float, z1: float, y: float) -> list[list[float]]:
    return [
        [x0, y, z0],
        [x1, y, z0],
        [x1, y, z1],
...
def _room_partition(room_index: int, footprint: tuple[float, float, float, float], atom_id: str) -> dict:
    x0, z0, x1, z1 = footprint
        ...
        "building_part_graph": {
            ...
            "room_membership": {
                f"room:{room_partition['room_index']}": [
                    ...
                    if f"room:{room_partition['room_index']}" in (node.get("room_ids") or [])
                ]
...
def test_seed_spill_does_not_expand_owned_room_set() -> None:
    occupied = [
        _cell("cell-a", 0, "room:0", (0.0, 0.0, 2.0, 2.0), part_id="building-part:left"),
        _cell("cell-b", 1, "room:1", (2.0, 0.0, 4.0, 2.0), part_id="building-part:right"),
    ]
    subparts = [
        {
...
def test_overlapping_support_projections_are_relabeled_disjointly() -> None:
    occupied = [
        _cell("cell-left", 0, "room:0", (0.0, 0.0, 4.0, 2.0), part_id="building-part:left"),
        _cell("cell-right", 1, "room:1", (2.0, 0.0, 6.0, 2.0), part_id="building-part:right"),
    ]
    roof_building = _roof_building(
        occupied_cells=occupied,
        room_partitions=[
...
def test_broad_seed_can_survive_on_multiple_owned_components_without_overlap() -> None:
    occupied = [
        _cell("cell-left", 0, "room:0", (0.0, 0.0, 2.0, 2.0), part_id="building-part:left"),
        _cell("cell-right", 1, "room:1", (3.0, 0.0, 5.0, 2.0), part_id="building-part:right"),
    ]
    subparts = [
        {
...
def test_target_building_artifact_has_shadow_zone_baseline() -> None:
    ...
    row = next(
        (item for item in rows if item.get("building_uuid") == "c87c1e25-ff00-44ec-b823-b0966c81af70"),
        None,
    )
    if row is None:
        return
    shadow_zones = [zone for zone in (row.get("zones") or []) if zone.get("part_id") is None]
