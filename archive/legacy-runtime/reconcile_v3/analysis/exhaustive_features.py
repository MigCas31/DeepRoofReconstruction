...
def _interior_angles_xy(corners_xyz: Sequence[Sequence[float]]) -> tuple[list[float], int]:
...
def _edge_records(corners_xyz: Sequence[Sequence[float]]) -> list[dict[str, Any]]:
    ...
    for i in range(len(pts)):
        ...
        dy = float(vec[1])
        az = math.degrees(math.atan2(float(vec[0]), float(vec[2]))) % 180.0 if length2 > _EPS else None
        out.append(
            {
                "a": a,
                "b": b,
...
def _grid_snap_fraction(corners_xyz: Sequence[Sequence[float]], grid: float) -> float | None:
    if not corners_xyz:
        return None
    snapped = 0
    for c in corners_xyz:
        ...
        snap_tol = GRID_SNAP_FINE_TOL_M if grid <= GRID_BUCKET_MEDIUM_M else GRID_SNAP_COARSE_TOL_M
...
def _fourier_boundary_features(
    ...
    out["fourier_energy_fraction_top_5"] = (
        float(np.sum(np.sort(energy)[-5:]) / total_energy) if total_energy > _EPS else None
    )
    return out


def _projection_xy(points_xyz: Sequence[Sequence[float]], axes: tuple[int, int]) -> np.ndarray:
    if not points_xyz:
        return np.empty((0, 2), dtype=float)
    return np.asarray([(float(p[axes[0]]), float(p[axes[1]])) for p in points_xyz], dtype=float)


def _hu_projection_features(points_xyz: Sequence[Sequence[float]]) -> dict[str, float | None]:
    projections = {
...
def _point_cloud_shape_features(points_xyz: Sequence[Sequence[float]]) -> dict[str, float | None]:
    out = {
...
def _part_for_poly(segment_poly: Polygon | None, building_context: dict | None) -> dict | None:
...
def _nearest_distance_to_polys(poly: Polygon | None, others: Iterable[Polygon]) -> float | None:
    if poly is None:
        return None
    best = None
    for other in others:
...
def _segment_peers(record: dict, building_context: dict | None) -> list[dict]:
    ...
    return [s for s in building_context.get("merged_roof_segments") or [] if s.get("id") != my_id]
...
def _ray_exit_distance(
    ...
    far = Point(origin.x + float(direction[0]) * span * 8.0, origin.y + float(direction[1]) * span * 8.0)
...
def exhaustive_features(
    ...
    *,
    ...
    az, incl = fe._plane_to_azimuth_incl(plane) if plane else (float("nan"), float("nan"))
    ...
    peer_polys = [(peer, _xz_poly(peer.get("corners") or peer.get("footprint_xz") or [])) for peer in peers]
    ...
    out["poly_unique_xy_count"] = len({(round(float(c[0]), 2), round(float(c[2]), 2)) for c in corners}) if corners else 0
    ...
    out["poly_right_angle_fraction"] = (
        float(sum(1 for a in angles if abs(a - 90.0) <= 5.0) / len(angles)) if angles else None
    )
    ...
    if ys:
        ...
        out["vtx_mid_band_count"] = int(len(y_arr) - out["vtx_ridge_band_count"] - out["vtx_eave_band_count"])
        ...
        out["vtx_y_cluster_count_k2"] = 1 if out["vtx_y_range_m"] < VERTEX_Y_CLUSTER_SPLIT_M else 2
        out["vtx_y_gap_at_median_m"] = float(np.median(y_arr[y_arr >= np.median(y_arr)]) - np.median(y_arr[y_arr <= np.median(y_arr)])) if len(y_arr) >= 2 else None
    else:
        for key in (
            "vtx_y_range_m",
    ...
    out["vtx_on_wall_top_count"] = int(sum(1 for y in ys if any(abs(y - wy) <= 0.1 for wy in wall_top_ys))) if ys and wall_top_ys else 0
    ...
    if boundary_poly is not None and corners:
        ...
        out["vtx_outside_building_footprint_max_m"] = float(max(outside)) if outside else 0.0
    else:
    ...
    out["vtx_on_floor_slab_count"] = (
        int(sum(1 for y in ys if any(abs(y - sy) <= 0.1 for sy in slab_ys))) if ys and slab_ys else 0
    )
    ...
    if corners:
        ...
        if building_context is not None:
            for seg in building_context.get("merged_roof_segments") or []:
                all_seg_corners.append(np.asarray(seg.get("corners") or [], dtype=float))
        valences = []
        for c in np.asarray(corners, dtype=float):
            ...
            for seg_pts in all_seg_corners:
                if seg_pts.size == 0:
                    continue
                if np.any(np.linalg.norm(seg_pts - c, axis=1) <= GRID_SNAP_COARSE_TOL_M):
                    count += 1
    ...
    out["edge_length_m_median"] = float(np.median(edge_lengths)) if edge_lengths else None
    ...
    out["edge_shortest_to_longest_ratio"] = (
        float(min(edge_lengths) / max(edge_lengths)) if edge_lengths and max(edge_lengths) > _EPS else None
    )
    if edge_lengths:
        logs = np.log1p(np.asarray(edge_lengths, dtype=float))
        hist, _ = np.histogram(logs, bins=min(5, max(2, len(edge_lengths))))
        out["edge_length_entropy_log"] = _entropy_from_counts(Counter({i: int(v) for i, v in enumerate(hist) if v > 0}))
    else:
    ...
    out["edge_turning_angle_sum_deg"] = float(sum(abs(t) for t in turns)) if turns else None
    ...
    out["edge_parallel_pair_fraction"] = float(parallel / pair_count) if edge_az else None
    ...
    for wid, info in (wall_index or {}).items():
        ...
        if not pts:
            continue
        wall_centroids.append((float(np.mean([p[0] for p in pts])), float(np.mean([p[2] for p in pts]))))
    for e in edges:
        length = e["length3"]
        mid = e["mid"]
        touch_peer = False
        if y_max is not None and e["horizontal"] and float(mid[1]) >= y_max - PLANE_INTERIOR_CLIP_MARGIN_M:
            ridge_len += length
            ridge_count += 1
        elif y_min is not None and e["horizontal"] and float(mid[1]) <= y_min + PLANE_INTERIOR_CLIP_MARGIN_M:
            eave_len += length
            eave_count += 1
        elif e["descending"]:
            if e["azimuth"] is not None and plane_az is not None and abs(fe._azimuth_diff_deg(e["azimuth"], plane_az) - 90.0) < 15.0:
                hip_len += length
                hip_count += 1
            else:
                rake_len += length
        ...
        for _, peer_poly in peer_polys:
            if peer_poly is None:
                continue
            line = LineString([(float(e["a"][0]), float(e["a"][2])), (float(e["b"][0]), float(e["b"][2]))])
    ...
    out["edge_free_length_fraction"] = float(free_len / out["edge_length_m_sum"]) if out["edge_length_m_sum"] and out["edge_length_m_sum"] > _EPS else None
    out["edge_ridge_to_eave_length_ratio"] = float(ridge_len / max(eave_len, _EPS)) if (ridge_len > 0 or eave_len > 0) else None
    out["edge_role_entropy"] = _entropy_from_counts(
        Counter({"ridge": ridge_count, "hip": hip_count, "valley": valley_count, "eave": eave_count, "rake": rake_count, "free": free_count})
    )
    ridge_az = None
    eave_az = None
    for e in edges:
        if ridge_az is None and e["horizontal"] and y_max is not None and float(e["mid"][1]) >= y_max - PLANE_INTERIOR_CLIP_MARGIN_M:
            ridge_az = e["azimuth"]
        if eave_az is None and e["horizontal"] and y_min is not None and float(e["mid"][1]) <= y_min + PLANE_INTERIOR_CLIP_MARGIN_M:
            eave_az = e["azimuth"]
    ...
    if nm is not None and np.linalg.norm(nm) > _EPS:
        ...
        for op in record.get("opposing_planes") or []:
            ...
            dihedrals.append(float(math.degrees(math.acos(max(-1.0, min(1.0, abs(float(np.dot(nm, on)))))))))
    ...
    out["edge_shared_dihedral_deg_mean"] = float(np.mean(dihedrals)) if dihedrals else None
    out["edge_shared_dihedral_deg_std"] = float(np.std(dihedrals)) if dihedrals else None
    out["edge_shared_dihedral_deg_count_under_120"] = int(sum(1 for d in dihedrals if d < 120.0))
    out["edge_shared_dihedral_deg_count_over_150"] = int(sum(1 for d in dihedrals if d > 150.0))
    ...
    for wid, info in (wall_index or {}).items():
        ...
        top_pts = [(float(p[0]), float(p[2])) for p in pts if abs(float(p[1]) - top_y) <= SCAN_NOISE_M]
        if len(top_pts) >= 2:
            top_line = None
            try:
        ...
        if wall_id in source_wall_ids:
            ...
            material = info.get("material") or info.get("wall_material") or info.get("classification")
    ...
    for e in edges:
        ...
        if e["azimuth"] is not None and wall_azimuths:
            nearest_wall_az = min(wall_azimuths, key=lambda waz: fe._azimuth_diff_deg(waz, e["azimuth"]))
            ...
            if abs(diff - 90.0) <= 10.0:
                orth_count += 1
    out["edge_to_nearest_wall_top_m_min"] = float(min(midpoint_dists)) if midpoint_dists else None
    out["edge_to_nearest_wall_top_m_median"] = float(np.median(midpoint_dists)) if midpoint_dists else None
    out["edge_aligned_with_wall_count"] = aligned_count
    out["edge_aligned_with_wall_length_fraction"] = float(aligned_len / out["edge_length_m_sum"]) if out["edge_length_m_sum"] and out["edge_length_m_sum"] > _EPS else None
    out["edge_orthogonal_to_wall_count"] = orth_count
    out["swall_length_median"] = float(np.median(resolved_wall_lengths)) if resolved_wall_lengths else None
    out["swall_top_y_min"] = float(min(resolved_wall_top_ys)) if resolved_wall_top_ys else None
    out["swall_top_y_max"] = float(max(resolved_wall_top_ys)) if resolved_wall_top_ys else None

    # Plane-fit / member distributions.
    out["plane_tilt_deg"] = plane_incl
    out["plane_slope_ratio"] = row.get("plane_rise_over_run")
    out["plane_pitch_is_medium"] = bool(20.0 <= plane_incl < 50.0) if plane_incl is not None else None
    out["plane_pitch_matches_dk_norm"] = bool(25.0 <= plane_incl <= 50.0) if plane_incl is not None else None
    ...
    for m in record.get("cluster_members") or []:
        ...
        if len(mp) >= 3 and len(plane) >= 3:
            ...
            if np.linalg.norm(mn) > _EPS and np.linalg.norm(nn) > _EPS:
                member_norm_cos.append(float(abs(np.dot(mn / np.linalg.norm(mn), nn / np.linalg.norm(nn)))))
        if len(mp) >= 4:
            member_ds.append(float(mp[3]))
    out["slant_residual_mae_m"] = float(np.mean(np.abs(member_residuals))) if member_residuals else None
    out["slant_residual_max_m"] = float(max(member_residuals)) if member_residuals else None
    out["slant_residual_iqr_m"] = _iqr(member_residuals)
    out["slant_residual_skewness"] = _skewness(member_residuals)
    out["plane_fit_r2"] = (
        float(1.0 - (np.var(member_residuals) / (np.var(ys) + _EPS))) if member_residuals and ys else None
    )
    ...
    out["plane_y_extent_vs_story_height"] = (
        float(out["plane_y_extent_m"] / story_h) if out["plane_y_extent_m"] is not None and story_h not in (None, 0) else None
    )
    out["plane_normal_consistency_index"] = float(np.mean(member_norm_cos)) if member_norm_cos else None
    out["plane_curvature_rms_m_per_m"] = (
        float(np.sqrt(np.mean(np.square(member_residuals))) / max(math.sqrt(own_area or 0.0), _EPS)) if member_residuals and own_area else None
    )
    out["plane_waviness_amplitude_m"] = (
        float(max(member_residuals) - min(member_residuals)) if len(member_residuals) >= 2 else None
    )
    if member_ds:
    ...
    if record.get("cluster_members"):
        ...
        if member_azs:
            ...
            out["member_azimuth_is_unimodal"] = bool(resultant > GABLE_UNIMODAL_RESULTANT_MIN)
            hist, _ = np.histogram(np.mod(member_azs, 180.0), bins=np.arange(0.0, 181.0, 15.0))
            peak_counts = sorted((int(v) for v in hist if v > 0), reverse=True)
            if len(peak_counts) >= 2 and peak_counts[0] > 0:
                out["member_azimuth_bimodality_index"] = float(peak_counts[1] / peak_counts[0])
            else:
        ...
        out["member_heuristic_rejected_fraction"] = float(heur.get("rejected", 0) / total_heur) if total_heur else None
        out["member_heuristic_uncertain_fraction"] = float((heur.get("uncertain", 0) + heur.get("not_evaluated", 0)) / total_heur) if total_heur else None
        out["member_heuristic_unanimous"] = bool(len([k for k, v in heur.items() if v > 0]) <= 1) if total_heur else None
        out["member_trace_rule_entropy"] = _entropy_from_counts(trace_rules)
        out["member_trace_top_rule"] = max(trace_rules.items(), key=lambda kv: kv[1])[0] if trace_rules else None
        ...
        for key in (
            ...
            out[key] = None if key not in ("member_heuristic_accepted_count", "member_heuristic_rejected_count", "member_trace_rules_count") else 0
    out["cluster_confidence_score"] = _safe_float((record.get("cluster_params") or {}).get("confidence"))
    normal_dot_min = _safe_float((record.get("cluster_params") or {}).get("normal_dot_min"))
    out["cluster_plane_incl_tolerance_deg"] = (
        float(math.degrees(math.acos(max(-1.0, min(1.0, normal_dot_min))))) if normal_dot_min is not None else None
    )
    out["cluster_plane_azimuth_tolerance_deg"] = out["cluster_plane_incl_tolerance_deg"]
    out["cluster_plane_d_tolerance_m"] = _safe_float((record.get("cluster_params") or {}).get("d_abs_max"))
    ...
    out["poly_centroid_x_rel_bld_center"] = (
        ...
        if out["poly_centroid_x"] is not None and row.get("bld_footprint_centroid_x") is not None
        else None
    )
    out["poly_centroid_z_rel_bld_center"] = (
        float(out["poly_centroid_z"] - row["bld_footprint_centroid_z"])
        if out["poly_centroid_z"] is not None and row.get("bld_footprint_centroid_z") is not None
        else None
    )
    if centroid is not None and row.get("bld_footprint_centroid_x") is not None and row.get("bld_footprint_centroid_z") is not None:
        d_center = math.hypot(
        ...
        out["distance_to_footprint_center_normalised"] = (
            float(d_center / math.sqrt(row["bld_footprint_area_m2"])) if row.get("bld_footprint_area_m2") else None
        )
    else:
    ...
    if centroid is not None and major_az is not None and row.get("bld_footprint_area_m2"):
        major_vec = np.asarray([math.sin(math.radians(major_az)), math.cos(math.radians(major_az))], dtype=float)
        rel = np.asarray(
            [
        ...
        out["poly_short_axis_projection_m"] = float(np.dot(rel, np.asarray([major_vec[1], -major_vec[0]], dtype=float)))
        out["poly_long_axis_projection_normalised"] = float(
            out["poly_long_axis_projection_m"] / max(math.sqrt(row["bld_footprint_area_m2"]), _EPS)
        )
    else:
    ...
    out["height_above_ground_fraction"] = (
        ...
        if out["height_above_ground_m"] is not None and row.get("bld_height_m") not in (None, 0)
        else None
    )
    out["plane_azimuth_sin"] = float(math.sin(math.radians(plane_az))) if plane_az is not None else None
    out["plane_azimuth_cos"] = float(math.cos(math.radians(plane_az))) if plane_az is not None else None
    ...
    out["plane_az_vs_nearest_wall_deg"] = (
        min(fe._azimuth_diff_deg(plane_az, waz) for waz in wall_azimuths) if plane_az is not None and wall_azimuths else None
    )
    out["plane_incl_sin"] = float(math.sin(math.radians(plane_incl))) if plane_incl is not None else None
    out["plane_incl_cos"] = float(math.cos(math.radians(plane_incl))) if plane_incl is not None else None
    out["plane_incl_bucket_5"] = _incl_bucket(plane_incl)
    out["plane_incl_is_dk_typical"] = bool(25.0 <= plane_incl <= 50.0) if plane_incl is not None else None
    out["plane_incl_too_steep_for_residential"] = bool(plane_incl > 60.0) if plane_incl is not None else None
    out["plane_incl_too_shallow_for_oblique"] = bool(plane_incl < 5.0) if plane_incl is not None else None
    normals = []
    ds_norm = []
    for m in record.get("cluster_members") or []:
        p = m.get("plane") or []
    ...
    out["area_vs_footprint_area_ratio"] = float(own_area / row["bld_footprint_area_m2"]) if own_area and row.get("bld_footprint_area_m2") else None
    out["perimeter_vs_footprint_perimeter_ratio"] = float(row["poly_perimeter_xz_m"] / row["bld_footprint_perimeter_m"]) if row.get("poly_perimeter_xz_m") and row.get("bld_footprint_perimeter_m") else None
    part_poly = _xz_poly(part.get("footprint_xz") or []) if part else None
    out["area_vs_part_area_ratio"] = float(own_area / part_poly.area) if own_area and part_poly is not None and part_poly.area > _EPS else None
    out["perimeter_vs_part_perimeter_ratio"] = float(row["poly_perimeter_xz_m"] / part_poly.length) if row.get("poly_perimeter_xz_m") and part_poly is not None and part_poly.length > _EPS else None
    out["plane_top_y_vs_bld_height_ratio"] = float(out["plane_top_y_m"] / row["bld_height_m"]) if out.get("plane_top_y_m") is not None and row.get("bld_height_m") not in (None, 0) else None
    out["plane_y_extent_vs_story_height_ratio"] = out["plane_y_extent_vs_story_height"]
    out["opposing_cluster_unique_count"] = len(set(record.get("opposing_cluster_canonicals") or []))
    if record.get("opposing_planes"):
        ...
        out["opposing_incl_diff_mean_deg"] = float(np.mean(op_diffs)) if op_diffs else None
        ...
        out["opposing_gable_incl_asymmetry_deg"] = op_diffs[0] if len(op_diffs) == 1 else None
        out["opposing_gable_azimuth_offset_deg"] = abs(op_az_diffs[0] - 180.0) if len(op_az_diffs) == 1 else None
        out["opposing_is_hip_trio"] = bool(len(record.get("opposing_planes") or []) == 3)
        out["opposing_is_hip_quartet"] = bool(len(record.get("opposing_planes") or []) == 4)
        out["opposing_hip_closure_angle_deg"] = float(sum(op_az_diffs)) if op_az_diffs else None
    else:
        for key in (
    ...
    for peer, peer_poly in peer_polys:
        ...
        paz, pincl = fe._plane_to_azimuth_incl(pplane) if pplane else (float("nan"), float("nan"))
        if plane_az is not None and plane_incl is not None and not math.isnan(paz) and not math.isnan(pincl):
            pdiff = abs(float(pplane[3]) - float(plane[3])) if len(pplane) >= 4 and len(plane) >= 4 else 999.0
            if fe._azimuth_diff_deg(plane_az, float(paz)) < 5.0 and abs(plane_incl - float(pincl)) < 3.0 and pdiff < COPLANAR_D_TOL_M:
                coplanar.append(peer_poly)
    out["opposing_same_part_count"] = same_part_count
    out["opposing_cross_part_count"] = cross_part_count
    out["opposing_cross_part_fraction"] = float(cross_part_count / max((record.get("opposing_planes") and len(record.get("opposing_planes"))) or 0, 1)) if record.get("opposing_planes") else None
    out["coplanar_peer_count"] = len(coplanar)
    if seg_poly is not None and coplanar:
        union = _safe_unary_union(coplanar)
        out["coplanar_peer_union_area_m2"] = float(union.area) if union is not None else None
        out["coplanar_peer_overlap_area_m2"] = float(sum(seg_poly.intersection(p).area for p in coplanar))
    ...
    out["swall_unresolved_count"] = max((row.get("cluster_member_count") or 0) - (row.get("swall_resolved_count") or 0), 0)
    exterior_count = int(
        sum(
            1
    ...
    out["swall_is_interior_fraction"] = (
        float(out["swall_is_interior_count"] / len(source_wall_stats)) if source_wall_stats else None
    )
    out["swall_supports_only_interior"] = bool(source_wall_stats) and exterior_count == 0
    ...
    room_ids = {rr.get("room_id") for rr in record.get("room_boundary_refs") or [] if rr.get("room_id")}
    ...
    room_slab_pairs = [
        (slab, poly)
        for slab, poly in zip(slabs, room_slab_polys)
        if poly is not None and slab.get("room_id") in touched_room_ids
    ]
    room_floor_areas = [float(poly.area) for _, poly in room_slab_pairs if poly.area > _EPS]
    out["room_floor_area_total_m2"] = float(sum(room_floor_areas)) if room_floor_areas else None
    out["room_floor_area_mean_m2"] = float(np.mean(room_floor_areas)) if room_floor_areas else None
    if seg_poly is not None and room_slab_pairs:
        ...
        total_overlap = float(sum(seg_poly.intersection(poly).area for _, poly in room_slab_pairs))
        out["seg_local_slant_fraction_of_touched_rooms"] = float(total_overlap / max(total_room_area, _EPS))
    else:
    ...
    if part is not None:
        ...
        part_story_indices = sorted(
            story for story in (part.get("stories") or []) if _coerce_int(story) is not None
        )
        part_story_indices = [_coerce_int(story) for story in part_story_indices]
        part_story_indices = [story for story in part_story_indices if story is not None]
        out["part_story_index_min"] = min(part_story_indices) if part_story_indices else None
        out["part_story_index_max"] = max(part_story_indices) if part_story_indices else None
        out["part_story_index_range"] = (
            max(part_story_indices) - min(part_story_indices) if part_story_indices else None
        )
        out["part_has_basement"] = bool(part_story_indices and min(part_story_indices) < 0)
    else:
    ...
    out["story_is_top_only"] = bool(len(room_story_indices) == 1 and room_story_indices and row.get("bld_story_count") and room_story_indices[0] >= row["bld_story_count"] - 1)
    ...
    out["story_index_range"] = (max(room_story_indices) - min(room_story_indices)) if room_story_indices else None
    ...
    if part is not None:
        ...
        out["part_gable_uncovered_region_area_m2"] = float(uncovered.area) if uncovered is not None else None
        if seg_poly is not None and uncovered is not None and own_area:
            out["seg_covers_gable_uncovered_region_fraction"] = float(seg_poly.intersection(uncovered).area / own_area)
        else:
            out["seg_covers_gable_uncovered_region_fraction"] = None
        if ridge_line and seg_poly is not None:
            ridge = LineString([(float(ridge_line[0][0]), float(ridge_line[0][2])), (float(ridge_line[1][0]), float(ridge_line[1][2]))])
            out["seg_intersects_gable_ridge_line"] = _safe_intersects(seg_poly.buffer(SCAN_NOISE_M), ridge)
        else:
    ...
    out["bld_slab_count"] = building_context.get("slab_count") if building_context else None
    out["bld_flat_ceiling_count"] = building_context.get("flat_ceiling_count") if building_context else None
    out["bld_slanted_roof_count"] = building_context.get("slanted_roof_count") if building_context else None
    out["bld_roof_proposal_count"] = building_context.get("roof_proposals_count") if building_context else None
    out["bld_merged_roof_segment_count"] = building_context.get("merged_roof_segments_count") if building_context else None
    out["bld_dormer_count"] = building_context.get("dormer_count") if building_context else None
    out["bld_unresolved_region_count"] = building_context.get("unresolved_region_count") if building_context else None
    out["bld_part_count"] = building_context.get("part_count") if building_context else None
    out["bld_proposal_density"] = (
        float(out["bld_merged_roof_segment_count"] / row["bld_footprint_area_m2"])
        if out.get("bld_merged_roof_segment_count") is not None and row.get("bld_footprint_area_m2")
        else None
    )
    out["bld_complexity_index"] = (
        float((out.get("bld_part_count") or 0) * (row.get("bld_story_count") or 0) * (row.get("bld_footprint_bbox_aspect") or 0))
        if out.get("bld_part_count") is not None
        else None
    )
    out["seg_nearest_slab_distance_m"] = _nearest_distance_to_polys(seg_poly, slab_polys)
    out["seg_is_above_slab_count"] = int(sum(1 for p in slab_polys if seg_poly is not None and p.intersects(seg_poly)))
    out["bld_slab_area_total_m2"] = float(sum(p.area for p in slab_polys)) if slab_polys else 0.0
    slanted_polys = [_xz_poly(s.get("corners") or s.get("footprint_xz") or []) for s in (building_context.get("slanted_roofs") or [])] if building_context else []
    slanted_polys = [p for p in slanted_polys if p is not None]
    out["bld_slanted_area_total_m2"] = float(sum(p.area for p in slanted_polys)) if slanted_polys else 0.0
    ...
    out["seg_small_partial_room_slant_score"] = (
        float(
            ...
            * max(0.0, 1.0 - min((out.get("bld_slanted_area_fraction") or 1.0) / 0.35, 1.0))
        )
        if out.get("seg_room_coverage_fraction_mean") is not None and out.get("bld_slanted_area_fraction") is not None
        else None
    )

    wall_ext = building_context.get("wall_extensions") or [] if building_context else []
    ext_polys = [_xz_poly(w.get("strip_corners") or []) for w in wall_ext]
    ext_pairs = [(w, p) for w, p in zip(wall_ext, ext_polys) if p is not None]
    if seg_poly is not None:
        touching = [(w, p) for w, p in ext_pairs if p.intersects(seg_poly)]
        out["seg_wall_extension_contact_count"] = len(touching)
        out["seg_wall_extension_total_length_m"] = float(sum(p.length for _, p in touching)) if touching else 0.0
        out["seg_has_behind_knee_wall_extension"] = bool(any(w.get("behind_knee_wall") for w, _ in touching))
    else:
    ...
    out["bld_knee_wall_total_length_m"] = float(sum(p.length for _, p in knee_pairs)) if knee_pairs else 0.0
    support_terms = []
    for w, p in knee_pairs:
        shell_len = float(p.length)
        if shell_len > _EPS:
            support_terms.append(float(p.intersection(seg_poly).length / shell_len) if seg_poly is not None else 0.0)
    ...
    out["bld_knee_wall_dropped_count"] = int(
        sum(1 for w, _ in knee_pairs if str((w.get("trace") or {}).get("decision", "")).lower() == "dropped")
    )
    if seg_poly is not None and knee_pairs:
        nearest_knee = min(knee_pairs, key=lambda item: float(seg_poly.distance(item[1])))
        ...
        out["seg_kneewall_span_fraction"] = (
            float(seg_poly.intersection(nearest_poly.buffer(WALL_SUPPORT_BUFFER_M)).area / max(seg_poly.area, _EPS))
        )
        ...
        out["seg_kneewall_eave_consistency"] = (
            float(1.0 / (1.0 + abs((row.get("plane_bottom_y_m") or 0.0) - max(float(pt[1]) for pt in strip))))
    ...
    flat_pairs = [(c, p) for c, p in zip(flats, flat_polys) if p is not None]
    if seg_poly is not None and own_area:
        ...
        out["seg_is_mostly_a_flat_ceiling"] = bool(
            overlap / own_area > FLAT_CEILING_DOMINANCE_RATIO and (plane_incl or 0.0) < 10.0
        )
    else:
        out["seg_flat_ceiling_overlap_m2"] = None
        out["seg_flat_ceiling_overlap_fraction"] = None
        out["seg_is_mostly_a_flat_ceiling"] = None
    out["bld_flat_ceiling_area_total_m2"] = float(sum(p.area for _, p in flat_pairs)) if flat_pairs else 0.0

    gaps = building_context.get("gaps") or [] if building_context else []
    gap_polys = [_xz_poly(g.get("footprint_xz") or []) for g in gaps]
    gap_pairs = [(g, p) for g, p in zip(gaps, gap_polys) if p is not None]
    out["seg_gap_proximity_m"] = _nearest_distance_to_polys(seg_poly, [p for _, p in gap_pairs])
    out["seg_gap_adjacent_count"] = int(sum(1 for _, p in gap_pairs if seg_poly is not None and seg_poly.distance(p) <= 0.5))
    out["bld_gap_floor_area_m2"] = float(sum(p.area for _, p in gap_pairs)) if gap_pairs else 0.0
    out["bld_gap_to_footprint_ratio"] = float(out["bld_gap_floor_area_m2"] / row["bld_footprint_area_m2"]) if row.get("bld_footprint_area_m2") else None
    out["bld_gap_status_entropy"] = _entropy_from_counts(Counter(g.get("status") for g, _ in gap_pairs if g.get("status")))
    ...
    unresolved_pairs = [(u, p) for u, p in zip(unresolved, unresolved_polys) if p is not None]
    out["seg_unresolved_region_proximity_m"] = _nearest_distance_to_polys(seg_poly, [p for _, p in unresolved_pairs])
    if seg_poly is not None:
        out["seg_unresolved_region_overlap_m2"] = float(sum(seg_poly.intersection(p).area for _, p in unresolved_pairs))
    else:
        out["seg_unresolved_region_overlap_m2"] = None
    out["bld_unresolved_region_area_total_m2"] = float(sum(p.area for _, p in unresolved_pairs)) if unresolved_pairs else 0.0
    if unresolved_pairs:
        reasons = Counter(u.get("reason") for u, _ in unresolved_pairs if u.get("reason"))
        out["bld_unresolved_reason_mode"] = max(reasons.items(), key=lambda kv: kv[1])[0] if reasons else None
    else:
    ...
    out["trace_decision_reason_token_entropy"] = _entropy_from_counts(Counter(reason_tokens)) if reason_tokens else None
    ...
    if seg_poly is not None:
        ...
        out["poly_outside_footprint_fraction"] = (
            float((row.get("overshoot_area_m2") or 0.0) / own_area) if own_area else None
        )
        out["distance_to_footprint_edge_normalised"] = (
            float((row.get("distance_to_footprint_edge_m") or 0.0) / math.sqrt(row.get("bld_footprint_area_m2") or 1.0))
            if row.get("distance_to_footprint_edge_m") is not None and row.get("bld_footprint_area_m2")
            else None
        )
        try:
            shell_core = polylabel(seg_poly)
            out["plane_interior_crossing_depth_m"] = float(shell_core.distance(seg_poly.boundary))
        except Exception:
        ...
        if centroid is not None and footprint_corners:
            corner_d = min(math.hypot(centroid.x - x, centroid.y - y) for x, y in footprint_corners)
    ...
    if boundary_poly is not None and edges:
        ...
        out["plane_exterior_edge_contact_fraction"] = (
            float(exterior_edge_len / max(total_edge_len, _EPS)) if total_edge_len > _EPS else None
        )
        out["plane_eave_edge_to_exterior_shell_m"] = float(min(eave_shell_dists)) if eave_shell_dists else None
    ...
    out["convex_deficiency"] = (
        1.0 - row["poly_convex_hull_ratio"] if row.get("poly_convex_hull_ratio") is not None else None
    )
    ...
    out["roundness_ratio"] = (
        float(4.0 * (own_area or 0.0) / (math.pi * (row.get("poly_min_rect_major_m") or 1.0) ** 2))
    ...
    radial_sig = adv._radial_signature(_ring_points_xy(corners), n_samples=16) if len(corners) >= 3 else []
    ...
    if seg_poly is not None and not seg_poly.is_empty:
        ...
        try:
            ...
            out["symmetry_reflect_principal_iou"] = _symmetry_iou(seg_poly, refl_principal)
            candidates = {
            ...
            best = max(
                ((angle, score) for angle, score in candidates.items() if score is not None),
    ...
    same_part_peers = [peer_poly for peer, peer_poly in peer_polys if peer_poly is not None and _same_part(peer, part, building_context)]
    all_seg_polys = [p for _, p in peer_polys if p is not None]
    if seg_poly is not None and part_poly is not None:
        union = _safe_unary_union([seg_poly, *same_part_peers]) if same_part_peers else seg_poly
        if union is None:
            union = seg_poly
        out["part_coverage_union_area_m2"] = float(union.area)
        out["part_coverage_union_to_footprint_ratio"] = float(union.area / part_poly.area) if part_poly.area > _EPS else None
        out["part_coverage_gap_area_m2"] = float(max(part_poly.area - union.area, 0.0))
        out["part_coverage_gap_fraction"] = float(out["part_coverage_gap_area_m2"] / part_poly.area) if part_poly.area > _EPS else None
        cover_sum = own_area + sum(float(p.area) for p in same_part_peers)
        out["part_coverage_over_cover_area_m2"] = float(max(cover_sum - union.area, 0.0))
        out["part_coverage_over_cover_fraction"] = float(out["part_coverage_over_cover_area_m2"] / max(union.area, _EPS))
        contrib_union = _safe_unary_union(same_part_peers) if same_part_peers else None
        if contrib_union is not None:
            out["this_seg_coverage_contribution_m2"] = float(max(union.area - contrib_union.area, 0.0))
        ...
        out["this_seg_coverage_contribution_fraction"] = (
            float(out["this_seg_coverage_contribution_m2"] / union.area) if union.area > _EPS else None
        )
        gap_region = part_poly.difference(union)
        out["this_seg_covers_gap_region"] = bool(not gap_region.is_empty and seg_poly.intersects(gap_region))
    else:
        for key in (
    ...
    if seg_poly is not None and boundary_poly is not None:
        all_union = _safe_unary_union([seg_poly, *all_seg_polys]) if all_seg_polys else seg_poly
        if all_union is None:
            all_union = seg_poly
        out["bld_coverage_union_area_m2"] = float(all_union.area)
        out["bld_coverage_union_to_footprint_ratio"] = float(all_union.area / boundary_poly.area) if boundary_poly.area > _EPS else None
        out["bld_coverage_gap_fraction"] = float(max(boundary_poly.area - all_union.area, 0.0) / boundary_poly.area) if boundary_poly.area > _EPS else None
        out["bld_coverage_over_cover_fraction"] = float(max((own_area or 0.0) + sum(float(p.area) for p in all_seg_polys) - all_union.area, 0.0) / max(all_union.area, _EPS))
    else:
    ...
    if top_vals and bottom_vals and row.get("plane_y_extent_m") is not None:
        ...
        out["this_seg_y_range_fraction"] = float(row["plane_y_extent_m"] / max(part_range, _EPS))
        out["this_seg_is_above_part_median_y"] = bool((row.get("plane_mid_y_m") or 0.0) >= float(np.median(top_vals)))
    else:
    ...
    if own_area is not None:
        all_areas = sorted([own_area, *peer_areas], reverse=True)
        out["seg_rank_in_bld_by_area"] = 1 + sum(1 for a in peer_areas if a > own_area)
        out["seg_percentile_area_in_bld"] = float(sum(1 for a in peer_areas if a <= own_area) / max(len(peer_areas), 1))
    else:
        out["seg_rank_in_bld_by_area"] = None
        out["seg_percentile_area_in_bld"] = None
    peer_tops = [max(float(c[1]) for c in (peer.get("corners") or [])) for peer, _ in peer_polys if peer.get("corners")]
    if row.get("plane_top_y_m") is not None:
        out["seg_rank_in_bld_by_top_y"] = 1 + sum(1 for y in peer_tops if y > row["plane_top_y_m"])
        out["segment_height_vs_max_segment_height"] = float(row["plane_top_y_m"] / max([row["plane_top_y_m"], *peer_tops])) if [row["plane_top_y_m"], *peer_tops] else None
    else:
    ...
    if plane_incl is not None:
        out["seg_rank_in_bld_by_incl"] = 1 + sum(1 for x in peer_incls if x > plane_incl)
        out["seg_percentile_incl_in_bld"] = float(sum(1 for x in peer_incls if x <= plane_incl) / max(len(peer_incls), 1))
        out["seg_is_bld_unique_by_incl"] = bool(sum(1 for x in peer_incls if abs(x - plane_incl) <= 5.0) == 0)
        peer_azs = []
        for peer, _ in peer_polys:
            paz, _ = fe._plane_to_azimuth_incl(peer.get("merged_plane") or [])
            if not math.isnan(paz):
                peer_azs.append(float(paz))
        out["seg_is_bld_unique_by_az"] = bool(sum(1 for x in peer_azs if fe._azimuth_diff_deg(x, plane_az or 0.0) <= 15.0) == 0) if plane_az is not None else None
    else:
    ...
    same_cluster = [peer for peer, _ in peer_polys if peer.get("cluster_canonical_id") == record.get("cluster_canonical_id")]
    same_cluster_areas = [
    ...
    out["seg_rank_in_cluster_by_area"] = 1 + sum(1 for a in same_cluster_areas if own_area is not None and a > own_area) if own_area is not None else None
    ...
    same_part_top_peers = [
        max(float(c[1]) for c in (peer.get("corners") or []))
        for peer, peer_poly in peer_polys
        if peer_poly is not None and _same_part(peer, part, building_context) and peer.get("corners")
    ]
    ...
    out["seg_rank_in_part_by_area"] = 1 + sum(1 for a in same_part_area_peers if own_area is not None and a > own_area) if own_area is not None else None
    out["seg_rank_in_part_by_height"] = 1 + sum(1 for y in same_part_top_peers if row.get("plane_top_y_m") is not None and y > row["plane_top_y_m"]) if row.get("plane_top_y_m") is not None else None
    out["seg_rank_in_part_by_incl"] = 1 + sum(1 for x in same_part_incl_peers if plane_incl is not None and x > plane_incl) if plane_incl is not None else None
    out["seg_fraction_of_part_roof_area"] = (
        float((own_area or 0.0) / ((own_area or 0.0) + sum(same_part_area_peers)))
        if own_area is not None and ((own_area or 0.0) + sum(same_part_area_peers)) > _EPS
        else None
    )
    out["seg_is_part_primary_roof"] = bool(
        out.get("seg_rank_in_part_by_area") == 1 and (plane_incl or 0.0) > 20.0
    ) if out.get("seg_rank_in_part_by_area") is not None else None
    ...
    out["member_count_vs_part_member_count"] = (
        float((row.get("cluster_member_count") or 0) / max((row.get("cluster_member_count") or 0) + sum((peer.get("features") or {}).get("member_count", 0) for peer, peer_poly in peer_polys if peer_poly is not None and _same_part(peer, part, building_context)), 1))
        if row.get("cluster_member_count") is not None
        else None
    )
    out["edge_count_vs_median_in_part"] = (
        float((row.get("edge_count") or 0) / max(np.median([row.get("edge_count") or 0] + [len(_edge_records(peer.get("corners") or [])) for peer, peer_poly in peer_polys if peer_poly is not None and _same_part(peer, part, building_context)]), 1.0))
        if row.get("edge_count") is not None
        else None
    )
    out["edges_vs_part_edges_ratio"] = (
        float((row.get("edge_count") or 0) / max((row.get("edge_count") or 0) + sum(len(_edge_records(peer.get("corners") or [])) for peer, peer_poly in peer_polys if peer_poly is not None and _same_part(peer, part, building_context)), 1))
        if row.get("edge_count") is not None
        else None
    )
    out["hull_efficiency_vs_part_median"] = (
        float((row.get("poly_convex_hull_ratio") or 0.0) / max(np.median([row.get("poly_convex_hull_ratio") or 0.0] + [(peer.get("features") or {}).get("poly_convex_hull_ratio", row.get("poly_convex_hull_ratio") or 0.0) for peer, peer_poly in peer_polys if peer_poly is not None and _same_part(peer, part, building_context)]), _EPS))
    ...
    if plane_az is not None:
        ...
        if same_part_peer_az:
            out["azimuth_spread_relative_to_part"] = float(
                ...
                / max(max(same_part_peer_az + [plane_az]) - min(same_part_peer_az + [plane_az]), _EPS)
            )
        else:
        ...
        if centroid is not None and len(coords) >= 2:
            for i in range(len(coords)):
                ...
                if nearest_dist is None or d < nearest_dist:
                    nearest_dist = d
                    nearest_edge = ((a[0] + b[0]) / 2 - centroid.x, (a[1] + b[1]) / 2 - centroid.y)
        if nearest_edge is not None:
            flow = np.asarray([math.sin(math.radians(plane_az)), math.cos(math.radians(plane_az))], dtype=float)
    ...
    if plane_incl is not None and row.get("height_above_ground_m") is not None and plane_incl > 1.0:
        out["drainage_to_ground_distance_m"] = float(row["height_above_ground_m"] / max(math.tan(math.radians(plane_incl)), _EPS))
    else:
    ...
    if seg_poly is not None and wall_top_lines:
        ...
        for e in edges:
            line = LineString([(float(e["a"][0]), float(e["a"][2])), (float(e["b"][0]), float(e["b"][2]))])
            touching = any(_safe_intersects(line.buffer(WALL_SUPPORT_BUFFER_M), wl) for wl in wall_top_lines)
            if touching:
                support_count += 1
        ...
        out["seg_supported_by_wall_fraction"] = float(support_lengths / max(out["edge_length_m_sum"] or 0.0, _EPS))
        out["seg_has_eave_overhang"] = bool(overhang > EAVE_OVERHANG_MIN_M)
        out["seg_overhang_length_m"] = float(overhang)
        out["seg_is_cantilevered"] = bool(support_lengths < 0.5 * max(out["edge_length_m_sum"] or 0.0, _EPS))
    else:
    ...
    out["seg_area_moment_about_centroid_m4"] = (
        float((own_area or 0.0) * ((row.get("poly_min_rect_major_m") or 0.0) ** 2 + (row.get("poly_min_rect_minor_m") or 0.0) ** 2) / 12.0)
        if own_area is not None
        else None
    )
    ...
    out["plane_top_y_below_br18_cap"] = bool((row.get("plane_top_y_m") or 0.0) < 12.0) if row.get("plane_top_y_m") is not None else None
    out["plane_top_y_above_building_physics_limit"] = bool((row.get("plane_top_y_m") or 0.0) > 20.0) if row.get("plane_top_y_m") is not None else None
    out["plane_bottom_y_below_slab"] = bool((row.get("plane_bottom_y_m") or 0.0) < (row.get("bld_y_min_m") or 0.0)) if row.get("plane_bottom_y_m") is not None and row.get("bld_y_min_m") is not None else None
    out["plane_normal_dot_gravity"] = float(abs(plane[1]) / np.linalg.norm(np.asarray(plane[:3], dtype=float))) if len(plane) >= 3 and np.linalg.norm(np.asarray(plane[:3], dtype=float)) > _EPS else None
    ...
    out["seg_ridge_is_at_part_top"] = bool(
        ridge_len > 0.0
        and row.get("plane_top_y_m") is not None
        and top_vals
        and abs(row["plane_top_y_m"] - max(top_vals)) <= PLANE_INTERIOR_CLIP_MARGIN_M
    ) if top_vals else None
    out["seg_eave_is_at_wall_top"] = bool(
        eave_len > 0.0
        and y_min is not None
        and wall_top_ys
        and min(abs(y_min - wy) for wy in wall_top_ys) <= PLANE_INTERIOR_CLIP_MARGIN_M
    ) if y_min is not None and wall_top_ys else None
    out["seg_ridge_colinear_with_part_major"] = bool(
        ridge_len > 0.0 and part is not None and (part.get("gable_extension") or {}).get("metrics", {}).get("major_az") is not None and plane_az is not None and fe._azimuth_diff_deg(plane_az, float((part.get("gable_extension") or {}).get("metrics", {}).get("major_az"))) < 15.0
    ) if ridge_len > 0.0 else None
    out["seg_ridge_colinear_with_bld_major"] = bool(ridge_len > 0.0 and plane_az is not None and major_az is not None and fe._azimuth_diff_deg(plane_az, major_az) < 15.0) if ridge_len > 0.0 else None
    ...
    if part_poly is not None:
        ...
        if len(part_coords) >= 3:
            ...
            major = max(pe1, pe2)
            minor = min(pe1, pe2)
            out["part_footprint_elongation"] = float(1.0 - (minor / major)) if major > _EPS else None
        else:
            out["part_footprint_elongation"] = None
        out["part_footprint_solidity"] = float(part_poly.area / part_hull.area) if part_hull.area > _EPS else None
    else:
        out["part_footprint_elongation"] = None
        out["part_footprint_solidity"] = None
    out["part_has_kneewall"] = bool(out.get("seg_has_behind_knee_wall_extension"))
    out["bld_has_attic_story"] = bool(out.get("bld_knee_wall_count") or out.get("typ_knee_extended_candidate"))
    ...
    out["bld_footprint_convexity_deficiency"] = row.get("bld_footprint_convexity_deficiency")
    out["bld_footprint_interior_ring_count"] = row.get("bld_footprint_interior_ring_count")
    ...
    out["bld_footprint_orientation_rank_cluster"] = (
        int(round((row.get("bld_footprint_principal_axis_deg") or 0.0) / 15.0))
    ...
    if room_ids:
        ...
        out["room_is_top_story_count"] = int(sum(1 for s in room_story_indices if row.get("bld_story_count") is not None and s >= row["bld_story_count"] - 1))
        out["room_is_top_story_fraction"] = float(out["room_is_top_story_count"] / max(len(room_story_indices), 1))
    else:
    ...
    dormer_polys = [_xz_poly(d.get("corners") or d.get("footprint_xz") or []) for d in dormers]
    dormer_pairs = [(d, p) for d, p in zip(dormers, dormer_polys) if p is not None]
    out["bld_dormer_total_front_wall_length_m"] = float(sum(p.length for _, p in dormer_pairs)) if dormer_pairs else 0.0
    out["bld_dormer_total_surface_area_m2"] = float(sum(p.area for _, p in dormer_pairs)) if dormer_pairs else 0.0
    ...
    out["room_has_dormer_count"] = int(sum(1 for d, _ in dormer_pairs if d.get("roof_surface_id")))
    out["seg_is_dormer_front"] = bool(any(d.get("front_wall_id") in (m.get("source_wall_id") for m in record.get("cluster_members") or []) for d, _ in dormer_pairs))
    out["seg_is_dormer_top"] = bool(any(d.get("roof_surface_id") == record.get("proposal_id") for d, _ in dormer_pairs))
    out["seg_dormer_proximity_m"] = _nearest_distance_to_polys(seg_poly, [p for _, p in dormer_pairs])
    out["seg_contains_dormer_locus_count"] = int(sum(1 for _, p in dormer_pairs if seg_poly is not None and _safe_intersects(seg_poly, p)))
    ...
    out["seg_requires_dormer_second_pass"] = bool(
        ...
        and (
            ...
            or (out.get("seg_dormer_proximity_m") is not None and out["seg_dormer_proximity_m"] <= 1.5)
        )
    )
    ...
    opp_areas = [
        float(peer_poly.area)
        for peer, peer_poly in peer_polys
        if peer_poly is not None and peer.get("cluster_canonical_id") in set(record.get("opposing_cluster_canonicals") or [])
    ]
    out["opposing_gable_area_ratio"] = (
        float((own_area or 0.0) / max(opp_areas[0], _EPS)) if own_area is not None and len(opp_areas) == 1 else None
    )
    out["opposing_gable_eave_y_asymmetry_m"] = (
        float(abs((row.get("plane_bottom_y_m") or 0.0) - min(float(c[1]) for c in (peers[0].get("corners") or []))))
    ...
    if len(peers) == 1 and ridge_az is not None:
        ...
        for e in peer_edges:
            if e["horizontal"] and peer_ymax is not None and float(e["mid"][1]) >= peer_ymax - PLANE_INTERIOR_CLIP_MARGIN_M:
    ...
    if same_part_drain:
        ...
        out["part_drainage_azimuth_resultant"] = float(math.degrees(math.atan2(resultant[0], resultant[1])) % 360.0)
        total_area = sum(area for _, area in same_part_drain)
        out["part_drainage_balance"] = float(np.linalg.norm(resultant) / max(total_area, _EPS))
        out["part_drainage_azimuths_spread_deg"] = float(max(spread) - min(spread)) if len(spread) >= 2 else 0.0
        quadrants = {int(((azv % 360.0) + 45.0) // 90.0) % 4 for azv, _ in same_part_drain}
    ...
    if centroid is not None and wall_centroids and plane_az is not None:
        gutter = min(wall_centroids, key=lambda pt: math.hypot(pt[0] - centroid.x, pt[1] - centroid.y))
        gutter_vec = np.asarray([gutter[0] - centroid.x, gutter[1] - centroid.y], dtype=float)
        flow = np.asarray([math.sin(math.radians(plane_az)), math.cos(math.radians(plane_az))], dtype=float)
    ...
    if centroid is not None and plane_az is not None:
        ...
        for peer, peer_poly in peer_polys:
            ...
            vec = np.asarray([peer_centroid.x - centroid.x, peer_centroid.y - centroid.y], dtype=float)
        ...
        out["seg_windward_score"] = (
            float(math.cos(math.radians(abs(fe._azimuth_diff_deg(plane_az, prevailing)))))
    ...
    out["typ_hip_candidate"] = bool(out.get("opposing_is_hip_trio") or out.get("opposing_is_hip_quartet"))
    out["typ_hip_closure_deg"] = out.get("opposing_hip_closure_angle_deg")
    out["typ_shed_candidate"] = bool((record.get("opposing_planes") and len(record.get("opposing_planes")) <= 1) or not record.get("opposing_planes")) and bool(plane_incl is not None and 5.0 <= plane_incl <= 30.0)
    out["typ_shed_is_architectural"] = bool((plane_incl or 0.0) > 10.0 and (own_area or 0.0) > 5.0)
    out["typ_shed_is_outbuilding_scale"] = bool(
        (own_area or 0.0) < 20.0 and (out.get("area_vs_footprint_area_ratio") or 0.0) < FLAT_CEILING_SPREAD_M
    )
    out["typ_flat_candidate"] = bool((plane_incl or 90.0) < 5.0) if plane_incl is not None else None
    out["typ_flat_is_ceiling_not_roof"] = out.get("seg_is_mostly_a_flat_ceiling")
    out["typ_pyramid_candidate"] = bool(
        out.get("opposing_is_hip_quartet") and (row.get("poly_bbox_aspect") or 99.0) < PYRAMID_ASPECT_MAX
    )
    out["typ_lplan_candidate"] = bool(row.get("bld_footprint_is_L_shape") and out.get("typ_gable_candidate"))
    out["typ_tplan_candidate"] = bool(row.get("bld_footprint_is_T_shape") and out.get("typ_gable_candidate"))
    out["typ_uplan_candidate"] = bool(row.get("bld_footprint_is_U_shape") and out.get("typ_gable_candidate"))
    out["typ_knee_extended_candidate"] = bool(out.get("seg_has_behind_knee_wall_extension"))
    out["typ_complex_candidate"] = bool((out.get("bld_slanted_roof_count") or 0) >= 5 and not any(out.get(k) for k in ("typ_gable_candidate", "typ_hip_candidate", "typ_shed_candidate", "typ_flat_candidate")))
    out["typ_gable_ridge_horizontality"] = (
        float(ridge_len / max(out.get("edge_ridge_length_m") or 0.0, _EPS)) if ridge_len > 0.0 else None
    )
    ridge_az = None
    eave_az = None
    for e in edges:
    ...
    out["typ_hip_has_apex_point"] = bool(out.get("typ_hip_candidate") and out.get("vtx_ridge_band_count", 0) <= 1)
    out["typ_hip_aspect_ratio"] = row.get("poly_bbox_aspect") if out.get("typ_hip_candidate") else None
    out["typ_hip_apex_height_above_eave_m"] = (
        float((row.get("plane_top_y_m") or 0.0) - (row.get("plane_bottom_y_m") or 0.0))
        if out.get("typ_hip_candidate") and row.get("plane_top_y_m") is not None and row.get("plane_bottom_y_m") is not None
        else None
    )
    lower = [float(x) for x in peer_incls if plane_incl is not None and x > plane_incl + 5.0]
    upper = [float(x) for x in peer_incls if plane_incl is not None and x < plane_incl - 5.0]
    out["typ_mansard_candidate"] = bool(lower and upper and out.get("seg_has_behind_knee_wall_extension"))
    ...
    out["typ_mansard_knuckle_y_m"] = row.get("plane_mid_y_m") if out.get("typ_mansard_candidate") else None
    out["typ_mansard_is_lower"] = (
        bool(plane_incl is not None and out.get("typ_mansard_lower_leg_incl_deg") is not None and plane_incl >= out["typ_mansard_lower_leg_incl_deg"] - 1.0)
        if out.get("typ_mansard_candidate")
        else None
    )
    out["typ_gambrel_candidate"] = bool(lower and upper and out.get("typ_gable_candidate"))
    out["typ_pyramid_apex_distance_m"] = (
        float(math.hypot(out.get("poly_long_axis_projection_m") or 0.0, out.get("poly_short_axis_projection_m") or 0.0))
        if out.get("typ_pyramid_candidate")
        else None
    )
    out["typ_lplan_leg_index"] = (
        0 if out.get("typ_lplan_candidate") and (out.get("poly_long_axis_projection_m") or 0.0) < 0 else 1
    ...
    out["typ_half_hip_candidate"] = bool(out.get("typ_gable_candidate") and out.get("seg_has_hip"))
    out["typ_hipped_gable_candidate"] = bool(out.get("typ_hip_candidate") and out.get("seg_has_rake"))
    out["typ_tower_candidate"] = bool(
        (row.get("poly_bbox_aspect") or 99.0) <= 1.5 and (own_area or 0.0) < 25.0 and (row.get("plane_top_y_m") or 0.0) > (row.get("plane_bottom_y_m") or 0.0)
    )
    out["typ_tower_is_conical"] = bool(out.get("typ_tower_candidate") and out.get("poly_vertex_count", 0) >= 6)

    out["adv_plane_clips_into_wall_interior"] = bool(
        (out.get("edge_to_nearest_wall_top_m_min") or 99.0) > 0.5
        and (row.get("plane_bottom_y_m") or 0.0) < (row.get("bld_y_min_m") or 0.0) + PLANE_INTERIOR_CLIP_MARGIN_M
    )
    out["adv_plane_clips_through_floor"] = bool((out.get("seg_is_above_slab_count") or 0) > 0 and (plane_incl or 0.0) > 5.0)
    out["adv_eave_below_floor"] = bool(y_min is not None and row.get("bld_y_min_m") is not None and y_min < row["bld_y_min_m"])
    out["adv_ridge_above_reasonable"] = bool(
        (row.get("plane_top_y_m") or 0.0) > (row.get("bld_height_m") or 0.0) + RIDGE_HEIGHT_REASONABLE_MARGIN_M
    ) if row.get("plane_top_y_m") is not None and row.get("bld_height_m") is not None else None
    out["adv_plane_is_vertical"] = bool((plane_incl or 0.0) > 85.0) if plane_incl is not None else None
    out["adv_plane_is_horizontal"] = bool((plane_incl or 90.0) < 2) if plane_incl is not None else None
    out["adv_is_thin_sliver"] = bool(
        (own_area or 1.0) < 1.0 and (row.get("poly_min_width_m") or 1.0) < THIN_SLIVER_WIDTH_M
    )
    ...
    out["adv_low_grid_snap"] = bool((out.get("vtx_grid_snap_fraction_0p05") or 0.0) < 0.1)
    out["adv_duplicates_accepted_peer"] = bool(len(coplanar) > 0 and (row.get("heuristic_label") == "accepted" or any((peer.get("heuristic_label") == "accepted") for peer, _ in peer_polys)))
    out["adv_duplicate_cluster_canonical"] = bool(len(same_cluster) > 0)
    out["adv_overlap_with_accepted_peer_fraction"] = float(max((seg_poly.intersection(p).area / max(seg_poly.union(p).area, _EPS)) for p in coplanar)) if seg_poly is not None and coplanar else None
    out["adv_is_orphan_oblique"] = bool((plane_incl or 0.0) >= 15.0 and (record.get("opposing_planes") is None or len(record.get("opposing_planes") or []) == 0)) if plane_incl is not None else None
    out["adv_gable_has_no_opposing"] = bool(out.get("typ_gable_candidate") and not record.get("opposing_planes"))
    out["adv_hip_has_wrong_peer_count"] = bool(out.get("typ_hip_candidate") and (len(record.get("opposing_planes") or []) < 3))
    out["adv_pitch_outside_dk_norm"] = bool(plane_incl is not None and not (25.0 <= plane_incl <= 50.0) and plane_incl >= 5.0)
    out["adv_cross_story_without_part_support"] = bool((row.get("member_story_delta_max") or 0.0) > 0.0 and (out.get("part_story_count") or 1) == 1)
    out["adv_above_top_story_ceiling"] = bool((row.get("plane_bottom_y_m") or 0.0) > (row.get("bld_y_max_m") or 0.0) + 0.5) if row.get("plane_bottom_y_m") is not None and row.get("bld_y_max_m") is not None else None

    out["artefact_vertex_on_grid_fraction"] = out.get("vtx_grid_snap_fraction_0p1")
    out["artefact_incl_near_discrete_bucket"] = bool(plane_incl is not None and min(abs(plane_incl - b) for b in _INCL_BUCKETS) <= 1.0)
    out["artefact_azimuth_near_cardinal"] = bool(plane_az is not None and min(fe._azimuth_diff_deg(plane_az, b) for b in (0.0, 90.0, 180.0, 270.0)) <= 3.0)
    ...
    out["artefact_duplicated_wall_signature"] = bool(
        len(source_wall_ids) > 0 and len(source_wall_ids) < len([m for m in record.get("cluster_members") or [] if m.get("source_wall_id")])
    )
    out["artefact_from_scan_cache_only"] = bool(str(trace.get("stage") or "").lower() == "cache")
    out["scan_quality_overlap_fraction"] = row.get("scan_quality_overlap_fraction")
    out["scan_quality_cross_floor_gap_density"] = row.get("scan_quality_cross_floor_gap_density")
    ...
    if aux_context is not None and seg_poly is not None:
        ...
        touched_room_indices = sorted(
            {
                idx
                for idx in (
                    rr.get("room_index") if rr.get("room_index") is not None else _parse_room_index(rr.get("room_id"))
                    for rr in (record.get("room_boundary_refs") or [])
                )
                if idx is not None
            }
        ...
        for cell in topo_cells:
            ...
            if cell_poly is not None and seg_poly.intersects(cell_poly.buffer(WALL_SUPPORT_BUFFER_M)):
        ...
        adj_edges = [
            ...
            if edge.get("from_id") in touched_room_graph_ids or edge.get("to_id") in touched_room_graph_ids
        ]
        thickness_vals = [
        ...
        if thickness_vals:
            ...
            aux_out["v2_source_wall_thickness_m_mean"] = aux_out["swall_thickness_mean_m"]
        ...
        if surface_roles:
            ...
            aux_out["v2_source_wall_is_exterior_fraction"] = float(exterior_count / len(surface_roles))

        building_parts = onto.get("building_parts") or []
        best_part, best_part_poly, _ = _segment_match_by_overlap(
            seg_poly,
            building_parts,
            poly_getter=lambda part_rec: _poly_from_xz_points(part_rec.get("polygon_xz") or []),
        )
        ...
        if best_cell is not None:
            ...
            aux_out["seg_cell_is_pure_flat"] = bool(best_cell.get("roof_surface_kind") == "flat")
            aux_out["seg_cell_is_pure_oblique"] = bool(best_cell.get("roof_surface_kind") == "oblique")
        ...
        best_atom, _, atom_overlap = _segment_match_by_overlap(
            seg_poly,
            semantic_atoms,
        ...
        aux_out["ont_coverage_confirmed_sloped_atom_count"] = int(
            sum(1 for atom in semantic_atoms if str(atom.get("sloped_coverage_state") or "") == "confirmed")
        )
        aux_out["ont_coverage_partial_sloped_atom_count"] = int(
            sum(1 for atom in semantic_atoms if str(atom.get("sloped_coverage_state") or "") == "partial")
        )
        aux_out["ont_evidence_edge_tier_mean"] = (
            float(np.mean([float(atom.get("support_evidence_score")) for atom in semantic_atoms if atom.get("support_evidence_score") is not None]))
            if semantic_atoms
            else None
        )
        aux_out["ont_evidence_supports_oblique"] = bool(
            any((atom.get("support_evidence_score") or 0) > 0 for atom in semantic_atoms)
        )
        ...
        if best_subpart is not None:
            ...
            aux_out["seg_subpart_member_count"] = len(best_subpart.get("room_indices") or [])
        aux_out["ont_coverage_subpart_count"] = len(coverage_subparts)
        aux_out["ont_coverage_gable_run_count"] = int(
            sum(1 for sub in coverage_subparts if str(sub.get("semantic_kind") or "") == "gable_run")
        )
        aux_out["ont_coverage_l_t_subpart_count"] = int(
            sum(
                1
                for sub in coverage_subparts
                if any(tag in str(sub.get("semantic_kind") or "") for tag in ("l_", "t_", "lplan", "tplan"))
            )
        )
        aux_out["v2_seg_is_in_gable_run_chain"] = bool(
            best_subpart is not None and str(best_subpart.get("semantic_kind") or "") == "gable_run"
        )
        aux_out["v2_seg_chain_length"] = int(
            sum(1 for sub in coverage_subparts if sub.get("roof_hypothesis_id") == (best_subpart or {}).get("roof_hypothesis_id"))
        ) if best_subpart is not None else None

        if best_part is not None:
            room_indices = [
                idx for idx in (best_part.get("room_indices") or []) if isinstance(idx, int)
            ]
            room_summaries = onto.get("room_summaries") or {}
            room_records = [room_summaries.get(f"room:{idx}", {}) for idx in room_indices]
            aux_out["ont_part_family_guess"] = best_part.get("roof_family_guess")
            hypothesis_ids = list(best_part.get("hypothesis_ids") or [])
            selected_ids = list(best_part.get("oblique_hypothesis_ids") or []) + list(best_part.get("flat_hypothesis_ids") or [])
            ...
            aux_out["ont_part_articulation_room_count"] = len(best_part.get("articulation_room_ids") or [])
            aux_out["ont_part_slant_ratio"] = (
                float(len(best_part.get("oblique_hypothesis_ids") or []) / max(len(hypothesis_ids), 1))
                if hypothesis_ids
                else 0.0
            )
            deltas = [
            ...
            aux_out["ont_part_max_room_slant_delta_m"] = float(max(deltas)) if deltas else None
            part_id = best_part.get("id")
            aux_out["part_knee_wall_count"] = int(
                sum(1 for wall in (onto.get("full_model_knee_walls") or []) if wall.get("part_id") == part_id)
            )
            part_roof_cells = [cell for cell in roof_cells if cell.get("part_id") == part_id]
            aux_out["ont_cell_complex_flat_cell_count"] = int(
                sum(1 for cell in part_roof_cells if cell.get("roof_surface_kind") == "flat")
            )
            aux_out["ont_cell_complex_oblique_cell_count"] = int(
                sum(1 for cell in part_roof_cells if cell.get("roof_surface_kind") == "oblique")
            )
            aux_out["ont_cell_complex_mixed_cell_count"] = int(
                sum(
                    1
                    for cell in part_roof_cells
                    if len({str(face.get("role") or "") for face in (cell.get("faces") or []) if face.get("role")}) > 2
                )
            )
            simple_flags = [
                bool(room_rec.get("covered_by_sloped_roof")) and not bool(room_rec.get("mixed"))
                for room_rec in room_records
                if room_rec
            ]
            mixed_flags = [bool(room_rec.get("mixed")) for room_rec in room_records if room_rec]
            aux_out["ont_room_is_simple_slant"] = bool(simple_flags[0]) if len(simple_flags) == 1 else None
            aux_out["seg_all_rooms_simple_slant"] = bool(simple_flags) and all(simple_flags)
            aux_out["ont_room_is_mixed"] = bool(mixed_flags[0]) if len(mixed_flags) == 1 else None
            aux_out["seg_any_room_mixed"] = bool(mixed_flags) and any(mixed_flags)
            aux_out["ont_part_mixed_room_fraction"] = (
                float(sum(1 for flag in mixed_flags if flag) / len(mixed_flags)) if mixed_flags else None
            )
            flat_flags = [
                "flat_ceiling" in (room_rec.get("roles") or []) for room_rec in room_records if room_rec
            ]
            aux_out["ont_thermal_ceiling_is_flat"] = bool(flat_flags[0]) if len(flat_flags) == 1 else None
            aux_out["ont_thermal_ceiling_count"] = int(sum(1 for flag in flat_flags if flag))

        oblique_surfaces = onto.get("roof_surfaces_oblique") or []
        best_oblique, best_oblique_poly, best_oblique_overlap = _segment_match_by_overlap(
            seg_poly,
            oblique_surfaces,
            poly_getter=lambda surf: _xz_poly(surf.get("corners") or []),
        )
        aux_out["xm_v1_oblique_match_exists"] = bool(best_oblique is not None and best_oblique_overlap > 0.0)
        aux_out["xm_v1_match_count"] = int(
            sum(
                1
                for surf in oblique_surfaces
                if _xz_poly(surf.get("corners") or []) is not None
                and seg_poly.intersects(_xz_poly(surf.get("corners") or []).buffer(SCAN_NOISE_M))
            )
        )
        ...
        aux_out["xm_v1_flat_match_exists"] = bool(best_flat is not None and best_flat_overlap > 0.0)
        aux_out["ont_flat_intermediate_candidate_count"] = int(
            sum(1 for surf in flat_surfaces if str(surf.get("kind") or "") == "intermediate")
        )
        ...
        aux_out["xm_v1_is_v3_orphan"] = bool(
            not aux_out["xm_v1_oblique_match_exists"] and not aux_out["xm_v1_flat_match_exists"]
        )
        if best_oblique is not None:
            ...
            surf_centroid = _xz_poly(best_oblique.get("corners") or []).centroid if _xz_poly(best_oblique.get("corners") or []) is not None else None
            aux_out["xm_v1_oblique_match_azimuth_diff_deg"] = (
                fe._azimuth_diff_deg(plane_az, surf_az) if plane_az is not None and surf_az is not None else None
            )
            aux_out["xm_v1_oblique_match_incl_diff_deg"] = (
                float(abs(plane_incl - surf_incl)) if plane_incl is not None and surf_incl is not None else None
            )
            aux_out["xm_v1_oblique_match_centroid_distance_m"] = (
                float(seg_poly.centroid.distance(surf_centroid)) if surf_centroid is not None else None
            )
            aux_out["seg_ceiling_plane_azimuth_match_deg"] = aux_out["xm_v1_oblique_match_azimuth_diff_deg"]
            selected_ids = set((best_part or {}).get("oblique_hypothesis_ids") or []) | set((best_part or {}).get("flat_hypothesis_ids") or [])
            aux_out["seg_hypothesis_match_selected"] = bool(best_oblique.get("roof_hypothesis_id") in selected_ids)
            aux_out["seg_hypothesis_match_score"] = (
                float(best_oblique_overlap / max(own_area or 0.0, _EPS)) if own_area else None
            )
        ...
        aux_out["xm_ontology_family_agrees"] = bool(typ_guess is not None and family == typ_guess) if family else None
        aux_out["xm_ontology_disagreement_mode"] = (
            None if aux_out["xm_ontology_family_agrees"] or typ_guess is None or not family else f"{typ_guess}_vs_{family}"
        )
        aux_out["xm_hypothesis_solver_selects_seg"] = aux_out.get("seg_hypothesis_match_selected")
        aux_out["xm_cell_complex_agrees_with_ridge_class"] = (
            bool(out.get("seg_has_ridge")) == bool(best_cell is not None and best_cell.get("cell_kind") in {"attic", "upper_void"})
            if best_cell is not None
            else None
        )
        aux_out["xm_v2_chain_membership_agrees"] = (
            aux_out.get("v2_seg_is_in_gable_run_chain") == (aux_out.get("seg_subpart_semantic_kind") == "gable_run")
            if aux_out.get("v2_seg_is_in_gable_run_chain") is not None
            else None
        )
        aux_out["xm_v2_gable_run_endpoint_matches_seg"] = (
            bool(aux_out.get("v2_seg_is_in_gable_run_chain")) and bool(out.get("seg_has_ridge") or out.get("seg_has_rake"))
            if aux_out.get("v2_seg_is_in_gable_run_chain") is not None
            else None
        )
    out.update(aux_out)
    ...
    out["cluster_algorithm_version"] = (record.get("cluster_params") or {}).get("algorithm_version")
    out["model_train_timestamp"] = None
    out["model_hash"] = None
    out["label_age_days_mean"] = None

    out["interaction_covered_side_x_member_accept"] = (
        float((row.get("covered_side_count") or 0.0) * (row.get("member_heuristic_accepted_fraction") or 0.0))
    )
    out["interaction_drainage_center_cos_x_incl"] = (
        float((row.get("drainage_to_building_center_cos") or 0.0) * (plane_incl or 0.0))
    )
    out["interaction_story_delta_x_part_story_count"] = (
        float((row.get("member_story_delta_max") or 0.0) * (out.get("part_story_count") or 0.0))
    )
    out["interaction_ridge_len_x_part_n_slanted"] = (
        float((ridge_len or 0.0) * ((part.get("gable_extension") or {}).get("metrics", {}).get("n_slanted_roofs") or 0.0))
        if part is not None
        else None
    )
    out["interaction_swall_distance_x_member_count"] = (
        float((row.get("swall_centroid_to_seg_mean_m") or 0.0) * (row.get("cluster_member_count") or 0.0))
    )
    out["interaction_area_x_interiority"] = (
        float((own_area or 0.0) * max(0.0, 1.0 - ((row.get("distance_to_footprint_edge_m") or 0.0) / max(math.sqrt(row.get("bld_footprint_area_m2") or 1.0), _EPS))))
        if own_area is not None
        else None
    )
    out["interaction_opposing_incl_diff_x_count"] = (
        float((out.get("opposing_incl_diff_max_deg") or 0.0) * (row.get("opposing_count") or 0.0))
    )
    out["interaction_normals_entropy_x_cluster_member_count"] = (
        float((row.get("normals_d_entropy") or 0.0) * (row.get("cluster_member_count") or 0.0))
    )
