...
def derived_features(row: dict) -> dict[str, Any]:
    ...
    if family == "gable" and plane_az is not None and (az0 is not None or az1 is not None):
        ...
        out["derived_plane_matches_gable_family"] = (
            bool(nearest is not None and nearest < 10.0)
        )
    ...
    try:
        k = int(row.get("part_gable_metric_n_slanted_roofs")) if row.get("part_gable_metric_n_slanted_roofs") is not None else None
    except (TypeError, ValueError):
        k = None
