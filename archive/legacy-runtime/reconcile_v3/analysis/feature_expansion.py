...
def plane_features(plane: Sequence[float]) -> dict[str, Any]:
    ...
    return {
        ...
        "plane_pitch_is_shallow": None if math.isnan(incl) else bool(5.0 <= incl < 15.0),
        "plane_pitch_is_architectural": None
        if math.isnan(incl)
...
def polygon_features(
    ...
    compactness = (4.0 * math.pi * area) / (perimeter * perimeter) if perimeter > _EPS else None
    ...
    try:
        ...
        n_simp = len(simplified.exterior.coords) - 1 if hasattr(simplified, "exterior") else n_orig
...
def neighbor_features(
    ...
    if opposing_planes:
        az_mine, incl_mine = _plane_to_azimuth_incl(plane) if plane else (float("nan"), float("nan"))
        azs, incls = [], []
        az_diffs, incl_diffs = [], []
        cos_mins = []
        nm = None
...
def cluster_member_features(cluster_members: Sequence[dict] | None) -> dict[str, Any]:
    ...
    out["member_unique_source_rooms"] = len({m.get("source_room_id") for m in cluster_members if m.get("source_room_id")})
    out["member_unique_source_walls"] = len({m.get("source_wall_id") for m in cluster_members if m.get("source_wall_id")})
    out["member_unique_slab_rooms"] = len({m.get("slab_room_id") for m in cluster_members if m.get("slab_room_id")})
    stories = set()
...
def cluster_quality_features(
    ...
    if cluster_params:
        ...
        out["cluster_param_normal_dot_min"] = _safe(cluster_params.get("normal_dot_min"))
    ...
    if cluster_members:
        ...
        if pre_polys:
            try:
                ...
                out["cluster_pre_clip_area_sum_m2"] = float(sum(p.area for p in pre_polys))
    ...
    if merged_poly is not None:
        ...
        if out["cluster_pre_clip_area_sum_m2"] and out["cluster_pre_clip_area_sum_m2"] > _EPS:
