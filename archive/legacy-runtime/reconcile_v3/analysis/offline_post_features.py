...
def _attach_oof_predictions(
    ...
    *,
    ...
    attached = None
    if len(oof) == len(df):
        if "building_uuid" not in oof.columns or oof["building_uuid"].astype(str).equals(df["building_uuid"].astype(str)):
            attached = oof.reset_index(drop=True)
    if attached is None and reference_features_path:
        ref = pd.read_parquet(reference_features_path, columns=["building_uuid", "proposal_id"])
        if len(ref) == len(oof):
            ref = ref.copy()
            ref["building_uuid"] = ref["building_uuid"].astype(str)
            ref["proposal_id"] = ref["proposal_id"].astype(str)
            ref["__row__"] = np.arange(len(ref))
            oof_join = oof.reset_index(drop=True).drop(columns=["building_uuid"], errors="ignore")
            keyed = ref.merge(oof_join, left_on="__row__", right_index=True, how="left")
            lookup = keyed.set_index(["building_uuid", "proposal_id"])
            idx = pd.MultiIndex.from_frame(df[["building_uuid", "proposal_id"]].astype(str))
...
def _numeric_feature_matrix(df: pd.DataFrame, *, max_features: int = 32) -> tuple[np.ndarray, list[str]]:
    ...
    variances = numeric.var(axis=0, skipna=True).fillna(0.0).sort_values(ascending=False)
...
def augment_dataframe(
    ...
    *,
    ...
    if gbm_pkl_path:
        ...
        if feature_names and models:
            ...
            if contrib.ndim == 2 and contrib.shape[1] >= len(feature_names):
                ...
                probs_abs = np.divide(abs_contrib, sums, out=np.zeros_like(abs_contrib), where=sums > _EPS)
    ...
    if "label" in df.columns:
        labels = df["label"]
        df["lbl_was_split_child"] = df.get("is_split_child").fillna(False).astype(bool) if "is_split_child" in df.columns else False
        ...
        df["lbl_has_merge_event"] = df.get("merge_mode").fillna(False).astype(bool) if "merge_mode" in df.columns else False
        ...
        if "proposal_id" in df.columns:
            ...
            if "ts" in ordered.columns:
                ordered = ordered.sort_values(["proposal_id", "ts", "_order"], kind="stable")
        ...
        if "cluster_canonical_id" in df.columns:
            df["sib_cluster_count"] = df.groupby("cluster_canonical_id")["proposal_id"].transform("count")
            df["sib_cluster_accept_fraction"] = df.groupby("cluster_canonical_id")["label"].transform(_accept_fraction)
            df["sib_peer_label_entropy"] = df.groupby("cluster_canonical_id")["label"].transform(_entropy)
            df["sib_peer_label_disagreement_rate"] = 1.0 - df.groupby("cluster_canonical_id")["label"].transform(
                lambda s: float(s.value_counts(normalize=True, dropna=True).max()) if not s.dropna().empty else np.nan
            )

        if {"building_uuid", "part_index"}.issubset(df.columns):
            df["sib_part_accept_fraction"] = df.groupby(["building_uuid", "part_index"])["label"].transform(_accept_fraction)

        if "building_uuid" in df.columns:
            df["sib_bld_accept_fraction"] = df.groupby("building_uuid")["label"].transform(_accept_fraction)
            ...
            if "ts" in df.columns:
                ...
                df["sib_labeled_before_me_count"] = temp["sib_labeled_before_me_count"].to_numpy()
                df["sib_labeled_after_me_count"] = temp["sib_labeled_after_me_count"].to_numpy()
            else:
        ...
        if {"plane_azimuth_deg", "plane_incl_deg", "plane_d"}.issubset(df.columns):
            ...
            df["sib_coplanar_accept_fraction"] = df.groupby("_coplanar_key")["label"].transform(_accept_fraction)
        ...
        if "heuristic_label" in df.columns:
            ...
            if {"building_uuid", "part_index"}.issubset(df.columns):
                df["xm_heuristic_disagreement_rate_in_part"] = 1.0 - df.groupby(
                    ["building_uuid", "part_index"]
                ).apply(lambda g: _agreement_fraction(g["heuristic_label"], g["label"])).reindex(
                    pd.MultiIndex.from_frame(df[["building_uuid", "part_index"]])
                ).to_numpy()

        if "plane_incl_deg" in df.columns:
            pitch_bucket = (np.round(df["plane_incl_deg"].fillna(-999.0) / 5.0) * 5.0).astype(str)
            df["lbl_pitch_bucket_accept_rate"] = labels.groupby(pitch_bucket).transform(_accept_fraction)

        if "plane_azimuth_deg" in df.columns:
            az_bucket = (np.round(df["plane_azimuth_deg"].fillna(-999.0) / 22.5) * 22.5).astype(str)
            df["lbl_azimuth_bucket_accept_rate"] = labels.groupby(az_bucket).transform(_accept_fraction)

        if "derived_part_roof_family_guess" in df.columns:
            fam = df["derived_part_roof_family_guess"].fillna("unknown")
            df["lbl_family_accept_rate"] = labels.groupby(fam).transform(_accept_fraction)

        if "trace_rule" in df.columns:
            rules = df["trace_rule"].fillna("unknown")
            df["trace_rule_accept_rate_prior"] = labels.groupby(rules).transform(_accept_fraction)
        ...
        if X.shape[1] > 0 and len(df) >= 16:
            ...
            if "label" in df.columns:
                ...
                for row_inds in inds:
                    ...
                    neigh_accept.append(float(np.mean(accept[use])) if len(use) else np.nan)
            ...
            df["meta_in_training_manifold"] = df["meta_isolation_forest_score"] >= iso_cut
