...
def solve_building(
    ...
    *,
    ...
    if selected:
        ...
        status2 = m.optimize(max_seconds=max(5.0, cfg.time_limit_s * _RUNNER_UP_TIME_FRACTION))
...
def solve_building_with_zones(
    ...
    *,
    ...
    if not zones:
        return solve_building(building_uuid, candidates, scan_footprint_xz, config=config)
    ...
    for zone in zones:
        ...
        poly = _poly_from_ring(
            [(float(pt[0]), float(pt[1])) for pt in ring if isinstance(pt, (list, tuple)) and len(pt) >= 2]
        )
        if poly is None:
            continue
    ...
    for zone_id, zone in zone_lookup.items():
        ...
        if zone.get("fallback_kind") and not (zone.get("seed_subpart_ids") or []):
            ...
            major_axis_azimuth_deg = gap_axis_candidates[0] if gap_axis_candidates else None
            aspect_ratio = None
            ...
            if major_axis_azimuth_deg is not None and (
                gap_axis_candidates or (aspect_ratio is not None and aspect_ratio >= _AXIS_HINT_MIN_ASPECT)
            ):
                zone_config = replace(
                    zone_config,
        ...
        if (
            zone.get("fallback_kind")
            and len(zone_result.selected_face_ids) > 8
        ):
    ...
    if scan_poly is not None:
        try:
            unassigned_remainder = scan_poly.difference(zone_union) if zone_union is not None else scan_poly
        except Exception:
    ...
    if unassigned_candidates and remainder_pieces:
        for piece_index, remainder_piece in enumerate(
            ...
            remainder_ring = [(float(x), float(z)) for x, z in list(remainder_piece.exterior.coords)[:-1]]
            ...
            for candidate in unassigned_candidates:
                ...
                if candidate_poly is None:
                    continue
                try:
                    overlap_area = float(candidate_poly.intersection(remainder_piece).area)
                except Exception:
                    overlap_area = 0.0
                if overlap_area < _PIECE_MIN_OVERLAP_M2:
                    continue
                piece_candidates.append(candidate)
            unassigned_zone_id = "__unassigned__" if len(remainder_pieces) == 1 else f"__unassigned__:{piece_index}"
            zone_result = solve_building(
                building_uuid,
                piece_candidates,
                remainder_ring,
    ...
    if weighted_coverage_den > 0.0:
        ...
        result.coverage_ratio = min(
            sum(float(row.get("coverage_ratio", 0.0) or 0.0) for row in zone_results) / len(zone_results),
            1.0,
        )
    ...
    low_confidence_zone_ids = [
        ...
        if row.get("zone_confidence") is not None and float(row.get("zone_confidence")) < 0.4
    ]
    reasons = [
