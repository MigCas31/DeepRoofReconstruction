...
def _bbox_vertical_overlap(left: list[float] | None, right: list[float] | None) -> float:
...
def _room_partitions_by_room(
    ...
    room_partitions = ((roof_building.get("ceiling_partitions") or {}).get("room_partitions") or [])
    for room_partition in room_partitions:
        ...
        outline_poly = floor_polygon_to_shapely(room_partition.get("room_outline") or [])
        if outline_poly is not None and not outline_poly.is_empty and outline_poly.area > _AREA_EPS:
    ...
    if bldg:
        for room_index, room in enumerate(bldg.get("rooms") or []):
            ...
            if floor_poly is not None and not floor_poly.is_empty and floor_poly.area > _AREA_EPS:
...
def _room_ids_for_component(cells: list[dict[str, Any]]) -> tuple[list[str], list[int]]:
    room_ids = sorted({str(cell.get("room_id")) for cell in cells if cell.get("room_id")})
    room_indices = sorted(
        {
...
def _split_cells_by_wings(
    *,
    ...
    """
    cell_bboxes = [
        cell.get("bbox_xyz") if isinstance(cell.get("bbox_xyz"), list) and len(cell.get("bbox_xyz") or []) >= 6 else None
        for cell in cells
    ]
    ...
    for idx, (cell, poly) in enumerate(zip(cells, polys)):
...
def _build_support_components(
    *,
    ...
    for i, left in enumerate(prepared):
        for j in range(i + 1, len(prepared)):
            ...
            if left["room_id"] and left["room_id"] == right["room_id"]:
                connected = True
            elif left["top_atom_ids"] and left["top_atom_ids"].intersection(right["top_atom_ids"]):
                connected = True
            elif (
                ...
                and left["poly"].buffer(_SUPPORT_TOUCH_TOL_M).intersects(
                    right["poly"].buffer(_SUPPORT_TOUCH_TOL_M)
                )
            ):
                connected = True
            else:
                ...
                if vertical_overlap >= _VERTICAL_OVERLAP_TOL_M:
                    ...
                    if same_part_hint:
                        try:
                            connected = left["poly"].buffer(_SUPPORT_TOUCH_TOL_M).intersects(
                                right["poly"].buffer(_SUPPORT_TOUCH_TOL_M)
                            )
                        except Exception:
                            connected = False
    ...
    for component_index, component in enumerate(raw_components):
        ...
        room_ids, room_indices = _room_ids_for_component(cells)
        for room_id in room_ids:
        ...
        for sub_index, sub in enumerate(sub_components):
            ...
            support_components.append(
                _SupportComponent(
                    id=(
                        f"{building_uuid}::hybrid-zone-support::"
                        f"{_stable_hash([str(component_index), str(sub_index), str(sorted(c['id'] for c in sub_cells))], 20)}"
                    ),
                    occupied_cell_ids=sorted(str(cell["id"]) for cell in sub_cells if cell.get("id")),
    ...
    for component in support_components:
        if component.volume_m3 >= _MIN_SUPPORT_VOLUME_M3:
            ...
            continue
        if any(room_component_counts.get(room_id, 0) <= 1 for room_id in component.room_ids):
            kept.append(component)
    return kept
    ...
    for subpart in (coverage_graph.get("subparts") or []):
        ...
        seeds.append(
            _SeedSubpart(
                ...
                part_hint_ids=sorted(str(value) for value in (subpart.get("part_ids") or []) if value),
...
def _seed_component_score(
    ...
    *,
    ...
    support_geom = footprint_override if footprint_override is not None else component.footprint_poly
    ...
    same_hypothesis = int(
        bool(seed.roof_hypothesis_id and seed.roof_hypothesis_id in component.roof_hypothesis_ids)
    )
    ...
    part_hint_match = int(bool(set(component.part_hint_ids).intersection(seed.part_hint_ids)))
    score = (
        6.0 * shared_atoms
        + 4.0 * shared_rooms
        + 3.0 * same_hypothesis
...
def _initial_seed_component_map(
    ...
    for seed in seeds:
        for component in support_components:
            score, overlap_area, _overlap_ratio, shared_atoms, shared_rooms = _seed_component_score(
                component, seed
            )
...
def _seed_spill_penalty(component: _SupportComponent, seeds: list[_SeedSubpart]) -> float:
    attached_rooms = {
        room_index
        for seed in seeds
    ...
    for component in support_components:
        ...
        if not cells:
            for piece in _polygons_above_area(poly, _OWNERSHIP_CELL_MIN_AREA_M2):
                cells.append(
                    {"poly": piece, "covering_component_ids": {component.id}}
                )
            continue
        ...
        for cell in cells:
            ...
            try:
                ...
                intersection = None
            for piece in _polygons_above_area(intersection, _OWNERSHIP_CELL_MIN_AREA_M2):
                next_cells.append(
                    {
                        "poly": piece,
                        "covering_component_ids": set(cell["covering_component_ids"]).union({component.id}),
                    }
                )
            try:
            ...
            for piece in _polygons_above_area(diff_existing, _OWNERSHIP_CELL_MIN_AREA_M2):
                next_cells.append(
                    {
                        "poly": piece,
        ...
        for residual in residual_parts:
            next_cells.append(
                {"poly": residual, "covering_component_ids": {component.id}}
            )
        cells = next_cells
    return cells
...
def _assign_support_projection_ownership(
    ...
    for cell in raw_cells:
        ...
        for component_id in cover_ids:
            component = component_by_id[component_id]
            cell_poly = cell["poly"]
            room_cover_ratio = _intersection_area(cell_poly, component_room_union.get(component_id)) / max(
                float(cell_poly.area), _AREA_EPS
            )
            seed_cover_ratio = _intersection_area(cell_poly, component_seed_union.get(component_id)) / max(
                float(cell_poly.area), _AREA_EPS
            )
            part_hint_specificity = (
                1.0
                if len(component.part_hint_ids) == 1
                else 0.5 if len(component.part_hint_ids) > 1 else 0.0
            )
    ...
    for left_idx in range(len(raw_cells)):
        for right_idx in range(left_idx + 1, len(raw_cells)):
            ...
            try:
                shared_len = float(left_poly.boundary.intersection(right_poly.boundary).length)
            except Exception:
                shared_len = 0.0
    ...
    for _ in range(2):
        ...
        for idx, cell in enumerate(raw_cells):
            ...
            if total_shared <= 0.0:
                continue
            target_owner, target_len = max(owner_lengths.items(), key=lambda item: (item[1], item[0]))
...
def _final_seed_component_map(
    ...
    for seed in seeds:
        ...
        for component in support_components:
            ...
            score, overlap_area, overlap_ratio, shared_atoms, shared_rooms = _seed_component_score(
                component,
                seed,
    ...
    for component_id, attached in component_seed_map.items():
        component_seed_map[component_id] = sorted(
            {seed.id: seed for seed in attached}.values(),
            key=lambda seed: (-seed.support_evidence_score, -float(seed.polygon.area), seed.id),
        )
    return component_seed_map
...
def _seed_pair_compatible(left: _SeedSubpart, right: _SeedSubpart) -> bool:
    if left.id == right.id:
        return True
    adjacency_hint = (
        right.id in left.adjacent_subpart_ids
        or left.id in right.adjacent_subpart_ids
    )
...
def _split_zone_pieces(geom, seed_centroids: list[tuple[float, float]]) -> list[Polygon]:
    geom = _normalize_geom(geom)
    if geom is None:
        return []
    try:
    ...
    for piece in decompose_polys(cleaned):
        ...
        if not keep:
            keep = any(piece.buffer(1e-6).contains(Point(x, z)) for x, z in seed_centroids)
        if keep:
            pieces.append(piece)
    if pieces:
        return pieces
...
def _owned_room_indices(
    ...
    local_indices = [
        room_index
        for room_index in component.room_indices
        if sum(_intersection_area(poly, owned_support) for poly in room_partition_polys.get(room_index, []))
...
def _split_piece_by_part_overlap(
    *,
    ...
    if len(distinct_part_ids) <= 1:
        return [{"poly": piece, "room_ids": piece_room_ids, "part_id": _zone_part_id(piece_room_ids, part_nodes)}]

    local_room_ids = [room_id for room_id in piece_room_ids if room_id in set(component.room_ids)]
    ...
    for room_id in piece_room_ids:
        if room_id in local_room_ids:
            continue
        memberships = [str(part_id) for part_id in room_membership.get(room_id, []) if part_id]
    ...
    for group_key, room_ids in group_room_ids.items():
        ...
        group_scores[group_key] = (overlap_sum, float(len(room_ids)), float(group_key == "__local__"))
            ...
            for cell in cells:
                ...
                for piece_part in _polygons_above_area(intersection, _AREA_EPS):
                    next_cells.append(
                        {
                            "poly": piece_part,
                            "covering_groups": set(cell["covering_groups"]).union({group_key}),
                        }
                    )
                try:
                    diff_existing = cell_poly.difference(poly)
                ...
                for piece_part in _polygons_above_area(diff_existing, _AREA_EPS):
                    next_cells.append(
                        {"poly": piece_part, "covering_groups": set(cell["covering_groups"])}
                    )
                updated_residuals: list[Polygon] = []
                for residual in residual_parts:
                    try:
        ...
        for subpiece in _expand_polygons(geom):
            ...
            sub_room_ids = [
                ...
                if any(
                    ...
                    for room_poly in room_partition_polys.get(int(room_id.split(":")[-1]), [])
                )
            ]
            if not sub_room_ids:
    ...
    try:
        residual = piece.difference(assigned_union) if assigned_union is not None else piece
    except Exception:
        residual = None
    residual_geom = _normalize_geom(residual)
    if residual_geom is not None and float(residual_geom.area) >= max(0.5, _RESIDUAL_MIN_RATIO_OF_PIECE * piece_area):
    ...
    return variants or [{"poly": piece, "room_ids": piece_room_ids, "part_id": _zone_part_id(piece_room_ids, part_nodes)}]
    ...
    for i, left in enumerate(zones):
        ...
        for j in range(i + 1, len(zones)):
            ...
            if (
                ...
                and left.get("support_component_id") == right.get("support_component_id")
                and list(left.get("room_ids") or []) == list(right.get("room_ids") or [])
            ):
                keep = max([left, right], key=lambda zone: (_zone_priority(zone), zone["id"]))
...
def derive_authoritative_zones(
    *,
    ...
    occupied_cells = (roof_building.get("occupied_room_cell_complex") or {}).get("cells") or []
    ...
    for component in support_components:
        ...
        owned_room_ids = sorted(f"room:{room_index}" for room_index in owned_room_indices)
        ...
        seed_groups = _seed_groups_for_component(attached_seeds) if attached_seeds else []
        if not seed_groups:
            if (
        ...
        for group_index, seed_group in enumerate(seed_groups):
            ...
            if seed_group:
                ...
                clipped = None
                try:
                    clipped = owned_support.intersection(seed_union) if seed_union is not None else None
                except Exception:
                    clipped = None
                if (
                    clipped is None
                    or getattr(clipped, "is_empty", True)
                    or float(clipped.area) < _SEED_CLIP_MIN_RATIO * float(owned_support.area)
                ):
            ...
            for piece_index, piece in enumerate(pieces):
                ...
                semantic_kinds = sorted({seed.semantic_kind for seed in seed_group if seed.semantic_kind})
                avg_azimuths = sorted(
                ...
                for variant_index, variant in enumerate(variants):
                    ...
                    piece_room_indices = sorted(int(room_id.split(":")[-1]) for room_id in variant_room_ids)
                    ...
                    for roof_cell, roof_poly in roof_cell_polys:
                        ...
                        if (
                            seed_hypothesis_ids
                            and str(roof_cell.get("roof_hypothesis_id") or "") not in seed_hypothesis_ids
                            and int(roof_cell.get("room_index", -1)) not in piece_room_indices
                        ):
                            continue
                        if roof_cell.get("id"):
                    ...
                    zone_id = (
                        f"{building_uuid}::hybrid-zone::"
                        f"{_stable_hash([component.id, str(group_seed_ids), str(piece_index), str(variant_index), str(part_id)], 20)}"
                    )
                    ...
                    zones.append(
                        {
                            ...
                            "owned_support_area_m2": round(float(owned_support.area), 6),
