...
def _expand_polygons(geom) -> list[ShapelyPolygon]:
    ...
    if isinstance(geom, MultiPolygon):
        return [poly for poly in geom.geoms if isinstance(poly, ShapelyPolygon) and not poly.is_empty]
    ...
    for (zone_id, cluster_canonical_id), records in grouped.items():
        ...
        for piece_index, piece in enumerate(pieces):
            ...
            consolidated_candidate = CandidateFace(
                ...
                extended=any(bool(member["candidate"].extended) for member in piece_members),
...
def build_candidate_faces(
    ...
    for zone in building_zones or []:
        ...
        prepared_zones.append({
            ...
            "room_ids": [str(room_id) for room_id in (zone.get("room_ids") or []) if room_id],
            "confidence": (
            ...
            "seed_subpart_ids": [
                str(value) for value in (zone.get("seed_subpart_ids") or []) if value
            ],
            "seed_hypothesis_ids": [
                str(value) for value in (zone.get("seed_hypothesis_ids") or []) if value
            ],
            "semantic_kinds": [
    ...
    for entry in prepared:
        ...
        if prepared_zones:
            for zone in prepared_zones:
                ...
                for piece_index, piece in enumerate(_expand_polygons(overlap)):
                    ...
                    azimuth_incompatible = (
                        bool(avg_azimuths)
                        and all(_azimuth_diff_deg(azimuth, zone_azimuth) > 35.0 for zone_azimuth in avg_azimuths)
                    )
                    if (
        ...
        for piece, zone, piece_index in zone_pieces:
            ...
            candidate = CandidateFace(
                ...
                zone_seed_subpart_ids=[] if zone is None else list(zone["seed_subpart_ids"]),
                zone_seed_hypothesis_ids=[] if zone is None else list(zone["seed_hypothesis_ids"]),
                zone_semantic_kinds=[] if zone is None else list(zone["semantic_kinds"]),
                zone_avg_azimuths_deg=[] if zone is None else list(zone["avg_azimuths_deg"]),
            ...
            candidate_records.append({
                ...
                "opposing_cluster_canonicals": {
                    str(value) for value in (seg.get("opposing_cluster_canonicals") or []) if value
                },
                "zone_id": None if zone is None else zone["id"],
            })
    ...
    for record in candidate_records:
        ...
        for other in candidate_records:
            if other is record:
                continue
            if other["cluster_canonical_id"] not in record["opposing_cluster_canonicals"]:
                continue
