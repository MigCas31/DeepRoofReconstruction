...
def validate_graph(graph: TopologyGraph) -> list[GraphDiagnostic]:
    ...
    adjacency_pairs = {(edge.from_id, edge.to_id): edge for edge in graph.edges if edge.type == "ADJACENT_TO"}
    ...
    above_pairs = {(edge.from_id, edge.to_id): edge for edge in graph.edges if edge.type == "ABOVE"}
    below_pairs = {(edge.from_id, edge.to_id): edge for edge in graph.edges if edge.type == "BELOW"}
    ...
    for node in graph.nodes:
        ...
        if floor_y is not None and ceil_y is not None and float(ceil_y) < float(floor_y):
            diagnostics.append(
                GraphDiagnostic(
    ...
    for gap in graph.nodes_by_type("Gap"):
        bounded = graph.edges_from(gap.id, "BOUNDED_BY") + graph.edges_to(gap.id, "BOUNDED_BY")
        room_refs = {
