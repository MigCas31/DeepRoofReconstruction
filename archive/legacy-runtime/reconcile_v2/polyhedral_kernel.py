...
def _clip_footprint_to_min_clearance(
    ...
    *,
    ...
    for point in clipped:
        if deduped and abs(point[0] - deduped[-1][0]) <= EPS and abs(point[1] - deduped[-1][1]) <= EPS:
            continue
        deduped.append(point)
    if len(deduped) >= 2 and abs(deduped[0][0] - deduped[-1][0]) <= EPS and abs(deduped[0][1] - deduped[-1][1]) <= EPS:
        deduped.pop()
...
def _perimeter_indices_from_original(
    ...
    for index, start in enumerate(clipped):
        ...
        if any(_point_on_segment_xz(midpoint, edge_start, edge_end) for edge_start, edge_end in original_edges):
            keep.add(index)
    return keep
...
def _direct_prism_faces(
    *,
    ...
    faces.append(
        {
            "id": f"arr-face:{_stable_hash([cell_id, 'direct-bottom', str(bottom_ordered)], 20)}",
            "kind": "bottom",
            "role": "slab",
            "source_kind": "bottom_cap",
            "corners": bottom_ordered,
            "corners_lattice": [[_snap(c[0]), _snap(c[1]), _snap(c[2])] for c in bottom_ordered],
    ...
    faces.append(
        {
            "id": f"arr-face:{_stable_hash([cell_id, 'direct-top', str(top_ordered)], 20)}",
            "kind": "top",
            "role": "roof" if top_surface_kind == "oblique" else "slab",
            "source_kind": top_surface_kind,
            "corners": list(reversed(top_ordered)),
            "corners_lattice": [[_snap(c[0]), _snap(c[1]), _snap(c[2])] for c in reversed(top_ordered)],
    ...
    for index in range(len(base_corners)):
        ...
        corners = [
            [_round6(base_corners[index][0]), _round6(base_corners[index][1]), _round6(base_corners[index][2])],
            [_round6(base_corners[nxt][0]), _round6(base_corners[nxt][1]), _round6(base_corners[nxt][2])],
            [_round6(top_corners[nxt][0]), _round6(top_corners[nxt][1]), _round6(top_corners[nxt][2])],
            [_round6(top_corners[index][0]), _round6(top_corners[index][1]), _round6(top_corners[index][2])],
        ]
        ...
        faces.append(
            {
                "id": f"arr-face:{_stable_hash([cell_id, f'direct-side:{index}', str(corners)], 20)}",
                "kind": "side",
                "role": side_plane.role,
                "source_kind": side_plane.source_kind,
                "corners": corners,
                "corners_lattice": [[_snap(c[0]), _snap(c[1]), _snap(c[2])] for c in corners],
    ...
    *,
    ...
    if top_surface_kind == "oblique":
        ...
        if len(clipped_footprint) < 3:
            return None
        side_face_indices = _perimeter_indices_from_original(region_footprint, clipped_footprint)
    ...
    top_face_vertex_count = max((len(face.get("corners") or []) for face in top_faces), default=0)
    if (
        len(clipped_footprint) >= 3
        and (
            not top_faces
