    ...
    for i in range(len(roofs)):
        ...
        for j in range(i + 1, len(roofs)):
            ...
            if abs_ridge_delta >= RIDGE_DELTA_WEAK_M:
                strength = "strong" if abs_ridge_delta >= RIDGE_DELTA_STRONG_M else "weak"
                results.append(
                    ExtDiscontinuity(
            ...
            # Azimuth mismatch — only strong (weak mismatches are just a hip or gable cap).
                ...
                results.append(
                    ExtDiscontinuity(
                        ...
                        rationale=(
                            ...
                            f"({roof_a.inclination_deg:.1f}° vs {roof_b.inclination_deg:.1f}°)"
                        ),
                    )
                )
