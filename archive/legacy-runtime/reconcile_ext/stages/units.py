...
def _principal_azimuth_deg(poly: Polygon) -> float:
    ...
    for i in range(len(coords)):
        ...
        bin_key = int(round(az / 2.0)) % 45
        for offset, weight in ((0, 1.0), (1, 0.5), (-1, 0.5)):
            bins[(bin_key + offset) % 45] = bins.get((bin_key + offset) % 45, 0.0) + weight * length
    if not bins:
        return 0.0
...
def _rectangular_faces(footprint: Polygon) -> tuple[list[Polygon], float]:
    ...
    faces = [shp_rotate(f, azimuth, origin=(0, 0), use_radians=False) for f in faces_rotated]
