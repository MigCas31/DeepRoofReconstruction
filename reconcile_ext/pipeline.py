    ...
    for rv in report.reflex_vertices:
        ...
        for d in report.discontinuities:
            if near(anchor, d.mid_xz):
                ...
                score += TIER2_EVIDENCE_WEIGHT * (1.0 if d.strength == "strong" else 0.5)
...
def run(snapshot: ExtSnapshot) -> ExtReport:
    ...
    if polys:
        ...
        if simplified.is_valid and not simplified.is_empty:
            ...
            merged = simplified
        y = snapshot.parts[0].footprint_xz[0][1] if snapshot.parts[0].footprint_xz else 0.0
        coords = list(merged.exterior.coords)
        if coords and coords[0] == coords[-1]:
            coords = coords[:-1]
        report.building_footprint_xz = [(float(x), float(y), float(z)) for x, z in coords]
    ...
    report.stats = {
        ...
        "seam_strong": sum(1 for s in report.seam_hypotheses if s.confidence == "strong"),
        "seam_medium": sum(1 for s in report.seam_hypotheses if s.confidence == "medium"),
