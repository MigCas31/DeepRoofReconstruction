...
def main():
    ...
    print("4. Compared rotations ACROSS floors (different floor = may have different session)")
    ...
    for uuid, name in TARGETS.items():
        ...
        for ft, fd in floor_data.items():
            if fd["yaw_std"] > 1.0:
                ...
                issues.append(f"INTRA-FLOOR SESSION RESTART on {ft} floor (yaw spread={max_yaw_diff:.1f}deg)")
        ...
        if len(floor_types) >= 2:
            ...
            for i in range(len(floor_types)):
                for j in range(i+1, len(floor_types)):
                    ...
                    yaw_diff = abs(floor_data[ft1]["mean_yaw"] - floor_data[ft2]["mean_yaw"])
                    if yaw_diff > 180:
                        yaw_diff = 360 - yaw_diff
                    if yaw_diff > 1.0:
                        cross_floor_yaw_diffs.append((ft1, ft2, yaw_diff))
                        issues.append(f"CROSS-FLOOR SESSION RESTART: {ft1}->{ft2} (yaw diff={yaw_diff:.1f}deg)")

        # Check Y-offset outliers within a floor
        for ft, fd in floor_data.items():
            if fd["y_vals"]:
                mean_y = np.mean(fd["y_vals"])
                for i, y in enumerate(fd["y_vals"]):
                    if abs(y - mean_y) > 1.0:
                        issues.append(f"Y-OFFSET OUTLIER on {ft} floor (room at y={y:.2f}m vs mean={mean_y:.2f}m)")
        ...
        for ft, fd in sorted(floor_data.items(), key=lambda x: x[1].get("created", 0)):
            ...
            y_outlier = any(abs(y - np.mean(fd["y_vals"])) > 1.0 for y in fd["y_vals"]) if fd["y_vals"] else False
            y_flag = " *** Y-OUTLIER ***" if y_outlier else ""
            print(f"    {ft:>10}: {fd['num_rooms']:>2} rooms | "
    ...
    print("=" * 120)
    print()
    print(f"{'Building':<25} {'Intra-Floor':>12} {'Cross-Floor':>12} {'Y-Outlier':>10} {'Category':<40}")
    print("-" * 120)
    ...
    2. CROSS-FLOOR SESSION RESTARTS (4 additional buildings):
       - Gerskov Bygade 24: First floor yaw=82.5deg, ground floor yaw=72.9deg (9.6deg diff).
         Each floor was scanned in a separate ARSession.
       - Gultvedgyden 6: Ground/first/second all at ~107.6deg, but basement at 26.2deg
         (81.5deg difference). The basement was scanned in a completely different session.
