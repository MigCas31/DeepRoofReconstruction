...
def analyze(uuid, name):
    ...
    for i, room in enumerate(rooms):
        ...
        if wall_ys:
            ...
            story_room_centroids[story].append((i, centroid_y, min(wall_ys), max(wall_ys)))
        ...
        for i in range(len(stories_set)):
            for j in range(i + 1, len(stories_set)):
                ...
                if s1 in story_ranges and s2 in story_ranges:
                    ...
                    if overlap > 0:
                        print(f"    Story {s1} <-> {s2}: OVERLAP = {overlap:.3f}m  (mean gap = {gap:.3f}m)")
                        print(f"      Story {s1}: Y=[{r1[0]:.3f}, {r1[1]:.3f}], mean={r1[2]:.3f}")
                        print(f"      Story {s2}: Y=[{r2[0]:.3f}, {r2[1]:.3f}], mean={r2[2]:.3f}")
                    else:
                        sep = -overlap
    ...
    for s in sorted(top_wall_story_ys.keys()):
        ...
        print(f"  Story {s}: {len(ys)} walls, Y=[{min(ys):.3f}, {max(ys):.3f}], mean_Y={np.mean(ys):.3f}")
        print(f"    X-range=[{xzs[:,0].min():.2f}, {xzs[:,0].max():.2f}], Z-range=[{xzs[:,1].min():.2f}, {xzs[:,1].max():.2f}]")
    ...
    if len(stories_set) > 1:
        ...
        for s in stories_set:
            ...
            if len(xzs) > 0:
                print(f"    Story {s}: X=[{xzs[:,0].min():.2f}, {xzs[:,0].max():.2f}], Z=[{xzs[:,1].min():.2f}, {xzs[:,1].max():.2f}]")
    ...
    for i, room in enumerate(rooms):
        ...
        if not walls:
            continue
        positions = [pos(w.get("transform")) for w in walls if pos(w.get("transform")) is not None]
        if not positions:
            continue
        parr = np.array(positions)
        spread = parr.max(axis=0) - parr.min(axis=0)
        if spread[0] > 15 or spread[2] > 15 or spread[1] > 4:
            print(f"    Room {i} (story {story}): wall spread X={spread[0]:.2f}, Y={spread[1]:.2f}, Z={spread[2]:.2f}")
    ...
    for w in data.get("walls", []):
        ...
        if len(dim) >= 3 and dim[2] == 0:
            zero_thick += 1
    if zero_thick > 0:
        print(f"\n  ** {zero_thick}/{len(data.get('walls',[]))} walls have zero thickness")
