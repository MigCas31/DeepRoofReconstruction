...
def _attach_ontology_gap_ids(closures, graph):
    ...
    for idx, closure in enumerate(closures):
        ...
        if best_id and best_area > 1e-5:
            ...
            closure["id"] = closure.get("id") or f"gc:{best_id}:{closure.get('type','closure')}:{idx}"
...
def compute_gap_closures(indicators, rooms_out, graph=None):
    ...
    for ind in indicators:
        ...
        def interp_wall_y(corners, t, bottom=True):
            if bottom:
            ...
            bot_edge = np.array(
                [corners[1][0] - corners[0][0], corners[1][2] - corners[0][2]]
            )
            ...
            for c in top_cs:
                ...
                ct = (
                    float(np.clip(np.dot(cxz - bot_p0, bot_edge) / bot_len2, 0, 1))
                    if bot_len2 > 1e-12
                    else 0.0
                )
        ...
        for t_val in [ov_min, ov_max]:
            ...
            p_ybot = interp_wall_y(pc, p_t, bottom=True)
            p_ytop = interp_wall_y(pc, p_t, bottom=False)

            w_t = edge_t(xz_pt, w0_xz, w_edge, w_elen)
            w_xz = w0_xz + w_t * w_edge if w_elen > 1e-6 else w0_xz
            w_ybot = interp_wall_y(wc, w_t, bottom=True)
            w_ytop = interp_wall_y(wc, w_t, bottom=False)
