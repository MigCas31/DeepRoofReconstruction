...
def extract_building(uuid, pipeline_dir, scan_cache_root, load_topology_graph=True):
    ...
    gap_closures = compute_gap_closures(exterior_indicators, rooms_out, graph=topology_graph)
    gap_walls = compute_gap_walls(
        cross_floor_gaps, rooms_out, story_y_map, gap_closures, graph=topology_graph
    )
