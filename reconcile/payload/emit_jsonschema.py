...
def emit_schema(root_type: type) -> dict[str, Any]:
    ...
    properties = {field.name: _schema_for(hints[field.name]) for field in dataclasses.fields(root_type)}
    return {
        ...
        "required": [field.name for field in dataclasses.fields(root_type)],
    }
