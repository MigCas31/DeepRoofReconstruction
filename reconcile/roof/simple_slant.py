...
from reconcile.roof.roof import ObliqueSurface, RoofCluster
...
def build_simple_slant_surfaces(model: BuildingModel, room_indices: set[int]) -> list[ObliqueSurface]:
    ...
    for room in model.rooms:
        ...
        surfaces.append(
            ObliqueSurface(
                corners=[[float(point[0]), float(point[1]), float(point[2])] for point in corners],
                plane=fit,
