...
from reconcile.roof.roof import ObliqueSurface, SplitObliqueSurface
...
def split_oblique_surfaces(obliques: list[ObliqueSurface], model: BuildingModel | None = None) -> list[SplitObliqueSurface]:
    ...
    return [
        SplitObliqueSurface(
            ...
            plane=surface.plane,
            arrangement_cell_id=f"cell:{idx}",
...
def _split_by_room_cells(obliques: list[ObliqueSurface], model: BuildingModel) -> list[SplitObliqueSurface]:
    room_cells = []
    ...
    for oblique_idx, surface in enumerate(obliques):
...
def _polygon_parts(geometry) -> list[Polygon]:
    ...
    if geometry.geom_type == "GeometryCollection":
        return [part for part in geometry.geoms if part.geom_type == "Polygon" and part.area >= MIN_SPLIT_AREA_M2]
    return []
