...
from reconcile.roof.geometry import edge_inclination_deg, xz_polygon_to_vec3
from reconcile.roof.roof import FlatSurface
...
def _intermediate_flats(model: BuildingModel, top_story: int) -> list[FlatSurface]:
    ...
    for room in model.rooms:
        ...
        if y is None:
            continue
        out.append(FlatSurface(corners=xz_polygon_to_vec3([(p[0], p[2]) for p in room.floor_polygon], y), story=room.story, dominant_story=top_story, source="intermediate"))
    return out
...
def _ceiling_flats(model: BuildingModel, top_story: int) -> list[FlatSurface]:
    ...
    for room in model.rooms:
        ...
        out.append(
            FlatSurface(
                corners=[[float(p[0]), float(p[1]), float(p[2])] for p in room.ceiling_polygon],
                story=room.story,
...
def _top_flat_edges(model: BuildingModel, top_story: int) -> list[tuple[list[float], list[float], float]]:
...
def _top_flats(model: BuildingModel, top_story: int) -> list[FlatSurface]:
    ...
    for group in groups:
        ...
        out.append(FlatSurface(corners=xz_polygon_to_vec3(ring, y), story=top_story, dominant_story=top_story, source="top"))
    return out
...
def build_flat_surfaces(model: BuildingModel) -> list[FlatSurface]:
    ...
    if ceiling_flats:
        return [*ceiling_flats, *(_top_flats(model, top_story) if top_story == 0 else [])]
