...
def plane_normal_from_azimuth_inclination(azimuth_deg: float, incl_deg: float) -> tuple[float, float, float]:
...
def segment_midpoint(a: Sequence[float], b: Sequence[float]) -> list[float]:
    return [(float(a[0]) + float(b[0])) / 2.0, (float(a[1]) + float(b[1])) / 2.0, (float(a[2]) + float(b[2])) / 2.0]
...
def xz_polygon_to_vec3(poly: Sequence[tuple[float, float]], y: float) -> list[list[float]]:
...
def wall_quads(wall) -> list[list[list[float]]]:
    ...
    strip = getattr(wall, "extension_strip", None)
    if strip is None and isinstance(wall, dict):
        strip = wall.get("extension_strip")
    if not strip:
        return quads
    is_nested = bool(strip and isinstance(strip[0], (list, tuple)) and strip[0] and isinstance(strip[0][0], (list, tuple)))
