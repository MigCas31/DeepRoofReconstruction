...
from reconcile.roof.roof import Footprint
...
def build_building_footprint(model: BuildingModel, room_indices: set[int] | None = None) -> Footprint | None:
    polygons = []
    ...
    if coords and coords[0] == coords[-1]:
        coords = coords[:-1]
    return Footprint(polygon_xz=[(float(x), float(z)) for x, z in coords], top_story=top_story, area=float(shrunk.area))
