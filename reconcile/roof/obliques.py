...
from reconcile._core.plane import FitFailure, Plane
...
from reconcile.roof.roof import ClippedRoofPlane, ObliqueSurface, RoofCluster
...
def build_oblique_surfaces(clipped_planes: list[ClippedRoofPlane], floor_y_by_story: dict[int, float]) -> list[ObliqueSurface]:
...
def _clip_y(poly: list[list[float]], bound: float, keep_greater: bool) -> list[list[float]]:
    out: list[list[float]] = []
    n = len(poly)
...
def build_raw_oblique_surfaces(model: BuildingModel, existing: list[ObliqueSurface]) -> list[ObliqueSurface]:
    ...
    for room in model.rooms:
        ...
        for raw in room.raw_ceiling_planes:
            ...
            if isinstance(fit, FitFailure):
                continue
            if _plane_residual(raw.corners, fit) > RAW_OBLIQUE_MAX_RESIDUAL_M:
                continue
            incl = degrees(atan2(hypot(fit.a, fit.c), abs(fit.b)))
            if incl <= RAW_OBLIQUE_MIN_INCL_DEG or incl >= RAW_OBLIQUE_MAX_INCL_DEG:
                continue
            if not _valid_raw_rectangle(raw.corners):
                continue
            avg_azimuth = degrees(atan2(-fit.a, -fit.c)) % 360.0
            if len(_ridge_like_edges(raw.corners, avg_azimuth)) < 2:
                continue
            ref_pt = [
                sum(p[0] for p in raw.corners) / len(raw.corners),
                sum(p[1] for p in raw.corners) / len(raw.corners),
                sum(p[2] for p in raw.corners) / len(raw.corners),
            ]
            if _duplicates_existing(ref_pt, incl, avg_azimuth, existing):
                continue
            out.append(
                ObliqueSurface(
                    corners=[[float(p[0]), float(p[1]), float(p[2])] for p in raw.corners],
                    plane=fit,
                    cluster=RoofCluster(segments=[], avg_incl=incl, avg_azimuth=avg_azimuth, ref_pt=ref_pt),
                    dominant_story=room.story,
                    ridge=_ridge_for_raw(raw.corners, avg_azimuth),
                )
            )
    return out
...
def _plane_residual(corners: list[list[float]], plane: Plane) -> float:
    return max(abs(plane.a * p[0] + plane.b * p[1] + plane.c * p[2] - plane.d) for p in corners)


def _ridge_like_edges(corners: list[list[float]], avg_azimuth: float) -> list[tuple[list[float], list[float], float]]:
...
def _duplicates_existing(ref_pt: list[float], incl: float, avg_azimuth: float, existing: list[ObliqueSurface]) -> bool:
