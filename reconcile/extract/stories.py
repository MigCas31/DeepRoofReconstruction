...
from reconcile.extract.geometry import corners_to_world
from reconcile.ingest.merged import MergedDoc
...
def _floor_polygons_from_merged(merged: MergedDoc) -> list[list[list[float]]]:
    ...
    for room in merged.rooms:
        ...
        if floor_items and floor_items[0].get("polygonCorners"):
            floors.append(corners_to_world(floor_items[0]["polygonCorners"], floor_items[0]["transform"]))
        else:
            floors.append([])
    return floors


def compute_room_stories(merged_or_floor_polygons: MergedDoc | Sequence[Sequence[Sequence[float]]]) -> StoryAssignment:
    if isinstance(merged_or_floor_polygons, MergedDoc):
        floor_polygons = _floor_polygons_from_merged(merged_or_floor_polygons)
    else:
        floor_polygons = [[list(point) for point in polygon] for polygon in merged_or_floor_polygons]

    floor_ys = [
        float(np.mean([corner[1] for corner in floor_polygon])) if floor_polygon else 0.0
    ...
    for floor_y in floor_ys:
        closest_y = min(sorted_ys, key=lambda y_value: abs(y_value - floor_y)) if sorted_ys else 0.0
...
def is_split_level(floor_polygons: Sequence[Sequence[Sequence[float]]], room_stories: Sequence[int]) -> bool:
    if not room_stories:
        return False
    ...
    for story, floor_polygon in zip(room_stories, floor_polygons):
        if not floor_polygon:
            continue
        ys_by_story.setdefault(story, []).append(float(np.mean([corner[1] for corner in floor_polygon])))
    mean_y = {story: float(np.mean(values)) for story, values in ys_by_story.items() if values}
    ordered = sorted(mean_y)
    deltas = [mean_y[ordered[idx + 1]] - mean_y[ordered[idx]] for idx in range(len(ordered) - 1)]
