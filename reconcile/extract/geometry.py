...
def corners_to_world(corners: Sequence[Sequence[float]], transform_flat: Sequence[float]) -> list[list[float]]:
    transform = parse_roomplan_transform(transform_flat)
    out: list[list[float]] = []
    for corner in corners:
        world = transform @ np.array([float(corner[0]), float(corner[1]), float(corner[2]), 1.0])
...
def hybrid_wall_corners(merged_wall: dict, raw_wall: dict, floor_y: float | None = None) -> list[list[float]]:
