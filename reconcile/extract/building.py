...
from reconcile.extract.geometry import (
    corners_to_world,
    hybrid_wall_corners,
    wall_world_corners,
)
from reconcile.extract.stories import compute_room_stories, is_split_level
from reconcile.ingest.merged import load_merged
from reconcile.ingest.room_transforms import (
    METHOD_RANK,
    RoomTransform,
    compute_room_transforms,
)
from reconcile.ingest.scan_cache import (
    find_scan_cache_dir,
    load_raw_ceilings,
    load_raw_rooms,
...
class ExtractedWall:
    ...
    source: str
    extension_strip: list[list[list[float]]] | None = None
...
def _raw_room_matches_merged(raw_room: dict, merged_room: dict) -> bool:
    raw_wall_ids = {wall.get("identifier") for wall in raw_room.get("walls", []) if wall.get("identifier")}
    merged_wall_ids = {wall.get("identifier") for wall in merged_room.get("walls", []) if wall.get("identifier")}
    return bool(raw_wall_ids & merged_wall_ids)


def _build_raw_to_merged_index(raw_rooms: list[tuple[str, dict]], merged_rooms: list[dict]) -> dict[str, int]:
    ...
    for room_name, raw_room in raw_rooms:
        ...
        for idx, merged_room in enumerate(merged_rooms):
            merged_ids = {wall.get("identifier") for wall in merged_room.get("walls", []) if wall.get("identifier")}
...
def _orient_walls_outward(walls: list[ExtractedWall], floor_polygon: list[list[float]]) -> list[ExtractedWall]:
    ...
    for wall in walls:
        ...
        if float(normal @ (wall_center - centroid)) < 0.0:
            oriented.append(ExtractedWall(id=wall.id, corners=list(reversed(corners)), source=wall.source))
        else:
            oriented.append(wall)
    return oriented
...
def _extract_room_walls(
    ...
    walls_merged = [
        ExtractedWall(id=wall["identifier"], corners=wall_world_corners(wall), source="merged-room")
    ...
    for wall in merged_room.get("walls", []):
        ...
        if raw_entry is None:
            walls_computed.append(
                ExtractedWall(id=wall_id, corners=wall_world_corners(wall), source="merged-room")
            )
            continue
        ...
        if transform.method == "floor-svd":
            ...
            floor_y = float(np.mean([corner[1] for corner in floor_polygon])) if floor_polygon else None
            walls_computed.append(
                ExtractedWall(
                    id=wall_id,
...
def _add_dedup_walls(
    ...
    for room_name, raw_room in raw_rooms:
        transform = transforms.get(room_name)
        if transform is None or transform.method != "floor-svd" or not _raw_room_matches_merged(raw_room, merged_room):
            continue
        for wall in raw_room.get("walls", []):
            ...
            if not wall_id or wall_id in added_ids or wall_id in global_added:
                continue
            in_any_merged = any(
                any(merged_wall.get("identifier") == wall_id for merged_wall in room.get("walls", []))
...
def _extract_openings(
    ...
    for item in merged_items:
        ...
        if raw_entry is not None and raw_entry[1].method == "floor-svd":
            ...
            if _opening_near_parent(transformed, parent_id, walls_computed):
                out.append(ExtractedElement(id=item_id, corners=transformed, source="scan-cache"))
                continue
        out.append(ExtractedElement(id=item_id, corners=wall_world_corners(item), source="merged-room"))
    ...
    for room_name, raw_room in raw_rooms:
        ...
        for item in raw_room.get(key, []):
            item_id = item.get("identifier")
            if not item_id or item_id in added_ids or item_id in global_added or item_id in merged_ids:
                continue
            transformed = transform.apply(wall_world_corners(item))
            if not _opening_near_parent(transformed, item.get("parentIdentifier"), walls_computed):
                continue
            out.append(ExtractedElement(id=item_id, corners=transformed, source="scan-cache-dedup"))
    ...
    for room_name, raw_room in raw_rooms:
        ...
        for item in raw_room.get("objects", []):
            ...
            if not item_id or item_id in added or item_id in global_added:
                continue
            out.append(ExtractedElement(id=item_id, corners=transform.apply(wall_world_corners(item)), source="scan-cache"))
            added.add(item_id)
            global_added.add(item_id)
    return out
...
def _remap_raw_ceilings(
    ...
    for room_name, merged_idx in raw_to_merged.items():
        ...
        for plane in ceiling.get("planes") or []:
            world_corners = corners_to_world(plane["corners_local"], plane["transform"])
            remapped_corners = [
                [round(float(corner[0]), 4), round(float(corner[1]), 4), round(float(corner[2]), 4)]
                for corner in transform.apply(world_corners)
            ]
            if len(remapped_corners) >= 3:
                remapped.append(RawCeilingPlane(corners=remapped_corners))
        if not remapped:
            continue
...
def extract_building_model(uuid: str, pipeline_dir: Path | str, scan_cache_root: Path | str | None) -> BuildingModel:
    ...
    for idx, merged_room in enumerate(merged_rooms):
        floor_polygon = assignment.floor_polygons[idx] if idx < len(assignment.floor_polygons) else []
        walls_merged, walls_computed, added_ids = _extract_room_walls(merged_room, floor_polygon, raw_wall_data)
        ...
        storages = _extract_storages(merged_room, raw_rooms, transforms, global_storages_added)
        raw_ceiling_planes, raw_ceiling_source = ceilings_by_merged.get(idx, ([], None))
        rooms.append(
            ExtractedRoom(
                index=idx,
                story=assignment.room_stories[idx] if idx < len(assignment.room_stories) else 0,
    ...
    from reconcile.extract.ceilings import infer_ceilings
    from reconcile.extract.extension import extend_walls_to_slabs
    from reconcile.extract.exterior import (
        compute_gap_closures,
        detect_exterior_gap_indicators,
    )
    from reconcile.extract.gaps import (
        assign_gaps_to_rooms,
        compute_cross_floor_gaps,
        compute_gap_walls,
        story_y_map_from_rooms,
    )
    from reconcile.extract.height_align import align_room_heights
    from reconcile.extract.overlaps import (
        clip_floor_overlaps,
        clip_walls_to_story_bounds,
    )
    from reconcile.extract.stitches import stitch_wall_gaps
    ...
    rooms = extend_walls_to_slabs(rooms)
    ...
    gap_walls, cross_floor_gaps = compute_gap_walls(
        cross_floor_gaps,
        rooms,
        story_y_map,
