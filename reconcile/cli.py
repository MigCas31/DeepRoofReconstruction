...
def reconcile_building(
    ...
    story_result = fix_building_stories(reconciled_building)
    ...
    reconciled_building.top_level_walls = [
        reconciled_by_id.get(w.identifier, w) for w in reconciled_building.top_level_walls
    ]
    reconciled_building.top_level_doors = [
        reconciled_by_id.get(d.identifier, d) for d in reconciled_building.top_level_doors
    ]
    reconciled_building.top_level_windows = [
        reconciled_by_id.get(w.identifier, w) for w in reconciled_building.top_level_windows
    ]
    ...
    # 7. Output
    write_reconciled(
        reconciled_building, thicknesses, quality, output_path,
        cross_floor_gaps=cross_floor_gaps,
...
def main():
    ...
    parser.add_argument("--scan-cache", required=True, help="Path to scan-cache directory")
