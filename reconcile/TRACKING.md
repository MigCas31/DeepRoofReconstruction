...
- `test_no_v2_graph_import`: `import reconcile; assert not any("reconcile_v2" in m for m in reconcile.__dict__.values()...)` — guards Phase 0 decision.
...
- **Finding**: The legacy path clusters roof segments by full directional
  ...
  clusters too early.
- **Decision**: refactor `reconcile.roof.clustering` and
  `reconcile.roof.planes` to match the legacy semantics: full 0-360
...
1. CI green (`pytest tests/reconcile_tiers/` + `python -m reconcile.build --validate-only` once Phase G lands).
...
- [ ] Coverage on `_core/` ≥ 90% (`pytest --cov=reconcile._core`)
...
- [x] `python -m reconcile.build --all --validate-only` exits 0 across all 223 `pipeline-outputs/` buildings present in this checkout
