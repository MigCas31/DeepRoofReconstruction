...
def build_quality_report(
    ...
    floor_gap_thicknesses: list = None,
    cross_floor_gaps: list = None,
    scan_session_count: int = 1,
    ...
    # Dimension deltas
    width_deltas = [
        r.divergences.get("dimensions.width", 0) for r in wall_reports if r.divergences.get("dimensions.width")
    ]
    height_deltas = [
        r.divergences.get("dimensions.height", 0) for r in wall_reports if r.divergences.get("dimensions.height")
    ]
    ...
    report = BuildingQualityReport(
        ...
        synthesized_count=sum(1 for r in wall_reports if "synthesized_in_merge" in r.flags),
...
def classify_building(report: BuildingQualityReport) -> str:
    ...
    if report.scan_session_count >= 3:
        red_conditions.append(f"multi_session ({report.scan_session_count} scan sessions)")
    if report.median_wall_displacement_m > 10:
        red_conditions.append(f"median_wall_displacement > 10m ({report.median_wall_displacement_m:.1f}m)")
    ...
    if report.cross_floor_gap_high_confidence > 3:
        yellow_conditions.append(f"cross_floor_gaps_high > 3 ({report.cross_floor_gap_high_confidence})")
    if report.story_change_ratio > 0.5:
        yellow_conditions.append(f"story_change_ratio > 50% ({report.story_change_ratio:.0%})")
    if report.median_wall_displacement_m > 7:
        yellow_conditions.append(f"median_wall_displacement > 7m ({report.median_wall_displacement_m:.1f}m)")
