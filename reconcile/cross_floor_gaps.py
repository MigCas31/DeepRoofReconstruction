...
def build_story_footprints(
    ...
    for story, room_polys in sorted(rooms_by_story.items()):
        ...
        sf = StoryFootprint(
            story=story, footprint_wkt=footprint.wkt,
            area_m2=footprint.area, room_count=len(rooms),
        )
...
def _best_filling_story(
    ...
    """Find which other story best fills this concavity. Returns (story, fill_fraction)."""
...
def _score_gap(
    ...
    if other:
        filling_count = sum(
            1 for s in other
            if all_footprints[s].intersection(region).area > area * 0.5
        )
    ...
    for story, (footprint, _sf, rooms) in footprint_data.items():
        ...
        for region in concavities:
            ...
            try:
                ...
                contact = shared.length / region.boundary.length if not shared.is_empty else 0
            except Exception:
                contact = 0

            affected = _find_affected_room(region, rooms)

            gaps.append(CrossFloorGap(
                story=story,
