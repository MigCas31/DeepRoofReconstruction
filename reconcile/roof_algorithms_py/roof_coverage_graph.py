...
def _largest_polygon(geom: Any) -> Polygon | None:
    ...
    if getattr(geom, "geom_type", "") == "MultiPolygon":
        try:
            return max(
                (part for part in geom.geoms if isinstance(part, Polygon) and part.area > AREA_EPS),
...
def build_roof_coverage_graph(
    *,
    ...
    for envelope_kind, surfaces in (("oblique", selected_oblique_surfaces), ("flat", selected_flat_surfaces)):
        for index, surface in enumerate(surfaces):
            polygon = _surface_poly(surface)
            if polygon is None:
                continue
            hypothesis_id = str(surface.get("roof_hypothesis_id") or f"{envelope_kind}:{index}")
            envelope_id = f"roof-envelope:{envelope_kind}:{_stable_hash([hypothesis_id, str(index)], 20)}"
            part_ids = [str(part_id) for part_id in hypothesis_membership.get(hypothesis_id, [])]
            envelope = {
                "id": envelope_id,
                "type": "RoofEnvelope",
    ...
    for room_partition in room_partitions:
        ...
        for partition in room_partition.get("partitions") or []:
            ...
            for envelope in envelopes:
                ...
                same_part = bool(part_id and part_id in (envelope.get("part_ids") or []))

                if envelope["envelope_kind"] == "oblique":
                    vertical_clearance = _surface_y_at(envelope["surface"], float(rep.x), float(rep.y)) - atom_base_y
                    if vertical_clearance > 0.12:
                        state = "confirmed"
                    elif overlap_ratio >= 0.2 or (same_part and part_family == "gable_or_multi_slope"):
                        state = "partial"
                    else:
                        state = "weak"
                    if (
                        {"none": 0, "weak": 1, "partial": 2, "confirmed": 3}[state]
                        > {"none": 0, "weak": 1, "partial": 2, "confirmed": 3}[best["sloped_state"]]
                        or (
                        ...
                        best.update(
                            {
                                ...
                                "sloped_vertical_clearance_m": round(vertical_clearance, 6),
                    ...
                    edges.append(
                        {
                            "id": f"edge:covers-atom:{_stable_hash([envelope['id'], atom_id], 20)}",
                            "type": "COVERS_ATOM",
                            "from": envelope["id"],
                            "to": atom_id,
                            "evidence": {
                        ...
                        best.update(
                            {
                                ...
                                "flat_shell_hypothesis_id": envelope["roof_hypothesis_id"],
                            }
                        )
    ...
    for envelope in envelopes:
        ...
        seeded_any = False
        envelope_parts = {str(part_id) for part_id in (envelope.get("part_ids") or []) if part_id}
        for room_index in seed_room_indices:
            ...
            if envelope_parts and room_parts and not envelope_parts.intersection(room_parts):
                continue
            try:
            ...
            if clipped_poly is None or clipped_poly.area <= AREA_EPS:
                try:
                    clipped_poly = _largest_polygon(room_poly.buffer(SEED_ROOM_BUFFER_M).intersection(envelope["polygon"]))
                except Exception:
                    clipped_poly = None
            if clipped_poly is None or clipped_poly.area <= AREA_EPS:
                continue
    ...
    for hypothesis_id, room_polys in room_polys_by_hypothesis.items():
        ...
        for room_index in room_ids:
            ...
            subpart_id = f"coverage-subpart:{_stable_hash([hypothesis_id, str(sorted(component))], 20)}"
            part_ids = [str(part_id) for part_id in hypothesis_membership.get(hypothesis_id, [])]
            union_poly = None
            for member in component:
                ...
                if member_poly is None:
                    continue
                union_poly = member_poly if union_poly is None else union_poly.union(member_poly)
            record = {
                "id": subpart_id,
                "roof_hypothesis_id": hypothesis_id,
                "room_indices": sorted(component),
                "part_ids": part_ids,
                "avg_azimuth": (oblique_surface_meta.get(hypothesis_id) or {}).get("avg_azimuth"),
    ...
    for atom_id, coverage in atom_coverage.items():
        ...
        if not hypothesis_id or str(coverage.get("sloped_state", "none")) not in {"confirmed", "partial"}:
            continue
    ...
    for idx, left_id in enumerate(subpart_ids):
        ...
        for right_id in subpart_ids[idx + 1 :]:
            ...
            if 140.0 <= diff <= 220.0:
                ...
                right_semantic = set([subpart_records[right_id].get("semantic_kind", "slope_run"), "gable_run"])
        ...
        serializable_subparts.append(
            {
                ...
                "adjacent_subpart_ids": sorted(set(subpart.get("adjacent_subpart_ids") or [])),
                "polygon_xz": (
                    [
                        [round(float(x), 6), round(float(y), 6)]
    ...
    return {
        ...
        "atom_subpart_membership": {atom_id: sorted(set(subparts)) for atom_id, subparts in atom_subpart_membership.items()},
        "metadata": {
            "envelope_count": len(nodes),
            "coverage_edge_count": len(edges),
            "confirmed_sloped_atom_count": sum(
                1 for cov in atom_coverage.values() if cov.get("sloped_state") == "confirmed"
            ),
            "partial_sloped_atom_count": sum(
                1 for cov in atom_coverage.values() if cov.get("sloped_state") == "partial"
            ),
            "subpart_count": len(serializable_subparts),
            "seeded_hypothesis_count": seeded_hypothesis_count,
            "gable_subpart_count": sum(1 for subpart in serializable_subparts if subpart.get("semantic_kind") == "gable_run"),
            "l_t_subpart_count": sum(1 for subpart in serializable_subparts if subpart.get("semantic_kind") == "l_t_branch"),
        },
    }
