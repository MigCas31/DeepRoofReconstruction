...
def build_roof_evidence_graph(
    *,
    ...
    room_polys = {room_index: poly for room_index, poly in room_polys.items() if poly is not None}
    ...
    for room_partition in room_partitions:
        ...
        for atom in room_partition.get("partitions") or []:
            ...
            subpart_ids = [str(subpart_id) for subpart_id in atom_subpart_membership.get(atom_id, [])]
            ...
            try:
                perimeter_len = float(atom_poly.boundary.intersection(room_boundary).length)
            except Exception:
                perimeter_len = 0.0
            sloped_context = atom.get("kind") == "oblique" or str(coverage.get("sloped_state", "none")) in {
                "confirmed",
                "partial",
            }
            ...
            if atom.get("kind") == "flat" and str(coverage.get("sloped_state", "none")) in {"confirmed", "partial"}:
                room_flat_cap_under_slope += 1
            room_upper_void_cell_count += sum(1 for cell in cells if cell.get("cell_kind") == "upper_void")
            room_attic_cell_count += sum(1 for cell in cells if cell.get("cell_kind") == "attic")
            hypothesis_id = atom.get("roof_hypothesis_id") or coverage.get("sloped_hypothesis_id")
            if hypothesis_id:
            ...
            atom_evidence[atom_id] = {
                ...
                "flat_cap_under_slope": atom.get("kind") == "flat" and str(coverage.get("sloped_state", "none")) in {"confirmed", "partial"},
                "subpart_ids": subpart_ids,
                "semantic_kinds": semantic_kinds,
                "upper_void_cell_count": sum(1 for cell in cells if cell.get("cell_kind") == "upper_void"),
                "attic_cell_count": sum(1 for cell in cells if cell.get("cell_kind") == "attic"),
            }

        knee_walls = knee_walls_by_room.get(room_index, [])
        knee_wall_span = sum(_knee_wall_bottom_span(knee_wall) for knee_wall in knee_walls)
        ...
        strong_perimeter_sloped = room_sloped_perimeter >= 1.5 or (room_sloped_perimeter / room_boundary_len) >= 0.25
        strong_knee_wall_signal = len(knee_walls) > 0 or knee_wall_span >= 0.75
        strong_gable_context = "gable_run" in semantic_kinds_on_room or opposed_pair_count > 0
        strong_upper_void_context = (
            room_upper_void_cell_count > 0
            or (
                room_flat_cap_under_slope > 0
        ...
        room_evidence[room_id] = {
            ...
            "sloped_perimeter_ratio": round(room_sloped_perimeter / room_boundary_len, 6),
    ...
    for subpart_id, subpart in subparts_by_id.items():
        room_indices = [int(room_index) for room_index in (subpart.get("room_indices") or []) if isinstance(room_index, int)]
        room_ids = [_room_key(room_index) for room_index in room_indices]
        strong_perimeter_room_count = sum(
            1 for room_id in room_ids if (room_evidence.get(room_id) or {}).get("strong_perimeter_sloped")
        )
        knee_wall_room_count = sum(
            1 for room_id in room_ids if (room_evidence.get(room_id) or {}).get("knee_wall_count", 0) > 0
        )
        flat_cap_room_count = sum(
            1 for room_id in room_ids if (room_evidence.get(room_id) or {}).get("flat_cap_under_slope_count", 0) > 0
        )
        evidence_score = (
    ...
    for part in building_part_graph.get("nodes") or []:
        ...
        gable_subpart_count = sum(
            1 for subpart_id in subpart_ids if (subpart_evidence.get(subpart_id) or {}).get("semantic_kind") == "gable_run"
        )
        branch_subpart_count = sum(
            1 for subpart_id in subpart_ids if (subpart_evidence.get(subpart_id) or {}).get("semantic_kind") == "l_t_branch"
        )
        ...
        elif strong_sloped_room_count > 0 or knee_wall_room_count > 0 or flat_cap_room_count > 0:
...
def annotate_roof_coverage_graph(
    *,
    ...
    for subpart in roof_coverage_graph.get("subparts") or []:
        ...
        annotated["support_evidence_score"] = int(evidence.get("evidence_score", 0) or 0)
        annotated["strong_perimeter_room_count"] = int(evidence.get("strong_perimeter_room_count", 0) or 0)
        annotated["knee_wall_room_count"] = int(evidence.get("knee_wall_room_count", 0) or 0)
        annotated["flat_cap_room_count"] = int(evidence.get("flat_cap_room_count", 0) or 0)
    ...
    metadata["evidence_strong_subpart_count"] = sum(
        1 for subpart in annotated_subparts if int(subpart.get("support_evidence_score", 0) or 0) >= 3
    )
