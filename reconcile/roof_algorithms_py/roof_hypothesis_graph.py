...
def _candidate_boundary_ids(surface: dict[str, Any]) -> list[str]:
    return sorted({str(boundary_id) for boundary_id in (surface.get("space_boundary_ids") or []) if boundary_id})


def _room_graph_support(graph, story: int, floor_polygon: list) -> tuple[str | None, float, str]:
    ...
    if decision["mode"] == "roof_candidate":
        ...
        return graph_room.id if graph_room is not None else None, (1.0 if state == "confirmed" else 0.8), state
    if decision["mode"] == "underdetermined":
        return graph_room.id if graph_room is not None else None, 0.55, decision.get("exposure_state", "partial")
    return graph_room.id if graph_room is not None else None, 0.1, decision.get("above_state", "confirmed")


def _candidate_base_score(kind: str, surface: dict[str, Any], graph_support: float) -> float:
    if kind == "oblique":
    ...
    *,
    ...
    for room_data in exposed_rooms:
        ...
        room_node = {
            ...
            "area_m2": round(_poly_xz(room_data.get("fp") or []).area, 6) if _poly_xz(room_data.get("fp") or []) else 0.0,
            "wall_top_y": round(float(room_data["wallTopY"]), 6),
            "wall_top_min_y": round(float(room_data["wallTopMin"]), 6),
            "slant_delta_m": round(float(room_data["wallTopY"] - room_data["wallTopMin"]), 6),
    ...
    for kind, surfaces in (("oblique", oblique_surfaces), ("flat", flat_surfaces)):
        for index, surface in enumerate(surfaces):
            ...
            if kind == "oblique":
                support_room_indices = sorted(set((surface.get("cluster") or {}).get("room_indices") or []))
            else:
                room_index = surface.get("room_index")
                support_room_indices = [int(room_index)] if isinstance(room_index, int) else []
            ...
            graph_support = sum(support_scores) / len(support_scores) if support_scores else 0.5
            ...
            node = {
                ...
                "support_score": round(_candidate_base_score(kind, surface, graph_support), 6),
                "selected": False,
            }
            nodes.append(node)
            ...
            for right_id in right_ids:
                ...
                if current is None or shared_length > current["shared_edge_length_m"]:
                    continuation_pairs[pair] = {
                        ...
                        "relation_state": edge_evidence.get("relation_state", "partial"),
                        "exact_face_incidence": bool(edge_evidence.get("exact_face_incidence")),
                        "partition_atom_pairs": edge_evidence.get("partition_atom_pairs") or [],
                    }
    ...
    return {
        ...
        "metadata": {
            ...
            "continuation_edge_count": sum(1 for edge in edges if edge["type"] == "CONTINUES_AS"),
        },
