...
def _bbox_gap(a: tuple[float, float, float, float] | None, b: tuple[float, float, float, float] | None) -> float:
...
def _edge(
    *,
    ...
    return {
        "id": f"edge:{edge_type.lower()}:{_stable_hash([edge_type, from_id, to_id, str(evidence)], 20)}",
        "type": edge_type,
        "from": from_id,
        "to": to_id,
        "evidence": evidence,
...
def build_roof_boundary_graph(boundary_model: dict[str, Any], graph=None) -> dict[str, Any]:
    faces_by_id = {
        face["id"]: face
    ...
    for incidence in partition.get("adjacency", []):
        ...
        entry["atom_pairs"].append(
            [incidence.get("from_atom_id"), incidence.get("to_atom_id")]
        )
    ...
    for (left_id, right_id), incidence in incidence_by_pair.items():
        ...
        adjacency_edge = room_adjacency.get((room_a, room_b)) or room_adjacency.get((room_b, room_a))
        adjacency_state = (
        ...
        normal_delta = _normal_angle_deg(left_face.get("normal"), right_face.get("normal"))
        ...
        if not (same_story and same_kind and same_plane_family and shared_edge_length > 0.01):
            continue
        ...
        edges.append(_edge(edge_type="CONTINUES_AS", from_id=left["id"], to_id=right["id"], evidence=evidence))
        edges.append(_edge(edge_type="CONTINUES_AS", from_id=right["id"], to_id=left["id"], evidence=evidence))
