...
def _largest_polygon_component(geom: Any) -> Polygon | None:
    ...
    if isinstance(geom, Polygon):
        return geom
    if isinstance(geom, MultiPolygon):
        polys = [poly for poly in geom.geoms if isinstance(poly, Polygon) and not poly.is_empty]
...
def _cell_faces(base: list[list[float]], top: list[list[float]]) -> list[dict[str, Any]]:
    faces = [
...
def _derive_knee_walls(
    *,
    ...
    for face_index, face in enumerate(faces):
        ...
        knee_walls.append(
            {
                "id": f"knee-wall:{_stable_hash([cell_id, str(face_index), str(corners)], 20)}",
                "cell_id": cell_id,
                "room_id": room_id,
    ...
    *,
    ...
    for room_partition in room_partitions:
        ...
        for atom in room_partition.get("partitions") or []:
            ...
            atom_is_upper_void_candidate = flat_role in {
                "ceiling_cap",
                "ambiguous_flat_over_sloped_part",
            } or room_has_oblique_atom or sloped_state in {"confirmed", "partial"}
            ...
            for surface_record in surface_records:
                if (
                    ...
                    and surface_record["roof_hypothesis_id"] != target_sloped_hypothesis_id
                ):
                    continue
                try:
            ...
            for region in regions:
                ...
                for overlap in overlaps:
                    try:
                        ...
                        continue
                    top_y = _surface_y_at(overlap["surface"], float(rep.x), float(rep.y))
                    if top_y <= base_y + MIN_UPPER_VOID_HEIGHT_M:
                        continue
                    same_part = part_id is not None and part_id in (overlap.get("part_ids") or [])
                    current_same_part = (
                        part_id is not None
                ...
                for convex_index, convex_region in enumerate(convex_regions):
                    ...
                    if len(coords) < 3:
                        continue
                    region_footprint = [(_snap(float(x)), _snap(float(z))) for x, z, *_ in coords]
                    perimeter_side_face_indices = _perimeter_side_face_indices(convex_region, atom_poly)
                    cell_id = f"roof-cell:{top_kind}:{_stable_hash([atom['id'], best_surface['roof_hypothesis_id'], str(region_footprint), str(convex_index)], 20)}"
                    cell = build_arranged_polyhedral_cell(
                        ...
                        base_y=base_y,
                        top_y_at=lambda x, z, surface=best_surface["surface"]: _surface_y_at(surface, x, z),
                    ...
                    edges.append(
                        {
                            "id": f"edge:atom-cell:{_stable_hash([str(atom['id']), cell_id], 20)}",
                            "type": "BOUNDS_CELL",
                            "from": str(atom["id"]),
                            "to": cell_id,
                        }
                    ...
                    edges.append(
                        {
                            "id": f"edge:cell-hypothesis:{_stable_hash([cell_id, best_surface['roof_hypothesis_id']], 20)}",
                            "type": "SUPPORTED_BY",
                            "from": cell_id,
