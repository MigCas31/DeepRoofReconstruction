        ...
        for right_room in room_indices[idx + 1 :]:
            ...
            try:
                is_adjacent = (
                    ...
                    or left_poly.boundary.intersection(right_poly.boundary).length > 0.05
                )
            except Exception:
                is_adjacent = False
            if is_adjacent:
...
def _continuation_stats(hypothesis_graph: dict[str, Any]) -> dict[str, dict[str, Any]]:
    ...
    for node in hypothesis_graph.get("nodes") or []:
        ...
        stats[hypothesis_id] = {
            ...
            "continuation_component_size": int(node.get("continuation_component_size", 1) or 1),
...
def continue_roof_envelopes(
    *,
    ...
    selected_room_assignments = {
        ...
        for room_id, assignments in (augmented_graph.get("selected_room_assignments") or {}).items()
    }
    ...
    for room_index, room_partition in room_partitions_by_index.items():
        ...
        if room_slant < 0.6:
            continue
        if any(partition.get("kind") == "oblique" for partition in (room_partition.get("partitions") or [])):
            continue
        ...
        part_has_structural_slope_semantics = any(
            str(subpart.get("semantic_kind", "slope_run")) in {"gable_run", "l_t_branch"}
        ...
        for atom in room_partition.get("partitions") or []:
            ...
            for hypothesis_id, surfaces in surfaces_by_hypothesis.items():
                ...
                adjacent_covered = sorted(
                    ...
                    if room_index_value in covered_rooms_by_hypothesis.get(hypothesis_id, set())
                )
                if not adjacent_covered:
                    continue
                semantic_kinds = {
                    str(subparts_by_id[subpart_id].get("semantic_kind", "slope_run"))
                    for room_key in (_room_key(room_idx) for room_idx in adjacent_covered)
                    for subpart_id in room_subpart_membership.get(room_key, [])
                    if subpart_id in subparts_by_id and subparts_by_id[subpart_id].get("roof_hypothesis_id") == hypothesis_id
                }
                source_atoms = []
                for source_atom in covered_atoms_by_hypothesis.get(hypothesis_id, []):
                    ...
                    try:
                        shared_boundary = float(
                            atom_poly.boundary.intersection(source_atom["poly"].boundary).length
                        )
                    except Exception:
                        shared_boundary = 0.0
                    ...
                    for subpart_id in source_atom.get("subpart_ids") or []:
                        if subpart_id in subparts_by_id:
                            semantic_kinds.add(str(subparts_by_id[subpart_id].get("semantic_kind", "slope_run")))
                ...
                source_story = int(
                    source_surface.get("story", source_surface.get("dominant_story", room_partition.get("story", 0))) or 0
                )
                ...
                if distance > MAX_CONTINUATION_DISTANCE_M:
                    continue
                clearance = float(_surface_y_at(source_surface, float(rep.x), float(rep.y)) - atom_base_y)
                ...
                if source_atoms:
                    source_incidence_score = max(
                        min(0.6, 0.2 * atom_data["shared_boundary_m"]) + max(0.0, 0.2 - atom_data["gap_distance"])
    ...
    for (room_index, hypothesis_id), proposals in continuation_proposals.items():
        ...
        for region_index, region in enumerate(merged_regions):
            ...
            continued_surface["continuation_atom_ids"] = [proposal["atom_id"] for proposal in proposals]
            continued_surface["continuation_score"] = round(float(best_proposal["best"]["score"]), 6)
            continued_surface["continuation_adjacent_room_indices"] = list(best_proposal["best"]["adjacent_covered"])
            continued_surface["continuation_distance_m"] = round(float(best_proposal["best"]["distance"]), 6)
            continued_surface["continuation_clearance_m"] = round(float(best_proposal["best"]["clearance"]), 6)
        ...
        graph_edges.append(
            {
                ...
                "synthetic": True,
                "evidence": {
                    "coverage_ratio": round(total_area / max(float(proposals[0]['room_area']), AREA_EPS), 6),
