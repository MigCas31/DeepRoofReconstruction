...
def run_roof_algorithms(bldg: dict, graph=None) -> dict:
    ...
    candidate_roof_surfaces = derive_roof_surfaces_from_boundary_model(candidate_boundary_model)
    candidate_roof_graph = build_roof_boundary_graph(candidate_boundary_model, graph=graph)
    ...
    selected_oblique_surfaces, selected_flat_surfaces = select_roof_surfaces_from_hypotheses(
        oblique_surfaces=candidate_roof_surfaces["oblique"],
        flat_surfaces=candidate_roof_surfaces["flat"],
    ...
    roof_flat_surfaces = []
    ceiling_flat_support_surfaces = []
    for surface in selected_flat_surfaces:
        hypothesis_id = surface.get("roof_hypothesis_id")
        role_info = flat_hypothesis_roles.get(str(hypothesis_id), {"role": "roof_flat", "reason": "default"})
        enriched = dict(surface)
        enriched["flat_role"] = role_info["role"]
        enriched["flat_role_reason"] = role_info["reason"]
        ceiling_flat_support_surfaces.append(enriched)
        if role_info["role"] == "ceiling_cap":
            continue
        roof_flat_surfaces.append(enriched)
    ...
    for d in dormer_candidates:
        ...
        geom = build_dormer_geometry(group, surf, room_walls=room.get("walls_merged", []))
