...
def _remove_collinear_xz(
    ...
    while changed:
        ...
        for i in range(n):
            ...
            cross = (curr[0] - prev[0]) * (nxt[2] - prev[2]) - (
                curr[2] - prev[2]
            ) * (nxt[0] - prev[0])
            if abs(cross) > eps:
                out.append(curr)
...
def _find_covering_planes(
    room_data: dict, ceiling_planes: list
) -> list[int]:
...
def _build_thermal_for_oblique_room(
    ...
    for pi in plane_indices:
        ...
        projected = [
            (p[0], plane_y_at(plane, p[0], p[2]), p[2]) for p in fp
        ]
        ...
        if len(clipped_for_render) >= 3:
            ...
            clipped_holes = _find_dormer_holes_for_surface(
                clipped_for_render, oblique_surfaces, surface_indices, fp,
                top_y, bottom_y
            )
            entry: dict = {
                "kind": "thermal-slant",
...
def _clip_by_lower_plane(
    ...
    for k in range(n):
        ...
        cv = plane_y_at(this_plane, curr[0], curr[2]) - plane_y_at(other_plane, curr[0], curr[2])
        nv = plane_y_at(this_plane, nxt[0], nxt[2]) - plane_y_at(other_plane, nxt[0], nxt[2])
        c_in = cv <= 0.0
        n_in = nv <= 0.0
        if c_in:
            out.append(curr)
...
def _find_dormer_holes_for_surface(
    ...
    """Find dormer cutout holes from oblique surfaces that overlap this thermal surface."""
    clipped_holes = []
    ...
    if fp_shapely is not None and full_xz_polys:
        ...
        if parts:
            ...
            if remainder is not None:
                ...
                for poly in cap_polys:
                    if len(poly) >= 3:
                        out.append({"kind": "thermal-cap", "story": story, "poly": poly})
...
def _build_knee_wall_ceilings(
    ...
    for i in range(1, len(stories)):
        ...
        geoms = [gap] if isinstance(gap, Polygon) else list(gap.geoms) if isinstance(gap, MultiPolygon) else []
        for g in geoms:
            ...
            centroid = g.centroid
            y = _floor_y_at_xz(
                story_floor_regions, upper_story,
                centroid.x, centroid.y,  # shapely 2D: x=X, y=Z
                fallback_y,
            )
            out.append({
                "kind": "thermal-knee",
...
def _find_best_matching_room(
    slant_ceiling: dict, exposed_rooms: list
) -> dict | None:
...
def build_thermal_ceilings(
    *,
    ...
    for di, d in enumerate(dormers or []):
        for ci, cheek in enumerate(d.get("cheeks", [])):
