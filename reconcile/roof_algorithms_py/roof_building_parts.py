...
def _selected_hypothesis_nodes(hypothesis_graph: dict[str, Any]) -> dict[str, dict[str, Any]]:
    return {
        node["id"]: node
...
def _all_room_nodes(
    *,
    ...
    nodes = {room_id: dict(room) for room_id, room in _room_nodes(exposed_rooms).items()}
    if bldg is None:
        return nodes
...
def _room_adjacency(room_nodes: dict[str, dict[str, Any]], graph) -> dict[str, set[str]]:
    ...
    if graph is None:
        ...
        for room_id, room in room_nodes.items():
            points = [(float(p[0]), float(p[2])) for p in (room.get("fp") or []) if len(p) >= 3]
            if len(points) < 3:
                continue
            poly = Polygon(points)
        ...
        for idx, left_id in enumerate(room_ids):
            ...
            for right_id in room_ids[idx + 1 :]:
                ...
                if left.distance(right) <= 0.25 or left.boundary.intersection(right.boundary).length > 0.05:
...
def _full_room_adjacency(room_nodes: dict[str, dict[str, Any]], graph) -> dict[str, set[str]]:
    ...
    *,
    ...
    while frontier:
        ...
        if candidate_counts:
            ...
            return sorted(part_id for part_id, score in candidate_counts.items() if score == best_score)[0]
        frontier = next_frontier
    return None


def _biconnected_room_components(adjacency: dict[str, set[str]]) -> tuple[list[set[str]], set[str]]:
...
def build_building_part_graph(
    *,
    ...
    all_room_nodes = _all_room_nodes(exposed_rooms=exposed_rooms, bldg=bldg, graph=graph)
    ...
    for index, component in enumerate(components):
        ...
        oblique_ids = sorted(
            ...
            if (selected_hypotheses.get(hypothesis_id) or {}).get("surface_kind") == "oblique"
        )
        flat_ids = sorted(
            hypothesis_id
            for hypothesis_id in hypothesis_ids
            if (selected_hypotheses.get(hypothesis_id) or {}).get("surface_kind") == "flat"
        )
        slant_ratio = (
        ...
        part_id = f"building-part:{_stable_hash([str(index), str(room_ids), str(oblique_ids), str(flat_ids)], 20)}"
        part_node = {
            "id": part_id,
            "type": "BuildingPart",
            "room_ids": room_ids,
            ...
            "articulation_room_ids": sorted(room_id for room_id in room_ids if room_id in articulation_rooms),
            "slant_ratio": round(slant_ratio, 6),
            "max_room_slant_delta_m": round(max(room_slants), 6) if room_slants else 0.0,
        ...
        for hypothesis_id in sorted(hypothesis_ids):
            ...
            part_edges.append(
                {
                    "id": f"edge:part-hypothesis:{_stable_hash([part_id, hypothesis_id], 20)}",
    ...
    if not part_nodes and all_room_nodes:
        ...
        for index, component in enumerate(all_components):
            room_ids = sorted(component)
            part_id = f"building-part:{_stable_hash(['full-building', str(index), str(room_ids)], 20)}"
            node = {
                "id": part_id,
                "type": "BuildingPart",
                "room_ids": room_ids,
    ...
    for room_id in sorted(all_room_nodes):
        ...
        part_nodes_by_id.setdefault(nearest_part_id, {}).setdefault("room_ids", []).append(room_id)
        part_edges.append(
            {
    ...
    return {
        "nodes": part_nodes,
        "edges": part_edges,
        "room_membership": {room_id: sorted(part_ids) for room_id, part_ids in room_part_membership.items()},
...
def refine_building_part_graph(
    *,
    ...
    part_evidence = {
        ...
        for part_id, evidence in (roof_evidence_graph.get("part_evidence") or {}).items()
    ...
    for node in building_part_graph.get("nodes") or []:
        ...
        refined["gable_subpart_count"] = int(evidence.get("gable_subpart_count", 0) or 0)
        refined["branch_subpart_count"] = int(evidence.get("branch_subpart_count", 0) or 0)
        refined["strong_sloped_room_count"] = int(evidence.get("strong_sloped_room_count", 0) or 0)
        refined["knee_wall_room_count"] = int(evidence.get("knee_wall_room_count", 0) or 0)
        refined["flat_cap_room_count"] = int(evidence.get("flat_cap_room_count", 0) or 0)
        refined["opposed_slope_pair_count"] = int(evidence.get("opposed_slope_pair_count", 0) or 0)
    ...
    refined["metadata"].update(
        {
            "evidence_refined_part_count": sum(
                ...
                if node.get("roof_family_guess") != node.get("roof_family_guess_initial")
            ),
...
def classify_selected_flat_hypothesis_roles(
    *,
    ...
    selected_hypotheses = _selected_hypothesis_nodes(hypothesis_graph)
    part_nodes = {
        node["id"]: node for node in (building_part_graph.get("nodes") or [])
    }
    ...
    for hypothesis_id, node in selected_hypotheses.items():
        if node.get("surface_kind") != "flat":
            continue
        support_rooms = [int(v) for v in (node.get("support_room_indices") or []) if isinstance(v, int)]
        support_room_id = _room_key(support_rooms[0]) if len(support_rooms) == 1 else None
        role = "roof_flat"
        reason = "selected_flat_hypothesis"
        if support_room_id is not None:
            parts = [part_nodes[part_id] for part_id in room_membership.get(support_room_id, []) if part_id in part_nodes]
