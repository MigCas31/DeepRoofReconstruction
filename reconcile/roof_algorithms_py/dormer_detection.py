...
def _wall_width_along_ridge(
    corners: list, ridge_x: float, ridge_z: float
) -> float:
...
def detect_dormers(
    ...
    atom_subpart_membership = ((roof_coverage_graph or {}).get("atom_subpart_membership") or {})
    covered_polys_by_hypothesis_room: dict[tuple[str, int], list[dict]] = {}
    for atom_id, region in ((roof_coverage_graph or {}).get("atom_regions") or {}).items():
        coverage = ((roof_coverage_graph or {}).get("atom_coverage") or {}).get(atom_id) or {}
        ...
        if not hypothesis_id or state not in {"confirmed", "partial"} or not isinstance(room_index, int):
            continue
        if len(polygon_xz) < 3:
            continue
        try:
        ...
        covered_polys_by_hypothesis_room.setdefault((str(hypothesis_id), room_index), []).append(
            {
                "poly": poly,
                "subpart_ids": subpart_ids,
                "semantic_kinds": semantic_kinds,
                "support_evidence_score": max(
                    int((subpart_evidence.get(subpart_id) or {}).get("evidence_score", 0) or 0)
                    for subpart_id in subpart_ids
                )
                if subpart_ids
                else 0,
    ...
    if top_boundary_graph is not None:
        for node in top_boundary_graph.get("nodes") or []:
            ...
            if not hypothesis_id or not isinstance(room_index, int):
                continue
            sloped_rooms_by_hypothesis.setdefault(str(hypothesis_id), set()).add(room_index)
    ...
    for surf_idx, surface in enumerate(oblique_roof_surfaces):
        ...
        allowed_room_indices = sloped_rooms_by_hypothesis.get(str(surface.get("roof_hypothesis_id")))
        ...
        for room_idx, room in enumerate(bldg.get("rooms", [])):
            if room.get("story") != dominant_story:
                continue
            if allowed_room_indices is not None and room_idx not in allowed_room_indices:
                continue
            ...
            for wall in all_walls:
                ...
                above = [
                    ...
                    for c, ry in zip(corners, roof_ys)
                ]
                if not any(above):
                    continue
                if all(above):
                    ...
                    min_gap = min(
                        c[1] - ry for c, ry in zip(corners, roof_ys)
                    )
                    if min_gap > MAX_BOTTOM_ABOVE_ROOF:
                        continue

                # 3. Size: dormer is smaller than the roof
                wall_width = _wall_width_along_ridge(
                    corners, ridge_x, ridge_z
                )
                ...
                wall_height = max(c[1] for c in corners) - min(
                ...
                if not _point_near_roof_xz(wcx, wcz, surface):
                    continue
                covered_regions = covered_polys_by_hypothesis_room.get((surface_hypothesis_id, room_idx), [])
                if covered_regions:
                    pt = Point(wcx, wcz)
                    matching_regions = [
                        region
...
def _is_knee_wall_front(wall: dict, knee_walls: list[dict], max_dist: float = 0.25, max_angle_deg: float = 20.0) -> bool:
    corners = wall.get("corners", [])
    if len(corners) < 2:
        return False
...
def group_dormer_walls(
    front_wall: dict, room_walls: list[dict]
) -> dict:
