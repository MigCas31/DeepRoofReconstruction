...
def _largest_polygon(geom: Any) -> Polygon | None:
    ...
    if isinstance(geom, Polygon):
        return geom
    polys = [poly for poly in getattr(geom, "geoms", []) if isinstance(poly, Polygon) and not poly.is_empty]
    if not polys:
        return None
...
def _footprint_from_polygon(poly: Polygon) -> list[tuple[float, float]]:
    def same_point(left: tuple[float, float], right: tuple[float, float], tol: float = 1e-5) -> bool:
...
def _room_walls(room: dict[str, Any]) -> list[dict[str, Any]]:
    computed = [wall for wall in (room.get("walls_computed") or []) if isinstance(wall, dict)]
    merged = [wall for wall in (room.get("walls_merged") or []) if isinstance(wall, dict)]
...
def _room_base_y(room: dict[str, Any]) -> float:
    ...
    for wall in _room_walls(room):
        ...
        if len(corners) < 2:
            continue
        min_y = min(float(corner[1]) for corner in corners)
        bottom_ys.extend(float(corner[1]) for corner in corners if float(corner[1]) <= min_y + 0.02)
    if bottom_ys:
...
def _partition_group_key(atom: dict[str, Any]) -> tuple[str, str, str, float | None]:
    ...
    rounded_top_y = round(float(top_y), 3) if isinstance(top_y, (int, float)) and kind == "flat" else None
...
def _boundary_overlap_length(face: dict[str, Any], boundary_geom: Polygon | LineString | None) -> float:
    ...
    if wall_edge is None:
        return 0.0
    try:
        boundary = boundary_geom.boundary if isinstance(boundary_geom, Polygon) else boundary_geom
...
def _annotate_boundary_classes(cell: dict[str, Any], room_poly: Polygon, story_union: Polygon | None) -> None:
...
def _synthetic_surface_from_polygon(poly: Polygon, *, top_y: float) -> dict[str, Any]:
    corners = [[float(x), float(top_y), float(z)] for x, z, *_ in list(poly.exterior.coords)[:-1]]
    return {
        "kind": "flat",
        "corners": corners,
...
def build_occupied_room_cell_complex(
    *,
    ...
    for room_index, room in enumerate(bldg.get("rooms") or []):
        ...
        part_ids = [str(part_id) for part_id in (room_membership.get(room_id) or []) if part_id]
        ...
        if top_y <= base_y + 0.05:
            continue
        for remainder_index, remainder_piece in enumerate(_decompose_polygons(remainder)):
    ...
    for cell in cells:
        for face in (cell.get("faces") or []):
            boundary_class = str((face.get("metadata") or {}).get("boundary_class") or "")
            if boundary_class:
