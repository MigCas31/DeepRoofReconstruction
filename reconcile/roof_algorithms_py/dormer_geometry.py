...
def _clip_cheek_against_walls(
    ...
    def from_uv(pts):
        return [
            (front[0] + u * dir_x, v, front[2] + u * dir_z)
            for u, v in pts
        ]
    ...
    for wc in wall_corner_lists:
        ...
        dists = [
            (c[0] - front[0]) * normal_x + (c[2] - front[2]) * normal_z
            for c in wc
        ]
        ...
        projs = [
            (c[0] - front[0]) * dir_x + (c[2] - front[2]) * dir_z
            for c in wc
        ]
