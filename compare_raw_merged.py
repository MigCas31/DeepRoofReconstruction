...
BUILDINGS = {
    "1f03f6e0": {
        ...
        "scan_dir": "scans_imgyiHP46oPmBK2ZQLlx3kLxLAR2_Bredballe_Byvej_63__7120_Vejle__st_1f03f6e0-dfe6-4b25-bb45-f44ad146c0a3_1772204075_129672",
    },
    "1d26eda3": {
        "name": "Gerskov Bygade 24",
        "uuid": "1d26eda3-c927-4cdf-a3b0-218036828a55",
        "scan_dir": "scans_ZbzSl5KqVsNNBG6I5p53yxeLoNT2_Gerskov_Bygade_24__5450_Otterup_1d26eda3-c927-4cdf-a3b0-218036828a55_1772632245_7456899",
    },
    "7dbc53a6": {
        "name": "Gultvedgyden 6",
        "uuid": "7dbc53a6-17e8-4806-83de-42286b95726c",
        "scan_dir": "scans_Bmgf8ROc4ZcAtPoPIi5P8vxm2VI3_Gultvedgyden_6__5772_Kv_rndrup_7dbc53a6-17e8-4806-83de-42286b95726c_1773653476_089257",
    },
    "e661e7b6": {
        "name": "Hesselvænget 2",
        "uuid": "e661e7b6-303d-415c-b378-2d9dd2fbfd6f",
        "scan_dir": "scans_ZbzSl5KqVsNNBG6I5p53yxeLoNT2_Hesselv_nget_2__5800_Nyborg_e661e7b6-303d-415c-b378-2d9dd2fbfd6f_1772704341_1621342",
    },
    "cb711a0b": {
        "name": "Kastanievej 6",
        "uuid": "cb711a0b-6e8d-4ae6-b008-af3297446dcc",
        "scan_dir": "scans_aKeHkHmE5fM8kkPth63xGUGOfz63_Kastanievej_6__5400_Bogense_cb711a0b-6e8d-4ae6-b008-af3297446dcc_1774438563_092342",
    },
    "b4b6f3ed": {
        "name": "Lucernevej 23",
        "uuid": "b4b6f3ed-7bfd-43a8-aeed-520b558bfa2b",
        "scan_dir": "scans_RzpAyFHQtPf6afmXkhqnIwPo9QU2_Lucernevej_23__8700_Horsens_b4b6f3ed-7bfd-43a8-aeed-520b558bfa2b_1772535414_466711",
    },
    "feccbd0c": {
        "name": "Morelvej 68",
        "uuid": "feccbd0c-0420-4775-b5b7-49b99559947e",
        "scan_dir": "scans_ZbzSl5KqVsNNBG6I5p53yxeLoNT2_Morelvej_68__5250_Odense_SV_feccbd0c-0420-4775-b5b7-49b99559947e_1770801053_970685",
    },
    "99ce0ab2": {
        "name": "Pilevej 30",
        "uuid": "99ce0ab2-eaac-406c-9fa2-707d7dbe30c5",
        "scan_dir": "scans_ZbzSl5KqVsNNBG6I5p53yxeLoNT2_Pilevej_30__5792_A_rslev_99ce0ab2-eaac-406c-9fa2-707d7dbe30c5_1775030332_775651",
    },
}
...
def load_raw_rooms(scan_dir_path):
    """Load all room JSONs from scan-cache, excluding data.json, arworldmap.json, ceiling_*.json"""
    rooms = {}
...
def analyze_building(short_uuid, info):
    ...
    # wall_id -> { "room_id": ..., "raw_transform": ..., "raw_translation": ..., "story": ... }
    raw_wall_index = {}
    raw_room_summaries = {}
    ...
    for room_id, summary in sorted(raw_room_summaries.items()):
        print(f"    Room '{room_id}': story={summary['story']}, "
              ...
              f"windows={summary['num_windows']}, floor_corners={summary['num_floor_corners']}")
    ...
    for room_id, summary in raw_room_summaries.items():
    ...
    if rooms_entirely_dropped:
        ...
        for rid in sorted(rooms_entirely_dropped):
            ...
            print(f"      Room '{rid}': story={s['story']}, walls={s['num_walls']}, "
    ...
    if large_diffs:
        ...
        for entry in large_diffs[:20]:
            ...
            print(f"        raw_story={entry['raw_story']} -> merged_story={entry['merged_story']}")
    ...
    for room_id in sorted(raw_room_summaries.keys()):
        ...
        if len(merged_stories) > 1:
            print(f"    *** SCATTERED *** Room '{room_id}' (raw story={raw_story}): "
                  f"walls ended up in merged stories {merged_stories}")
        elif merged_stories and list(merged_stories)[0] != raw_story:
            print(f"    STORY CHANGED: Room '{room_id}' raw_story={raw_story} -> "
                  f"merged_story={list(merged_stories)[0]}")

        if len(merged_room_indices) > 1:
            print(f"    *** SPLIT *** Room '{room_id}': walls split across "
    ...
    for room_id in sorted(raw_room_summaries.items(), key=lambda x: x[0]):
        ...
        if best_merged_idx is not None and best_overlap > 0:
            ...
            if raw_ref and merged_ref:
                ...
                if ref_dist > 2.0:
                    print(f"    *** LARGE REF TRANSFORM SHIFT *** Room '{room_id}' -> "
                          f"merged room {best_merged_idx}: ref origin moved {ref_dist:.2f}m")
                    print(f"      raw_ref_pos=({raw_ref_t[0]:.2f}, {raw_ref_t[1]:.2f}, {raw_ref_t[2]:.2f})")
                    print(f"      merged_ref_pos=({merged_ref_t[0]:.2f}, {merged_ref_t[1]:.2f}, {merged_ref_t[2]:.2f})")

            # Also compute average wall displacement for this room
            displacements = []
            for wid in room_walls:
                if wid in merged_wall_index:
                    d = dist3d(raw_wall_index[wid]["translation"],
            ...
            if displacements:
                ...
                if avg_d > 2.0:
                    print(f"    *** HIGH AVG WALL DISPLACEMENT *** Room '{room_id}' -> "
                          f"merged room {best_merged_idx}: avg={avg_d:.2f}m, max={max_d:.2f}m "
                          f"({len(displacements)} walls)")
        else:
            if room_walls:
                print(f"    Room '{room_id}': NO matching merged room found (all walls dropped)")
