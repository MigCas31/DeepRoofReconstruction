...
def _node_point(shared_nodes: dict[str, list[float]], nid: str) -> tuple[float, float, float] | None:
...
def _build_edge_topology(
    ...
    *,
            ...
            sorted(
                {
                    ...
                    if (cluster_id := surface_cluster_by_id.get(str(ref.get("surface_id") or "")))
                }
            )
        )
    ...
    node_points = {
        nid: p
        for nid in node_ids
        if (p := _node_point(shared_nodes, nid)) is not None
    }
        ...
        for rpi, rpoly in enumerate(render_polys):
            ...
            for hi, hole in enumerate(rpoly.interiors):
                ...
                for p in h3:
                    ...
                    split_nodes.add(nid)
                rhole_loops.append(
                    {"id": f"render-hole:{ci}:{rpi}:{hi}", "node_ids": h_ids}
                )
