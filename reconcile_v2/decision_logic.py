...
def _relation_state_from_edges(edges: list) -> str:
    if not edges:
        return "unknown"
    states = [str((edge.evidence or {}).get("relation_state", "confirmed")) for edge in edges]
...
def classify_room_relation_state(graph: TopologyGraph, room: GraphNode) -> dict[str, Any]:
    ...
    adjacency_edges = [edge for edge in graph.edges_from(room.id, "ADJACENT_TO") if edge.to_id.startswith("room:")]
    return {
        "room_id": room.id,
...
def classify_gap_decision(graph: TopologyGraph, gap: GraphNode) -> dict[str, Any]:
    ...
    if gap_kind == "cross_story":
        if any(profile["above_state"] in {"partial", "unknown"} for profile in relation_profiles):
            return {
                "action": "inspect",
...
def classify_overlap_resolution(graph: TopologyGraph, room: GraphNode) -> dict[str, Any]:
