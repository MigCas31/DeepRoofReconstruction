...
def _story_floor_offsets(building: Building) -> dict[str, float]:
    ...
    for room in building.rooms:
        ...
        floor_values_by_story.setdefault(int(room.story), []).append((room.identifier, floor_y))

    offsets: dict[str, float] = {}
    for story, entries in floor_values_by_story.items():
        values = sorted(v for _room_id, v in entries)
        if not values:
            continue
        mid = len(values) // 2
        median = values[mid] if len(values) % 2 == 1 else (values[mid - 1] + values[mid]) * 0.5
...
def _adjacency_relation_state(
    *,
    ...
    vertical_factor = 1.0 if floor_delta_m <= 0.15 else (0.25 if floor_delta_m <= 0.50 else 0.05)
...
def _gap_relation_state(confidence_score: float, compactness: float, perimeter_contact_pct: float) -> tuple[float, str]:
