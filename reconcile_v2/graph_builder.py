...
def _story_stats_from_cells(cell_complex: CellComplex) -> dict[int, dict[str, float | int]]:
...
def _classify_surface_roles(graph: TopologyGraph) -> None:
    ...
    for edge in graph.edges:
        if edge.type == "CONTAINS" and edge.to_id.startswith("surface:") and edge.from_id.startswith("room:"):
        ...
        if kind == "floor":
            ...
            if container_count.get(node.id, 0) > 1:
                ...
                parent_rooms = [
                    edge.from_id
                    for edge in graph.edges
                    if edge.type == "CONTAINS" and edge.to_id == node.id and edge.from_id.startswith("room:")
                ]
                props["surface_role"] = (
                    "exterior_wall"
...
def _emit_surface_connectivity(graph: TopologyGraph) -> None:
    ...
    room_adjacency = {
        ...
        for edge in graph.edges
        if edge.type == "ADJACENT_TO" and edge.from_id.startswith("room:") and edge.to_id.startswith("room:")
    }
    room_surfaces: dict[str, list[GraphNode]] = {}
    for edge in graph.edges:
        if edge.type != "CONTAINS" or not edge.from_id.startswith("room:") or not edge.to_id.startswith("surface:"):
            continue
        ...
        for room_b, surfaces_b in room_surfaces.items():
            ...
            for surface_a in surfaces_a:
                for surface_b in surfaces_b:
                    ...
                    if gap > 0.2:
                        continue
                    for left, right in ((surface_a.id, surface_b.id), (surface_b.id, surface_a.id)):
                        _put_edge(
                            edges,
                            GraphEdge(
                                ...
                                evidence={"bbox_gap_m": round(gap, 6), "junction_type": "BUTT"},
                            ),
                        )
...
def _emit_interference_edges(graph: TopologyGraph) -> None:
    edges = {edge.id: edge for edge in graph.edges}
    rooms = [node for node in graph.nodes if node.type == "Room" and isinstance(node.story, int)]
    ...
    for edge in graph.edges:
        ...
        if edge.type in {"ABOVE", "BELOW"}:
            ...
            if shared_area <= 0.0 and existing_state is not None:
                ...
                continue
            lower_area = float((lower.properties or {}).get("area_m2", 0.0)) if lower else 0.0
            upper_area = float((upper.properties or {}).get("area_m2", 0.0)) if upper else 0.0
            ...
            support_ratio = min(lower_ratio, upper_ratio) if lower_ratio and upper_ratio else max(lower_ratio, upper_ratio)
            derived_support = round(support_ratio, 6)
            derived_state = "confirmed" if support_ratio >= 0.35 else ("partial" if support_ratio >= 0.05 else "weak")
            evidence["support_ratio"] = min(float(existing_support), derived_support) if existing_support is not None else derived_support
            evidence["relation_state"] = _merge_relation_state(str(existing_state), derived_state) if existing_state is not None else derived_state
            ...
            if shared_area <= 0.0 and existing_state is not None:
                edge.evidence = evidence
                continue
            room_area = float((room.properties or {}).get("area_m2", 0.0)) if room else 0.0
            room_height = 0.0
            if room is not None:
            ...
            span_ratio = side_span / max(room_area ** 0.5, 1e-6) if room_area > 1e-6 else 0.0
            derived_support = round(span_ratio, 6)
            derived_state = "confirmed" if span_ratio >= 0.35 else ("partial" if span_ratio >= 0.05 else "weak")
            evidence["support_ratio"] = min(float(existing_support), derived_support) if existing_support is not None else derived_support
            evidence["relation_state"] = _merge_relation_state(str(existing_state), derived_state) if existing_state is not None else derived_state
            ...
            left_height = 0.0
            right_height = 0.0
            left_area = float((left.properties or {}).get("area_m2", 0.0)) if left else 0.0
            right_area = float((right.properties or {}).get("area_m2", 0.0)) if right else 0.0
            if left is not None:
                left_height = max(
            ...
            width = shared_area / max(min(v for v in [left_height, right_height] if v > 1e-6) if any(v > 1e-6 for v in [left_height, right_height]) else 1.0, 1e-6)
            support_ratio = width / max(min(v ** 0.5 for v in [left_area, right_area] if v > 1e-6) if any(v > 1e-6 for v in [left_area, right_area]) else 1.0, 1e-6)
            derived_support = round(support_ratio, 6)
            derived_state = "confirmed" if support_ratio >= 0.35 else ("partial" if support_ratio >= 0.05 else "weak")
            evidence["support_ratio"] = min(float(existing_support), derived_support) if existing_support is not None else derived_support
            evidence["relation_state"] = _merge_relation_state(str(existing_state), derived_state) if existing_state is not None else derived_state
...
def _enrich_with_cells(graph: TopologyGraph, cell_complex: CellComplex) -> None:
    ...
    for cell in cell_complex.cells:
        ...
        if cell.kind == "gap":
            ...
            if source_gap_id is None:
                source_gap_id = f"gap:derived:{_stable_hash([cell.id, str(cell.story), str(cell.bbox_xyz)], 20)}"
                if source_gap_id not in nodes:
                    ...
                    nodes[source_gap_id] = GraphNode(
                        ...
                        source_ids=[cell.id],
                        properties={
                            "gap_kind": (cell.properties or {}).get("gap_kind", "intra_story"),
                    ...
                    if isinstance(cell.story, int):
                        ...
                        if story_node_id:
                            _put_edge(
                                edges,
                                GraphEdge(
                                    id=_edge_id("HAS_GAP", story_node_id, source_gap_id, "derived-cell"),
                                    type="HAS_GAP",
            ...
            if source_node.type == "Room":
                for boundary_edge in graph.edges_from(source_node.id, "CONTAINS"):
                    ...
                    _put_edge(
                        edges,
                        GraphEdge(
                            id=_edge_id("BOUNDED_BY", source_node.id, boundary_edge.to_id),
                            type="BOUNDED_BY",
                            from_id=source_node.id,
    ...
    for adjacency in cell_complex.adjacency:
        ...
        if adjacency.relation == "adjacent_to":
            if a_source and b_source and a_source.startswith("room:") and b_source.startswith("room:"):
                ...
                _put_relation_edge(
                    ...
                    GraphEdge(
                        ...
                        to_id=a.id,
                        source="cell_decomposition",
                        evidence={"shared_face_area": round(adjacency.shared_face_area, 6)},
                    ),
                )
            ...
            elif a_kind == "gap" and b_source and b_source.startswith("room:") and a_gap_source:
                _put_edge(
                    edges,
                    GraphEdge(
            ...
            elif b_kind == "gap" and a_source and a_source.startswith("room:") and b_gap_source:
                _put_edge(
                    edges,
                    GraphEdge(
...
def _apply_story_and_room_properties(graph: TopologyGraph, cell_complex: CellComplex) -> None:
    ...
    for node in graph.nodes:
        if node.type == "Story" and isinstance(node.story, int):
            ...
            if stats:
                ...
                if room_cells:
                    area = sum(
                        Polygon([(float(p[0]), float(p[1])) for p in (cell.properties or {}).get("xz_footprint", [])]).area
                        for cell in room_cells
                        if isinstance((cell.properties or {}).get("xz_footprint"), list)
                        and len((cell.properties or {}).get("xz_footprint")) >= 3
                    )
                    volume = sum(float((cell.properties or {}).get("volume_m3", 0.0)) for cell in room_cells)
    ...
    for room_id, room_node in room_nodes.items():
        room_node.properties["adjacent_room_count"] = len(
            [edge for edge in graph.edges_from(room_id, "ADJACENT_TO") if edge.to_id.startswith("room:")]
        )
