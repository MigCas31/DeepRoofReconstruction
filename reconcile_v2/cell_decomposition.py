...
def _snap_poly_coords(points: list[tuple[float, float]]) -> list[tuple[float, float]]:
    ...
    for x, z in points:
        ...
        if snapped and abs(snapped[-1][0] - sx) <= EPS and abs(snapped[-1][1] - sz) <= EPS:
            continue
        snapped.append((sx, sz))
    if len(snapped) >= 2 and abs(snapped[0][0] - snapped[-1][0]) <= EPS and abs(snapped[0][1] - snapped[-1][1]) <= EPS:
        snapped.pop()
    return snapped
...
def _build_face(cell_id: str, face_kind: str, vertices: list[Vec3]) -> Face3D:
    ...
    if abs(plane[1]) > 1.0 - 1e-3:
        area = _polygon_area_xz(
            [(_from_lattice(_snap_to_lattice(v.x)), _from_lattice(_snap_to_lattice(v.z))) for v in vertices]
        )
    else:
    ...
    fid = f"face:{_stable_hash([cell_id, face_kind, str([v.as_list() for v in vertices])], 20)}"
    return Face3D(
        id=fid,
...
def _face_from_arranged_record(face: dict[str, Any]) -> Face3D | None:
    ...
    return Face3D(
        ...
        source_kind=str(face.get("source_kind")) if face.get("source_kind") is not None else None,
...
def _story_heights(graph) -> tuple[dict[int, float], dict[int, float]]:
    ...
    floor_y = {
        story: min(vals)
        for story, vals in floor_map.items()
        if vals
    }
    ...
    for idx, story in enumerate(sorted_stories[:-1]):
        ...
        if story in floor_y and next_story in floor_y:
            top_y[story] = min(top_y.get(story, floor_y[next_story]), floor_y[next_story])
...
def _room_footprint(graph, node) -> list[tuple[float, float]] | None:
    ...
    if not isinstance(bbox, list) or len(bbox) != 4:
        return None
    min_x, min_z, max_x, max_z = [_from_lattice(_snap_to_lattice(float(v))) for v in bbox]
...
def _cell_from_polygon(
    *,
    ...
    for part_index, subpoly in enumerate(_convex_subpolygons(polygon)):
        ...
        faces = tuple(
            face_obj
            for face_obj in (_face_from_arranged_record(face) for face in (arranged.get("faces") or []))
        ...
        out.append(
            Cell3D(
                ...
                story=story,
                source_id=source_id,
                centroid=Vec3(float(centroid_raw[0]), float(centroid_raw[1]), float(centroid_raw[2])),
...
def _plane_y_at_xz(plane: tuple[float, float, float, float], x: float, z: float, fallback_y: float) -> float:
    nx, ny, nz, d = plane
    if abs(ny) <= EPS:
        return fallback_y
    return _from_lattice(
        _snap_to_lattice((-(nx * x + nz * z + d)) / ny)
    )


def split_planar_faces_exact_on_lattice(face_records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for face in face_records:
        ...
        if len(snapped_corners) < 3:
            continue
        plane = _plane_from_points(snapped_corners[0], snapped_corners[1], snapped_corners[2])
        prepared.append(
            {
                "id": face["id"],
    ...
    geoms = [arrangement] if getattr(arrangement, "geom_type", "") != "MultiLineString" else list(arrangement.geoms)
...
def _layered_story_cells(graph, floor_y: dict[int, float], top_y: dict[int, float]) -> list[Cell3D]:
    cells: list[Cell3D] = []
    ...
    for story, room_entries in story_rooms.items():
        ...
        for idx, atom in enumerate(polygonize(geoms)):
            ...
            touches_envelope = atom.boundary.intersection(envelope.boundary).length > EPS
            if best_gap is not None and best_gap_area > EPS:
                gap_cells = _cell_from_polygon(
                    ...
                    floor_y=fy,
                    ceiling_y=cy,
                    boundary_kind="closure" if (best_gap.properties or {}).get("gap_kind") == "cross_story" else "virtual",
                    properties={"gap_kind": (best_gap.properties or {}).get("gap_kind")},
                )
...
def _gap_cell(node, floor_y: float, ceiling_y: float) -> Cell3D | None:
    ...
    if not poly.is_valid:
        try:
            ...
            return None
    min_x, min_z, max_x, max_z = [_from_lattice(_snap_to_lattice(v)) for v in poly.bounds]
    floor_y = _from_lattice(_snap_to_lattice(floor_y))
    ceiling_y = _from_lattice(_snap_to_lattice(ceiling_y))
    if max_x - min_x <= EPS or max_z - min_z <= EPS or ceiling_y - floor_y <= EPS:
        return None
    ...
    return Cell3D(
        ...
        faces=_prism_faces(
            ...
            floor_y=floor_y,
            ceiling_y=ceiling_y,
            boundary_kind="closure" if (node.properties or {}).get("gap_kind") == "cross_story" else "virtual",
        ),
        properties={
...
def _outside_cell(cells: list[Cell3D]) -> Cell3D | None:
    if not cells:
        return None
    min_x = _from_lattice(min(_snap_to_lattice(c.bbox_xyz[0]) for c in cells) - LATTICE_SCALE_MM)
    min_y = _from_lattice(min(_snap_to_lattice(c.bbox_xyz[1]) for c in cells) - LATTICE_SCALE_MM)
    min_z = _from_lattice(min(_snap_to_lattice(c.bbox_xyz[2]) for c in cells) - LATTICE_SCALE_MM)
    max_x = _from_lattice(max(_snap_to_lattice(c.bbox_xyz[3]) for c in cells) + LATTICE_SCALE_MM)
    max_y = _from_lattice(max(_snap_to_lattice(c.bbox_xyz[4]) for c in cells) + LATTICE_SCALE_MM)
    max_z = _from_lattice(max(_snap_to_lattice(c.bbox_xyz[5]) for c in cells) + LATTICE_SCALE_MM)
    cid = "cell:outside"
    return Cell3D(
        id=cid,
        kind="outside",
        story=None,
        source_id=None,
        centroid=Vec3((min_x + max_x) * 0.5, (min_y + max_y) * 0.5, (min_z + max_z) * 0.5),
...
def _infer_adjacency(cells: list[Cell3D]) -> list[CellAdjacency]:
    ...
    for cell in cells:
        ...
        if isinstance(footprint, list) and len(footprint) >= 3 and cell.kind != "outside":
    ...
    for i, a in enumerate(cells):
        for b in cells[i + 1 :]:
            ...
            if poly_a is not None and poly_b is not None and a.story == b.story and y_ov > EPS:
        ...
        if exposed_len > EPS:
            out.append(
                CellAdjacency(
                    ...
                    relation="adjacent_to",
                    shared_face_area=exposed_len * (cell.bbox_xyz[4] - cell.bbox_xyz[1]),
                    axis="outside",
                )
            )
    return out
...
def build_cell_complex(graph) -> CellComplex:
    ...
    return CellComplex(cells=tuple(cells), adjacency=tuple(adjacency), metadata=metadata)
