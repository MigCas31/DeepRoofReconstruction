...
Reads the existing producer artefacts (buildings_3d.json + roof_algorithms_py_results.json)
...
Audits:
  ...
  2. oblique_split coverage: of buildings with any obliques, fraction with oblique_split[].
  3. Loftrum thermal-cap incidence: count buildings with ceiling.thermal[*].kind == "thermal-cap".
  4. Tier-8 cohort: enumerate buildings the current classifier puts in tier 8 with their
     (n_oblique, n_flat, surface azimuths/inclinations) so we can hand-pick HIP/MANSARD samples.
...
def audit_oblique_split_coverage(roofs: dict) -> dict:
    ...
    return {
        ...
        "split_coverage_pct": (
            round(100 * with_split / max(1, with_obl), 1)
        ),
...
def main() -> int:
    ...
    print(f"  disagree:                       {a1['disagreements']}  ({a1['disagreement_rate_pct']}%)")
    print()
    ...
    print(f"  with obliques + split:          {a2['with_oblique_and_split']}  ({a2['split_coverage_pct']}%)")
    ...
    for k, v in sorted(a3["kind_counts"].items(), key=lambda kv: -kv[1]):
        print(f"    {k:30s} {v:4d}")
    print(f"  thermal-cap buildings:          {a3['thermal_cap_buildings']}  ({a3['thermal_cap_pct']}%)")
    print()
    ...
    if "error" in a4:
        ...
        for t, c in a4["tier_distribution"].items():
            print(f"    tier {t}: {c}")
        print(f"  tier 8 (Other):                {a4['tier8_count']}  ({a4['tier8_pct']}%)")
        print(f"  See {OUTPUT_PATH} for tier8_samples (n_oblique + cluster az/incl per UUID).")
