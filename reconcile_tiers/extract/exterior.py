...
def _canonicalize_wall_quad(corners: list[list[float]]) -> list[list[float]]:
    ...
    def nearest(x: float, z: float, candidates: list[list[float]]) -> list[float]:
        return min(candidates, key=lambda corner: (corner[0] - x) ** 2 + (corner[2] - z) ** 2)

    return [list(bl), list(br), list(nearest(br[0], br[2], top)), list(nearest(bl[0], bl[2], top))]
    ...
    def consolidate(profile, take_max: bool) -> list[tuple[float, float]]:
        ...
        for t, y in sorted(profile, key=lambda point: point[0]):
            if out and abs(out[-1][0] - t) < 1e-6:
                out[-1] = (out[-1][0], max(out[-1][1], y) if take_max else min(out[-1][1], y))
            else:
                out.append((t, y))
        return out

    if sum(point[1] for point in forward) / len(forward) >= sum(point[1] for point in backward) / len(backward):
    ...
    *,
    ...
    for sign in (1.0, -1.0):
        ...
        for wall, wall_room_idx in story_walls.get(story, []):
            ...
            if dist_along < 0.20 or dist_along > 1.0:
                continue
            ...
            if wall_normal is None:
                continue
            angle_deg = math.degrees(math.acos(min(abs(float(np.dot(normal_xz, wall_normal))), 1.0)))
            if angle_deg > 15.0:
                continue
...
def detect_exterior_gap_indicators(rooms: list[ExtractedRoom]) -> list[ExteriorGapIndicator]:
    ...
    for room_idx, room in enumerate(rooms):
        for element_type, elements in (("door", room.doors), ("opening", room.openings)):
        ...
        for storage in room.storages:
            ...
            for wall, _wall_room_idx in story_walls.get(room.story, []):
                ...
                if edge_len < 1e-6:
                    continue
                perp_dist = abs(float(edge[0] * (storage_xz - w0)[1] - edge[1] * (storage_xz - w0)[0])) / edge_len
                if perp_dist < 0.05:
                    flush = True
                    break
            if not flush:
...
def compute_gap_closures(indicators: list[ExteriorGapIndicator]) -> list[GapClosure]:
    ...
    for indicator in indicators:
        if len(indicator.wall_corners) < 4 or len(indicator.element_corners) < 4:
            continue
        wall_corners = np.asarray(_canonicalize_wall_quad(indicator.wall_corners), dtype=float)
        parent_corners = np.asarray(_canonicalize_wall_quad(indicator.element_corners), dtype=float)
        ...
        def project(points):
            return [
                float(np.dot(np.array([point[0], point[2]]) - np.array([origin[0], origin[2]]), axis2d))
                for point in points
            ]
        ...
        parent_raw = _canonicalize_wall_quad(indicator.element_corners)
        wall_raw = _canonicalize_wall_quad(indicator.wall_corners)
        origin_xz = (float(origin[0]), float(origin[2]))
        parent_top, parent_bottom = _wall_top_bottom_profiles(parent_raw, axis2d, origin_xz)
        ...
        for t_val in (overlap_min, overlap_max):
            ...
            side_pts.append((parent_xz, parent_ybot, parent_ytop, wall_xz, wall_ybot, wall_ytop))
            closures.append(
                GapClosure(
                    corners=[
