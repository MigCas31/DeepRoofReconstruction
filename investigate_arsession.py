...
def analyze_building(uuid, name, is_control=False):
    ...
    for fl in floors:
        ...
        for r in rooms:
            ...
            if r_created:
                ...
                all_room_timestamps.append((r_created, r.get("name", "?"), fl_type, fl_id, r.get("id")))
    ...
    if len(floors) >= 2:
        floor_created = [(fl.get("created"), fl.get("floorType")) for fl in floors if fl.get("created")]
        floor_created.sort()
        inter_gaps = []
    ...
    for story, rooms in room_ref_origins_by_story.items():
        ...
        # Compare all pairs
        transforms = [r["referenceOriginTransform"] for r in rooms if r["referenceOriginTransform"]]
        if len(transforms) < 2:
            continue
...
def print_report(results):
    ...
    print("SUMMARY TABLE")
    print("=" * 100)
    header = f"{'Building':<30} {'Type':<8} {'Floors':>6} {'Rooms':>6} {'Gaps>5m':>8} {'RefOrigin Var':>14} {'AzJumps':>8} {'WorldMap':>20}"
    print(header)
    print("-" * 100)
    ...
    for r in results:
        ...
        max_trans = max((v.get("max_translation_diff_m", 0) for v in ref_var.values()), default=0)
        ...
        print(f"{r['name']:<30} {label:<8} {num_floors:>6} {total_rooms:>6} {gaps:>8} {ref_str:>14} {az_jumps:>8} {wm_str:>20}")
    ...
    for r in results:
        ...
        for fl in r.get("floors", []):
            print(f"    {fl['type']:>10} (id={fl['id'][:20]}...): {fl['num_rooms']} rooms, created={fl.get('created')}")
            if "max_intra_floor_gap_sec" in fl:
                print(f"      Intra-floor room gaps: min={fl['min_intra_floor_gap_sec']:.0f}s, max={fl['max_intra_floor_gap_sec']:.0f}s ({fl['max_intra_floor_gap_sec']/60:.1f}min)")
    ...
    print(f"\n  Metric                    | Targets (n={len(targets)})        | Controls (n={len(controls)})")
    print(f"  {'':->28}|{'':->30}|{'':->30}")
    if t_rv:
        print(f"  Max refOrigin trans diff  | mean={np.mean(t_rv):.6f}m max={np.max(t_rv):.6f}m | mean={np.mean(c_rv):.6f}m max={np.max(c_rv):.6f}m")
    if t_gc:
        print(f"  Timestamp gaps > 5min     | mean={np.mean(t_gc):.1f}  max={np.max(t_gc)}              | mean={np.mean(c_gc):.1f}  max={np.max(c_gc)}")
    if t_az:
        print(f"  Azimuth transform jumps   | mean={np.mean(t_az):.1f}  max={np.max(t_az)}              | mean={np.mean(c_az):.1f}  max={np.max(c_az)}")
