...
the orientation of the ARSession's world coordinate frame relative to true north/gravity.
...
def pick_controls(exclude, n=10):
    ...
    for d in sorted(CACHE.iterdir()):
        ...
        if uuid and uuid not in exclude:
            if (d / "data.json").exists() and (PIPELINE / uuid / "merged.json").exists():
    ...
    for fl in floors:
        ...
        for i in range(len(room_list)):
            for j in range(i+1, len(room_list)):
                angle = rotation_angle_between(room_list[i]["rotation"], room_list[j]["rotation"])
        ...
        session_groups = []
        assigned = set()
        for i, rm in enumerate(room_list):
            if i in assigned:
                continue
            group = [i]
            assigned.add(i)
        ...
        for g in session_groups:
            timestamps = [room_list[idx]["created"] for idx in g if room_list[idx].get("created")]
            if timestamps:
...
def main():
    ...
    print("=" * 120)
    print()
    print("METHODOLOGY: For rooms scanned in the same ARSession, the rotation component of")
    print("referenceOriginTransform must be (nearly) identical, because it encodes the fixed")
    print("relationship between the room's local axes and the ARSession's world axes.")
    print("Translation differs per room (different physical locations), but rotation should not.")
    print("If rotation differs > 1 degree between rooms on the same floor, it indicates a")
    ...
    # === Summary Table ===
    print("=" * 120)
    print(f"{'Building':<35} {'Type':<7} {'Flrs':>4} {'MaxRotDiff':>10} {'#SessGroups':>11} {'Restart?':>8}  Session details")
    print("-" * 120)
    ...
    for r in target_results + control_results:
        ...
        print(f"{r['name']:<35} {label:<7} {total_floors:>4} {maxrot:>10} {total_groups:>11} {restart:>8}  {detail_str}")
    ...
    for r in target_results + control_results:
        ...
        for fa in r["floor_analyses"]:
            restart_flag = " *** SESSION RESTART ***" if fa["has_session_restart"] else ""
            print(f"\n  Floor: {fa['floor_type']:<10} | {fa['num_rooms_analyzed']} rooms | "
            ...
            for sg in fa["session_groups"]:
                ...
                print(f"    Session Group {sg['group_id']}: {sg['num_rooms']} rooms, "
            ...
            if fa["num_session_groups"] > 1:
                groups_by_time = sorted(fa["session_groups"], key=lambda x: x["time_start"] or 0)
                for i in range(1, len(groups_by_time)):
                    gap = (groups_by_time[i]["time_start"] or 0) - (groups_by_time[i-1]["time_end"] or 0)
                    yaw_diff = abs(groups_by_time[i]["avg_yaw_deg"] - groups_by_time[i-1]["avg_yaw_deg"])
                    print(f"    >>> GAP between Group {groups_by_time[i-1]['group_id']} -> "
        ...
        for story, room_data in sorted(by_story.items()):
            ...
            for i in range(len(room_data)):
                for j in range(i+1, len(room_data)):
                    angle = rotation_angle_between(room_data[i]["rotation"], room_data[j]["rotation"])
    ...
    if t_restart_count > 0:
        ...
        for r in target_results:
            if r["has_session_restart_evidence"]:
                print(f"    - {r['name']}: floors with restart = {r['session_restart_floors']}, "
    ...
    print("  re-aligning rooms. However, if the original scan data had coordinate frame")
