...
def analyze_building(b3d, batch_entry):
    ...
    if dists:
        ...
        outlier_rooms = [r for r in room_analyses if r["dist_from_centroid_2d"] > 2 * median_dist and median_dist > 0.5]
    ...
    for s, yvs in sorted(story_ys.items()):
        story_ranges[s] = {"min_y": min(yvs), "max_y": max(yvs), "range_y": max(yvs) - min(yvs)}
    ...
    for room in rooms:
        for wall in room.get("walls_merged", []):
            ...
            if d > info["max_room_dist"] * 1.5 and d > 10:
                far_walls.append({"id": wall.get("id", "?"), "dist": d, "story": room.get("story")})
    ...
    for i_r, room in enumerate(rooms):
        ...
        if fp:
            ...
            floor_sigs.append((i_r, room.get("story"), round(c[0], 1), round(c[1], 1), round(c[2], 1), len(fp)))

    duplicates = []
    for i_a in range(len(floor_sigs)):
        for i_b in range(i_a+1, len(floor_sigs)):
            a, b = floor_sigs[i_a], floor_sigs[i_b]
            if abs(a[2]-b[2]) < 0.5 and abs(a[3]-b[3]) < 0.5 and abs(a[4]-b[4]) < 0.5:
...
def print_building_report(uuid, info, label=""):
    ...
    print(f"  Classification: {info.get('classification', '?')}  |  Flags: {', '.join(info.get('flags', [])) or 'none'}")
    print(f"{'='*90}")
    ...
    print(f"  Stories: {info.get('stories_found')} found, {info.get('stories_changed')} changed")
    print(f"  Rooms: {info.get('room_count')}  |  Cross-floor gaps: {info.get('cross_floor_gaps')}")
    print(f"  Walls: {info.get('merged_walls_total')} merged / {info.get('computed_walls_total')} computed")

    if info.get("match_rate") is not None:
        print(f"  Match rate: {fmt(info['match_rate'])}  |  Wall reduction: {fmt(info.get('wall_reduction_pct'))}%")
        print(f"  Mean width delta: {fmt(info.get('mean_width_delta_cm'))} cm  |  Mean height delta: {fmt(info.get('mean_height_delta_cm'))} cm")
    ...
    for r in info.get("outlier_rooms", []):
        print(f"    Room {r['room_idx']} (story {r['story']}): dist={fmt(r['dist_from_centroid_2d'])} m, extent={fmt(r['room_extent'])} m")

    if info.get("far_walls_count", 0) > 0:
        print(f"  *** FAR WALLS (>1.5x max room dist & >10m): {info['far_walls_count']}")

    if info.get("duplicate_rooms"):
        print(f"  *** POTENTIALLY OVERLAPPING ROOMS: {len(info['duplicate_rooms'])} pairs")
...
def main():
    ...
    green_yellow = [u for u, b in batch_map.items()
                    if b.get("classification") in ("GREEN", "YELLOW")
                    and u not in PROBLEM_UUIDS]
    ...
    print("="*130)
    header = f"{'Address':<30} {'Class':>6} {'Rooms':>5} {'Stories':>7} {'ExtentX':>8} {'ExtentY':>8} {'ExtentZ':>8} {'MaxRmDist':>9} {'Outliers':>8} {'Overlaps':>8} {'FarWalls':>8} {'Flags':>5}"
    print(header)
    print("-"*130)
    ...
    def print_summary_row(uuid, info, tag=""):
        ...
        print(f"{addr:<30} {cls:>6} {rooms:>5} {stories:>7} {ex:>8} {ey:>8} {ez:>8} {mrd:>9} {outl:>8} {ovlp:>8} {fw:>8} {fl:>5}")
    ...
    print("="*80)
    for label, results in [("PROBLEM", problem_results), ("COMPARISON", comparison_results)]:
        ...
        max_dists = [r["max_room_dist"] for r in results.values() if "max_room_dist" in r]
        outliers = [r["outlier_rooms_count"] for r in results.values() if "outlier_rooms_count" in r]
        overlaps = [len(r.get("duplicate_rooms", [])) for r in results.values()]

        print(f"\n  {label} (n={len(results)}):")
        if extents_x:
            print(f"    Extent X:  mean={sum(extents_x)/len(extents_x):.2f}  max={max(extents_x):.2f}")
            print(f"    Extent Y:  mean={sum(extents_y)/len(extents_y):.2f}  max={max(extents_y):.2f}")
            print(f"    Extent Z:  mean={sum(extents_z)/len(extents_z):.2f}  max={max(extents_z):.2f}")
            print(f"    MaxRmDist: mean={sum(max_dists)/len(max_dists):.2f}  max={max(max_dists):.2f}")
