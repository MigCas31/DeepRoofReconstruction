...
def _summary(plane_rows: list[dict], bldg_rows: list[dict]) -> dict:
    ...
    return {
        ...
        "buildings_with_most_oblique": [
            {"uuid": r["uuid"], "n_oblique": r["n_oblique_ge_5deg"], "n_planes": r["n_planes"]}
            for r in sorted(bldg_rows, key=lambda b: b["n_oblique_ge_5deg"], reverse=True)[:20]
        ],
    }
...
def main() -> None:
    ...
    print(f"Wrote {len(plane_rows)} plane rows across {summary['n_buildings_with_any_raw']}"
          f"/{summary['n_buildings']} buildings.")
    print(f"Oblique (>={OBLIQUE_MIN_INCL_DEG} deg): {summary['n_oblique_ge_5deg']}"
