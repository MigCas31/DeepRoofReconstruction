...
def main() -> int:
    ...
    ap.add_argument("--building", type=str, default=None,
                    help="Solve only this building UUID.")
    ...
    if args.building:
        ...
        if not corpus:
            print(f"Building {args.building} not found in {args.candidates}",
                  file=sys.stderr)
            return 2
    if args.limit:
    ...
    for i, b in enumerate(corpus):
        ...
        res = solve_building(uuid, candidates, fp, config=cfg)
    ...
    md = [
        ...
        "",
        f"Corpus: `{args.candidates}` — {total} buildings "
    ...
    print(f"{solved}/{total} solved, {auto_accept}/{total} auto-accept, "
