...
def run_clipping_with_threshold(bldg, ceiling_planes, building_footprint,
                                 exposed_rooms, top_story, all_stories,
                                 floors_by_story, story_floor_y, use_new):
...
def process_building(uuid, pipeline_dir, scan_cache_root):
    ...
    old_surfaces = run_clipping_with_threshold(
        bldg, ceiling_planes, building_footprint, exposed_rooms,
        footprint_info["top_story"], story_index["all_stories"],
        story_index["floors_by_story"], story_stats["story_floor_y"],
        use_new=False,
    )
    ...
    new_surfaces = run_clipping_with_threshold(
        bldg, ceiling_planes_new, building_footprint, exposed_rooms,
        footprint_info["top_story"], story_index["all_stories"],
        story_index["floors_by_story"], story_stats["story_floor_y"],
        use_new=True,
    )
...
def main():
    ...
    for i, uuid in enumerate(uuids):
        ...
        if (i + 1) % 25 == 0:
            print(f"  [{i+1}/{len(uuids)}] processed, {len(changed)} changed so far...")

    # Summary
    print(f"\n{'='*80}")
    print(f"RESULTS: {total} buildings processed, {skipped} skipped, {len(errors)} errors")
    print(f"{'='*80}\n")
    ...
    if not changed:
        ...
        # Categorize
        surfaces_lost = [c for c in changed if c["new_n_surfaces"] < c["old_n_surfaces"]]
        surfaces_same = [c for c in changed if c["new_n_surfaces"] == c["old_n_surfaces"]]
        ...
        print(f"  Surfaces same count but different area: {len(surfaces_same)} buildings")
        ...
        for c in changed:
            ...
            print(f"  {c['uuid']}:{flag}")
            print(f"    planes={c['n_planes']}  surfaces: {c['old_n_surfaces']}→{c['new_n_surfaces']}")
            print(f"    area: {c['old_total_area']}→{c['new_total_area']} (diff={c['area_diff']})")
            print(f"    overlap: {c['old_overlap']}→{c['new_overlap']} (diff={c['overlap_diff']})")
