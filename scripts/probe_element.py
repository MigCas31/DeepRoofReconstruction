"""Quick geometric probes for a single element ID — stick-out, neighbors, missing-at-point.
...
def check_flatness(element: dict, roof_results_for_uuid: dict) -> dict:
    ...
    if (
        ...
        if stored_role == "flat" and nearest["roof_kind"] == "oblique" and roof_incl > 10:
            cross_checks.append(
                f"nearest overhead roof is oblique @ {roof_incl}° "
                f"(xz overlap {nearest['xz_overlap_m2']} m², dy={nearest['vertical_delta_m']}m)"
                " — corroborates sloped"
            )
...
def compute_stickout(
    ...
    max_overhang = max(
        (v["distance_to_boundary_m"] for v in vertex_overhangs if v["outside_footprint"]),
        default=0.0,
    )
...
def _atom_records(roof_results_for_uuid: dict) -> list[dict]:
    ...
    for subkind in ("oblique", "flat"):
        for idx, item in enumerate(rs.get(subkind, []) or []):
            records.append(
                {
                    "id": item.get("boundary_face_id") or f"roof_surfaces.{subkind}[{idx}]",
...
def _summarize_human(result: dict) -> str:
    ...
    if mode == "element":
        ...
        lines.append(f"kind={elem.get('kind')}  source_id={elem.get('source_id', elem.get('id'))}")
        ...
        if fl:
            ...
            for o in (fl.get("overhead_roof_surfaces") or [])[:3]:
                lines.append(
                    f"  overhead: {o['roof_kind']} roof incl={o['roof_inclination_deg']}° "
        ...
        if lv.get("available"):
            lines.append(
                ...
                f"elem_y={lv.get('element_y_mean')} (range {lv.get('element_y_range')})  "
                f"room={lv.get('room_index')} story={lv.get('story')}"
            )
            if lv.get("sibling_count"):
                lines.append(
                    f"  siblings: n={lv['sibling_count']}  "
                    f"y_mean={lv.get('sibling_y_mean')} range={lv.get('sibling_y_range')}  "
        ...
        for r in n[:8]:
            lines.append(
                f"  - {r['id']}  d={r['centroid_distance_m']}m  dy={r['vertical_delta_m']}m  "
                f"source={r['source']}  same_room={r['same_room']}  same_hyp={r['same_hypothesis']}"
            )
    else:
        pt = result.get("point", {})
        lines.append(f"point=({pt.get('x')}, {pt.get('y')}, {pt.get('z')})  radius_m={result.get('radius_m')}")
        lines.append(f"containing={len(result.get('containing', []))}  nearby={len(result.get('nearby', []))}")
        for r in result.get("containing", [])[:8]:
            lines.append(
                f"  [contains] {r['id']}  dy={r['vertical_delta_m']}m  source={r['source']}"
            )
        for r in result.get("nearby", [])[:8]:
            lines.append(
                f"  [nearby]   {r['id']}  d={r['centroid_distance_m']}m  dy={r['vertical_delta_m']}m  source={r['source']}"
            )
...
def _resolve_element(
    ...
    if is_ontology_kind(parsed.kind):
        resolution = find_element([], token, roof_results=roof_results)
        atom = resolution.get("atom") or {}
        corners = atom.get("poly") or atom.get("corners") or []
    ...
    if buildings_path is None or not buildings_path.exists():
        raise SystemExit(
            f"Legacy kind '{parsed.kind}' needs --buildings-json; {buildings_path} not found"
        )
    buildings = json.loads(buildings_path.read_text())
    resolution = find_element(buildings, token)
...
def _cli() -> int:
    ...
    if args.element_id:
        ...
        element = _resolve_element(args.element_id, roof_results_all, args.buildings_json)
        ...
        try:
            ...
            raise SystemExit(f"--point must be 'x,y,z' (got: {args.point!r})")
