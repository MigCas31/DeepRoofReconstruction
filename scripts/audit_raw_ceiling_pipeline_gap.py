...
class RawPlane:
    ...
    def element_id(self) -> str:
        return f"{self.uuid}::ceiling-raw::{self.story}:{self.room_index}:{self.plane_index}"
...
def _collect_raw_planes(building: dict, exposed_keys: set[tuple[int, int]]) -> list[RawPlane]:
    uuid = building["uuid"]
    out: list[RawPlane] = []
        ...
        out.append(
            {
                ...
                "raw_vs_pipeline_agrees": (
                    majority == pipeline_class if pipeline_class in {"oblique", "flat"} else False
                ),
...
def _summary(plane_rows: list[dict], room_rows: list[dict]) -> dict:
    ...
    top_uncovered_oblique = sorted(
        [r for r in plane_rows if r["classification"] == "uncovered" and float(r["inclination_deg"]) >= OBLIQUE_MIN_INCL_DEG],
        key=lambda r: float(r["area_xz_m2"]), reverse=True,
    )[:20]
    ...
    def _slim(row: dict) -> dict:
        return {
            k: row[k]
            for k in ("element_id", "inclination_deg", "azimuth_deg", "area_xz_m2",
                      "coverage_oblique", "coverage_flat", "best_oblique_id",
                      "best_flat_id", "normal_dot_best_oblique", "classification")
        }
...
def main() -> None:
    ...
    print(f"Scored {len(plane_rows)} raw planes across {summary['n_rooms']} exposed rooms")
    print("Classification counts:", summary["classification_counts"])
    print("Raw-vs-pipeline per-room agreement:", summary["raw_vs_pipeline_class_counts"])
