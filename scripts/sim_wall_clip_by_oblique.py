...
def main():
    ...
    symptom = [
        r for r in all_records if (r["overhang_m"] or 0) > COHORT_TOL_M
    ]
    ...
    near_clip = [
        ...
        if r["has_overhead_oblique"]
        and 0 < (r["overhang_m"] or 0) <= COHORT_TOL_M
    ]
    below_plane = [
        r
        for r in all_records
    ...
    print(f"  └ overhang > {COHORT_TOL_M:.2f}m (symptom):   {len(symptom)}  ← fix REPAIRS")
    print(f"  └ 0 < overhang ≤ {COHORT_TOL_M:.2f}m (minor):  {len(near_clip)}  ← fix trims <{COHORT_TOL_M*100:.0f}cm (borderline)")
    print(f"  └ overhang ≤ 0 (wall below slope):  {len(below_plane)}  ← fix NO-OP")
    print(f"walls with no overhead oblique:       {total_walls - walls_with_overhead}  ← fix NEUTRAL")
    ...
    print(f"unchanged  (not under oblique):                           {total_walls - walls_with_overhead}")
    print(f"no-op      (wall below slope already):                    {len(below_plane)}")
    print(f"borderline (tiny trim ≤{COHORT_TOL_M:.2f}m; potential micro-regression): {len(near_clip)}")
