...
def _footprint_lines(roof_result: dict):
    ...
    n = len(fp)
    for i in range(n):
        ax, az = float(fp[i][0]), float(fp[i][2]) if len(fp[i]) >= 3 else float(fp[i][1])
        bx, bz = float(fp[(i + 1) % n][0]), float(fp[(i + 1) % n][2]) if len(fp[(i + 1) % n]) >= 3 else float(fp[(i + 1) % n][1])
...
def _collect_edges(building, roof_result):
    ...
    for (story, ri, pi, corners, normal, incl) in planes:
        ...
        for j in range(n):
            ...
            edge_records.append((story, ri, pi, a, b, dy, horizontal, normal, incl, length))
    ...
    for story, recs in by_story.items():
        for i, rec in enumerate(recs):
            ...
            for j in range(len(recs)):
                if i == j:
                    continue
                (_, ri2, pi2, a2, b2, dy2, horiz2, normal2, incl2, len2) = recs[j]
            ...
            if match_partner is not None:
                label = "ridge_or_hip"
                partner_id = (
                    f"{building['uuid']}::ceiling-raw::{story}:{match_partner[0]}:{match_partner[1]}"
                )
            ...
            edge_rows.append(
                {
                    "uuid": building["uuid"],
                    "plane_element_id": f"{building['uuid']}::ceiling-raw::{story}:{ri}:{pi}",
                    "story": story,
                    "room_index": ri,
                    "plane_index": pi,
...
def main() -> None:
    ...
    for building in buildings:
        ...
        n_oblique_clusters = sum(
            1 for cl in roof_result.get("valid_clusters") or [] if cl.get("avgIncl", 0) > 0
        )
        n_oblique_surfaces = len((roof_result.get("roof_surfaces") or {}).get("oblique") or [])
        bldg_rows.append(
            {
                "uuid": uuid,
    ...
    top_under_clustered = sorted(bldg_rows, key=lambda r: r["ridge_minus_clusters"], reverse=True)[:15]
    ...
    summary = {
        ...
        "top_under_clustered": [
            {k: r[k] for k in ("uuid", "ridge_or_hip_edges", "pipeline_oblique_clusters", "ridge_minus_clusters")}
    ...
    print(f"Edges total: {corpus['total_edges']}  horizontal: {corpus['horizontal_edges']}"
          f"  sloped: {corpus['sloped_edges']}")
    print(f"Horizontal labels — ridge_or_hip: {corpus['ridge_or_hip']}"
