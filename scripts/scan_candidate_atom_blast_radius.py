...
def main() -> int:
    ...
    for idx, uuid in enumerate(uuids, 1):
        ...
        candidates = (
            role_counts.get("attic_floor_candidate", 0)
            + role_counts.get("flat_transition_cap_candidate", 0)
        )
    ...
    print()
    print(f"Buildings with candidate atoms:                  {len(with_cand)} / {len(ok)}")
    print(f"Buildings emitting atom-level unresolved surfs:  {len(with_unres)} / {len(ok)}")
    print()
