...
def _build_rows():
    ...
    for bldg in buildings:
        ...
        for ri, room in enumerate(rooms):
            ...
            n_disagree = sum(1 for r in plane_rows if r["classification"] in DISAGREE_CLASSES)
            ...
            rows.append(
                {
                    ...
                    "disagreement_rate": round(n_disagree / n_planes, 3) if n_planes else 0.0,
...
def _analyze(rows):
    ...
    report = {
        "corpus": {"n_planes": corpus[1], "n_disagree": corpus[0], "rate": round(corpus[2], 4)},
        "bool_features": {},
        "numeric_features": {},
        "cross_features": {},
    }
    ...
    for feat in bool_feats:
        ...
        report["bool_features"][feat] = {
            "true": {"rooms": len(pos), "planes": np_, "disagree": dp, "rate": round(rp, 4)},
            "false": {"rooms": len(neg), "planes": nn, "disagree": dn, "rate": round(rn, 4)},
            "relative_risk": round(rp / rn, 2) if rn else None,
        }

    num_specs = {
        "exterior_edge_frac": ([0, 0.2, 0.5, 0.8, 1.0001], ["<0.2_interior", "0.2-0.5", "0.5-0.8", "0.8-1.0_exterior"]),
        "floor_area_m2": ([0, 5, 10, 20, 40, 99999], ["<5", "5-10", "10-20", "20-40", "40+"]),
        "perimeter_m": ([0, 10, 15, 25, 40, 9999], ["<10", "10-15", "15-25", "25-40", "40+"]),
        "max_floor_dim_m": ([0, 3, 5, 8, 12, 9999], ["<3", "3-5", "5-8", "8-12", "12+"]),
        "aspect_ratio": ([1, 1.3, 2, 3, 999], ["<1.3_square", "1.3-2", "2-3", "3+_elongated"]),
    ...
    for feat, (edges, labels) in num_specs.items():
        ...
        for label in labels:
            ...
            out[label] = {"rooms": len(bucket), "planes": n, "disagree": d, "rate": round(rate, 4)}
    ...
    for key, bucket in sorted(cross2.items()):
        d, n, rate = _weighted_rate(bucket)
        cross2_out[key] = {"rooms": len(bucket), "planes": n, "disagree": d, "rate": round(rate, 4)}
...
def _print(report):
    ...
    for feat, d in report["bool_features"].items():
        t, f = d["true"], d["false"]
        print(
            f"  {feat:<20} true: rooms={t['rooms']:>4} planes={t['planes']:>4} rate={t['rate']:.3f}   "
            f"false: rooms={f['rooms']:>4} planes={f['planes']:>4} rate={f['rate']:.3f}   RR={d['relative_risk']}"
        )
    print("\nNumeric features:")
    for feat, d in sorted(report["numeric_features"].items(), key=lambda kv: -kv[1].get("_spread", 0)):
        print(f"  {feat} (spread={d.get('_spread', 0):.3f}):")
        for label, info in d.items():
            if label.startswith("_"):
                continue
            print(f"    {label:<24} rooms={info['rooms']:>4} planes={info['planes']:>4} rate={info['rate']:.3f}")
    print("\nCross features:")
    for name, buckets in report["cross_features"].items():
        print(f"  {name}:")
        for key, info in sorted(buckets.items()):
            print(f"    {key:<30} rooms={info['rooms']:>4} planes={info['planes']:>4} rate={info['rate']:.3f}")
