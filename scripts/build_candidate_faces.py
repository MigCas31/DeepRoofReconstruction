...
def _scan_footprint_for(uuid: str, pipeline_dir: Path, scan_cache: Path):
    """Return the scan-derived footprint ring (list of (x, z)) or ``None``."""
    ...
    return info.get("building_footprint")


def main() -> int:
    ap = argparse.ArgumentParser()
    ap.add_argument("--results", default="reconcile/reconcile_v3_results.json", type=Path)
    ...
    for i, building in enumerate(corpus):
        ...
        try:
            fp = _scan_footprint_for(uuid, args.pipeline_dir, args.scan_cache)
        except Exception as exc:
            errors.append((uuid, f"footprint: {type(exc).__name__}: {exc}"))
            fp = None

        try:
            faces = build_candidate_faces(uuid, merged, fp)
        except Exception as exc:
    ...
    print(f"{n_total_candidates} candidate faces across "
