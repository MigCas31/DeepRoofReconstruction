...
def _obb_aspect_ratio(coords) -> float:
    try:
        ...
        if len(pts) < 4:
            return float("nan")
        sides = [
            math.dist(pts[i], pts[(i + 1) % len(pts)])
...
def _compute_metrics(uuid: str, bldg: dict) -> tuple[FootprintMetrics, dict]:
    ...
    if not exposed_rooms:
        m = FootprintMetrics(
            uuid=uuid, status="no_exposed_rooms", n_exposed_rooms=0,
            n_vertices=0, area_m2=0.0, is_valid=False, is_simple=False,
            hull_area_m2=0.0, convexity=float("nan"),
            obb_aspect_ratio=float("nan"),
            unbuffered_union_area_m2=0.0, buffer_bridged_ratio=float("nan"),
    ...
    if not footprint or len(footprint) < 3:
        m = FootprintMetrics(
            uuid=uuid, status="no_footprint", n_exposed_rooms=len(exposed_rooms),
            n_vertices=0, area_m2=0.0, is_valid=False, is_simple=False,
            hull_area_m2=0.0, convexity=float("nan"),
            obb_aspect_ratio=float("nan"),
    ...
    fell_back = (
        abs(convexity - 1.0) < 1e-3
        and unbuffered <= 1e-6
    )

    metrics = FootprintMetrics(
        uuid=uuid, status="ok", n_exposed_rooms=len(exposed_rooms),
        n_vertices=len(footprint), area_m2=area,
        is_valid=bool(poly.is_valid), is_simple=bool(poly.is_simple),
        hull_area_m2=hull_area, convexity=convexity,
...
def main() -> int:
    ...
    for i, uuid in enumerate(uuids):
        try:
            ...
            if not bldg:
                metrics = FootprintMetrics(
                    uuid=uuid, status="error", n_exposed_rooms=0,
                    n_vertices=0, area_m2=0.0, is_valid=False, is_simple=False,
                    hull_area_m2=0.0, convexity=float("nan"),
                    obb_aspect_ratio=float("nan"),
                    unbuffered_union_area_m2=0.0,
                    buffer_bridged_ratio=float("nan"),
                    fell_back_to_hull=False, error="extract_building returned None",
                )
    ...
    # Rank
    scored = sorted(
        results, key=lambda pair: _suspicion_score(pair[0]), reverse=True
    )

    # Write per-building metrics
    metrics_path = args.out_dir / "metrics.json"
    metrics_path.write_text(json.dumps(
        [asdict(m) for m, _ in results], indent=2, default=lambda x: None
    ))
