...
from reconcile_v3.analysis import (  # noqa: E402
...
def main() -> int:
    ...
    ap.add_argument(
        ...
        help="Reference full-corpus feature parquet used to align OOF predictions by proposal id.",
    )
    ...
    if not args.skip_advanced:
        ...
        print(
            f"=== Wall index — {n_walls} walls across {len(wall_idx)} buildings ==="
        )
        ...
        for row, rec in zip(rows, records):
            row.update(
                advf.advanced_features(
                    ...
                    building_context=ctx_index.get(row["building_uuid"]) if ctx_index else None,
                )
            )
    ...
    print(
        f"=== Phase 2 — feature expansion: {len(rows)} rows × "
    ...
    print(f"wrote {feat_out} ({len(feat_df)} rows × {len(feat_df.columns)} cols)")
