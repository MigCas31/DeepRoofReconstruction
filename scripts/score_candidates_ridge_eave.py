...
def _plane_eave_y_clusters(plane: list[float],
...
def _resolve_eave_pair(clusters_a: list[tuple[float, int]],
                       clusters_b: list[tuple[float, int]]) -> tuple[float, float] | None:
...
def _plane_group_eave_segment(points_xz: np.ndarray, downslope_xz: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
...
def _clip_line_to_obb(pt3: np.ndarray, dir3: np.ndarray, obb: dict) -> tuple[np.ndarray, np.ndarray] | None:
...
def _build_plane_groups(
    ...
    for key, entries in buckets.items():
        ...
        try:
            buffered = unary_union([p.buffer(PLANE_GROUP_CONNECT_BUFFER_M) for p in member_polys])
        except Exception:
            continue
        ...
        for comp_idx, comp in enumerate(components):
            ...
            groups.append({
                ...
                "max_support": round(max(float(m.get("support_m2", 0.0) or 0.0) for m in comp_members), 3),
...
def _ridge_segment_on_scan(plane_a: list[float], plane_b: list[float],
                           scan_poly: ShapelyPolygon) -> tuple[np.ndarray, np.ndarray, np.ndarray] | None:
...
def _split_scan_by_ridge(scan_poly: ShapelyPolygon,
                         ridge_xz_a: np.ndarray, ridge_xz_b: np.ndarray) -> list[ShapelyPolygon]:
    ...
    try:
        ...
        return [scan_poly]
    pieces = [g for g in result.geoms if g.area > 1e-6] if hasattr(result, "geoms") else [result]
    return pieces


def _assign_sides(pieces: list[ShapelyPolygon], ridge_mid_xz: np.ndarray,
                  ds_a: np.ndarray, ds_b: np.ndarray) -> tuple[ShapelyPolygon | None, ShapelyPolygon | None]:
...
def _eave_geometry(piece: ShapelyPolygon, ridge_line: ShapelyLineString,
                   tol: float) -> tuple[float, ShapelyLineString | None]:
...
def _score_plane_pair(pg_a: dict,
                      pg_b: dict,
                      scan_poly: ShapelyPolygon | None,
                      wall_tops: np.ndarray | None = None) -> dict | None:
    ...
    if pair_domain.is_empty:
        return None
    clip_domain = scan_poly if (scan_poly is not None and not scan_poly.is_empty
                                and scan_poly.intersects(pair_domain)) else pair_domain
    ...
    if eave_a_y is None or eave_b_y is None:
        ...
        eave_height_parity = math.exp(-(eave_diff_m / EAVE_HEIGHT_PARITY_SIGMA_M) ** 2)
    ...
    side_a, side_b = _assign_sides(sig_pieces, ridge_mid_xz, ds_a, ds_b) if sig_pieces else (None, None)
    ...
    eave_a_len, eave_a_geom = _eave_geometry(side_a, ridge_line_xz, eave_tol) if side_a else (0.0, None)
    eave_b_len, eave_b_geom = _eave_geometry(side_b, ridge_line_xz, eave_tol) if side_b else (0.0, None)
    ...
    return {
        ...
        "ridge_reach_gap_m": None if ridge_reach_gap_m is None else round(ridge_reach_gap_m, 3),
        "scan_y_max_a": None if pg_a.get("scan_y_max") is None else round(float(pg_a["scan_y_max"]), 3),
        "scan_y_max_b": None if pg_b.get("scan_y_max") is None else round(float(pg_b["scan_y_max"]), 3),
...
def _score_building(entry: dict,
                    ...
                    envelope_fp_by_uuid: dict[str, ShapelyPolygon] | None = None) -> dict:
    ...
    for pg in plane_groups:
        member_parent_ids = [m.get("parent_segment_id") for m in pg.get("members") or []]
        if scan_y_max_by_parent:
            parent_maxes = [scan_y_max_by_parent.get(pid) for pid in member_parent_ids if pid]
            parent_maxes = sorted((v for v in parent_maxes if v is not None), reverse=True)
        else:
            parent_maxes = []
    ...
    if wall_top_y is not None and scan_y_max_by_parent and plane_groups:
        ...
        passing_groups = [pg for pg in plane_groups
                          if plane_passes.get(pg["key"][0], False)]
    ...
    selected_pgids: set[str] = {
        pgid for pgid, best in best_per_group.items()
    ...
    for c in cands:
        ...
        scored_candidates.append({
            ...
            "plane_group_id": pgid,
            "is_plane_group_rep": bool(pgid and group_by_id[pgid]["rep_id"] == c["id"]),
            "best_partner_id": best.get("best_partner_id") if best else None,
            "best_partner_plane_group_id": best.get("best_partner_plane_group_id") if best else None,
    ...
    selected_polys = [pg["union"] for pg in plane_groups
    ...
    return {
        ...
        "n_candidates": len([c for c in cands if c.get("area_m2", 0.0) >= MIN_CANDIDATE_AREA_M2]),
...
def main() -> int:
    ...
    ap.add_argument(
        ...
        type=Path,
        help="buildings_3d.json — top-story walls_merged provide physical eave elevations",
    )
    ap.add_argument(
        "--v3-results",
        default="reconcile/reconcile_v3_results.json",
        type=Path,
        help="reconcile_v3_results.json — merged_roof_segments provide physical scan Y extents",
    )
    ap.add_argument(
        "--out",
    ...
    if args.buildings.exists():
        ...
        print(f"  {n_with}/{len(wall_tops_by_uuid)} buildings have ≥2 top-story wall tops")
        print(f"  {len(building_fp_by_uuid)}/{len(wall_tops_by_uuid)} buildings have room-floor footprint unions")
        print(f"  {len(envelope_fp_by_uuid)}/{len(wall_tops_by_uuid)} buildings have envelope (rooms + gaps) unions")
    else:
    ...
    with args.candidates.open("rb") as handle:
        for entry in ijson.items(handle, "item", use_float=True):
            ...
            try:
                ...
                print(f"  error {entry.get('building_uuid')}: {type(exc).__name__}: {exc}")
                continue
            results.append(out)
            n_scored += 1
