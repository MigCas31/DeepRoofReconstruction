...
def _classify_building(
    *,
    ...
    has_fenestration = any(
        counts.get(name, 0) > 0
        for name in ("base_window", "base_door", "base_opening")
    )
    roof_cells = int((part_payload.get("metadata") or {}).get("roof_cell_count", 0) or 0)
    occupied_cells = int(
...
def _classify_missing_roof_payload(
    *,
    ...
    unresolved_count = int((summary.get("metadata") or {}).get("unresolved_region_count", 0) or 0)
    if flat_count > 0 and oblique_count == 0 and roof_cells == 0 and evidence_rooms == 0:
...
def _roof_mode(payload_metadata: dict) -> str:
    ...
    coverage_patch = int(payload_metadata.get("roof_coverage_patch_surface_count", 0) or 0)
...
def _area_metrics(summary: dict, part_payload: dict, roof: dict) -> dict[str, float | None]:
    payload_surfaces = [
        surface for surface in (part_payload.get("renderable_surfaces") or []) if isinstance(surface, dict)
    ]
    summary_surfaces = [
        surface for surface in (summary.get("renderable_surfaces") or []) if isinstance(surface, dict)
    ]
    roof_surfaces = roof.get("roof_surfaces") or {}
...
def _markdown_table(rows: list[dict]) -> str:
    if not rows:
        return "_none_"
    header = (
        "| uuid | total | base_exterior | base_interior | floor | ceiling | roof | knee | unresolved | missing_roof_class | seconds |"
    )
    divider = "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- | ---: |"
    ...
    for row in rows:
        ...
        lines.append(
            "| {uuid} | {total} | {bew} | {biw} | {floor} | {ceiling} | {roof} | {knee} | {unresolved} | {missing_roof_class} | {seconds:.3f} |".format(
                uuid=row["uuid"],
...
def _write_markdown(path: Path, report: dict) -> None:
    ...
    lines = [
        ...
        f"- avg_renderable_surface_total: `{aggregate['avg_renderable_surface_total']:.1f}`",
        "",
        "## Coverage Buckets",
        "",
        ...
        f"- summary_is_ceiling_only: `{bucket_counts.get('summary_is_ceiling_only', 0)}`",
        "",
    ...
    for category in TARGET_CATEGORIES:
        lines.append(
            f"- `{category}` present in `{category_building_counts.get(category, 0)}` buildings, "
...
def main() -> None:
        ...
        report = {
            ...
            "aggregate": {
                **aggregate,
                ...
                "category_building_counts": dict(sorted(category_building_counts.items())),
                "category_surface_totals": dict(sorted(category_surface_totals.items())),
            },
            "results": results,
            "errors": errors,
        }
    ...
    aggregate["category_building_counts"] = dict(sorted(category_building_counts.items()))
    ...
    print(
        json.dumps(
            {
                "aggregate": aggregate,
                "top_missing_roof_examples": [row["uuid"] for row in report["top_missing_roof_examples"][:10]],
                "top_unresolved_examples": [row["uuid"] for row in report["top_unresolved_examples"][:10]],
            },
            indent=2,
        )
    )
