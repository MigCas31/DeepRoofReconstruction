...
def _classify_building(building, Point, Polygon, unary_union):
    ...
    for room in rooms:
        ...
        for wall in room.get("walls_computed") or []:
            ...
            for story_above, poly, slab_y in broad_candidates:
                ...
                if dist > XZ_NEAR_TOL:
                    continue
                if best is None or dist < best[0] or (dist == best[0] and story_above < best[1]):
    ...
    for i in range(len(ordered_stories) - 1):
        lo = sum(floor_y_by_story[ordered_stories[i]]) / len(floor_y_by_story[ordered_stories[i]])
        hi = sum(floor_y_by_story[ordered_stories[i + 1]]) / len(floor_y_by_story[ordered_stories[i + 1]])
        deltas.append(hi - lo)
...
def main():
    ...
    for b in buildings:
        rows, is_split, rooms_per_story = _classify_building(b, Point, Polygon, unary_union)
        by_building[b["uuid"]] = (rows, is_split, rooms_per_story)
        if is_split:
            split_level_uuids.add(b["uuid"])

    overall = Counter()
    per_building_fixable = Counter()  # fixable = slab_s+k_ok (k>=2) or slab_s+1_ok (but already_extended=False means untriggered)

    print(f"buildings total: {len(buildings)}")
    print(f"split-level (single-room-story OR story spacing < 2.0m): {len(split_level_uuids)}")
    print()
    ...
    for uuid, (rows, is_split, rpp) in by_building.items():
        for r in rows:
    ...
    print("Per-building (split-level) with slab_s+k_ok (k>=2) walls — these will be FIXED by the Phase B change:")
    ...
    for uuid, n in items:
        ...
        total = len(rows)
        print(f"  {uuid[:8]}  walls_total={total:4d}  ext_now={exts_now:4d}  fixable_s+k={n:4d}")
    print()
