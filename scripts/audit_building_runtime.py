...
def _row_failure_modes(row: dict[str, Any]) -> list[str]:
    ...
    if occupied_cell_count > 0 and fallback_cell_count / max(occupied_cell_count, 1) >= 0.4:
...
def _summarize(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    error_rows = [row for row in rows if str(row.get("status") or "").endswith("_error")]
    return {
        ...
        "avg_total_runtime_s": round(
            sum(float(row.get("timings_s", {}).get("total", 0.0) or 0.0) for row in rows) / max(total, 1),
            3,
        ),
        "avg_topology_runtime_s": round(
            sum(float(row.get("timings_s", {}).get("topology", 0.0) or 0.0) for row in rows) / max(total, 1),
            3,
        ),
        "max_total_runtime_s": round(
            max((float(row.get("timings_s", {}).get("total", 0.0) or 0.0) for row in rows), default=0.0),
            3,
        ),
        "max_topology_runtime_s": round(
            max((float(row.get("timings_s", {}).get("topology", 0.0) or 0.0) for row in rows), default=0.0),
            3,
        ),
        "failure_mode_counts": {
            mode: sum(1 for row in rows if mode in (row.get("failure_modes") or []))
            for mode in sorted({mode for row in rows for mode in (row.get("failure_modes") or [])})
        },
    }


def _write_report(output_path: Path, rows: list[dict[str, Any]], started_at: float) -> None:
...
def _run_one(uuid: str) -> dict[str, Any]:
    ...
    with warnings.catch_warnings(record=True) as caught:
        ...
        try:
            graph = build_topology_graph(merged_path=merged_path, scan_dir=SCAN_CACHE_ROOT, uuid=uuid)
    ...
    roof_cells_meta = ((roof or {}).get("roof_cell_complex") or {}).get("metadata") or {}
    ...
    row = {
        ...
        "occupied_fallback_cell_count": int(occupied_meta.get("fallback_cell_count") or 0),
        "occupied_atom_bound_cell_count": int(occupied_meta.get("atom_bound_cell_count") or 0),
        "occupied_exterior_wall_faces": int((occupied_meta.get("face_class_counts") or {}).get("exterior_wall") or 0),
        "occupied_interior_wall_faces": int((occupied_meta.get("face_class_counts") or {}).get("interior_wall") or 0),
        ...
        "roof_upper_void_cell_count": int(roof_cells_meta.get("upper_void_cell_count") or 0),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Checkpointed building runtime and ontology audit.")
