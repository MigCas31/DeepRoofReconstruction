...
def main() -> None:
    ...
    target_uuids = [
        ...
        and int(((row.get("payload_metadata") or {}).get("roof_fallback_surface_count", 0)) or 0) > 0
    ]
    ...
    def _write_report() -> None:
        ...
        report = {
            ...
            "example_buildings": [
                ...
                for row in sorted(results, key=lambda row: (-row["roof_fallback_surface_count"], row["uuid"]))[:15]
            ],
        }
        ...
        lines = [
            ...
            f"- roomless_fallback_surface_count: `{aggregate['roomless_fallback_surface_count']}`",
        ...
        for row in report["example_buildings"]:
            lines.append(f"- `{row['uuid']}` fallback_surfaces=`{row['roof_fallback_surface_count']}`")
            for room in row["fallback_rooms"]:
                lines.append(
                    f"  room `{room['room_index']}` bucket=`{room['bucket']}` count=`{room['fallback_surface_count']}` evidence=`{room['room_summary']['roof_evidence_score']}`"
                )
            for surface in row["roomless_fallback_surfaces"]:
                lines.append(
                    f"  roomless hypothesis=`{surface['roof_hypothesis_id']}` story=`{surface['story']}` has_patch_for_hypothesis=`{surface['has_patch_for_hypothesis']}`"
                )
        md_path.write_text("\n".join(lines))
        return report
                ...
                roomless_fallback_surfaces.append(
                    {
                        ...
                        "has_patch_for_hypothesis": str(surface.get("roof_hypothesis_id") or "") in patch_hypotheses,
                    }
                )
                continue
            ...
            room_rows.append(
                {
                    ...
                    "room_summary": {
                        ...
                        "partially_covered_by_sloped_roof": bool(room_summary.get("partially_covered_by_sloped_roof")),
                        "strong_perimeter_sloped": bool(room_summary.get("strong_perimeter_sloped")),
                        "strong_knee_wall_signal": bool(room_summary.get("strong_knee_wall_signal")),
        ...
        results.append(
            {
                ...
                "roof_cell_count": int((payload.get("metadata") or {}).get("roof_cell_count", 0) or 0),
                "roof_exact_flat_surface_count": int((payload.get("metadata") or {}).get("roof_exact_flat_surface_count", 0) or 0),
                "roof_coverage_patch_surface_count": int((payload.get("metadata") or {}).get("roof_coverage_patch_surface_count", 0) or 0),
                "roof_fallback_surface_count": int((payload.get("metadata") or {}).get("roof_fallback_surface_count", 0) or 0),
                "fallback_rooms": room_rows,
                "roomless_fallback_surfaces": roomless_fallback_surfaces,
            }
        )
        aggregate["avg_runtime_s"] = sum(row["timings_s"]["total"] for row in results) / len(results)
        _write_report()
