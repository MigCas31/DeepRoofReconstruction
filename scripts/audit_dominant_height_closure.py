...
def cluster_heights(tops_and_weights: list[tuple[float, float]], tol: float) -> list[tuple[float, float]]:
...
def compute_cohort(walls: list[dict]) -> dict | None:
    ...
    if total_w <= 0:
        return None
    clusters = cluster_heights([(w["top_y"], w["span"]) for w in walls], COHORT_TOLERANCE_M)
    if not clusters:
        return None
...
def format_hist(values: list[float], edges: list[float]) -> str:
    ...
    prev = None
    for e in edges:
        labels.append(f"<{e:.2f}" if prev is None else f"<{e:.2f}")
        prev = e
...
def run(input_path: Path, building_filter: str | None) -> int:
    ...
    print(f"Thresholds: cohort_tol={COHORT_TOLERANCE_M}, min_cov={MIN_COHORT_COVERAGE},")
    print(
        f"            max_span_frac={MAX_OUTLIER_SPAN_FRAC}, max_delta={MAX_DELTA_M},"
    )
    print(
        f"            min_promotion_delta={MIN_PROMOTION_DELTA_M},"
    )
    print(
        f"            floor_tol={FLOOR_TOLERANCE_M}, colin_angle={COLINEAR_ANGLE_DEG}°,"
    )
    print(
        f"            colin_offset={COLINEAR_OFFSET_M}, colin_gap={COLINEAR_GAP_M}"
    )
    print("-" * 72)
    ...
    print(
        f"Stories with raw candidates : {stories_with_raw_candidates}"
    )
    print(
        f"Stories with floor-passing  : {stories_with_floor_candidates}"
    )
    print(
        f"Stories with all-passing    : {stories_with_all_candidates}"
    )
