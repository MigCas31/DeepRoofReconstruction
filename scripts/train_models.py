...
from reconcile_v3.analysis.modelling import run_phase4  # noqa: E402
...
def main() -> int:
    ...
    if not feat_path.exists():
        print(f"ERROR: {feat_path} not found; run scripts/analyze_labels.py first.", file=sys.stderr)
        return 2

    df = pd.read_parquet(feat_path)
    print(f"loaded {len(df)} rows × {len(df.columns)} cols from {feat_path.name}")
