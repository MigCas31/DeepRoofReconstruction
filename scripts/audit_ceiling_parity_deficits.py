...
def _classify_failure_mode(
    *,
    ...
    if flat_ceiling_area > semantic_sensitive_area * 1.5 and semantic_overlap_ratio < 0.6:
...
def main() -> None:
    ...
    ok_rows = [row for row in (report.get("results") or []) if row.get("status") == "ok"]
    ranked_rows = sorted(
        ok_rows,
        key=lambda row: (
            -float((row.get("area_metrics") or {}).get("semantic_ceiling_heuristic_only_projected_area_m2", 0.0) or 0.0),
            float((row.get("area_metrics") or {}).get("semantic_ceiling_overlap_ratio_vs_heuristic") or 0.0),
            row["uuid"],
        ),
    )
    targets = [
        ...
        if float((row.get("area_metrics") or {}).get("heuristic_ceiling_area_m2", 0.0) or 0.0) > 1.0
    ][:25]
    ...
    for row in targets:
        ...
        for node in (top_boundary_graph.get("nodes") or []):
            if not isinstance(node, dict) or node.get("type") != "TopBoundaryAtom":
                continue
            role_area[str(node.get("role") or "")] += float(node.get("area_m2", 0.0) or 0.0)
        total_rooms = len(
            {
        ...
        mixed_room_count = sum(1 for room in room_partitions if isinstance(room, dict) and room.get("mixed"))
        ...
        failure_mode = _classify_failure_mode(
            ...
            shell_overlap_ratio=area_metrics.get("shell_ceiling_overlap_ratio_vs_heuristic"),
            semantic_ratio=area_metrics.get("ceiling_area_ratio"),
            semantic_overlap_ratio=area_metrics.get("semantic_ceiling_overlap_ratio_vs_heuristic"),
            role_area=role_area,
    ...
    lines = [
        ...
        f"- avg_semantic_ceiling_overlap_ratio_vs_heuristic: `{aggregate['avg_semantic_ceiling_overlap_ratio_vs_heuristic']}`",
        f"- avg_shell_ceiling_overlap_ratio_vs_heuristic: `{aggregate['avg_shell_ceiling_overlap_ratio_vs_heuristic']}`",
        "",
        "## Failure Modes",
        "",
    ]
    ...
    for row in results:
        lines.append(
            f"- `{row['uuid']}` mode=`{row['failure_mode']}` semantic_overlap=`{row['area_metrics'].get('semantic_ceiling_overlap_ratio_vs_heuristic')}` shell_overlap=`{row['area_metrics'].get('shell_ceiling_overlap_ratio_vs_heuristic')}` heuristic_only=`{row['area_metrics'].get('semantic_ceiling_heuristic_only_projected_area_m2')}`"
        )
        lines.append(
            f"  rooms total/exposed/resolved/mixed = `{row['room_counts']['total_rooms']}/{row['room_counts']['exposed_rooms']}/{row['room_counts']['resolved_top_boundary_rooms']}/{row['room_counts']['mixed_ceiling_partition_rooms']}`"
        )
        lines.append(
            f"  role_area = `{row['top_boundary_role_area_m2']}`"
        )
