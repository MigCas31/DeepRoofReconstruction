...
Target: building 24e8aaa7-ec15-4a72-be5f-c67b95a53411, story 0, Room 1, endpoint B of wall
...
def main() -> int:
    ...
    print(f"story {TARGET_STORY}: wall-height median={median_h:.3f} m, cutoff={cutoff:.3f} m")
    ...
    for room_idx, room in rooms:
        for wall_idx, wall in enumerate(room["walls_computed"]):
            ...
            if h < MIN_WALL_HEIGHT or h < cutoff:
                print(f"  FILTERED room={room_idx} wall_idx={wall_idx} id={wall.get('id','')[:8]} h={h:.2f}")
                continue
            wid = wall.get("id", "")
            wall_eps = []
            for ei in (0, 1):
                ...
                endpoints.append(
                    (corners[ei][0], corners[ei][2], corners[ei][1], yt,
                     (room_idx, wall_idx), ei, wid)
                )
                wall_eps.append(len(endpoints) - 1)
            if wid in (TARGET_WALL_A, TARGET_WALL_B):
                target_ep_by_wall[wid] = wall_eps

    # Pick the two endpoints that are actually facing each other (smallest cross-pair distance)
    ...
    if target_a_endpoint_idx is None:
        print(f"target wall {TARGET_WALL_A[:8]} not in pool — height filter excluded it")
        return 2
    if target_b_endpoint_idx is None:
        print(f"partner wall {TARGET_WALL_B[:8]} not in pool — height filter excluded it")
        return 2
    ...
    print(f"target A (E5C3F73F B-end): x={ta[0]:.3f} z={ta[1]:.3f}  room={ta[4][0]} wall_idx={ta[4][1]} end={ta[5]}")
    print(f"partner (572EE06C A-end): x={tb[0]:.3f} z={tb[1]:.3f}  room={tb[4][0]} wall_idx={tb[4][1]} end={tb[5]}")
    expected_dist = math.hypot(ta[0] - tb[0], ta[1] - tb[1])
    print(f"expected pair distance: {expected_dist:.3f} m  (max_gap={MAX_GAP}, mutual_thresh={MUTUAL_THRESH})")

    # Replay the inner best-match loop for the target endpoint
    x1, z1, _yb1, _yt1, wk1, end1, wid1 = ta
    best_dist = MAX_GAP
    best_idx = -1
    candidates = []
    ...
    if chosen[6] != TARGET_WALL_B:
        print("\nBLOCK GATE: a CLOSER endpoint on a DIFFERENT wall displaced the expected partner")
        print(f"  target picked {chosen[6][:8]} at {best_dist:.3f} m instead of 572EE06C at {expected_dist:.3f} m")
        return 0
    ...
    if best_dist <= MUTUAL_THRESH:
        ...
        if reverse_best != target_a_endpoint_idx:
            ...
            print(f"  partner's best is index {reverse_best}, not target ({target_a_endpoint_idx})")
            return 0
    else:
        print(f"\nmutual-best gate SKIPPED (best_dist {best_dist:.3f} > mutual_thresh {MUTUAL_THRESH})")

    print("\nNO BLOCK: pair should have been stitched; check that stitch output was actually generated in buildings_3d.json")
    return 0
