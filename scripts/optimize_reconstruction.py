...
def _sample_config(rng: random.Random, time_limit_s: float = 5.0) -> SolverConfig:
    ...
    return SolverConfig(
        ...
        k_azimuth_bins=int(round(_u("k_azimuth_bins"))),
...
def _run_trial(
    cfg: SolverConfig, pilot: list[dict],
    ...
    review_flags = 0
    for b in pilot:
        res = solve_building(b["uuid"], b["candidates"], b["scan_fp"], config=cfg)
        selected = []
...
def main() -> int:
    ...
    if args.promote:
        ...
        with hp_path.open("w") as h:
            ...
            promoted = {k: v for k, v in best["config"].items()
                        if k != "time_limit_s"}
