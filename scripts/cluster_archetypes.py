...
def _top_distinguishing(
    ...
    return [(feature_names[i], float(cluster_mean[i]), float(cluster_mean[i])) for i in order]
...
def cluster(
    ...
    print(
        ...
        f"(keep {pca_variance:.0%} var, explained={pca.explained_variance_ratio_.sum():.3f})"
    )
    ...
    if method == "hdbscan":
        ...
        print(f"HDBSCAN -> {n_found} clusters + {n_noise} noise ({100*n_noise/len(df):.1f}%)")
    elif method == "kmeans":
...
def write_report(
    ...
    real.sort(key=lambda s: (s["accept_rate"] if s["accept_rate"] == s["accept_rate"] else 2.0))
    ...
    for s in real:
        ...
        top_str = f"`{top[0]}` ({top[1]:+.1f}σ)"
        lines.append(
            f"| {s['cluster']} | {s['size']} | {s['n_labeled']} | "
            f"{_fmt_rate(s['accept_rate'])} | {_fmt_rate(s['heur_accept_rate'])} | {top_str} |"
        )
    lines.append("")
    ...
    for s in real:
        cid = s["cluster"]
        lines.append(f"### Cluster {cid} — {s['size']} members · accept {_fmt_rate(s['accept_rate'])} "
                     f"({s['n_accept']}/{s['n_labeled']} labeled) · heuristic "
                     f"{_fmt_rate(s['heur_accept_rate'])}\n")
        lines.append("**Most distinguishing features** (z vs grand mean, cluster raw mean):")
        lines.append("")
    ...
    if noise is not None and noise["size"] > 0:
        lines.append(
            ...
            f"{_fmt_rate(noise['accept_rate'])} ({noise['n_accept']}/{noise['n_labeled']} labeled)\n"
        )
        lines.append(
...
def main() -> int:
    ...
    ap.add_argument("--selection", choices=["eom", "leaf"], default="leaf",
    ...
    write_report(summaries, Path(args.output_md), Path(args.output_csv), df_out, args.method)
    return 0
