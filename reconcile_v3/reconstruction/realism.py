...
reference (``slanted_roofs`` ∪ ``flat_ceilings``) per building, all in
the XZ plane.
...
* ``azimuth_mismatch_frac``
                    ...
                    >30°. Catches cases where BIP selects a plane on
...
def score_building(
    ...
    """
    sel_polys = [
        p for p in (_poly_from_ring(f.get("footprint_xz")) for f in selected_faces)
        if p is not None
    ]
...
def aggregate_scores(scores: list[RealismScore]) -> dict[str, Any]:
    ...
    return {
        ...
        "frac_iou_ge_0_9": (
            sum(1 for i in ious if i >= 0.9) / len(ious) if ious else 0.0
        ),
