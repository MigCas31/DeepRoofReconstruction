...
def _iter_all_ys(building: dict) -> Iterable[float]:
    for room in building.get("rooms", []) or []:
        for c in room.get("floor_polygon", []) or []:
            yield float(c[1])
        for wall in (room.get("walls_merged") or []) + (room.get("walls_computed") or []):
...
def _shape_flags(poly: Polygon) -> dict[str, bool | None]:
    ...
    return {
        "bld_footprint_is_L_shape": bool(reflex == 1 and (solidity or 0.0) < PART_SHAPE_L_SOLIDITY_MAX),
        "bld_footprint_is_T_shape": bool(reflex == 2 and (solidity or 0.0) < PART_SHAPE_T_SOLIDITY_MAX),
        "bld_footprint_is_U_shape": bool(reflex >= 2 and (solidity or 0.0) < PART_SHAPE_U_SOLIDITY_MAX),
    ...
    for room in building.get("rooms", []) or []:
        for wall in (room.get("walls_merged") or []) + (room.get("walls_computed") or []):
            ...
            if principal is None:
                continue
            az = math.degrees(math.atan2(float(principal[0]), float(principal[1]))) % 180.0
            length = 0.0
            for i in range(len(pts)):
                for j in range(i + 1, len(pts)):
                    length = max(
                        length,
                        math.hypot(float(pts[i, 0] - pts[j, 0]), float(pts[i, 2] - pts[j, 2])),
                    )
            bins[round(az / 10.0) * 10.0] = bins.get(round(az / 10.0) * 10.0, 0.0) + max(length, 1e-3)
    if not bins:
        return {
...
def compute_building_features(building: dict) -> dict[str, Any]:
    ...
    out: dict[str, Any] = {
        ...
        "bld_wall_count": sum(
            len(r.get("walls_merged") or []) + len(r.get("walls_computed") or []) for r in rooms
        ),
            ...
            out.update(
                {
                    ...
                    "bld_footprint_convexity_deficiency": (1.0 - solidity) if solidity is not None else None,
    ...
    if out["bld_height_m"] is not None and out["bld_story_count"] > 0:
        out["bld_typical_story_height_m"] = float(out["bld_height_m"] / out["bld_story_count"])
    out["bld_has_basement"] = bool(any((room.get("story") or 0) < 0 for room in rooms if room.get("story") is not None))

    overlap = building.get("overlap_metrics") or {}
    room_count = float(len(rooms) or 0.0)
    footprint_area = out.get("bld_footprint_area_m2")
    overlap_fraction = (
        float(overlap.get("floor_overlap_count") or 0.0) / room_count if room_count > 0 else None
    )
    wall_clip_fraction = (
        float(overlap.get("walls_clipped") or 0.0) / float(overlap.get("walls_checked") or 0.0)
    ...
    out["bld_scan_quality_score"] = float(sum(scan_terms) / len(scan_terms)) if scan_terms else None
...
def load_building_features(buildings_3d_path: Path) -> dict[str, dict[str, Any]]:
    ...
    if not isinstance(buildings, list):
        raise ValueError(
            f"Expected a list of buildings in {buildings_3d_path}, got {type(buildings).__name__}"
        )
