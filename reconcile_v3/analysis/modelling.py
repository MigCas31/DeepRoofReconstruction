...
def prepare_data(df: pd.DataFrame) -> PreparedData:
    ...
    counts = (
        labeled.groupby("building_uuid")["building_uuid"].transform("count").astype(float)
    )
    ...
    feature_cols = [
        ...
        if (_is_model_feature_col(c) and labeled[c].dtype.kind not in ("O", "U")) or c.startswith("piece_kind_is_")
    ]
    ...
    for c in X.columns:
        if X[c].dtype == bool:
            ...
            X[c] = X[c].astype("category").cat.codes.replace(-1, np.nan).astype("float32")
...
def _calibrate_isotonic(y_true: np.ndarray, p_raw: np.ndarray) -> tuple[IsotonicRegression, np.ndarray]:
...
def _binary_metrics(
    ...
    out: dict[str, Any] = {
        ...
        "recall_accept": float(recall_score(y_true, y_pred, pos_label=1, zero_division=0.0)),
        "precision_reject": float(
            precision_score(y_true, y_pred, pos_label=0, zero_division=0.0)
        ),
        "recall_reject": float(recall_score(y_true, y_pred, pos_label=0, zero_division=0.0)),
    ...
    if p_score is not None:
        ...
        if mask.sum() >= 2 and len(np.unique(y_true[mask])) >= 2:
            ...
            out["pr_auc"] = float(
                average_precision_score(y_true[mask], p_score[mask])
            )
    return out
...
def _calibration_curve(y_true: np.ndarray, p: np.ndarray, n_bins: int = 10) -> dict:
    ...
    return {"bins": edges.tolist(), "mean_true": means_true, "mean_pred": means_pred, "counts": counts}
...
def _plot_calibration(
    raw_curve: dict, cal_curve: dict, out_path: Path, heuristic_rate: tuple[float, float]
) -> None:
    import matplotlib
...
def _plot_pr(
    ...
    ax.scatter([h_rec], [h_prec], s=80, marker="s", color="tab:green", label="heuristic")
    ax.axhline(y_true.mean(), ls=":", color="black", lw=1, label=f"base rate={y_true.mean():.2f}")
...
def run_phase4(
    ...
    print(
        f"prepared: {data.X.shape[0]} rows × {data.X.shape[1]} features; "
    ...
    heuristic_rate = (
        ...
        float(data.y[data.heuristic_pred == 1].mean() if (data.heuristic_pred == 1).any() else 0.0),
    )
...
def _write_report(
    ...
    lines.append("| Model | F1 accept | F1 reject | Prec accept | Recall accept | ROC AUC | PR AUC |\n")
    lines.append("|---|---|---|---|---|---|---|\n")
    ...
    lines.append(
        f"**Gate (GBM F1 accept ≥ heuristic + 0.10):** {'PASS' if gate['passes'] else 'FAIL'} "
        f"(Δ = {gate['delta']:.3f})\n\n"
    )

    lines.append("## Confusion matrices (rows: truth, cols: prediction; classes [reject, accept])\n\n")
    ...
    lines.append("| building_uuid | n_labels | accept_rate_true | accept_rate_pred | F1 accept | errors |\n")
    lines.append("|---|---|---|---|---|---|\n")
    ...
    if imps:
        mean_imp = pd.concat(imps).groupby("feature")["importance"].mean().sort_values(ascending=False)
