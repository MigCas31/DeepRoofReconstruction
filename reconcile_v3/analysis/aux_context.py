...
def build_for_uuid(uuid: str, *, repo_root: Path | None = None) -> dict[str, Any]:
    ...
    topology = {
        ...
        "cell_complex_cells": [_cell_record(cell) for cell in (topology_cell_complex.get("cells") or [])],
    }
    ontology = {
        ...
        "full_model_renderable_surfaces": [
            _surface_record(surface) for surface in (full_model.get("renderable_surfaces") or [])
        ],
        "roof_surfaces_oblique": [
            _roof_surface_record(surface) for surface in ((roof.get("roof_surfaces") or {}).get("oblique") or [])
        ],
        "roof_surfaces_flat": [
            _roof_surface_record(surface) for surface in ((roof.get("roof_surfaces") or {}).get("flat") or [])
        ],
    }
    return {
