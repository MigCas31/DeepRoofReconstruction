...
_KEEP_PART_FIELDS = ("id", "room_ids", "footprint_xz", "stories", "gable_extension", "trace")
_KEEP_WE_FIELDS = ("id", "wall_id", "room_id", "strip_corners", "behind_knee_wall", "trace")
...
_KEEP_GAP_FIELDS = ("id", "footprint_xz", "floor_y", "ceiling_y", "between_rooms", "status", "trace")
_KEEP_UNRESOLVED_FIELDS = ("id", "room_id", "footprint_xz", "y_range", "reason", "context", "trace")
    ...
    with results_path.open("rb") as f:
        for b in ijson.items(f, "item", use_float=True):
            ...
            out[uuid] = {
                ...
                "wall_extensions": [_pick(w, _KEEP_WE_FIELDS) for w in b.get("wall_extensions") or []],
                "dormers": [_pick(d, _KEEP_DORMER_FIELDS) for d in b.get("dormers") or []],
                "slanted_roofs": [_pick(s, _KEEP_SR_FIELDS) for s in b.get("slanted_roofs") or []],
                "merged_roof_segments": [_pick(s, _KEEP_SEG_FIELDS) for s in b.get("merged_roof_segments") or []],
                "slabs": [_pick(s, _KEEP_SLAB_FIELDS) for s in b.get("slabs") or []],
                "flat_ceilings": [_pick(c, _KEEP_FLAT_FIELDS) for c in b.get("flat_ceilings") or []],
                "gaps": [_pick(g, _KEEP_GAP_FIELDS) for g in b.get("gaps") or []],
                "unresolved": [_pick(u, _KEEP_UNRESOLVED_FIELDS) for u in b.get("unresolved") or []],
