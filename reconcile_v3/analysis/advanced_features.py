...
def _wall_azimuth_incl_length_top_bottom(
    ...
    for i in range(n):
        for j in range(i + 1, n):
            d = float(
                math.hypot(pts[i, 0] - pts[j, 0], pts[i, 2] - pts[j, 2])
            )
            if d > max_d:
                max_d = d
    ...
    try:
        ...
        if nl > _EPS:
            ...
            incl_from_horiz = math.degrees(math.acos(max(-1.0, min(1.0, abs(float(normal[1]))))))
...
def source_wall_features(
    ...
    if incls:
        ...
        out["swall_incl_is_nonvertical_fraction"] = float(
            np.mean(arr > 2.0)
        )
    ...
    if segment_poly is not None:
        cxcz = np.asarray(
            [[w["centroid_x"], w["centroid_z"]] for w in resolved if w.get("centroid_x") is not None],
            dtype=float,
        )
        if len(cxcz):
    ...
    out["swall_fraction_walls_merged"] = float(
        sum(1 for k in kinds if k == "walls_merged") / len(kinds)
    ) if kinds else None
    ...
    if slabs and room_ids:
        same = sum(
            1 for s, r in zip(slabs, room_ids) if s and r and s == r
        )
    ...
    out["swall_stories_touched"] = (
        float(len(stories)) if stories else 0.0
    )
...
def _moment_integrand(x1: float, y1: float, x2: float, y2: float, p: int, q: int) -> float:
...
def shape_descriptor_features(
    ...
    if hasattr(hull, "exterior"):
        ...
        if len(coords):
            ...
            reock = (
                float(poly.area) / (math.pi * r_enc * r_enc)
                if r_enc > _EPS
                else None
            )
        else:
    ...
    schwartzberg = (
        (2.0 * math.pi * equiv_r) / float(poly.length)
        if poly.length > _EPS
        else None
    )
...
def vertex3d_features(
    ...
    if n >= 1:
        ...
        by = pts[:, 1]
        bz = pts[:, 2]
        vol = float((bx.max() - bx.min()) * (by.max() - by.min()) * (bz.max() - bz.min()))
        diag = float(
            math.sqrt(
                (bx.max() - bx.min()) ** 2
                + (by.max() - by.min()) ** 2
...
def normal_stats_features(cluster_members: Sequence[dict] | None) -> dict[str, Any]:
    ...
    if ns:
        ...
        if mean_resultant < 1.0 - _EPS:
            kappa = (3.0 * mean_resultant - mean_resultant ** 3) / (1.0 - mean_resultant ** 2)
    ...
    if building_context:
        ...
        if union_polys:
            try:
                ...
                out["final_roof_union_area_m2"] = float(sum(p.area for p in union_polys))
    return out
