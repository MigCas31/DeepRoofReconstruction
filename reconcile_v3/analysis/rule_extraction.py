...
class LeafRule:
    leaf_id: int
    conditions: list[str]  # e.g. ["drainage_to_building_center_cos <= -0.412", "covered_side_count > 2"]
    support: int
    reject_count: int
    accept_count: int
...
def _walk_leaves(
    ...
    def path_for(leaf: int) -> list[str]:
        path_nodes: list[tuple[int, bool]] = []  # (node, went_left)
    ...
    for leaf_id in np.where(is_leaf)[0]:
        ...
        rj_pick = rng.choice(reject_idx, size=min(n_reject_examples, len(reject_idx)), replace=False)
        ac_pick = (
            rng.choice(accept_idx, size=min(n_accept_examples, len(accept_idx)), replace=False)
            if len(accept_idx)
...
def write_markdown(
    ...
    lines.append(
        f"Rules extracted from a single `DecisionTreeClassifier(max_depth={tree_depth}, "
    ...
    lines.append(
        f"- gates: **support ≥ {min_support}**, **purity ≥ {min_purity:.2f}** (reject fraction)\n"
    )
    lines.append(
