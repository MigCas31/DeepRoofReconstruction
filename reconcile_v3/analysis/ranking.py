...
def _cramers_v(series: pd.Series, labels: pd.Series) -> float | None:
    """Cramér's V on a 2-way contingency table (features × labels)."""
...
def _mutual_info_numeric(
    ...
    try:
        ...
        return {}
    numeric_cols = [
        c for c in feature_cols if pd.api.types.is_numeric_dtype(df[c]) and df[c].notna().any()
    ]
    if not numeric_cols:
        return {}
    ...
    return dict(zip(keep, map(float, mi)))
    ...
    *,
    ...
    for _, row in top.iterrows():
        ...
        if row["kind"] == "continuous":
            ...
            ax.hist(a.clip(lo, hi), bins=40, alpha=0.55, label="accept", color="tab:green", density=True)
            ax.hist(r.clip(lo, hi), bins=40, alpha=0.55, label="reject", color="tab:red", density=True)
            ax.set_xlabel(col)
