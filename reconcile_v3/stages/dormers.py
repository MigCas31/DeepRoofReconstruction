"""dormers — V3 adapter around ``reconcile.roof_algorithms_py.dormer_detection.detect_dormers``.
        ...
        "rooms": [
            {
                ...
                "walls_computed": [
                    {"id": w.get("id"), "corners": [list(c) for c in w.get("corners") or []]}
                    for w in r.walls
                ],
                "walls_merged": [
                    {"id": w.get("id"), "corners": [list(c) for c in w.get("corners") or []]}
...
def _cluster_surfaces(
    ...
    for idx, cl in enumerate(clusters):
        ...
        projections = [x * ridge_dx + z * ridge_dz for x, z in zip(xs, zs)]
        ...
        for s in cl["segs"]:
            story_counts[int(s.get("story", 0))] = story_counts.get(int(s.get("story", 0)), 0) + 1
        if story_counts:
        ...
        surfaces.append(
            {
                ...
                "ridge": {"x": ridge_dx, "z": ridge_dz, "min": ridge_min, "max": ridge_max},
