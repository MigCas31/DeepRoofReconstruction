...
def _slab_y(slab: V3Slab) -> float:
    return float(sum(c[1] for c in slab.polygon) / len(slab.polygon)) if slab.polygon else 0.0
...
def extend_walls_to_slab_above(
    ...
    for room in rooms:
        ...
        ) if room.floor_polygon else (0.0, 0.0)
        ...
        for wall in room.walls:
                ...
                unresolved.append(
                    V3UnresolvedRegion(
                        ...
                        trace=HypothesisTrace(
                            ...
                            inputs={"delta_m": round(delta, 3), "threshold": KNEE_DELTA_M},
                            decision_reason=(
                ...
                extensions.append(
                    V3WallExtension(
                        ...
                        trace=HypothesisTrace(
                            ...
                            decision_reason="Δ under SCAN_NOISE_M — raise wall to slab directly.",
                        ),
                    )
                )
                continue
            ...
            if slab_poly.contains(pt):
                ...
                extensions.append(
                    V3WallExtension(
                        id=make_element_id(
                            building_uuid, "v3-wall-extension", f"{extend_branch}-{inner}"
                        ),
                        wall_id=wall_id,
                        room_id=room.identifier,
                        strip_corners=strip_corners,
                        ...
                        trace=HypothesisTrace(
                            ...
                            decision_reason=(
                                "Slab above contains wall midpoint — raise wall to slab."
                            ),
                        ),
                    )
                )
                ...
                if outside_dist <= EAVE_OVERHANG_M:
                    ...
                    extensions.append(
                        V3WallExtension(
                            id=make_element_id(
                                building_uuid, "v3-wall-extension", f"cantilever-{inner}"
                            ),
                            wall_id=wall_id,
                            room_id=room.identifier,
                            strip_corners=strip_corners,
                    ...
                    if has_hypothesis:
                            ...
                            V3WallExtension(
                                ...
                                trace=HypothesisTrace(
                                    ...
                                    decision_reason=(
                                        ...
                                        "room has a SlopeHypothesis — treat as knee wall; "
                                        "do NOT extend."
                                    ),
                                ),
                            )
                        ...
                        unresolved.append(
                            V3UnresolvedRegion(
                                ...
                                trace=HypothesisTrace(
                                    ...
                                    decision_reason=(
                                        "Knee wall region above wall top — flat_ceilings "
                        ...
                        unresolved.append(
                            V3UnresolvedRegion(
                                id=make_element_id(
                                    building_uuid, "v3-unresolved", f"knee-indet-{inner}"
                                ),
                                room_id=room.identifier,
                                footprint_xz=[(float(c[0]), 0.0, float(c[2])) for c in corners],
                                y_range=(float(y_top), float(y_slab)),
