...
def project_xz_onto_plane(
    ...
    return [
        (float(x), float(-(a * x + c * z + d) / b), float(z))
        for x, _, z in corners_xz
    ]
...
def _outward_direction(wall_xz: tuple[float, float], centroid: tuple[float, float]) -> tuple[float, float]:
...
def _missing_exterior_edges(
    ...
    for i in range(len(coords) - 1):
        ...
        if edge.length < SCAN_NOISE_M:
            continue
        if part_boundary.distance(edge.interpolate(0.5, normalized=True)) > SCAN_NOISE_M:
            continue
        covered = False
        for wm in wall_mids:
...
def cluster_oblique_to_roof_faces(
    ...
    for idx, cl in enumerate(clusters):
        ...
        ratifying_polys = [room_floor_polys[rid] for rid in room_ids if rid in room_floor_polys]
    ...
    for room in rooms:
                ...
                if _wall_near_part_boundary(short_wall, nearest_part_boundary):
                    ...
                    if n > 1e-6:
                        ...
                        if direction is not None:
                            ...
                            if angle_diff(cluster_az, _azimuth_from_direction(asym_dir)) < CLUSTER_AZIMUTH_TOL_DEG:
                                confidence = min(1.0, confidence + CONFIDENCE_BOOST_ASYMMETRIC)
                        else:
                            direction = asym_dir
        ...
        if room.identifier in room_polys and nearest_part_boundary is not None:
            missing = _missing_exterior_edges(room, room_polys[room.identifier], nearest_part_boundary)
            if missing:
                edge = missing[0]
                mid = (
                    (edge[0][0] + edge[1][0]) / 2,
                ...
                if direction is not None:
                    ...
                    if angle_diff(dir_az, out_az) < CLUSTER_AZIMUTH_TOL_DEG:
                        confidence = min(1.0, confidence + CONFIDENCE_BOOST_MISSING_WALL)
                else:
                    direction = out_dir
        ...
        if confidence < SLOPE_HYPOTHESIS_MIN_CONFIDENCE:
            fp_xz = [(float(x), 0.0, float(z)) for x, _, z in room.floor_polygon] if room.floor_polygon else []
            unresolved.append(
                V3UnresolvedRegion(
                    id=make_element_id(
                    ...
                    context={"sources": list(sources), "confidence": round(confidence, 3)},
                    trace=HypothesisTrace(
                        stage="slanted_roofs",
