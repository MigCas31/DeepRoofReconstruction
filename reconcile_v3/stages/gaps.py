...
def _gap_polygon(gap: dict) -> Polygon | None:
    ...
    return poly if isinstance(poly, Polygon) and poly.area > SCAN_NOISE_M * SCAN_NOISE_M else None
...
def close_obvious_gaps(
    ...
    for idx, gap in enumerate(detected):
        ...
        if ceiling_y is None:
            fallback_ys = [y for y in (_room_ceiling_y(r) for r, _ in rooms_with_polys if r.story == story) if y is not None]
            if fallback_ys:
        ...
        ceilings_out.append(
            V3FlatCeiling(
                ...
                room_id=None,
                footprint_xz=[(float(c[0]), ceiling_y, float(c[2])) for c in gap["corners"]],
                y=ceiling_y,
                over="gap",
                trace=HypothesisTrace(
                    stage="gaps",
