...
def _plane_azimuth_inclination(
    ...
    """Downslope azimuth (degrees, 0=+Z, clockwise) and inclination from plane (a,b,c,d)."""
    a, b, c, _ = plane
...
def _part_arch_flat_counts(
    building: V3Building, rooms: list[V3Room]
) -> dict[str, int]:
    ...
    *,
    ...
    if coverage_only and len(tier2) == 1:
        return GableExtension(
            ...
            uncovered_region_xz=None,
            decision_reason=(
                "Gable-geometry passes and footprint is already covered — nothing to extend."
            ),
        )
    if ridge_only and len(tier2) == 1:
        return GableExtension(
            ...
            uncovered_region_xz=uncovered_coords,
            decision_reason=(
                "Ridge runs across the short axis — flag for review, do not auto-extend."
            ),
        )
    return GableExtension(
        status="not_gable",
