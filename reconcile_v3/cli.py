...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        help="Where to write the aggregate results (default: reconcile/reconcile_v3_results.json)",
    )
    ...
    if args.uuid:
        ...
        write_collection(args.output, [building])
        print(
            f"Wrote {args.output} (1 building, {len(building.slabs)} slabs, "
    ...
    if args.all:
        ...
        for idx, uuid in enumerate(uuids, 1):
            try:
                ...
                print(f"  ! skipped {uuid}: {type(exc).__name__}: {exc}", file=sys.stderr)
            if idx % 25 == 0:
