...
RAW_CEILING_PLANE_SPLITS_PATHS: dict[str, Path] = {
    "v1": ROOT_DIR.parent / "reports" / "raw_ceiling_plane_scorer" / "plane_extent_splits.json",
    "v2": ROOT_DIR.parent / ".context" / "raw_ceiling_plane_scorer_v2_full" / "plane_extent_splits.json",
}
RAW_CEILING_PLANE_SPLITS_CACHE_BY_VERSION: dict[str, dict] = {"v1": {}, "v2": {}}
RAW_CEILING_PLANE_SPLITS_CACHE_MTIME_BY_VERSION: dict[str, float] = {"v1": 0.0, "v2": 0.0}
...
def _gate_unsupported_v2_pieces(pieces: list[dict]) -> list[dict]:
    ...
    for i, piece in enumerate(pieces):
        ...
        try:
            ...
            covered = 0.0
        if self_poly.area > 0 and covered / self_poly.area >= SLANTED_PIECE_DOMINATION_COVERAGE:
            keep_flags[i] = False
...
def _load_v2_final_pieces(uuid: str, *, gate_unsupported: bool = False) -> list[dict]:
    ...
    if (
        not RAW_CEILING_PLANE_SPLITS_CACHE_BY_VERSION.get(version)
    ...
    buildings = (
        RAW_CEILING_PLANE_SPLITS_CACHE_BY_VERSION[version].get("buildings")
        or {}
    )
...
def _filter_cross_floor_gap_ceiling_lids_for_full_model(
    ...
    for record in records:
        item = dict(record)
        if item.get("ceiling_corners") and _ceiling_cap_is_covered_by_nearby_slanted_roof(
...
def _is_flat_patch(corners, coeffs) -> bool:
    ...
    return (
        (incl is not None and incl <= FLAT_PATCH_MAX_INCL_DEG)
        or _y_span(corners) <= FLAT_PATCH_MAX_Y_SPAN_M
    )
...
def _raw_ceiling_fallback_for_uuid(uuid: str, building: dict) -> list[dict]:
    ...
    for room_idx, room in enumerate(rooms):
        if (
            room_idx in raw_flat_source_rooms
            or room_idx in partition_flat_source_rooms
        ):
            continue
        poly = room.get("ceiling_polygon") or []
...
def _roof_results_newer_than_v2_sidecar() -> bool:
    try:
        ...
        sidecar_mtime = (
            sidecar_path.stat().st_mtime if sidecar_path.exists() else 0.0
        )
    except OSError:
        return False
...
def _building_has_gable(uuid: str) -> bool:
    ...
    try:
        try:
            ...
            from complexity_tiers import classify_building
        return bool(
            classify_building(bldg, ROOF_RESULTS_CACHE.get(uuid))["signals"].get("has_gable")
        )
    except Exception:
        return False
...
def _envelope_clip_pieces(pieces: list[dict], envelope, *, source_label: str) -> list[dict]:
    """Clip raw slanted pieces (corners-only) to a Shapely envelope, keeping the original metadata."""
    try:
        from shapely.geometry import Polygon
        from shapely.validation import make_valid
    except ImportError:
    ...
    for piece in pieces:
        ...
        for piece_out in _shapely_difference_to_3d_pieces(
            xz_poly, None, coeffs, min_area=0.05,
        ):
            out.append({
                **piece,
...
def _slanted_pieces_for_uuid(uuid: str) -> list[dict]:
    ...
    if _use_backend_arrangement_for_slanted(uuid):
        ...
        return _envelope_clip_pieces(backend_pieces, envelope, source_label="roof_arrangement")
    ...
    for piece in source_pieces:
        ...
        if envelope is not None:
            ...
            if xz_poly.is_empty or xz_poly.area < 0.05:
                continue
        for piece_out in _shapely_difference_to_3d_pieces(
            xz_poly, cutouts_xz, coeffs, min_area=0.05, interior_planes=cutouts,
        ):
            entry = {
                "corners": piece_out["poly"],
                "holes": piece_out["holes"],
...
class ViewerHandler(SimpleHTTPRequestHandler):
    ...
    def _handle_building_merged(self, query: str) -> None:
        ...
        if not has_backend_arrangement:
            thermal_kinds.update(
                {"thermal-dormer-cheek", "thermal-dormer-header"}
            )
        knee_walls = [
        ...
        gap_walls = _filter_gap_ceiling_caps_for_full_model(
            gap_walls, slanted_roof_xz
        )
    ...
    def _handle_raw_ceiling_plane_splits(self, query: str):
        ...
        Query:
          - ``version=v1`` (default): ``reports/raw_ceiling_plane_scorer/plane_extent_splits.json``
          - ``version=v2``: ``.context/raw_ceiling_plane_scorer_v2_full/plane_extent_splits.json``
        Missing files return an empty payload with ``available: false``.
        """
        global RAW_CEILING_PLANE_SPLITS_CACHE_BY_VERSION, RAW_CEILING_PLANE_SPLITS_CACHE_MTIME_BY_VERSION
        params = urllib.parse.parse_qs(query or "")
        requested_version = str((params.get("version") or ["v1"])[0] or "v1").strip().lower()
        version = requested_version if requested_version in RAW_CEILING_PLANE_SPLITS_PATHS else "v1"
        path = RAW_CEILING_PLANE_SPLITS_PATHS[version]
        mtime = (
            path.stat().st_mtime
            if path.exists()
        ...
        if (
            ...
            RAW_CEILING_PLANE_SPLITS_CACHE_MTIME_BY_VERSION[version] = mtime
        body = json.dumps(RAW_CEILING_PLANE_SPLITS_CACHE_BY_VERSION[version]).encode("utf-8")
        self.send_response(HTTPStatus.OK)
        self.send_header("Content-Type", "application/json")
        self.send_header("Cache-Control", "no-store")
        self.send_header("Access-Control-Allow-Origin", "*")
