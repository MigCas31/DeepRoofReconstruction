...
def analyze_cross_floor_consistency(uuid, name):
    ...
    if len(result["floors"]) >= 2:
        y_means = [(fl["type"], fl["mean_y"]) for fl in result["floors"] if fl["mean_y"] is not None]
        yaw_means = [(fl["type"], fl["mean_yaw"]) for fl in result["floors"] if fl["mean_yaw"] is not None]
...
def analyze_azimuth_detail(uuid, name):
    ...
    for sid, azs in by_story.items():
        ...
        if first_transforms:
            trans = [ft["translation"] for ft in first_transforms if ft["translation"] is not None]
            if len(trans) >= 2:
                trans = np.array(trans)
                spread = np.max(trans, axis=0) - np.min(trans, axis=0)
                max_dist = max(np.linalg.norm(trans[i] - trans[j])
                              for i in range(len(trans)) for j in range(i+1, len(trans)))
                result["stories"][sid] = {
...
def main():
    ...
    for uuid, name in ALL_TARGETS.items():
        ...
        for fl in r["floors"]:
            ...
            print(f"    {fl['type']:>10}: mean_y={fl['mean_y']:.3f}m (std={fl['std_y']:.4f}), "
                  f"mean_yaw={fl['mean_yaw']:.1f}deg (std={fl.get('yaw_std', 0):.3f}){yaw_flag}")

        if "cross_floor_yaw_range" in r:
            consistent = "YES" if r["cross_floor_yaw_consistent"] else "NO *** DIFFERENT SESSIONS ***"
            print(f"    Cross-floor yaw range: {r['cross_floor_yaw_range']:.2f}deg -> Consistent: {consistent}")
    ...
    for uuid, name in ALL_TARGETS.items():
        ...
        for sid, info in r.get("stories", {}).items():
            print(f"    Story {sid[:20]}...: {info['num_azimuths']} azimuths, "
    ...
    for uuid, name in ALL_TARGETS.items():
        ...
        if not r:
            continue
        tag = "[RESTART]" if uuid in RESTART_BUILDINGS else "[NO-RESTART]"
        print(f"  {tag} {r['name']}: {r['num_rooms']} rooms, {r['num_walls']} walls, "
              f"{r['num_doors']} doors, {r['num_windows']} windows, stories={r['stories']}")
