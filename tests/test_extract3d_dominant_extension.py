...
def _wall_quad(x0: float, z0: float, x1: float, z1: float, top_y: float, bot_y: float = FLOOR_Y) -> dict:
...
def test_fixtureA_short_wall_colinear_with_neighbour_is_promoted():
    ...
    lifted = [c for c in ext["extended_corners"] if c[1] == pytest.approx(DOMINANT_TOP_Y)]
    assert len(lifted) == 2
...
def test_fixtureC_stepped_floor_blocks_promotion():
    ...
    short = _wall_quad(1.0, 0.10, 3.0, 0.10, 2.25, bot_y=0.30)  # top delta OK, floor delta = 0.30
