...
def _make_roof_surface(
    az: float = 297.6, incl: float = 46.0, cx: float = 0.0, cz: float = 0.0, cy: float = 2.0
) -> dict:
    ...
    corners = [
        (cx - ridge_x * 3 + slope_x * 3, cy - math.sin(incl_rad) * 3, cz - ridge_z * 3 + slope_z * 3),
        (cx + ridge_x * 3 + slope_x * 3, cy - math.sin(incl_rad) * 3, cz + ridge_z * 3 + slope_z * 3),
        (cx + ridge_x * 3 - slope_x * 3, cy + math.sin(incl_rad) * 3, cz + ridge_z * 3 - slope_z * 3),
        (cx - ridge_x * 3 - slope_x * 3, cy + math.sin(incl_rad) * 3, cz - ridge_z * 3 - slope_z * 3),
    ]
...
class TestDetectDormers:
    def test_detects_protruding_wall(self):
        ...
        roof_y_at_wall = plane_y_at(plane, 0.0, 1.0)
        wall_corners = [
            (-1, 0, 1), (1, 0, 1),
            (1, 3, 1), (-1, 3, 1),
        ]
    ...
    def test_ignores_wall_parallel_to_ridge(self):
        ...
        # A wall facing east (normal along X) has dot(normal, ridge) ~ 1 → parallel → rejected
        wall_corners = [
            (0, 0, -1), (0, 0, 1),
            (0, 3, 1), (0, 3, -1),
        ]
    ...
    def test_uses_top_boundary_atoms_to_filter_candidate_rooms(self):
        ...
        bldg = {
            "rooms": [
                {"story": 1, "walls_merged": [_make_wall("OTHER", wall_corners), tall_wall], "walls_computed": []},
                {"story": 1, "walls_merged": [_make_wall("FRONT", wall_corners), tall_wall], "walls_computed": []},
            ]
        }
        top_boundary_graph = {
            "nodes": [
    ...
    def test_uses_coverage_polygon_to_filter_wall_location(self):
        ...
        bldg = {
            "rooms": [
                {
                    "story": 1,
                    "walls_merged": [_make_wall("INSIDE", inside_wall), _make_wall("OUTSIDE", outside_wall), tall_wall],
                    "walls_computed": [],
                }
            ]
        }
        ...
        roof_coverage_graph = {
            "atom_coverage": {
                "atom:1": {"sloped_hypothesis_id": "roof-hypothesis:oblique:0", "sloped_state": "confirmed"}
            },
            "atom_regions": {
                "atom:1": {
                    "room_index": 0,
    ...
    def test_allows_strong_branch_supported_region(self):
        ...
        roof_evidence_graph = {
            "subpart_evidence": {
                "subpart:branch:0": {"evidence_score": 4}
            }
        }
...
class TestWalkToRoofHeight:
    ...
    def test_returns_none_for_impossible(self):
        ...
        # Roof at (0,0) is already at y=10, asking for y=2 going uphill → impossible (negative t)
        result = _walk_to_roof_height((0, 0, 0), 2.0, plane, slope_x, slope_z)
    ...
    def test_cutout_points_on_roof_plane(self):
        ...
        for pt in cutout:
            ...
            assert abs(pt[1] - expected_y) < 0.1, (
                f"Point {pt} not on roof plane: expected y={expected_y:.3f}, got y={pt[1]:.3f}"
            )
...
class TestBuildDormerGeometry:
    ...
    def test_cheek_clipped_no_overlap_with_room_wall(self):
        ...
        geom = build_dormer_geometry(
            group, roof, room_walls=[front, blocking_wall]
        )
