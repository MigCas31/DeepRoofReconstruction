...
from reconcile.roof_algorithms_py.roof_partitioning import derive_room_ceiling_partitions


def _project_xz(poly3d: list[list[float]] | list[tuple[float, float, float]]) -> Polygon:
    return Polygon([(float(x), float(z)) for x, _, z in poly3d])


def test_derive_room_ceiling_partitions_repairs_snapped_oblique_atom_regression() -> None:
    exposed_room = {
        "room_index": 6,
        "story": 1,
