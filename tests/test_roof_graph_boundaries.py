...
def _make_graph(exposed: bool, above: bool = False, adjacent: bool = False) -> TopologyGraph:
    nodes = [
        GraphNode(
            id="room:r0",
            type="Room",
    ...
    edges = [
        GraphEdge(id="contains:room-floor", type="CONTAINS", from_id="room:r0", to_id="surface:floor-1"),
        GraphEdge(id="contains:room-wall", type="CONTAINS", from_id="room:r0", to_id="surface:wall-1"),
        GraphEdge(id="contains:room-boundary", type="CONTAINS", from_id="room:r0", to_id="boundary:wall-1"),
        GraphEdge(id="bounds:boundary-room", type="BOUNDS", from_id="boundary:wall-1", to_id="room:r0"),
        GraphEdge(id="bounds:boundary-surface", type="BOUNDS", from_id="boundary:wall-1", to_id="surface:wall-1"),
    ]
    if exposed:
        edges.append(
            GraphEdge(id="exposes:room-outside", type="EXPOSES_TO", from_id="room:r0", to_id="cell:outside")
        )
    if above:
        edges.extend(
            [
                GraphEdge(id="above:r-above-r0", type="ABOVE", from_id="room:r-above", to_id="room:r0"),
                GraphEdge(id="below:r0-r-above", type="BELOW", from_id="room:r0", to_id="room:r-above"),
            ]
        )
    if adjacent:
        edges.extend(
            [
                GraphEdge(id="adj:r0-r1", type="ADJACENT_TO", from_id="room:r0", to_id="room:r1"),
                GraphEdge(id="adj:r1-r0", type="ADJACENT_TO", from_id="room:r1", to_id="room:r0"),
            ]
        )
...
def test_build_ceiling_planes_can_borrow_adjacent_roof_room_support() -> None:
    ...
    planes = build_ceiling_planes([cluster], bldg={"rooms": [room0, room1]}, graph=graph)
...
def test_boundary_face_model_emits_shared_space_boundary_records() -> None:
    ...
    boundary_model = build_boundary_face_model(bldg=bldg, roof_result=roof_result, graph=graph)
    derived = derive_roof_surfaces_from_boundary_model(boundary_model)

    wall_boundary = next(boundary for boundary in boundary_model["boundaries"] if boundary["role"] == "wall")
    slab_boundary = next(boundary for boundary in boundary_model["boundaries"] if boundary["role"] == "slab")
...
def test_roof_boundary_graph_emits_continuation_edges() -> None:
    ...
    boundary_model = build_boundary_face_model(bldg={"rooms": [room0, room1]}, roof_result=roof_result, graph=graph)
    roof_graph = build_roof_boundary_graph(boundary_model, graph=graph)

    continuation_edges = [edge for edge in roof_graph["edges"] if edge["type"] == "CONTINUES_AS"]
    assert continuation_edges
    assert all(edge["evidence"]["relation_state"] in {"confirmed", "partial"} for edge in continuation_edges)
    assert all(edge["evidence"]["exact_face_incidence"] is True for edge in continuation_edges)
    assert all(edge["evidence"]["shared_edge_length_m"] > 0.0 for edge in continuation_edges)
    assert roof_graph["metadata"]["partition_face_count"] >= 2


def test_roof_boundary_graph_does_not_continue_across_different_oblique_planes() -> None:
    room0 = _make_room()
    room1 = {
        **_make_room(),
        "floor_polygon": [
