...
def test_opposing_gable_faces_remain_separate_directional_clusters():
    ...
    assert [len(cluster.segments) for cluster in clusters] == [MIN_CLUSTER_SIZE, MIN_CLUSTER_SIZE]
    assert angle_diff_deg(clusters[0].avg_azimuth, clusters[1].avg_azimuth) == pytest.approx(180.0, abs=1.0)
