...
def test_planes_clip_and_lift_to_oblique_surfaces_on_the_same_plane():
    ...
    assert all(fp_poly.buffer(1e-6).contains(Point(x, z)) or fp_poly.touches(Point(x, z)) for x, z in clipped[0].polygon_xz)
...
def test_plane_clipping_uses_contributing_room_support_not_full_footprint():
    ...
    far_wall = ExtractedWall(
        id="far-flat-wall",
        corners=[[12.0, 0.0, 0.0], [16.0, 0.0, 0.0], [16.0, 2.4, 0.0], [12.0, 2.4, 0.0]],
        source="test",
    )
    ...
    model = replace(base, rooms=[base.rooms[0], far_room], scan_rooms_found=2, scan_rooms_transformed=2)
    footprint = build_building_footprint(model)
    clusters = cluster_oblique_segments(collect_oblique_segments(model))
    planes = build_roof_planes(clusters, footprint)
...
def test_raw_oblique_surfaces_require_clean_large_rectangles():
    ...
    assert len(build_raw_oblique_surfaces(replace(model, rooms=[clean_room, model.rooms[1]]), [])) == 1
    assert build_raw_oblique_surfaces(replace(model, rooms=[small_room, model.rooms[1]]), []) == []
