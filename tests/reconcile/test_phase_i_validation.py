...
from reconcile_tiers.build import build_tier_payload, payload_json
from reconcile_tiers.classify.tiers import classify_building
from reconcile_tiers.payload.schema import CeilingSource, KneeWallKind, payload_to_dict
...
def _metrics(payload_dict: dict) -> dict:
    ...
    return {
        ...
        "n_dormers": sum(1 for wall in knee_walls if wall["kind"] == KneeWallKind.DORMER_HEADER.value),
        "n_ceiling_pieces": len(ceiling),
        "n_knee_walls": len(knee_walls),
        "n_thermal_caps": sum(1 for piece in ceiling if piece["source"] == CeilingSource.THERMAL_CAP.value),
        "total_ceiling_xz_area": round(sum(_xz_area(piece["corners"]) for piece in ceiling), 4),
    }
...
def test_cohort_metrics_match_committed_tolerances(uuid: str):
    ...
    for key in ("tier", "roof_type", "n_oblique", "n_flat", "n_dormers", "n_ceiling_pieces", "n_knee_walls", "n_thermal_caps"):
        assert current[key] == expected[key]
    assert abs(current["total_ceiling_xz_area"] - expected["total_ceiling_xz_area"]) <= tolerances["total_ceiling_xz_area_abs_m2"]


def test_tier_counts_vs_current_tier_index_within_five_percent():
    ...
    for building in buildings:
        uuid = building["uuid"]
        roof = roofs.get(uuid)
        legacy_counts[legacy_classify_building(building, roof)["tier"]] += 1
        current_counts[classify_building(building, roof)["tier"]] += 1

    for tier, legacy_count in legacy_counts.items():
        tolerance = max(1, math.ceil(legacy_count * 0.05))
        assert abs(current_counts[tier] - legacy_count) <= tolerance
...
def _pixel_diff_fraction(actual: Path, expected: Path) -> float:
    ...
    for idx in range(0, len(rgba_a), 4):
        if max(abs(rgba_a[idx + channel] - rgba_b[idx + channel]) for channel in range(4)) > 16:
            mismatches += 1
...
def phase_i_static_root(tmp_path_factory):
    ...
    (root / "pipeline-outputs" / "tier_index.json").write_text(json.dumps({"buildings": buildings}, indent=2, sort_keys=True) + "\n")
    return root
    ...
    with sync_playwright() as playwright:
        ...
        try:
            for idx, uuid in enumerate(COHORT_UUIDS):
                ...
                page.wait_for_function("window.__tierState && window.__tierState.locatorCount > 0", timeout=20000)
                page.wait_for_timeout(500)
                actual = tmp_path / f"{uuid}.png"
                page.screenshot(path=str(actual))
                expected = GOLDEN_ROOT / "screenshots" / f"{uuid}.png"
                assert _pixel_diff_fraction(actual, expected) <= SCREENSHOT_MAX_DIFF_FRACTION
        finally:
            browser.close()
