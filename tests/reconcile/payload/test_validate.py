...
def test_payload_invariants_enforced_for_wrong_horizontal_lid_winding():
    ...
    bad_floor = HorizontalLid(corners=list(reversed(payload.rooms[0].floor.corners)))
...
def test_vertical_plane_is_rejected_with_path():
    payload = sample_payload()
    bad_piece = dataclasses.replace(payload.ceiling[0], plane=Plane(1.0, 0.01, 0.0, 0.0))
...
def test_ceiling_corners_must_lie_on_plane():
    ...
    with pytest.raises(PayloadInvariantError, match="plane.y_at"):
        validate_payload(payload)
