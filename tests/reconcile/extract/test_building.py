...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            50,
            50,
            {"scan-cache": 49, "scan-cache-dedup": 1},
...
def test_extract_building_model_matches_legacy_wall_and_floor_counts(
    ...
    assert sum(len(room.walls_computed) for room in model.rooms) == expected_computed_walls
    sources = Counter(
        wall.source
    ...
    assert Counter(item.source for room in model.rooms for item in room.doors) == expected_doors
    assert Counter(item.source for room in model.rooms for item in room.windows) == expected_windows
    assert Counter(item.source for room in model.rooms for item in room.openings) == expected_openings
    assert Counter(item.source for room in model.rooms for item in room.storages) == expected_storages
    assert sum(len(room.raw_ceiling_planes) for room in model.rooms) == expected_raw_ceiling_planes
    assert Counter(
