...
def test_merged_segments_emitted_with_full_provenance() -> None:
    ...
    assert seg.room_boundary_refs and seg.room_boundary_refs[0]["kind"] in {"room", "gap"}
...
def test_opposing_seams_split_into_both_sides() -> None:
    ...
    both_sides = [c for c, sides in sides_seen_by_cluster.items() if {"rain", "covered"} <= sides]
    assert both_sides, (
