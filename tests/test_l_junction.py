...
def _make_plane(azi: float, incl: float, ref_x: float, ref_z: float, ref_y: float = 3.0):
...
class TestDetectAndExpandLJunctions:
    def _make_l_shaped_setup(self):
        ...
        return planes, plane_clipped, footprint, detect_and_expand_l_junctions, point_in_poly_2d
    ...
    def test_ridge_bounds_extended(self):
        planes, plane_clipped, fp, detect_fn, pip_fn = self._make_l_shaped_setup()
        orig_bounds = [
            (pc["ridgeMin"], pc["ridgeMax"]) for pc in plane_clipped
        ]
        detect_fn(
            ceiling_planes=planes,
            plane_clipped=plane_clipped,
        ...
        for i, pc in enumerate(plane_clipped):
            if (pc["ridgeMin"] < orig_bounds[i][0] - 0.01 or
                    pc["ridgeMax"] > orig_bounds[i][1] + 0.01):
                any_extended = True
                break
        assert any_extended
    ...
    def test_all_y_ranges_reasonable(self, roof_result):
        ...
        for i, o in enumerate(oblique):
            ...
            assert max(ys) < allowable_roof_peak, (
                f"oblique[{i}] y_max={max(ys):.2f} exceeds structure_y_max+4.0 ({allowable_roof_peak:.2f})"
            )
