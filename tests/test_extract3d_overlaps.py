...
def _wall(wall_id: str, x0: float, z0: float, x1: float, z1: float, y0: float = 0.0, y1: float = 2.4) -> dict:
    return {
        "id": wall_id,
        "corners": [
            [x0, y0, z0],
...
def test_clip_floor_overlaps_removes_long_shared_wall_even_when_midpoint_is_outside_overlap() -> None:
    rooms_out = [
        _room(
            (0.0, 0.0, 4.0, 2.0),
    ...
    assert [wall["id"] for wall in clipped[1]["walls_removed_overlap"]] == ["loser-wall"]
    assert any(
    ...
    assert floor_polygon_to_shapely(clipped[0]["floor_polygon"]).intersection(
        floor_polygon_to_shapely(clipped[1]["floor_polygon"])
    ).area == 0.0


def test_clip_floor_overlaps_keeps_parallel_wall_when_offset_exceeds_coverage_tolerance() -> None:
    rooms_out = [
        _room(
            (0.0, 0.0, 4.0, 2.0),
            [_wall("winner-wall", 0.0, 0.0, 4.0, 0.0)],
...
def test_clip_floor_overlaps_keeps_wall_that_touches_removed_region_without_winner_coverage() -> None:
    rooms_out = [
        _room(
            (0.0, 0.0, 4.0, 2.0),
            [_wall("winner-wall", 0.0, 0.0, 4.0, 0.0)],
...
def test_clipped_floor_ownership_can_drive_non_overlapping_ceiling_ownership_downstream() -> None:
    rooms_out = [
        _room(
            (0.0, 0.0, 4.0, 2.0),
            [_wall("winner-wall", 0.0, 0.0, 4.0, 0.0)],
    ...
    for room in clipped:
        ...
        derived_ceiling = [[corner[0], ceiling_y, corner[2]] for corner in room["floor_polygon"]]
...
def test_project_line_interval_accepts_multiline_geometry() -> None:
    ...
    start, end = _project_line_interval(line, np.array([0.0, 0.0]), np.array([1.0, 0.0]))
