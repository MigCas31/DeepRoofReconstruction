...
@pytest.mark.skipif(not PIPELINE_ROOT.exists() or not SCAN_CACHE.exists(), reason="real pipeline data unavailable")
...
def test_roof_pipeline_produces_reasonable_outputs(uuid: str) -> None:
    ...
    building = extract_building(uuid, PIPELINE_ROOT, SCAN_CACHE, load_topology_graph=False)
    ...
    if cell_complex:
        ...
        if cells:
            assert len(knee_walls) <= len(cells) * 3, (
                f"Knee-wall count ({len(knee_walls)}) exceeds 3x cell count ({len(cells)})"
            )
        # Each knee wall should have reasonable area
        for kw in knee_walls:
            if "face_area_m2" in kw:
                assert kw["face_area_m2"] >= 0.1, f"Knee wall too small: {kw['face_area_m2']}"
