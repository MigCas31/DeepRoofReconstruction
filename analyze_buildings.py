...
all_uuids = [d for d in os.listdir(BASE) if os.path.isdir(os.path.join(BASE, d)) and d not in problem_uuids]
random.seed(42)
...
for uuid in control_uuids:
    ...
    if os.path.exists(dp):
        try:
            with open(dp) as f:
                d = json.load(f)
            name = d.get("address", {}).get("street", "") or d.get("name", "") or uuid[:12]
        except:
            pass
...
def parse_transform(t):
    """Parse a flat 16-element list into a 4x4 matrix (column-major as used by RoomPlan)."""
...
def analyze_building(uuid, name):
    ...
    for i, room in enumerate(rooms):
        ...
        for w_idx, wall in enumerate(room.get("walls", [])):
            ...
            if std.max() > 0:
                ...
                if z_scores.max() > 3:
                    ...
                    result["issues"].append(
                        f"Room {i} refOrigin outlier: {axis}-axis z-score={z_scores.max():.1f}, pos={pos.tolist()}"
                    )
    ...
    if len(stories) > 1:
            ...
            for j_s in range(i_s + 1, len(sorted_stories)):
                ...
                if overlap > 0:
                    result["issues"].append(
                        ...
                        f"(story{s1}: [{r1[0]:.3f}, {r1[1]:.3f}], story{s2}: [{r2[0]:.3f}, {r2[1]:.3f}])"
                    )
                else:
                    gap = -overlap
                    ...
                    if gap < 0.5:
                        result["issues"].append(
                            f"Story {s1} to {s2}: suspiciously small Y-gap of {gap:.3f}m"
                        )
                    elif gap > 6.0:
                        result["issues"].append(
                            f"Story {s1} to {s2}: suspiciously large Y-gap of {gap:.3f}m"
                        )
        ...
        for i_s in range(len(sorted_stories) - 1):
            ...
            if story_y_ranges[s_lo][2] > story_y_ranges[s_hi][2]:
                result["issues"].append(
                    f"INVERTED stories: story {s_lo} mean_y={story_y_ranges[s_lo][2]:.3f} > story {s_hi} mean_y={story_y_ranges[s_hi][2]:.3f}"
                )
    ...
    for w in walls:
        ...
        if w_pos is not None:
            ...
            if w_dim and max(w_dim[:2]) > 20:
                result["issues"].append(
                    f"Wall {w.get('identifier','?')[:8]}: extreme dimension {w_dim} (story {w_story})"
                )

            # Check for walls at extreme positions
            if abs(w_pos[0]) > 50 or abs(w_pos[1]) > 20 or abs(w_pos[2]) > 50:
                result["issues"].append(
                    f"Wall {w.get('identifier','?')[:8]}: extreme position {w_pos.tolist()} (story {w_story})"
                )
    ...
    if wall_positions:
        ...
        outlier_count = 0
        for s, pos in wall_positions:
            z = np.abs(pos - mean) / (std + 1e-10)
            if z.max() > 4:
                outlier_count += 1
        if outlier_count > 0:
            result["issues"].append(f"{outlier_count} wall position outliers (>4 std from mean)")
    ...
    if all_positions:
        ...
        if bbox[1] > 10:
            result["issues"].append(
                f"Very tall building: Y-range={bbox[1]:.2f}m"
            )
    ...
    if len(ref_origins) >= 2:
        ...
        if dists.max() > 30:
            result["issues"].append(
                f"Max pairwise refOrigin distance: {dists.max():.2f}m (>30m, rooms may be misaligned)"
            )
...
def print_report(result):
    ...
    print(f"  Rooms: {result['num_rooms']}  |  Walls: {result['num_walls']}  |  Floors: {result['num_floors']}")
    ...
    if "ref_origin_stats" in result:
        ...
        print(f"  RefOrigin range (X,Y,Z): [{st['range'][0]:.2f}, {st['range'][1]:.2f}, {st['range'][2]:.2f}]")
        print(f"  RefOrigin std (X,Y,Z): [{st['std'][0]:.2f}, {st['std'][1]:.2f}, {st['std'][2]:.2f}]")

    if "ref_origin_pairwise_dist" in result:
        d = result["ref_origin_pairwise_dist"]
        print(f"  RefOrigin pairwise dist: mean={d['mean']:.2f}, max={d['max']:.2f}, std={d['std']:.2f}")

    if "ref_yaw_range" in result:
        print(f"  RefOrigin yaw range: {np.degrees(result['ref_yaw_range']):.1f} deg, {result['unique_ref_yaws']} unique orientations")
    ...
    if "wall_pos_stats" in result:
        ...
        print(f"  Wall position range (X,Y,Z): [{ws['range'][0]:.2f}, {ws['range'][1]:.2f}, {ws['range'][2]:.2f}]")

    if "wall_dim_stats" in result:
        wd = result["wall_dim_stats"]
        print(f"  Wall max dimensions: [{wd['max'][0]:.2f}, {wd['max'][1]:.2f}, {wd['max'][2]:.2f}]")
...
print(f"\n{'Building':<30} {'Rooms':>5} {'Walls':>5} {'Stories':>8} {'BBox X':>7} {'BBox Y':>7} {'BBox Z':>7} {'RefDist':>8} {'Yaw Rng':>8} {'Issues':>6}")
print("-" * 110)
