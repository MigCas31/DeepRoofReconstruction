...
BASE = Path("/Users/martincollignon/conductor/workspaces/look-ma-no-hands/tirana/pipeline-outputs")
CACHE = Path("/Users/martincollignon/conductor/workspaces/look-ma-no-hands/tirana/.scan-cache")
...
random.seed(42)
all_uuids = sorted([d.name for d in BASE.iterdir() if d.is_dir() and (d / "merged.json").exists()])
...
def cluster_yaws(yaws, eps=2.0):
    ...
    if len(clusters) > 1:
        ...
        wrap_diff = min(abs(last_yaw - first_yaw), abs(last_yaw - first_yaw + 360), abs(last_yaw - first_yaw - 360))
        if wrap_diff <= eps:
...
def analyze_building(uuid):
    ...
    for i in range(1, len(rooms)):
        ...
        time_gaps.append({
            ...
            "same_session": abs(rooms[i]["yaw"] - rooms[i - 1]["yaw"]) < 2.0 or abs(abs(rooms[i]["yaw"] - rooms[i - 1]["yaw"]) - 360) < 2.0,
        })
            ...
            for r in srms:
                for si in session_info:
                    if r["name"] in si["room_names"]:
                        session_yaw_map[r["name"]] = (r["centroid"][1], si["session_id"])
                        break
            y_offset_info[str(story)] = {
                "y_range_m": round(float(y_range), 3),
                "num_rooms": len(srms),
                "room_y_session": {k: (round(float(v[0]), 3), v[1]) for k, v in session_yaw_map.items()},
            }
    ...
    if len(session_info) > 1:
        ...
        for si_a in range(len(yaw_clusters)):
            for si_b in range(si_a + 1, len(yaw_clusters)):
                rooms_a = [rooms[idx] for idx, _ in yaw_clusters[si_a] if rooms[idx]["centroid"] is not None]
                rooms_b = [rooms[idx] for idx, _ in yaw_clusters[si_b] if rooms[idx]["centroid"] is not None]
                if rooms_a and rooms_b:
                    ...
                    # Yaw difference
                    yaw_diff = abs(session_info[si_a]["mean_yaw_deg"] - session_info[si_b]["mean_yaw_deg"])
            ...
            for b in range(a + 1, len(occs)):
                ...
                if t_diff > 0.01 or r_diff > 0.5:
                    wall_disagreements.append({
                        "wall_id": wid[:12],
                        "rooms": (rooms[occs[a][0]]["name"], rooms[occs[b][0]]["name"]),
...
def print_report(label, r):
    ...
    if r.get("error"):
        ...
        return
    print(f"  Rooms: {r['num_rooms']} | Floors: {r['num_floors']} | "
    ...
    if switches:
        ...
        for sw in switches:
            ...
            print(f"    {sw['from']:20s} -> {sw['to']:20s}  yaw_change={sw['yaw_change']}deg {tg}")
    ...
    if wd:
        ...
        for d in wd[:5]:
            print(f"    {d['wall_id']} between {d['rooms']}: "
...
def main():
    ...
    print("  SUMMARY TABLE")
    print(f"{'='*90}")
    print(f"\n{'UUID':<40} {'Type':<8} {'Rm':<4} {'Sess':<5} {'RSes':<5} {'TGap':<5} {'SwYaw':<6} {'YOff':<5} {'ShWD':<5} {'Class':<7} {'GapCm':<6}")
    print("-" * 110)
    ...
    prob_sess = [r["num_sessions"] for l, r in all_results if l == "PROBLEM" and not r.get("error")]
    ctrl_sess = [r["num_sessions"] for l, r in all_results if l == "CONTROL" and not r.get("error")]
    ...
    print(f"  Control buildings with multiple sessions: {ctrl_multi}/{len(ctrl_sess)} "
    ...
    for label, r in all_results:
        ...
        if r["num_sessions"] > 1:
            ...
            print(f"    {r['uuid'][:20]}.. {label:8s} "
                  f"sessions={r['num_sessions']}, yaw_diffs={yaw_diffs}deg, "
                  f"class={r.get('recon_class')}, gap_median={r.get('recon_gap_median_cm')}cm")
