...
The key question: within a SINGLE floor, do rooms have different referenceOriginTransforms?
...
def analyze_per_floor_ref_origins(uuid, name):
    """For each floor in data.json, load each room's JSON and compare referenceOriginTransform."""
    ...
    for fl in floors:
        ...
        transforms = [(r["translation"], r["rotation"], r["name"], r["created"])
                       for r in room_data_list if r["translation"] is not None]
...
def analyze_merged_per_story(uuid):
    ...
    for story, rots in by_story.items():
        ...
        if len(translations) >= 2:
            max_d = max(np.linalg.norm(translations[i] - translations[j])
                        for i in range(len(translations)) for j in range(i+1, len(translations)))
        else:
            max_d = 0
        result[story] = {
            "num_rooms": len(rots),
...
def main():
    ...
    for r in target_results:
        flag = " *** INTRA-FLOOR VARIATION ***" if r["has_intra_floor_variation"] else ""
        print(f"\n{'='*80}")
        ...
        for fl in r["floors"]:
            ...
            print(f"\n  Floor: {fl['floor_type']:>10} | {fl['num_rooms']} rooms | "
                  ...
                  f"max_trans={fl['max_trans_diff_m']:.3f}m | max_rot={fl['max_rot_diff_deg']:.2f}deg{var_flag}")
            ...
            for g in fl["groups"]:
                ...
                print(f"    Group {g['group_id']}: {len(g['rooms'])} rooms - {g['rooms']}")
                print(f"      Translation: [{t[0]:.3f}, {t[1]:.3f}, {t[2]:.3f}]{time_range}")
    ...
    if target_intra_floor_vars:
        print(f"\n    TARGET translation diffs : mean={np.mean(target_intra_floor_vars):.3f}m, "
              f"max={np.max(target_intra_floor_vars):.3f}m")
    if target_intra_floor_rots:
        print(f"    TARGET rotation diffs   : mean={np.mean(target_intra_floor_rots):.2f}deg, "
              f"max={np.max(target_intra_floor_rots):.2f}deg")
    if control_intra_floor_vars:
        print(f"    CONTROL translation diffs: mean={np.mean(control_intra_floor_vars):.3f}m, "
              f"max={np.max(control_intra_floor_vars):.3f}m")
    if control_intra_floor_rots:
        print(f"    CONTROL rotation diffs  : mean={np.mean(control_intra_floor_rots):.2f}deg, "
    ...
    for r in target_results + control_results:
        if len(r["floors"]) > 1:
            ...
            for fl in r["floors"]:
                if fl["groups"]:
                    floor_translations.append((fl["floor_type"], fl["groups"][0]["translation"]))
