...
def analyze_building(uuid):
    ...
    for i, room in enumerate(rooms):
        ...
        result["room_details"].append({
            ...
            "wall_height_mean": np.mean(room_wall_heights) if room_wall_heights else 0,
            "wall_height_std": np.std(room_wall_heights) if room_wall_heights else 0,
    ...
    if all_wall_positions:
        ...
        dists = [math.sqrt((p[0]-building_centroid[0])**2 +
        ...
        if room_centroids:
            room_dists = [math.sqrt((rc[0]-building_centroid[0])**2 +
            ...
            result["rooms_far_from_centroid"] = [
                ...
                for i, d in enumerate(room_dists) if d > 10
            ]
            ...
            if len(room_dists) > 2:
                ...
                result["room_outliers_2std"] = [
                    (result["room_details"][i], d)
                    for i, d in enumerate(room_dists) if d > mean_d + 2 * std_d
                ]
            else:
                result["room_outliers_2std"] = []
        else:
        ...
        for k in ["wall_x_spread", "wall_y_spread", "wall_z_spread",
                   "wall_max_dist", "wall_mean_dist",
...
def main():
    ...
    header = (f"{'Building':<28} {'Rooms':>5} {'Walls':>5} {'Stry':>4} "
    ...
    for uuid, r in sorted(problem_results.items(), key=lambda x: x[1]["name"]):
        print(f"{r['name']:<28} {r['num_rooms']:>5} {r['num_walls_global']:>5} "
              f"{len(r['stories']):>4} "
              f"{r['wall_x_spread']:>7.2f} {r['wall_y_spread']:>7.2f} {r['wall_z_spread']:>7.2f} "
              f"{r['wall_max_dist']:>8.2f} {r.get('room_max_dist_from_bldg_centroid',0):>7.2f} "
    ...
    print("\n--- REFERENCE BUILDINGS ---")
    for uuid, r in sorted(reference_results.items(), key=lambda x: x[1]["name"]):
        print(f"{r['name']:<28} {r['num_rooms']:>5} {r['num_walls_global']:>5} "
              f"{len(r['stories']):>4} "
              f"{r['wall_x_spread']:>7.2f} {r['wall_y_spread']:>7.2f} {r['wall_z_spread']:>7.2f} "
              f"{r['wall_max_dist']:>8.2f} {r.get('room_max_dist_from_bldg_centroid',0):>7.2f} "
    ...
    def agg(results, field):
        vals = [r[field] for r in results.values() if field in r and r[field] is not None]
        if not vals:
            return (0, 0, 0, 0)
    ...
    print(f"{'Metric':<42} {'PROB mean':>10} {'PROB med':>10} {'REF mean':>10} {'REF med':>10} {'Diff':>10}")
    print("-" * 95)
    for field, label in metrics:
        p = agg(problem_results, field)
        r = agg(reference_results, field)
        diff = p[0] - r[0]
        print(f"{label:<42} {p[0]:>10.3f} {p[1]:>10.3f} {r[0]:>10.3f} {r[1]:>10.3f} {diff:>+10.3f}")
    ...
    for uuid, r in sorted(problem_results.items(), key=lambda x: x[1]["name"]):
        ...
        for s, (ymin, ymax, yrange) in sorted(y_ranges.items()):
            print(f"    story {s}: Y range [{ymin:.2f}, {ymax:.2f}] spread={yrange:.2f}m")
    ...
    for uuid, name in sorted(PROBLEM_BUILDINGS.items(), key=lambda x: x[1]):
        ...
        if recon:
            ...
            print(f"    Walls: {recon.get('wall_count_merged')} merged / {recon.get('wall_count_scan')} per-scan "
            ...
            print(f"    Floor gap: {recon.get('floor_gap_median_cm')} cm ({recon.get('floor_gap_measurements')} measurements)")
    ...
    for uuid, name in sorted(REFERENCE_BUILDINGS.items(), key=lambda x: x[1]):
        recon = get_reconciliation(uuid)
        if recon:
            print(f"  {name}: class={recon.get('classification')}, "
    ...
    for uuid, r in sorted(problem_results.items(), key=lambda x: x[1]["name"]):
        for rd, d in r.get("rooms_far_from_centroid", []):
            any_found = True
            print(f"  {r['name']} room[{rd['index']}] story={rd['story']}: "
                  f"centroid=({rd['centroid'][0]:.2f}, {rd['centroid'][1]:.2f}, {rd['centroid'][2]:.2f}) "
                  f"dist={d:.2f}m")
    if not any_found:
        print("  None found.")
    ...
    for uuid, r in sorted(problem_results.items(), key=lambda x: x[1]["name"]):
        ...
        print(f"\n  {r['name']} ({r['num_rooms']} rooms, bldg centroid=({bc[0]:.2f}, {bc[1]:.2f}, {bc[2]:.2f}))")
        print(f"    {'Rm':>3} {'Stry':>4} {'Walls':>5} {'CentX':>8} {'CentY':>8} {'CentZ':>8} "
              f"{'DistBC':>7} {'Xext':>6} {'Yext':>6} {'Zext':>6} {'WHmean':>6} {'WHstd':>6}")
        for rd in r["room_details"]:
            dist = math.sqrt((rd['centroid'][0]-bc[0])**2 +
                             (rd['centroid'][1]-bc[1])**2 +
                             (rd['centroid'][2]-bc[2])**2)
            marker = " <<<" if dist > 10 else ""
            print(f"    {rd['index']:>3} {rd['story']:>4} {rd['wall_count']:>5} "
                  f"{rd['centroid'][0]:>8.2f} {rd['centroid'][1]:>8.2f} {rd['centroid'][2]:>8.2f} "
                  f"{dist:>7.2f} {rd['x_extent']:>6.2f} {rd['y_extent']:>6.2f} {rd['z_extent']:>6.2f} "
    ...
    print(f"    Problem: {prob_multi}/{len(problem_results)} multi-story "
          f"({prob_multi/len(problem_results)*100:.0f}%)")
    print(f"    Reference: {ref_multi}/{len(reference_results)} multi-story "
